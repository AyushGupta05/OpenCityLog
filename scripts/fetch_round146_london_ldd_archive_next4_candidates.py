...
TASK_ID = "round146_london_ldd_archive_next4_candidates"
SCRIPT_PATH = "scripts/fetch_round146_london_ldd_archive_next4_candidates.py"
OUT_DIR = Path("tmp/subagents/round146_london_ldd_archive_next4")
...
def any_prior_contains(prior_packs, key, value):
    return any(value in pack[key] for pack in prior_packs)
...
def rewrite_round146_candidate(candidate, row, excel_row):
    ...
    candidate["transformation_method"] = (
        ...
        "round132, round135, and round144 candidate packs; filtered to Current permission status Completed, Completed Date "
        "from 2008-01-01 through 2026-05-19, valid Greater London Easting/Northing point, and "
...
def candidate_for(row, excel_row, score, flags, floorspace_refs, bedroom_refs):
    ...
    return rewrite_round146_candidate(candidate, row, excel_row)
...
def write_notes(candidate_count, summary):
    text = f"""# Round 146 London LDD archive next4 candidates
...
def main():
    ...
    for excel_row, row in base.iter_xlsx_dict_rows(permissions_xlsx):
        ...
        if not reason and (
            ...
            reason = "duplicate inside round146 batch scan"
    ...
    selection_summary = {
        ...
        **prior_pack_summary(prior_packs, dedupe),
        "dedupe_fields": [
            ...
            "city/title/date",
            "prior round132, round135, and round144 candidate packs as fallback dedupe inputs if corpus ingestion is incomplete",
            "same fields inside the round146 batch scan",
        ],
        "filters": [
    ...
    print(json.dumps({
        ...
        "round132CandidateRowsSeen": len(prior_packs[0][2]["rows"]),
        "round132CandidateRowsNowInManualCorpus": len(prior_packs[0][2]["rows"] & dedupe["rows"]),
        "round135CandidateRowsSeen": len(prior_packs[1][2]["rows"]),
        "round135CandidateRowsNowInManualCorpus": len(prior_packs[1][2]["rows"] & dedupe["rows"]),
        "round144CandidateRowsSeen": len(prior_packs[2][2]["rows"]),
        "round144CandidateRowsNowInManualCorpus": len(prior_packs[2][2]["rows"] & dedupe["rows"]),
