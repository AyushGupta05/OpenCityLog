...
function replaceRequired(value, needle, replacement, label) {
  if (!value.includes(needle)) {
    ...
    throw new Error(`Round373 source transform could not find ${label}: ${needle}`);
  }
...
function assertContains(value, needle, label) {
  if (!value.includes(needle)) {
    throw new Error(`Round373 source transform failed to include ${label}: ${needle}`);
  }
}

function transformRound362Script(source) {
  let transformed = source
    .replace(/round362_london_pld_lifecycle_next42/g, "round373_london_pld_lifecycle_next45")
    .replace(/round362\.london_pld_lifecycle_next42/g, "round373.london_pld_lifecycle_next45")
    .replace(/Bims5Round362PldLifecycleNext42/g, "Bims5Round373PldLifecycleNext45")
    .replace(/Round362/g, "Round373")
    .replace(/Round 362/g, "Round 373")
    .replace(/round362/g, "round373")
    .replace(/next42/g, "next45")
    .replace(/MAX_PRIOR_ROUND = 358/g, "MAX_PRIOR_ROUND = 369")
    .replace(/through round358/g, "through round369");
  ...
  transformed = replaceRequired(
    ...
    '      "round272, round276, round284, round288, round298, round301, round306, round311, round316, round323, round327, round328, round333, round338, round343, round348, round350, round354, round358, round362, round365, and round369"\\n    );',
  ...
  transformed = replaceRequired(
    ...
    "prior round327 through round369 fetch assertions"
  );

  transformed = replaceRequired(
    transformed,
    '    .replace(/round276/g, "round358");',
    '    .replace(/round276/g, "round369");',
    "validator prior cutoff prose replacement"
  );

  assertContains(transformed, 'const ROUND_NAME = "round373_london_pld_lifecycle_next45";', "round name");
  assertContains(transformed, 'const ROUND = "round373";', "generated round id");
  assertContains(transformed, "MAX_PRIOR_ROUND = 369", "prior cutoff");
  ...
  assertContains(transformed, "through round369", "dedupe prose cutoff");
  return transformed;
}
...
function main() {
  ...
  vm.runInNewContext(
    ...
    {
      filename: "generated_round373_london_pld_lifecycle_next45_candidates.js"
    }
  );
}
