...
const GENERATED_FILENAME = "scripts/fetch_round389_nyc_dob_co_next45_candidates.generated.js";
const ROUND = "round389";
const ROUND_TITLE = "Round389";
const NEXT_LABEL = "next45";
const NEXT_TITLE = "Next45";
...
function transformRound308Source(source) {
  let transformed = source.replace(/\r\n/g, "\n")
    ...
    .replaceAll("through round303", "through round386");
  ...
  transformed = replaceFirst(
    ...
    `${round303File}\n];`,
    `${priorRoundFiles}\n];`,
    "explicit duplicate files through Round386"
  );
  transformed = replaceOnce(
    transformed,
    `${round303File}\n];`,
    `${priorRoundFiles}\n];`,
    "required screened files through Round386"
  );
  ...
  transformed = replaceOnce(
    ...
    "screened rounds include Round308 through Round386"
  );
  ...
  transformed = replaceOnce(
    ...
    "validation check fields through Round386"
  );
  ...
  transformed = replaceOnce(
    ...
    "validation report lines through Round386"
  );
  ...
  assertContains(transformed, "round389_nyc_dob_co_next45", "Round389 output path");
  assertContains(transformed, "Round389 NYC DOB CO Next45 Candidate Pack", "Round389 notes title");
  assertContains(transformed, "nyc_dob_co_round389_legacy_", "Round389 candidate ID prefix");
  assertContains(transformed, "tmp/subagents/round386_nyc_dob_co_next44/candidates.json", "Round386 screened file");
  assertContains(transformed, "through round386", "duplicate-screening text through Round386");
  assertContains(transformed, "required_round386_screened", "Round386 validation result");
  ...
  assertContains(transformed, `transformation_method: "${ROUND_TITLE} queried`, "Round389 source audit transformation method");
  assertContains(transformed, "scripts/fetch_round389_nyc_dob_co_next45_candidates.js standalone validation artifact", "Round389 validator label");
...
async function main() {
  ...
  if (typeof sandbox.module.exports.main !== "function") {
    throw new Error("Transformed Round389 generator did not export main().");
  }
