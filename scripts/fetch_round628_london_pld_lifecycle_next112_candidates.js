...
const ROUND628_OUT_DIR = path.join(
  ROOT,
  "tmp",
  "subagents",
...
function replaceRequired(value, needle, replacement, label) {
  if (!value.includes(needle)) {
    ...
    throw new Error(`Round628 wrapper transform could not find ${label}: ${needle}`);
  }
...
function transformRound616Wrapper(source) {
  let transformed = source
    .replace(/round616_london_pld_lifecycle_next109/g, "round628_london_pld_lifecycle_next112")
    .replace(/round616\.london_pld_lifecycle_next109/g, "round628.london_pld_lifecycle_next112")
    .replace(/Bims5Round616PldLifecycleNext109/g, "Bims5Round628PldLifecycleNext112")
    .replace(/Round 616/g, "Round 628")
    .replace(/Round616/g, "Round628")
    .replace(/round616/g, "round628")
    .replace(/next109/g, "next112")
    .replace(/MAX_PRIOR_ROUND = 614/g, "MAX_PRIOR_ROUND = 625")
    .replace(/through round614/g, "through round625")
    .replace(/packRound\(entry\.name\) > 614/g, "packRound(entry.name) > 625")
    .replace(/\.replace\(\/round276\/g, "round614"\);/g, '.replace(/round276/g, "round625");')
    .replace(/round614 prior PLD lifecycle pack was scanned/g, "round625 prior PLD lifecycle pack was scanned")
    .replace(
      /pack\) => pack\.label === "round614_london_pld_lifecycle_next108" && pack\.exists === true/g,
      'pack) => pack.label === "round625_london_pld_lifecycle_next111" && pack.exists === true'
    );
  ...
  transformed = replaceRequired(
    ...
    "round625 prior pack insertion"
  );

  assertContains(transformed, 'const ROUND_NAME = "round628_london_pld_lifecycle_next112";', "round name");
  assertContains(transformed, 'const ROUND = "round628";', "transformed fetch round id");
  assertContains(transformed, "MAX_PRIOR_ROUND = 625", "prior cutoff");
  assertContains(transformed, "through round625", "dedupe cutoff text");
  ...
  assertContains(transformed, "addRound628RetrievedAtAlias", "candidate retrieved_at alias");
  assertContains(transformed, 'CURRENT_ROUND_NAME = "round628_london_pld_lifecycle_next112"', "validator round name");
  assertContains(transformed, "round628_strict_duplicate_audit", "strict duplicate audit label");
...
async function main() {
  process.once("beforeExit", () => {
    if (!fs.existsSync(ROUND628_OUT_DIR)) return;
    for (const entry of fs.readdirSync(ROUND628_OUT_DIR, { withFileTypes: true })) {
      if (!entry.isFile()) continue;
      if (!/\.(json|md)$/i.test(entry.name)) continue;
      const filePath = path.join(ROUND628_OUT_DIR, entry.name);
      const before = fs.readFileSync(filePath, "utf8");
      const after = before.split(INHERITED_ACCESS_DATE).join(ROUND628_ACCESS_DATE);
  ...
  vm.runInNewContext(transformed, sandbox, {
    filename: "generated_round628_london_pld_lifecycle_next112_candidates.js"
  });
}
