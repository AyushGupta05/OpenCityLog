...
const GENERATED_FILENAME = "scripts/fetch_round318_nyc_dob_co_next28_candidates.generated.js";
...
function transformRound308Source(source) {
  let transformed = source.replace(/\r\n/g, "\n")
    .replaceAll("round308", "round318")
    .replaceAll("Round308", "Round318")
    .replaceAll("next26", "next28")
    .replaceAll("Next26", "Next28")
    .replaceAll("through round303", "through round313");
  ...
  transformed = replaceFirst(
    ...
    `${round303File}\n];`,
    `${priorRoundFiles}\n];`,
    "explicit duplicate Round308/Round313 files"
  );
  transformed = replaceOnce(
    transformed,
    `${round303File}\n];`,
    `${priorRoundFiles}\n];`,
    "required Round308/Round313 screened files"
  );
  transformed = replaceOnce(
    transformed,
    '  "300",\n  "303"\n];',
    '  "300",\n  "303",\n  "308",\n  "313"\n];',
    "screened rounds include Round308 and Round313"
  );
  transformed = replaceOnce(
    transformed,
    ...
    "Round308/Round313 validation check fields"
  );
  transformed = replaceOnce(
    transformed,
    "  const sourceAuditEntry = {\n    ...metadata,\n    reliability:",
    "  const sourceAuditEntry = {\n    ...metadata,\n    accessed_at: ACCESSED_AT,\n    confidence: \"documented\",\n    method: \"Round318 queried the official NYC Open Data legacy DOB Certificate Of Occupancy API, filtered and grouped issued rows, screened duplicates, and retained row-level provenance.\",\n    transformation_method: \"Round318 queried the official NYC Open Data legacy DOB Certificate Of Occupancy API, filtered and grouped issued rows, screened duplicates, and retained row-level provenance.\",\n    limitations: \"Legacy CO issuance is an administrative/legal DOB record. It is not actual occupancy, public opening, completion for all spaces, construction completion, final built form, safety outcome, or affordability outcome. Coordinates are DOB/Open Data geocoded address points.\",\n    reliability:",
  ...
  transformed = replaceOnce(
    ...
    "Round308/Round313 validation report lines"
  );
  transformed = replaceOnce(
    transformed,
    "main().catch((error) => {\n  console.error(error);\n  process.exit(1);\n});",
  ...
  assertContains(transformed, "round318_nyc_dob_co_next28", "Round318 output path");
  assertContains(transformed, "Round318 NYC DOB CO Next28 Candidate Pack", "Round318 notes title");
  assertContains(transformed, "nyc_dob_co_round318_legacy_", "Round318 candidate ID prefix");
  ...
  assertContains(transformed, '  "308",\n  "313"\n];', "screened rounds include 308 and 313");
  assertContains(transformed, "through round313", "duplicate-screening text through Round313");
  ...
  assertContains(transformed, "transformation_method: \"Round318 queried", "source audit transformation method");
  assertContains(transformed, "scripts/fetch_round318_nyc_dob_co_next28_candidates.js standalone validation artifact", "Round318 validator label");
...
async function main() {
  ...
  if (typeof sandbox.module.exports.main !== "function") {
    throw new Error("Transformed Round318 generator did not export main().");
  }
