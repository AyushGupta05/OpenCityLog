...
OUT_DIR = Path("tmp/subagents/round305_london_nhle_heritage_tail3")
...
SOURCE_ID = "historic-england-nhle-open-data-round305-tail3"
...
def get_json(url, params=None):
    ...
    request = urllib.request.Request(url, headers={"User-Agent": "Bims-5 round305 NHLE tail3 fetcher"})
...
def candidate_from_feature(query, feature, point):
    ...
    candidate_id = f"london-nhle-round305-{layer_slug}-{field_slug}-{list_entry}-{date_compact(event_date)}"
    source_record_id = (
    ...
    return {
        ...
        "transformation_method": (
            "scripts/fetch_round305_london_nhle_heritage_tail3_candidates.py queried the official Historic "
...
def candidate_from_delisted_feature(feature, point):
    ...
    candidate_id = f"london-nhle-round305-de-designated-sites-date-removed-{record_key}-{date_compact(event_date)}"
    source_record_id = (
    ...
    summary = (
        ...
        "occupation, condition, outcome, forecast, simulation, or causation."
    )
    if decision_text:
    ...
    return {
        ...
        "source_dataset_id": f"{DELISTED_SOURCE_ID}-round305-tail3-layer-0",
        "source_layer_id": 0,
        ...
        "limitations": (
            ...
            "change, heritage benefit, impact, forecast, simulation, or causation."
        ),
        "transformation_method": (
            "scripts/fetch_round305_london_nhle_heritage_tail3_candidates.py queried the official Historic "
...
def make_summary(existing, result):
    ...
    return {
        ...
        "worker_scope": "round305 London Historic England/NHLE heritage-designation tail3",
        ...
        "duplicate_screening": {
            ...
            "screening_note": (
                ...
                "prior Historic England/NHLE London heritage packs through round294, for event IDs, source keys, "
...
def notes_md(summary):
    lines = [
        "# Round305 London NHLE Heritage Tail3",
        "",
        "This pack harvests additional official Historic England/NHLE administrative heritage records for London after the earlier London/NHLE heritage packs through round294.",
        "",
    ...
    lines.extend(
        [
            "",
            "Caveat: these are administrative Historic England/NHLE dates only. Do not treat designation, listing, inscription, certificate/status-start or expiry, amendment, or de-designation/removal dates as construction, demolition, repair, opening, occupation, condition, forecast, simulation, or causal evidence.",
            "",
        ]
    )
    return "\n".join(lines)
...
def main():
    ...
    pack = {
        "metadata": {
            "schema_version": "round305_london_nhle_heritage_tail3_candidates_v1",
            "generated_at": f"{ACCESS_DATE}T00:00:00Z",
            "city_id": "london",
            "source_id": SOURCE_ID,
            "date_window": {"start": DATE_START, "end": DATE_END},
