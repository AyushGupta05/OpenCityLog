...
const ROUND_NAME = "round436_london_pld_lifecycle_next59";
...
function replaceRequired(value, needle, replacement, label) {
  if (!value.includes(needle)) {
    ...
    throw new Error(`Round436 source transform could not find ${label}: ${needle}`);
  }
...
function assertContains(value, needle, label) {
  if (!value.includes(needle)) {
    throw new Error(`Round436 source transform failed to include ${label}: ${needle}`);
  }
}
...
function transformRound404Source(source) {
  ...
  transformed = transformed
    .replace(/round404_london_pld_lifecycle_next52/g, "round436_london_pld_lifecycle_next59")
    .replace(/round404\.london_pld_lifecycle_next52/g, "round436.london_pld_lifecycle_next59")
    .replace(/Bims5Round404PldLifecycleNext52/g, "Bims5Round436PldLifecycleNext59")
    .replace(/Round 404/g, "Round 436")
    .replace(/Round404/g, "Round436")
    .replace(/round404/g, "round436")
    .replace(/next52/g, "next59")
    .replace(/MAX_PRIOR_ROUND = 398/g, "MAX_PRIOR_ROUND = 432")
    .replace(/through round398/g, "through round432");
  ...
  transformed = replaceRequired(
    ...
    "round404 through round432 prior PLD lifecycle packs"
  );
  ...
  assertContains(transformed, 'const ROUND_NAME = "round436_london_pld_lifecycle_next59";', "round name");
  assertContains(transformed, 'const ROUND = "round436";', "transformed fetch round id");
  assertContains(transformed, "MAX_PRIOR_ROUND = 432", "prior cutoff");
  assertContains(transformed, "through round432", "dedupe cutoff text");
  ...
  assertContains(transformed, 'CURRENT_ROUND_NAME = "round436_london_pld_lifecycle_next59"', "validator round name");
...
async function runTransformedRound404() {
  ...
  vm.runInNewContext(transformed, sandbox, {
    filename: "generated_round436_london_pld_lifecycle_next59_candidates.js"
  });

  if (typeof sandboxModule.exports !== "function") {
    throw new Error("Round436 transform did not export the fetch main() function.");
  }
...
function addRound436RequestedAliases() {
...
function validateAndRewriteReadbackJson() {
  ...
  const ok =
    ...
    /through round432/i.test(summary.dedupe_basis || "") &&
    (summary.duplicate_index?.prior_packs || []).some(
      (pack) => pack.label === "round432_london_pld_lifecycle_next58" && pack.exists === true
    );

  if (!ok) {
    throw new Error("Readback validation failed for Round436 output pack.");
  }
  ...
  writeJson(readbackFile, {
    ...
    readback_checks: [
      ...
      "dedupe basis includes PLD lifecycle packs through round432",
      "round432 prior PLD lifecycle pack was scanned",
  ...
  if (!finalReadback.ok || !finalReadback.files?.["readback.json"]?.exists) {
    throw new Error("Final readback self-check failed for Round436 output pack.");
  }
}
