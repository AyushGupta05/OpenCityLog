...
const candidateFiles = [
  "tmp/subagents/london_arch_candidates_round106_heritage_listings.json",
  "tmp/subagents/london_arch_candidates_round106_schools_libraries_health.json",
  "tmp/subagents/nyc_arch_candidates_round106_lpc_landmarks.json",
  "tmp/subagents/nyc_arch_candidates_round106_civic_infrastructure_facilities.json",
  "tmp/subagents/belfast_arch_candidates_round106_hed_listings.json",
  "tmp/subagents/belfast_arch_candidates_round106_health_education_civic.json"
];
...
const publisherFor = (candidate) => {
  ...
  if (url.includes("belfasttrust.hscni.net")) return "Belfast Health and Social Care Trust";
  ...
  if (url.includes("nyc.gov/site/lpc")) return "NYC Landmarks Preservation Commission";
  if (url.includes("nyc.gov/site/ddc") || url.includes("nyc.gov/html/ddc")) return "NYC Department of Design and Construction";
  if (url.includes("nycgovparks.org") || url.includes("nycgovparks.org") || url.includes("nycgovparks.org")) return "NYC Parks";
  if (url.includes("edc.nyc")) return "New York City Economic Development Corporation";
  if (url.includes("nhs.uk") || url.includes("hscni.net")) return "Public health trust or NHS website";
...
const termsNoteFor = (candidate) => {
  ...
  if (url.includes("nyc.gov") || url.includes("nycgovparks.org") || url.includes("edc.nyc")) {
    return "NYC public or partner website terms apply; factual metadata and source URL only, no page text or images reproduced.";
  }
  if (url.includes("hscni.net") || url.includes("nhs.uk")) {
    return "Public health website terms apply; factual metadata and source URL only, no page text or media reproduced.";
  }
...
if (candidateRows.length !== 47) {
  throw new Error(`Expected 47 round106 candidates, found ${candidateRows.length}`);
}
...
const toRecord = (candidate) => {
  ...
  return {
    ...
    transformation_method: safeText(
      ...
      `Manual round106 source-family extraction from ${candidate.__file}; duplicate checked by event_id, source URL, source record ID, and title terms; normalized into the architecture milestone schema with date, source, confidence, geometry, and limitation fields preserved.`
    )
  };
};
...
const tmpPath = `${path}.round106.tmp`;
