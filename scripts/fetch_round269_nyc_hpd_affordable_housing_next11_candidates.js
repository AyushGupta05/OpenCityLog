...
function replaceChecked(pattern, replacement, label) {
  ...
  if (next === source) {
    throw new Error(`Unable to adapt round251 HPD fetcher for round269: missing ${label}.`);
  }
  source = next;
}
...
replaceChecked(
  ...
  'const outDir = "tmp/subagents/round269_nyc_hpd_affordable_housing_next11";',
  "output directory"
);
replaceChecked('const accessedAt = "2026-05-19";', 'const accessedAt = "2026-05-20";', "access date");
replaceChecked('const roundName = "Round251";', 'const roundName = "Round269";', "round name");
replaceChecked("const maxScreeningRound = 246;", "const maxScreeningRound = 263;", "screening round");
replaceChecked(/Round251/g, "Round269", "round label references");
replaceChecked(/round251\.nyc/g, "round269.nyc", "schema round prefix");
replaceChecked(
  /nyc_hpd_affordable_housing_next8/g,
  "nyc_hpd_affordable_housing_next11",
  "next pack label"
);
replaceChecked(
...
replaceChecked(
  ...
  "candidate_files_considered_through_round263",
  "candidate-files summary key"
);
replaceChecked(
  "async function main()",
  `const round269SelectionCache = new WeakMap();
...
function isClearlySupportedRound269PreservationCandidate(candidate) {
...
function isRound269PreferredCandidate(candidate) {
  if (candidate.source_date_field !== "building_completion_date") return false;
  if (isPreservation(candidate.raw_row)) return isClearlySupportedRound269PreservationCandidate(candidate);
  return true;
}

function selectedRound269CandidateIds(rankedCandidates) {
  if (round269SelectionCache.has(rankedCandidates)) return round269SelectionCache.get(rankedCandidates);
  const selectedIds = new Set();

  for (const candidate of rankedCandidates) {
    if (selectedIds.size >= targetCount) break;
    if (isRound269PreferredCandidate(candidate)) selectedIds.add(candidate.event_id);
  }
  ...
  round269SelectionCache.set(rankedCandidates, selectedIds);
  return selectedIds;
}

async function main()`,
  "round269 supported preservation selection helper"
);
replaceChecked(
  ".slice(0, targetCount)",
  ".filter((candidate, index, rankedCandidates) => selectedRound269CandidateIds(rankedCandidates).has(candidate.event_id))",
  "round269 selection call"
);
replaceChecked(
  ...
  "Source date priority is applied first: building_completion_date rows, then project_completion_date rows, then project_start_date rows. Round269 keeps the accepted pack to building_completion_date rows where available; Preservation rows are admitted only when the selected row has HPD building_completion_date, source coordinates, positive unit counts, and Extended Affordability Only = No. Within each date-field tier, New Construction rows remain ahead of Preservation rows before unit-count score, senior/supportive text signals, and newer dates.",
  "round269 ranking summary"
);
replaceChecked(
...
replaceChecked(
  ...
  '      "Coordinates are source geocoded points and should not be displayed as exact building footprints or project boundaries.",\n      "Round269 Preservation rows are retained only when HPD reports building_completion_date, coordinates, positive units, and Extended Affordability Only = No; they remain administrative delivery records, not independent proof of full rehabilitation scope or occupancy.",',
...
replaceChecked(
  ...
  "including round229, round234, round236, round241, round246, round251, round255, and round263",
  "explicit overlap caveat"
);
replaceChecked(/through round246/g, "through round263", "screening prose");

const runAdaptedRound251Fetcher = new Function(
  "require",
  "fetch",
  `${source}\n//# sourceURL=fetch_round269_nyc_hpd_affordable_housing_next11_candidates.adapted.js`
);
