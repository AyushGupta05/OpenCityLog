...
const ROUND_ID = "round171_belfast_planning_statistics_next4";
...
const REJECTION_DETAIL_CAP = 800;

const PRIOR_PACKS = [
  ["round131_belfast_planning_statistics", "round131_belfast_planning_statistics"],
  ["round134_belfast_planning_statistics_deep", "round134_belfast_planning_statistics_deep"],
  ["round137_belfast_planning_statistics_more", "round137_belfast_planning_statistics_more"],
  ["round145_belfast_planning_statistics_next", "round145_belfast_planning_statistics_next"],
  ["round151_belfast_planning_statistics_next2", "round151_belfast_planning_statistics_next2"],
  ["round165_belfast_planning_statistics_next3", "round165_belfast_planning_statistics_next3"],
  [ROUND_ID, ROUND_ID, true]
].map(([roundId, folder, optional = false]) => ({
  round_id: roundId,
  path: path.join(ROOT_DIR, "tmp", "subagents", folder, "candidates.json"),
  optional
}));
  ...
  if (!fs.existsSync(pack.path)) {
    if (pack.optional) {
      return {
        ...
        application_ids: 0,
        source_rows: 0,
        source_record_aliases: 0,
        apps: new Set(),
        sourceRecords: new Set(),
        sourceRecordValues: new Set()
      };
    }
...
function assessNext4Quality(record, baseAssessment, existingApps) {
  if (baseAssessment.adminOrMinor || baseAssessment.smallDomestic || baseAssessment.lowSignalWorks) {
    return { passes: false, reason: "base_minor_admin_or_domestic_exclusion" };
  }
  if (baseAssessment.score < MIN_SCORE) {
    return { passes: false, reason: "below_round171_score_floor" };
  }
  ...
  const units = baseAssessment.scale.units || 0;
  const storeys = baseAssessment.scale.storeys || 0;
  const strongScale = units >= 10 || storeys >= 3;
  const hasResidentialUse = /\b(apartments?|flats?|dwellings?|houses?|homes?|townhouses?|residential units?|social housing|student accommodation|student rooms?|studio rooms?)\b/i.test(proposal);
  const hasBuiltFacility = /\b(hotel|office|offices|retail|restaurant|cafe|bakery|supermarket|convenience store|warehouse|factory|workshop|industrial|community|school|college|university|clinic|hospital|social club|sports dome|gym|leisure|apartments?|dwellings?|housing)\b/i.test(proposal);
  const hasSubstantialAction = /\b(development|redevelopment|construction|erection|new build|conversion|convert|change of use|extension|roof top extension|rooftop extension|alterations?|refurbishment|restoration|partial demolition|demolition|associated site works)\b/i.test(proposal);
  const likelyRevisionOnly = /\b(renewal of previous|renewal of permission|variation of conditions?|section 54|amended drawings|additional information|amended plans|amendments? to (?:previously )?(?:approved|accepted)|alteration to an extant approval|extant approval|relocation of|removal of|replacement with|regularisation)\b/i.test(combined) &&
    !strongScale &&
    !/\b(listed warehouses|former|vacant|bank building|church building|residential conversion|roof top extension|sports dome)\b/i.test(proposal);
  const minorSiteOrFitout = /\b(fencing|fence|gate|boundary|handrail|safety guarding|perimeter|car park|parking layout|internal alterations? only|internal works|shopfront|shop front|frontage|flue|extract(?:ion)?|air conditioning|telecom|antenna|CCTV|mezzanine floor|beauty treatment|training school|garden pavilion|pool pavilion|art studio|single dwelling|existing dwelling|to dwelling|domestic)\b/i.test(combined) &&
    !/\b(?:\d{2,4}\s*(?:no\.?\s*)?(?:apartments?|dwellings?|houses?|residential units?)|listed warehouses|social housing|sports dome|community hall|church building|former convent|bank building)\b/i.test(proposal);
  const smallHealthcareOrRetailSwap = /\bchange of use\b/i.test(proposal) &&
    /\b(?:veterinary|medical|dental|orthodontic|offices?|office accommodation|shop|estate agents?|massage treatment rooms?|cafe|restaurant)\b/i.test(proposal) &&
    /\b(?:retail|shop|class A1|dental surgery|orthodontic surgery|clinic|cafe)\b/i.test(proposal) &&
    !strongScale &&
    !/\b(former|vacant|listed|community|school|college|university|hospital|social club|sports dome|industrial|factory|workshop|hotel|office building)\b/i.test(proposal);
  const floorOnlyChangeOfUse = /\bchange of use\b/i.test(proposal) &&
    /\b(?:ground|first|second|third|fourth)\s+floor(?:\s+only)?\b/i.test(proposal) &&
    !strongScale &&
    !/\b(university|college|community|hospital|social club|sports dome|residential|apartments?)\b/i.test(proposal);
  const useOnlySmallPremises = /\bchange of use\b/i.test(proposal) &&
    !strongScale &&
    !/\b(former|vacant|listed|warehouse|church|bank building|community|school|college|university|hospital|clinic|apartments?|residential|social club|sports dome|industrial|factory|workshop|hotel|office building)\b/i.test(proposal);

  if (likelyRevisionOnly) {
    return { passes: false, reason: "revision_or_renewal_without_independent_scale" };
  }
  if (minorSiteOrFitout) {
    return { passes: false, reason: "minor_site_frontage_internal_or_domestic_works" };
  }
  if (useOnlySmallPremises) {
    return { passes: false, reason: "small_change_of_use_without_architecture_scale" };
  }
  if (smallHealthcareOrRetailSwap) {
    return { passes: false, reason: "small_healthcare_or_retail_use_swap" };
  }
  if (floorOnlyChangeOfUse) {
    return { passes: false, reason: "floor_only_change_of_use_without_scale" };
  }

  if (hasResidentialUse && strongScale && hasBuiltFacility) {
    return {
      passes: true,
      qualityGate: "scaled_residential_residual_next4",
      qualityReasons: ["scaled residual residential approval"]
    };
  }
  if (
    (signals.includes("listed high-signal conversion") || /listed building|listed warehouses|listed/i.test(combined)) &&
    hasSubstantialAction &&
    hasBuiltFacility &&
    !/\b(frontage|shopfront|safety guarding|handrail|internal alterations? only)\b/i.test(proposal)
  ) {
    return {
      passes: true,
      qualityGate: "listed_adaptive_reuse_or_built_use_next4",
      qualityReasons: ["listed/adaptive built-use residual approval"]
    };
  }
  if (
    (signals.includes("civic facility works") || /\b(school|college|university|community hall|community centre|sports dome|hospital|clinic|social club|scout hall|after-school|boxing gym)\b/i.test(proposal)) &&
    hasSubstantialAction &&
    !minorSiteOrFitout
  ) {
    return {
      passes: true,
      qualityGate: "civic_or_public_facility_next4",
      qualityReasons: ["civic/community facility residual approval"]
    };
  }
  if (
    (signals.includes("industrial or production-space works") || /\b(warehouse|industrial|factory|workshop|storage and distribution|office building|hotel|commercial building)\b/i.test(proposal)) &&
    hasSubstantialAction &&
    hasBuiltFacility &&
    !minorSiteOrFitout
  ) {
    return {
      passes: true,
      qualityGate: "industrial_or_commercial_built_use_next4",
      qualityReasons: ["industrial/commercial built-use residual approval"]
    };
  }

  return { passes: false, reason: "not_substantial_enough_for_round171" };
}
...
function addRejection(rejections, counts, record, reason, extra = {}) {
  ...
  rejections.push({
    ...
    proposal: helpers.truncate(record.proposal, 260),
    ...extra
  });
}
...
function limitationsFor() {
  return [
    ...
    "Round171 keeps only residual rows that are post-dedupe against the live manual corpus and the round131, round134, round137, round145, round151, and round165 Belfast planning-statistics packs, approved, in the Belfast coordinate envelope, and strong enough for a scaled residential, listed/adaptive reuse, civic/community facility, or industrial/commercial built-use residual gate. Existing same-round output is read for rerun awareness because the script overwrites this pack rather than appending to it.",
...
function candidateFor(record) {
  ...
  return {
    ...
    transformation_method: "scripts/fetch_round171_belfast_planning_statistics_next4_candidates.js reused the round145 local DfI planning-statistics CSV parser, deduped against all planning application IDs in the live manual architecture corpus plus round131, round134, round137, round145, round151, and round165 Belfast planning-statistics candidate application IDs/source rows, read any existing same-round output for rerun awareness, rejected residual rows that referenced already-used application IDs, kept only approved Belfast rows dated 2008-01-01 through 2026-05-19 where locally available CSV coverage exists and approximate in-city coordinates are present, required residual scaled residential, listed/adaptive reuse, civic/community facility, or industrial/commercial built-use signals, excluded domestic/minor/admin/signage/telecom/frontage-only/internal-fitout/site-only rows, and preserved file/row provenance."
  };
}
...
function validateCandidates(candidates, existingApps, existingSourceRecords) {
  ...
  for (const candidate of candidates) {
    ...
    if (!/administrative/i.test(candidate.observed_change) || !/not evidence/i.test(candidate.observed_change)) {
      throw new Error(`Missing planning approval caveat in observed_change for ${candidate.candidate_id}`);
    }
  }
}
...
function sourceAudit(fileAudits, summary, decision) {
  return {
    ...
    coverage_checked: `Local planning statistics CSV rows filtered for approved Belfast decisions from ${DATE_START} through ${summary.latest_defensible_source_date}, after dedupe against ${summary.corpus_all_application_ids} live-corpus application IDs and ${summary.prior_pack_application_ids} prior-pack application IDs from round131, round134, round137, round145, round151, and round165. Existing round171 output is read and reported for rerun awareness because this script overwrites its own output files rather than appending.`,
    ...
    transformation_method: "Live manual-corpus app-ID/source-row dedupe, explicit round131/round134/round137/round145/round151/round165 candidate app-ID/source-row dedupe, same-round output readback for rerun awareness, linked-application rejection, Belfast authority/LPA and coordinate-envelope check, approved decision filter, 2008-01-01 through 2026-05-19 requested date window bounded by available local CSVs, residual high-signal scoring, exclusion of minor domestic/admin/signage/telecom/frontage-only/internal-fitout/site-only rows, deterministic sorting, and required-provenance validation.",
    scoring_summary: {
      min_score: MIN_SCORE,
      max_candidates: MAX_CANDIDATES,
      emitted_candidates: summary.accepted_candidates,
      quality_gate: "A residual row must pass one round171 gate: scaled residential approval, listed/adaptive built-use approval, civic/community facility approval, or industrial/commercial built-use approval."
    },
    ...
    source_audits: [
      {
        ...
        license_or_terms_note: LICENSE,
        attribution: ATTRIBUTION,
        coverage_years_checked: `${DATE_START} through ${summary.latest_defensible_source_date}; local files cover releases present in data/raw/planning_statistics.`,
...
function buildPack() {
  ...
  for (const prior of priorKeys) {
    if (prior.round_id === ROUND_ID) continue;
  ...
  let latestDefensibleSourceDate = DATE_START;

  for (const record of rows) {
    if (record.date && record.date > latestDefensibleSourceDate && record.date <= DATE_END) {
      latestDefensibleSourceDate = record.date;
    }
    ...
    if (!record.date || record.date < DATE_START || record.date > DATE_END) {
      addRejection(rejections, rejectionCounts, record, "outside_round171_window");
      continue;
    }
    ...
    if (selectedAppIds.has(record.appIdKey)) {
      addRejection(rejections, rejectionCounts, record, "duplicate_app_id_in_round171_selection");
      continue;
    }

    const baseAssessment = helpers.assessQuality(record);
    const next4Assessment = assessNext4Quality(record, baseAssessment, existingApps);
    if (!next4Assessment.passes) {
      addRejection(rejections, rejectionCounts, record, next4Assessment.reason, {
        score: baseAssessment.score,
        reasons: baseAssessment.qualitySignals,
        linked_existing_application_ids: next4Assessment.linkedExistingAppIds
      });
      continue;
    }
    ...
    selectedRecords.push({
      ...
      qualitySignals: [...new Set([
        ...
        ...next4Assessment.qualityReasons
      ])],
      qualityGate: next4Assessment.qualityGate
    });
  }
  ...
  const priorPackSummaries = priorKeys.map((prior) => ({
    ...
    path: prior.path,
    optional: Boolean(prior.optional),
    missing: Boolean(prior.missing),
  ...
  const dedupePriorKeys = priorKeys.filter((prior) => prior.round_id !== ROUND_ID);
  const sameRoundKey = priorKeys.find((prior) => prior.round_id === ROUND_ID);
  const priorPackApps = new Set(dedupePriorKeys.flatMap((prior) => [...prior.apps]));
  const priorPackSourceRows = new Set(dedupePriorKeys.flatMap((prior) => [...prior.sourceRecordValues]));
  const priorPackSourceRecordAliases = new Set(dedupePriorKeys.flatMap((prior) => [...prior.sourceRecords]));
  ...
  const summary = {
    ...
    prior_pack_application_ids: priorPackApps.size,
    prior_pack_source_rows: priorPackSourceRows.size,
    prior_pack_source_record_aliases: priorPackSourceRecordAliases.size,
    same_round_existing_output: sameRoundKey ? {
      missing: Boolean(sameRoundKey.missing),
      candidate_count: sameRoundKey.candidate_count,
      application_ids: sameRoundKey.application_ids,
      source_rows: sameRoundKey.source_rows,
      source_record_aliases: sameRoundKey.source_record_aliases,
      handling: "read for rerun awareness; not used as a suppressing dedupe set because this script overwrites its own output files"
    } : null,
    ...
    latest_defensible_source_date: latestDefensibleSourceDate,
    ...
    audit_decision: candidates.length > 0
      ? "pack_generated_high_quality_residual_rows"
  ...
  const decision = candidates.length > 0
    ...
    : "Do not append another pack from this tranche; remaining post-dedupe rows did not clear the round171 residual high-signal quality gate.";
...
function writeNotes(file, summary) {
  const lines = [
    `# ${ROUND_ID}`,
    "",
    ...
    "## Scope",
    "",
    `- Date window: ${DATE_START} through ${summary.latest_defensible_source_date}.`,
    ...
    "## Result",
    "",
    `- Candidates retained: ${summary.accepted_candidates}.`,
    `- Eligible before cap: ${summary.eligible_before_cap}.`,
    `- Candidate cap: ${summary.candidate_cap}.`,
    `- Counts by year: ${JSON.stringify(summary.counts_by_year)}.`,
    `- Counts by quality gate: ${JSON.stringify(summary.counts_by_quality_gate)}.`,
    "",
    "## Caveat",
    "",
...
function main() {
  ...
  const output = {
    ...
    scope_note: "Fourth post-dedupe Belfast planning-statistics residual pack limited to approved rows with scaled residential, listed/adaptive reuse, civic/community facility, or industrial/commercial built-use evidence beyond the live manual corpus and round131/round134/round137/round145/round151/round165 outputs. Existing round171 output is read for rerun awareness because this script overwrites the same pack. Approval is not evidence of construction, opening, occupation, completion, delivery, final built form, or causal effects.",
  ...
  writeNotes(NOTES_PATH, summary);
  writeJson(REJECTIONS_PATH, rejectedOutput);
  writeJson(REJECTED_PATH, rejectedOutput);

  console.log(JSON.stringify({
    candidates: candidates.length,
    audit_decision: summary.audit_decision,
    latest_defensible_source_date: summary.latest_defensible_source_date,
    ...
    notes_path: path.relative(ROOT_DIR, NOTES_PATH).replace(/\\/g, "/"),
    rejections_path: path.relative(ROOT_DIR, REJECTIONS_PATH).replace(/\\/g, "/"),
    rejected_path: path.relative(ROOT_DIR, REJECTED_PATH).replace(/\\/g, "/")
  }, null, 2));
}
