...
function replaceRequired(value, needle, replacement, label) {
  if (!value.includes(needle)) {
    ...
    throw new Error(`Round387 source transform could not find ${label}: ${needle}`);
  }
...
function assertContains(value, needle, label) {
  if (!value.includes(needle)) {
    throw new Error(`Round387 source transform failed to include ${label}: ${needle}`);
  }
}

function transformRound362Script(source) {
  let transformed = source
    .replace(/round362_london_pld_lifecycle_next42/g, "round387_london_pld_lifecycle_next49")
    .replace(/round362\.london_pld_lifecycle_next42/g, "round387.london_pld_lifecycle_next49")
    .replace(/Bims5Round362PldLifecycleNext42/g, "Bims5Round387PldLifecycleNext49")
    .replace(/Round362/g, "Round387")
    .replace(/Round 362/g, "Round 387")
    .replace(/round362/g, "round387")
    .replace(/next42/g, "next49")
    .replace(/MAX_PRIOR_ROUND = 358/g, "MAX_PRIOR_ROUND = 384")
    .replace(/through round358/g, "through round384");
  ...
  transformed = replaceRequired(
    ...
    '      "round272, round276, round284, round288, round298, round301, round306, round311, round316, round323, round327, round328, round333, round338, round343, round348, round350, round354, round358, round362, round365, round369, round373, round377, round380, and round384"\\n    );',
  ...
  transformed = replaceRequired(
    ...
    "prior round327 through round384 fetch assertions"
  );

  transformed = replaceRequired(
    transformed,
    '    .replace(/round276/g, "round358");',
    '    .replace(/round276/g, "round384");',
    "validator prior cutoff prose replacement"
  );

  assertContains(transformed, 'const ROUND_NAME = "round387_london_pld_lifecycle_next49";', "round name");
  assertContains(transformed, 'const ROUND = "round387";', "generated round id");
  assertContains(transformed, "MAX_PRIOR_ROUND = 384", "prior cutoff");
  ...
  assertContains(transformed, "through round384", "dedupe prose cutoff");
  return transformed;
}
...
function main() {
  ...
  vm.runInNewContext(
    ...
    {
      filename: "generated_round387_london_pld_lifecycle_next49_candidates.js"
    }
  );
}
