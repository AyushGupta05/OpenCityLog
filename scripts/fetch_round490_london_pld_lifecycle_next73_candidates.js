...
function replaceRequired(value, needle, replacement, label) {
  if (!value.includes(needle)) {
    ...
    throw new Error(`Round490 source transform could not find ${label}: ${needle}`);
  }
...
function transformRound488Script(source) {
  let transformed = source
    .replace(/round488_london_pld_lifecycle_next72/g, "round490_london_pld_lifecycle_next73")
    .replace(/round488\.london_pld_lifecycle_next72/g, "round490.london_pld_lifecycle_next73")
    .replace(/Bims5Round488PldLifecycleNext72/g, "Bims5Round490PldLifecycleNext73")
    .replace(/Round 488/g, "Round 490")
    .replace(/Round488/g, "Round490")
    .replace(/round488/g, "round490")
    .replace(/next72/g, "next73")
    .replace(/MAX_PRIOR_ROUND = 485/g, "MAX_PRIOR_ROUND = 488")
    .replace(/through round485/g, "through round488")
    .replace(/packRound\(entry\.name\) > 485/g, "packRound(entry.name) > 488")
    .replace(/\.replace\(\/round276\/g, "round485"\);/g, '.replace(/round276/g, "round488");');
  ...
  transformed = replaceRequired(
    ...
    "round488 prior pack insertion"
  );

  transformed = transformed
    .replace(
      /pack\) => pack\.label === "round485_london_pld_lifecycle_next71" && pack\.exists === true/g,
      'pack) => pack.label === "round488_london_pld_lifecycle_next72" && pack.exists === true'
    )
    .replace(/round485 prior PLD lifecycle pack was scanned/g, "round488 prior PLD lifecycle pack was scanned");

  assertContains(transformed, 'const ROUND_NAME = "round490_london_pld_lifecycle_next73";', "round name");
  assertContains(transformed, 'const ROUND = "round490";', "transformed fetch round id");
  assertContains(transformed, "MAX_PRIOR_ROUND = 488", "prior cutoff");
  assertContains(transformed, "through round488", "dedupe cutoff text");
  ...
  assertContains(transformed, "addRound490RetrievedAtAlias", "candidate retrieved_at alias");
  assertContains(transformed, 'CURRENT_ROUND_NAME = "round490_london_pld_lifecycle_next73"', "validator round name");
  assertContains(transformed, "round490_strict_duplicate_audit", "strict duplicate audit label");
...
async function main() {
  ...
  vm.runInNewContext(transformed, sandbox, {
    filename: "generated_round490_london_pld_lifecycle_next73_candidates.js"
  });
}
