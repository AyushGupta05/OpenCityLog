...
const OUTPUT_DIR = path.join(__dirname, "..", "tmp", "subagents", "round588_nyc_dob_legacy_permit_next23");
...
function normalizeOutputBoundaryText() {
  ...
  for (const name of fs.readdirSync(OUTPUT_DIR)) {
    ...
    const normalized = text
      .replaceAll("through Round568", "through Round583")
      .replaceAll("treating Round568", "treating Round583")
      .replaceAll("through Round572", "through Round583")
      .replaceAll("treating Round572", "treating Round583")
      .replaceAll("through Round574", "through Round583")
      .replaceAll("treating Round574", "treating Round583")
...
if (sourceHash !== EXPECTED_SOURCE_SHA256) {
  throw new Error(
    `Round588 expected Round572 worker source hash ${EXPECTED_SOURCE_SHA256}, got ${sourceHash}. ` +
      "Review the upstream pattern before regenerating this candidate pack."
  );
}

const round588Source = source
  .replace("const ROUND = 572;", "const ROUND = 588;")
  .replace(
    'const SLUG = "round572_nyc_dob_legacy_permit_next19";',
    'const SLUG = "round588_nyc_dob_legacy_permit_next23";'
  )
  .replace("const DEDUPE_BOUNDARY_ROUND = 568;", "const DEDUPE_BOUNDARY_ROUND = 583;")
  .replaceAll(
    "fetch_round572_nyc_dob_legacy_permit_next19_candidates.js",
    "fetch_round588_nyc_dob_legacy_permit_next23_candidates.js"
  )
  .replaceAll("round572_nyc_dob_legacy_permit_next19", "round588_nyc_dob_legacy_permit_next23")
  .replaceAll("Round572", "Round588")
  .replaceAll('"Round563", "Round568"', '"Round578", "Round583"')
  .replace(
  ...
  .replaceAll("through Round578", "through Round583")
  .replaceAll("treating Round578", "treating Round583")
  .replaceAll("next19", "next23");

vm.runInNewContext(round588Source, {
  __dirname,
  __filename,
  require,
  console,
