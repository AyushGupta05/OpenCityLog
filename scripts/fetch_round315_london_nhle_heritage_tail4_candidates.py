...
OUT_DIR = Path("tmp/subagents/round315_london_nhle_heritage_tail4")
...
SOURCE_ID = "historic-england-nhle-open-data-round315-tail4"
...
def get_json(url, params=None):
    ...
    request = urllib.request.Request(url, headers={"User-Agent": "Bims-5 round315 NHLE tail4 fetcher"})
...
def candidate_from_feature(query, feature, point):
    ...
    candidate_id = f"london-nhle-round315-{layer_slug}-{field_slug}-{list_entry}-{date_compact(event_date)}"
    source_record_id = (
    ...
    return {
        ...
        "transformation_method": (
            "scripts/fetch_round315_london_nhle_heritage_tail4_candidates.py queried the official Historic "
...
def candidate_from_delisted_feature(feature, point):
    ...
    candidate_id = f"london-nhle-round315-de-designated-sites-date-removed-{record_key}-{date_compact(event_date)}"
    source_record_id = (
    ...
    return {
        ...
        "source_dataset_id": f"{DELISTED_SOURCE_ID}-round315-tail4-layer-0",
        "source_layer_id": 0,
...
def make_summary(existing, result):
    ...
    return {
        ...
        "worker_scope": "round315 London Historic England/NHLE heritage-designation tail4",
        ...
        "exhaustion_note": (
            ...
            "the manual architecture corpus and prior candidate packs through round305."
            if not candidates
            else None
        ),
        ...
        "duplicate_screening": {
            ...
            "screening_note": (
                ...
                "prior Historic England/NHLE London heritage packs through round305, for event IDs, source keys, "
        ...
        "critical_interpretation_note": (
            ...
            "occupation, repair, condition, or broader outcome evidence."
        ),
...
def notes_md(summary):
    lines = [
        "# Round315 London NHLE Heritage Tail4",
        "",
        "This pack harvests additional official Historic England/NHLE administrative heritage records for London after the earlier London/NHLE heritage packs through round305.",
        "",
    ...
    lines.extend(
        [
            "",
            "Caveat: these are administrative Historic England/NHLE dates only. Do not treat designation, listing, inscription, certificate/status-start or expiry, amendment, or de-designation/removal dates as construction, demolition, repair, opening, occupation, condition, or broader outcome evidence.",
            "",
        ]
    )
    return "\n".join(lines)
...
def main():
    ...
    pack = {
        "metadata": {
            "schema_version": "round315_london_nhle_heritage_tail4_candidates_v1",
            "generated_at": f"{ACCESS_DATE}T00:00:00Z",
            "city_id": "london",
            "source_id": SOURCE_ID,
            "date_window": {"start": DATE_START, "end": DATE_END},
