...
function replaceRequired(value, needle, replacement, label) {
  if (!value.includes(needle)) {
    ...
    throw new Error(`Round496 source transform could not find ${label}: ${needle}`);
  }
...
function transformRound488Script(source) {
  let transformed = source
    .replace(/round488_london_pld_lifecycle_next72/g, "round496_london_pld_lifecycle_next74")
    .replace(/round488\.london_pld_lifecycle_next72/g, "round496.london_pld_lifecycle_next74")
    .replace(/Bims5Round488PldLifecycleNext72/g, "Bims5Round496PldLifecycleNext74")
    .replace(/Round 488/g, "Round 496")
    .replace(/Round488/g, "Round496")
    .replace(/round488/g, "round496")
    .replace(/next72/g, "next74")
    .replace(/MAX_PRIOR_ROUND = 485/g, "MAX_PRIOR_ROUND = 490")
    .replace(/through round485/g, "through round490")
    .replace(/packRound\(entry\.name\) > 485/g, "packRound(entry.name) > 490")
    .replace(/\.replace\(\/round276\/g, "round485"\);/g, '.replace(/round276/g, "round490");');
  ...
  transformed = replaceRequired(
    ...
    "round488 and round490 prior pack insertion"
  );

  transformed = transformed
    .replace(
      /pack\) => pack\.label === "round485_london_pld_lifecycle_next71" && pack\.exists === true/g,
      'pack) => pack.label === "round490_london_pld_lifecycle_next73" && pack.exists === true'
    )
    .replace(/round485 prior PLD lifecycle pack was scanned/g, "round490 prior PLD lifecycle pack was scanned");

  assertContains(transformed, 'const ROUND_NAME = "round496_london_pld_lifecycle_next74";', "round name");
  assertContains(transformed, 'const ROUND = "round496";', "transformed fetch round id");
  assertContains(transformed, "MAX_PRIOR_ROUND = 490", "prior cutoff");
  assertContains(transformed, "through round490", "dedupe cutoff text");
  ...
  assertContains(transformed, "addRound496RetrievedAtAlias", "candidate retrieved_at alias");
  assertContains(transformed, 'CURRENT_ROUND_NAME = "round496_london_pld_lifecycle_next74"', "validator round name");
  assertContains(transformed, "round496_strict_duplicate_audit", "strict duplicate audit label");
...
async function main() {
  ...
  vm.runInNewContext(transformed, sandbox, {
    filename: "generated_round496_london_pld_lifecycle_next74_candidates.js"
  });
}
