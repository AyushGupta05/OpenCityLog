...
const ROUND_NAME = "round529_london_pld_lifecycle_next82";
...
function replaceRequired(value, needle, replacement, label) {
  if (!value.includes(needle)) {
    ...
    throw new Error(`Round529 source transform could not find ${label}: ${needle}`);
  }
...
function transformRound488Script(source) {
  let transformed = source
    .replace(/round488_london_pld_lifecycle_next72/g, "round529_london_pld_lifecycle_next82")
    .replace(/round488\.london_pld_lifecycle_next72/g, "round529.london_pld_lifecycle_next82")
    .replace(/Bims5Round488PldLifecycleNext72/g, "Bims5Round529PldLifecycleNext82")
    .replace(/Round 488/g, "Round 529")
    .replace(/Round488/g, "Round529")
    .replace(/round488/g, "round529")
    .replace(/next72/g, "next82")
    .replace(/MAX_PRIOR_ROUND = 485/g, "MAX_PRIOR_ROUND = 526")
    .replace(/through round485/g, "through round526")
    .replace(/packRound\(entry\.name\) > 485/g, "packRound(entry.name) > 526")
    .replace(/\.replace\(\/round276\/g, "round485"\);/g, '.replace(/round276/g, "round526");');
  ...
  transformed = replaceRequired(
    ...
    "round488 through round526 prior pack insertion"
  );

  transformed = transformed
    .replace(
      /pack\) => pack\.label === "round485_london_pld_lifecycle_next71" && pack\.exists === true/g,
      'pack) => pack.label === "round526_london_pld_lifecycle_next81" && pack.exists === true'
    )
    .replace(/round485 prior PLD lifecycle pack was scanned/g, "round526 prior PLD lifecycle pack was scanned");

  assertContains(transformed, 'const ROUND_NAME = "round529_london_pld_lifecycle_next82";', "round name");
  assertContains(transformed, 'const ROUND = "round529";', "transformed fetch round id");
  assertContains(transformed, "MAX_PRIOR_ROUND = 526", "prior cutoff");
  assertContains(transformed, "through round526", "dedupe cutoff text");
  ...
  assertContains(transformed, "round504_london_pld_lifecycle_next76", "round504 prior pack");
  assertContains(transformed, "round508_london_pld_lifecycle_next77", "round508 prior pack");
  assertContains(transformed, "round512_london_pld_lifecycle_next78", "round512 prior pack");
  ...
  assertContains(transformed, "addRound529RetrievedAtAlias", "candidate retrieved_at alias");
  assertContains(transformed, 'CURRENT_ROUND_NAME = "round529_london_pld_lifecycle_next82"', "validator round name");
  assertContains(transformed, "round529_strict_duplicate_audit", "strict duplicate audit label");
  ...
  while (Date.now() < deadline) {
    ...
    if (filesReady) {
      try {
        ...
        const candidateCount = candidates.candidate_count || (candidates.candidates || []).length;
        if (
          candidateCount > 0 &&
...
function addRound529SourceRowRefs() {
  ...
  pack.round529_postprocess = {
    source_row_ref_alias_count: aliasCount,
    note: "Round529 wrapper added explicit source_row_ref/source_row_refs aliases from the official PLD row/API URLs after the inherited lifecycle worker completed."
  };
  writeJson(file, pack);
}

function updateRound529Readback() {
  ...
  readback.round529_postprocess_checks = [
...
async function main() {
  ...
  vm.runInNewContext(transformed, sandbox, {
    filename: "generated_round529_london_pld_lifecycle_next82_candidates.js"
  });
  await waitForGeneratedOutputs(startedAtMs);
  addRound529SourceRowRefs();
  updateRound529Readback();
}
