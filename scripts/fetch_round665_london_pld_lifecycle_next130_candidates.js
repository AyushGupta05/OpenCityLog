...
function replaceRequired(source, search, replacement, label) {
  if (!source.includes(search)) {
    throw new Error(`Round665 wrapper transform could not find ${label}: ${search}`);
  }
...
// Advance the current wrapper from Round649/next122 to Round665/next130.
source = replaceRequired(source, "Round649", "Round665", "camel round");
source = replaceRequired(source, "Round 649", "Round 665", "display round");
source = replaceRequired(source, "ROUND649", "ROUND665", "upper round");
source = replaceRequired(source, "round649_london_pld_lifecycle_next122", "round665_london_pld_lifecycle_next130", "round name");
source = replaceRequired(source, "round649.london_pld_lifecycle_next122", "round665.london_pld_lifecycle_next130", "round dotted name");
source = replaceRequired(source, "round649", "round665", "lower round");
source = replaceRequired(source, "next122", "next130", "next label");

// Advance the scratch-pack dedupe boundary so generated Round665 excludes Round663.
// Later PLD rows already appended to the live manual corpus are still excluded by corpus scanning.
source = replaceRequired(source, "MAX_PRIOR_ROUND = 647", "MAX_PRIOR_ROUND = 663", "prior cutoff");
source = replaceRequired(source, "through round647", "through round663", "prior cutoff prose");
source = replaceRequired(source, "packRound(entry.name) > 647", "packRound(entry.name) > 663", "prior pack filter");
source = replaceRequired(source, '.replace(/round276/g, "round647");', '.replace(/round276/g, "round663");', "prior round replacement");
source = replaceRequired(source, "round647 prior PLD lifecycle pack was scanned", "round663 prior PLD lifecycle pack was scanned", "prior scan text");
source = replaceRequired(
  source,
  'pack) => pack.label === "round647_london_pld_lifecycle_next121" && pack.exists === true',
  'pack) => pack.label === "round663_london_pld_lifecycle_next129" && pack.exists === true',
  "prior pack existence check",
);
source = replaceRequired(
  source,
  "round628, round629, round630, round634, round636, round638, round641, round643, round645, and round647",
  "round628, round629, round630, round634, round636, round638, round641, round643, round645, round647, round649, round653, round655, round657, round659, round661, and round663",
  "prior prose list",
);
source = replaceRequired(
  source,
...
if (!source.includes('const ROUND_NAME = "round665_london_pld_lifecycle_next130";')) {
  throw new Error("Round665 wrapper did not advance the generated round name");
}
if (!source.includes("MAX_PRIOR_ROUND = 663") || !source.includes("through round663")) {
  throw new Error("Round665 wrapper did not advance the dedupe boundary");
}
if (!source.includes("round663_london_pld_lifecycle_next129")) {
  throw new Error("Round665 wrapper did not retain Round663 as a prior pack");
}
...
vm.runInNewContext(source, {
  ...
  filename: "generated_round665_wrapper.js",
});
