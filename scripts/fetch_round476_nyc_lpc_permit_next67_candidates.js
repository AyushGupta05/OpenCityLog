...
source = source
  .replace(/const ROUND = 471;/g, "const ROUND = 476;")
  .replace(/round471_nyc_lpc_permit_next66/g, "round476_nyc_lpc_permit_next67")
  .replace(/round471/g, "round476")
  .replace(/Round471/g, "Round476")
  .replace(/next66/g, "next67")
  .replace(/Next66/g, "Next67")
  .replace(/through round466/g, "through round471");
...
function writeRound476DuplicateAudit() {
  if (duplicateAuditWritten || process.exitCode) return;
  duplicateAuditWritten = true;

  const root = path.resolve(__dirname, "..");
  const outDir = path.join(root, "tmp", "subagents", "round476_nyc_lpc_permit_next67");
  ...
  const audit = {
    ...
    audit_scope: "Round476 duplicate audit for NYC LPC Permit Application Information next67 candidates.",
    dedupe_boundary: {
      latest_lpc_permit_pack: "tmp/subagents/round471_nyc_lpc_permit_next66/candidates.json",
...
process.on("beforeExit", writeRound476DuplicateAudit);
