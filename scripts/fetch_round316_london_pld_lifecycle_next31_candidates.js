...
const ROUND_NAME = "round316_london_pld_lifecycle_next31";
...
function assertContains(value, needle, label) {
  if (!value.includes(needle)) {
    throw new Error(`Round316 source transform failed to include ${label}: ${needle}`);
  }
}

function assertAbsent(value, needle, label) {
  if (value.includes(needle)) {
    throw new Error(`Round316 source transform retained ${label}: ${needle}`);
  }
}
...
function replaceRequired(value, needle, replacement, label) {
  if (!value.includes(needle)) {
    ...
    throw new Error(`Round316 source transform could not find ${label}: ${needle}`);
  }
...
function transformRound284Fetch(source) {
  let transformed = source
    .replace(/const ROUND = "round284";/, 'const ROUND = "round316";')
    .replace(
      /const ROUND_NAME = "round284_london_pld_lifecycle_next25";/,
      'const ROUND_NAME = "round316_london_pld_lifecycle_next31";'
    )
    .replace(/const MAX_PRIOR_ROUND = 276;/, "const MAX_PRIOR_ROUND = 311;")
    .replace(/Bims5Round284PldLifecycleNext25/g, "Bims5Round316PldLifecycleNext31")
    .replace(/round284_london_pld_lifecycle_next25/g, "round316_london_pld_lifecycle_next31")
    .replace(/round284\.london_pld_lifecycle_next25/g, "round316.london_pld_lifecycle_next31")
    .replace(/Round284/g, "Round316")
    .replace(/Round 284/g, "Round 316")
    .replace(/round284/g, "round316")
    .replace(/next25/g, "next31")
    .replace(/through round276/g, "through round311")
    .replace(
      /round272, and round276/g,
      "round272, round276, round284, round288, round298, round301, round306, and round311"
    );
  ...
  assertContains(transformed, 'const ROUND = "round316";', "round id");
  ...
  assertContains(transformed, "through round311", "dedupe cutoff");
...
function transformRound284Validator(source) {
  let transformed = source
    ...
    .replace(
      ...
      'const CURRENT_ROUND_NAME = "round316_london_pld_lifecycle_next31";'
    )
    .replace(/round284_london_pld_lifecycle_next25/g, "round316_london_pld_lifecycle_next31")
    .replace(/round284/g, "round316")
    .replace(/round284:/g, "round316:")
    .replace(/round276/g, "round311");
  ...
  assertContains(transformed, "MAX_PRIOR_ROUND = 311", "validator cutoff");
  assertContains(transformed, 'CURRENT_ROUND_NAME = "round316_london_pld_lifecycle_next31"', "validator current round");
...
async function runTransformedFetch() {
  ...
  vm.runInNewContext(transformed, sandbox, {
    filename: "generated_round316_london_pld_lifecycle_next31_candidates.js"
  });
  if (typeof sandboxModule.exports !== "function") {
    throw new Error("Round316 transform did not export the fetch main() function.");
  }
...
function writeValidator() {
  ...
  fs.writeFileSync(path.join(OUT_DIR, "validate_round316_pack.js"), transformed);
}
