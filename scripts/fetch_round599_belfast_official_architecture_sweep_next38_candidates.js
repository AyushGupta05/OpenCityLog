...
const ROUND_ID = "round599_belfast_official_architecture_sweep_next38";
const OUT_DIR = path.join("tmp", "subagents", ROUND_ID);
const SCRIPT_PATH =
  "scripts/fetch_round599_belfast_official_architecture_sweep_next38_candidates.js";
const GENERATED_AT = "2026-05-20";
const ACCESSED_AT = "2026-05-20";
const CITY_ID = "belfast";
const TARGET_CANDIDATE_COUNT = 30;
const DEDUPE_BOUNDARY_ROUND = 592;
const DATE_WINDOW = {
  start: "2008-01-01",
  end: "2026-05-20"
};
...
const DFI_2023_24_PUBLICATION_URL =
  "https://www.infrastructure-ni.gov.uk/publications/northern-ireland-planning-statistics-april-2023-march-2024";
const DFI_2023_24_CSV_URL =
  "https://www.infrastructure-ni.gov.uk/system/files/publications/infrastructure/planning-statistics-2023-24-dataset.csv";
...
const PLANNING_DATASETS = [
  {
    key: "2024-25",
    file_name: "planning-statistics-2024-25-dataset.csv",
    source_id: "dfi-planning-statistics-2024-25-round599",
  ...
  {
    key: "2023-24",
    file_name: "planning-statistics-2023-24-dataset.csv",
    source_id: "dfi-planning-statistics-2023-24-round599",
    source_name: "Northern Ireland planning statistics 2023/24 annual dataset",
    source_url: DFI_2023_24_PUBLICATION_URL,
    source_dataset_url: DFI_2023_24_CSV_URL,
    coverage_years: "2023-2024",
    publication_date: "2024-08-01"
  }
].map((dataset) => ({
  ...dataset,
  local_path: path.join("data", "raw", "planning_statistics", dataset.file_name),
  ...
  geographic_scope:
    "Belfast planning authority rows in Northern Ireland planning statistics",
  granularity:
...
const METHOD = [
  "Round599 official Belfast architecture sweep next38 after the completed Round592 next37 pack.",
  "Accepted records are selected from residual Department for Infrastructure Northern Ireland annual planning-statistics Belfast rows only where the source row supplies an approved planning, listed-building, demolition-consent or other-consent decision date and official Easting/Northing.",
  ...
  "Round592 is treated as the latest Belfast official architecture sweep dedupe boundary; official page-only, geometry-ref-only, aggregate, duplicate-project, status-only, signage-only, equipment-only, condition-only and lower-priority private-use leads are retained separately."
].join(" ");

const TRANSFORMATION_METHOD = `${SCRIPT_PATH}#round599OfficialArchitectureSweepNext38`;

const SOURCES = {
  dfiPlanningStats: {
    source_id: "dfi-planning-statistics-annual-round599",
    source_name: "Northern Ireland planning statistics annual datasets",
    publisher: "Department for Infrastructure, Northern Ireland",
    source_url: DFI_PLANNING_ACTIVITY_URL,
    ...
    attribution:
      "Contains public sector information from the Department for Infrastructure licensed under the Open Government Licence v3.0 where applicable.",
    publisher_terms_url: DFI_TERMS_URL,
    coverage_years: "2023-2025 annual CSVs checked for this sweep",
...
const SEARCH_QUERIES_CHECKED = [
  "local DfI planning-statistics annual Belfast residual APP_ID scan after Round592 accepted/rejected/readback dedupe",
  "local duplicate scan: data/manual_drops/architecture_milestones plus official Belfast architecture sweep candidate/rejected/readback packs through Round592",
  "source review: DfI planning activity statistics page plus 2024/25 and 2023/24 annual application-level CSVs",
...
function canonicalHeader(header) {
  ...
  if (key === "Status@31Mar") return "StatusAt31Mar";
  if (key === "Id") return "ID";
  if (key === "Location") return "SiteAddress";
  if (key === "AppCategory") return "Classification";
  return key;
}
...
function parseCsv(text, dataset) {
  ...
  return rows.slice(1).map((values, index) => {
    ...
    headers.forEach((header, columnIndex) => {
      record[header] = values[columnIndex] === undefined ? "" : values[columnIndex].trim();
    });
    record.row_number = index + 2;
    record.source_file_name = dataset.file_name;
    record.source_file_path = dataset.local_path;
    record.source_dataset_key = dataset.key;
    record.source_publication_date = dataset.publication_date;
    record.source_url = dataset.source_url;
    record.source_dataset_url = dataset.source_dataset_url;
    record.source_id = dataset.source_id;
    record.source_name = dataset.source_name;
    record.source_coverage_years = dataset.coverage_years;
    return record;
  });
}

function readPlanningRows() {
  return PLANNING_DATASETS.flatMap((dataset) => {
    const text = readTextIfExists(dataset.local_path);
    if (!text) throw new Error(`Missing planning CSV: ${dataset.local_path}`);
    return parseCsv(text, dataset);
  });
}
...
function sourceRecordIdFor(row) {
  return [
    `APP_ID:${row.ID}`,
    `FILE:${row.source_file_path.replace(/\\/g, "/")}`,
    `ROW:${row.row_number}`
  ].join("; ");
}

function planningSourceForRow(row) {
  const dataset = PLANNING_DATASETS.find((item) => item.key === row.source_dataset_key);
  if (!dataset) {
    throw new Error(`Missing source metadata for planning row ${row.ID}`);
  }
  return dataset;
}
...
function candidateEventId(row, decisionDate) {
  ...
  return `bfs_arch_round599_${site}_${appKey}_approval_${decisionDate.slice(0, 4)}`;
}
...
function candidateFromPlanningRow(item, duplicateHaystack) {
  const row = item.row;
  const source = planningSourceForRow(row);
  ...
  return {
    ...
    source_fields: {
      ...
      source_publication_url: row.source_url,
    ...
    provenance_links: [
      ...
      { rel: "annual_publication", href: source.source_url },
      { rel: "source_dataset", href: source.source_dataset_url },
      { rel: "license", href: OGL_URL },
      { rel: "coordinate_reference_system", href: EPSG_29902_URL }
    ],
    duplicate_check_terms: duplicateTerms,
    duplicate_review:
      `APP_ID, event_id and source_record_id were absent from the manual architecture corpus and official Belfast architecture sweep candidate/rejected/readback outputs through Round${DEDUPE_BOUNDARY_ROUND} checked by the Round599 duplicate scan.`,
...
function sampleSourceRecordId(rows, label) {
  ...
  const files = [
    ...new Set(
      rows
        .map((item) => (item.row ? item.row.source_file_path : item.source_file_path))
        .filter(Boolean)
        .map((filePath) => filePath.replace(/\\/g, "/"))
    )
  ];
  const fileSummary = files.length
    ? files.map((filePath) => `FILE:${filePath}`).join("; ")
    : PLANNING_DATASETS.map((dataset) => `FILE:${dataset.local_path.replace(/\\/g, "/")}`).join("; ");
  if (!ids.length) return `${label}: no sampled APP_IDs; ${fileSummary}`;
  return `EXAMPLES:${ids.map((id) => `APP_ID:${id}`).join("; ")}; ${fileSummary}`;
}
...
function planningRejectRecord(idSuffix, title, rejectionCategory, reason, rows, duplicateHaystack, terms) {
  ...
  const id = `bfs_arch_round599_reject_${idSuffix}`;
  ...
  return {
    ...
    reason,
    source_id: `round599-reject-${idSuffix}`,
...
function buildRejectedRecords(rows, duplicateHaystack, eligible, selected) {
  ...
  const rejected = [
    planningRejectRecord(
      ...
      duplicateRows,
      duplicateHaystack,
      ["Round592", "APP_ID duplicate", "manual architecture corpus"]
    )
  ];
  ...
  rejected.push(
    planningRejectRecord(
      ...
      `Additional non-duplicate, point-backed DfI rows passed the Round599 architecture filters but were retained outside the promoted pack to keep this next sweep close to the requested ${TARGET_CANDIDATE_COUNT}-candidate target. They may be reconsidered in a later residual pass.`,
      tailRows,
      duplicateHaystack,
  ...
  rejected.push(
    staticRejectRecord(
      {
        id: "bfs_arch_round599_reject_page_only_or_no_point_official_leads",
        ...
        source_id: "round599-reject-page-only-or-no-point-official-leads",
        source_record_id:
  ...
  rejected.push(
    staticRejectRecord(
      {
        id: "bfs_arch_round599_reject_aggregate_only_dfi_quarterly_sources",
        ...
        source_id: "round599-reject-aggregate-only-dfi-quarterly-sources",
        source_record_id:
  ...
  rejected.push(
    staticRejectRecord(
      {
        id: "bfs_arch_round599_reject_hed_status_or_geometry_ref_only_leads",
        ...
        source_id: "round599-reject-hed-status-or-geometry-ref-only-leads",
        source_record_id:
          "HED_FEATURE_SERVICE: Historic Buildings layer and Heritage at Risk layer reviewed; no new dated point-event rows promoted in this DfI residual next38 pack",
...
function validate(candidates, rejected, duplicateHaystack, expectedCandidateCount) {
  ...
  if (candidates.length !== expectedCandidateCount) {
    errors.push(
      `Expected ${expectedCandidateCount} promoted next38 candidates, found ${candidates.length}`
    );
  }
...
function buildSourceAudit(candidates, rejected, duplicateHaystack) {
  return {
    ...
    sources_checked: [
      ...PLANNING_DATASETS.map((dataset) => {
        const acceptedForDataset = candidates.filter(
          (candidate) => candidate.source_fields.source_dataset_key === dataset.key
        );
        return {
          ...dataset,
          local_file: dataset.local_path,
          local_file_sha256: sha256(readTextIfExists(dataset.local_path)),
          accepted_records: acceptedForDataset.length,
          accepted_app_ids: acceptedForDataset.map(
            (candidate) => candidate.source_fields.ID
          ),
          disposition: acceptedForDataset.length
            ...
            : `checked for residual point-backed Belfast planning/statutory-consent rows after duplicate review through Round${DEDUPE_BOUNDARY_ROUND}; no rows promoted from this annual CSV in Round599`
        };
      }),
      {
        ...SOURCES.epsg29902,
      ...
      {
        source_id: "dfi-planning-statistics-2025-26-quarterly-reviewed-round599",
        source_name: "Northern Ireland planning statistics 2025/26 provisional quarterly publications",
        publisher: "Department for Infrastructure, Northern Ireland",
        source_url: DFI_2025_26_Q3_URL,
      ...
      {
        source_id: "hed-historic-buildings-and-harni-reviewed-round599",
        source_name: "Historic Environment Division GIS Data",
        publisher: "Department for Communities Historic Environment Division",
        source_url: HED_FEATURE_SERVICE_URL,
        layer_urls: [HED_HISTORIC_BUILDINGS_LAYER_URL, HED_HERITAGE_AT_RISK_LAYER_URL],
      ...
      {
        source_id: "bcc-and-planning-portal-pages-reviewed-round599",
        source_name: "Northern Ireland Planning Portal and Belfast City Council project pages",
        publisher: "Department for Infrastructure / Belfast City Council planning authority",
        source_url: PLANNING_PORTAL_URL,
