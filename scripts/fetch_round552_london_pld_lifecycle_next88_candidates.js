...
function replaceRequired(value, needle, replacement, label) {
  if (!value.includes(needle)) {
    ...
    throw new Error(`Round552 wrapper transform could not find ${label}: ${needle}`);
  }
...
function transformRound529Wrapper(source) {
  let transformed = source
    .replace(/round529_london_pld_lifecycle_next82/g, "round552_london_pld_lifecycle_next88")
    .replace(/round529\.london_pld_lifecycle_next82/g, "round552.london_pld_lifecycle_next88")
    .replace(/Bims5Round529PldLifecycleNext82/g, "Bims5Round552PldLifecycleNext88")
    .replace(/Round 529/g, "Round 552")
    .replace(/Round529/g, "Round552")
    .replace(/round529/g, "round552")
    .replace(/next82/g, "next88")
    .replace(/MAX_PRIOR_ROUND = 526/g, "MAX_PRIOR_ROUND = 550")
    .replace(/through round526/g, "through round550")
    .replace(/packRound\(entry\.name\) > 526/g, "packRound(entry.name) > 550")
    .replace(/\.replace\(\/round276\/g, "round526"\);/g, '.replace(/round276/g, "round550");');
  ...
  transformed = replaceRequired(
    ...
    "round529, round534, round537, round542, round547, and round550 prior pack insertion"
  );

  transformed = transformed
    .replace(
      /pack\) => pack\.label === "round526_london_pld_lifecycle_next81" && pack\.exists === true/g,
      'pack) => pack.label === "round550_london_pld_lifecycle_next87" && pack.exists === true'
    )
    .replace(/round526 prior PLD lifecycle pack was scanned/g, "round550 prior PLD lifecycle pack was scanned");

  assertContains(transformed, 'const ROUND_NAME = "round552_london_pld_lifecycle_next88";', "round name");
  assertContains(transformed, 'const ROUND = "round552";', "transformed fetch round id");
  assertContains(transformed, "MAX_PRIOR_ROUND = 550", "prior cutoff");
  assertContains(transformed, "through round550", "dedupe cutoff text");
  ...
  assertContains(transformed, "addRound552RetrievedAtAlias", "candidate retrieved_at alias");
  assertContains(transformed, 'CURRENT_ROUND_NAME = "round552_london_pld_lifecycle_next88"', "validator round name");
  assertContains(transformed, "round552_strict_duplicate_audit", "strict duplicate audit label");
...
async function main() {
  ...
  vm.runInNewContext(transformed, sandbox, {
    filename: "generated_round552_london_pld_lifecycle_next88_candidates.js"
  });
}
