...
const SCRIPT_PATH = path.join("scripts", "fetch_round480_nyc_dob_now_next32_candidates.js");
const OUT_DIR = path.join("tmp", "subagents", "round480_nyc_dob_now_next32");
...
const DOB_NOW_ROUNDS_THROUGH_473 = [
...
const PRIOR_DOB_ROUND_REGEX_THROUGH_473 = [
  "round112",
  "round117",
  "round119",
  "round133",
...
function replaceRequired(source, needle, replacement, label) {
  if (!source.includes(needle)) {
    throw new Error(`Round480 template patch failed; missing ${label || needle}`);
  }
...
function replaceOnce(source, needle, replacement, label) {
  ...
  if (count !== 1) {
    throw new Error(`Round480 template patch failed for ${label}: expected one match, found ${count}`);
  }
...
function transformRound149Source(source) {
  ...
  transformed = replaceRequired(transformed, "below_high_signal_scale_threshold", "below_round480_architecture_signal_threshold", "rejection threshold label");
  ...
  const priorText = "manual corpus plus prior DOB NOW/DOB permit/DOB Certificate of Occupancy candidate packs through Round473";
  const priorSlash = "manual-corpus/DOB NOW/DOB permit/DOB CO packs through Round473";
  ...
  transformed = replaceRequired(transformed, "round133|round136|round143", PRIOR_DOB_ROUND_REGEX_THROUGH_473, "prior round regex");
  transformed = replaceRequired(transformed, "screened_round133_round136_round143_files", "screened_prior_candidate_files_through_round473", "prior summary field");

  const duplicateFilterHelper = `
function round480AllowedDuplicateIndexFile(file) {
  const normalized = String(file || "").replace(/\\\\/g, "/");
  if (normalized.startsWith("${OUT_DIR_POSIX}")) return false;
  const match = normalized.match(/round(\\d+)/i);
  if (!match) return true;
  return Number(match[1]) <= 473;
}
...
`;
  ...
  transformed = replaceOnce(
    ...
    '  const candidateFiles = walkJsonCandidateFiles("tmp/subagents").filter(round480AllowedDuplicateIndexFile);',
    "duplicate index cutoff"
  );

  transformed = transformed
    .replace(/round149/g, "round480")
    .replace(/Round149/g, "Round480")
    .replace(/nyc_dob_now_next/g, "nyc_dob_now_next32")
    .replace(/NYC DOB NOW Next/g, "NYC DOB NOW Next32")
    .replace(/NYC DOB NOW next/g, "NYC DOB NOW next32")
    .replace(/Round480 generator queried DOB NOW/g, "Round480 generator queried official DOB NOW")
    ...
    .replace(/round480_nyc_dob_now_next3232/g, "round480_nyc_dob_now_next32")
    .replace(/nyc_dob_now_next3232/g, "nyc_dob_now_next32");
...
async function runTransformedTemplateMain() {
  ...
  if (typeof sandbox.module.exports.main !== "function") {
    throw new Error("Transformed Round480 DOB NOW generator did not export main().");
  }
...
function isRequestedPriorDobFile(file) {
  ...
  if (!round || round > 473) return false;
...
function corpusStats() {
  const stats = {
    ...
    records: 0,
    contains_round473: false,
    round473_records: 0
  };
  if (!stats.present) return stats;
  const records = candidateArray(readJson(CORPUS_PATH));
  stats.records = records.length;
  stats.round473_records = records.filter((record) => {
    const text = JSON.stringify(record);
    return text.includes("round473") || text.includes("Round473") || text.includes("nyc_dob_now_next31");
  }).length;
  stats.contains_round473 = stats.round473_records > 0;
  return stats;
}
...
function buildRequestedDuplicateScopeCheck(candidates) {
  ...
  return {
    schema_version: "round480.nyc_dob_now_next32_strict_duplicate_audit.v1",
    generated_at: GENERATED_AT,
    accessed_at: ACCESSED_AT,
    scope: "Manual architecture corpus plus NYC DOB NOW/DOB permit/DOB Certificate of Occupancy candidate packs with round number <= 473, including tmp/subagents/round473_nyc_dob_now_next31/candidates.json.",
    ...
    round473_candidates_checked: priorDobFiles.some((file) => /round473_nyc_dob_now_next31\/candidates\.json/i.test(file)),
...
function normalizeCandidateArtifacts(candidatesDoc, strictAudit) {
  ...
  for (const candidate of candidates) {
    ...
    candidate.duplicate_check_note = "Exact event_id, source URL/record ID, DOB NOW job/permit/tracking identifiers, source-date tokens, and title/date/site keys were screened against the manual corpus plus NYC DOB NOW/DOB permit/DOB CO candidate packs through Round473.";
    candidate.transformation_method = `Round480 queried official NYC Open Data DOB NOW approved permit rows (${DATASET_ENDPOINTS.permits.id}) for ${START_DATE} through ${END_DATE}, joined retained permit rows to DOB NOW job application filing rows (${DATASET_ENDPOINTS.applications.id}), kept one nonduplicate architecture-signal administrative permit row per base DOB NOW job, and preserved row-level permit/application provenance. Duplicate scope: manual corpus plus prior DOB administrative candidate packs through Round473.`;
  ...
  candidatesDoc.selection_summary = {
    ...
    prior_scope: {
      ...
      dob_now_rounds_through: "round473",
      dob_now_rounds_screened: DOB_NOW_ROUNDS_THROUGH_473,
...
function patchGeneratedArtifacts() {
  ...
  const sourceAudits = (candidatesDoc.source_audits || []).map((source) => ({
    ...
    granularity: source.dataset_id === DATASET_ENDPOINTS.permits.id
      ? "One approved permit row; this generator retains one substantial initial permit row per base DOB NOW job filing beyond the manual corpus and DOB administrative candidate packs through Round473."
  ...
  sourceAuditDoc.schema_version = "round480.nyc_dob_now_next32_source_audit.v1";
  sourceAuditDoc.generated_at = GENERATED_AT;
  sourceAuditDoc.accessed_at = ACCESSED_AT;
  sourceAuditDoc.audit_scope = "Official NYC DOB NOW Build approved permits and job application filings, selected for residual administrative milestone candidate discovery after screening the manual corpus and DOB administrative candidate packs through Round473.";
  ...
  summaryDoc.schema_version = "round480.nyc_dob_now_next32_summary.v1";
  summaryDoc.generated_at = GENERATED_AT;
  summaryDoc.accessed_at = ACCESSED_AT;
  summaryDoc.output_files = ALL_OUTPUTS;
  summaryDoc.worker = "Round480 NYC DOB NOW Next32 candidates";
  ...
  summaryDoc.duplicate_screening = {
    ...
    cutoff_round: 473,
    round473_candidates_checked: strictAudit.round473_candidates_checked
  };
  ...
  rejectedDoc.schema_version = "round480.nyc_dob_now_next32_rejections.v1";
  ...
  const notes = [
    ...
    "## Duplicate Screening",
    "",
    `Strict requested audit checked ${strictAudit.prior_dob_files_checked} prior DOB administrative candidate files plus the manual corpus through Round473. Round473 pack checked: ${strictAudit.round473_candidates_checked}.`,
...
function buildReadbackAndValidation() {
  ...
  if (strictAudit.round473_candidates_checked !== true) errors.push("strict duplicate audit did not confirm Round473 screening");
  ...
  const readback = {
    schema_version: "round480.nyc_dob_now_next32_readback.v1",
    generated_at: GENERATED_AT,
    accessed_at: ACCESSED_AT,
    ok: errors.length === 0,
  ...
  const validation = {
    schema_version: "round480.nyc_dob_now_next32_validation.v1",
    generated_at: GENERATED_AT,
    accessed_at: ACCESSED_AT,
    ok: errors.length === 0,
  ...
  const validationReport = {
    schema_version: "round480.nyc_dob_now_next32_validation_report.v1",
    ok: errors.length === 0,
    passed: errors.length === 0,
    checked_at: GENERATED_AT,
    accessed_at: ACCESSED_AT,
    validator: "scripts/fetch_round480_nyc_dob_now_next32_candidates.js --validate-only; validates emitted JSON, strict requested duplicate scope, provenance fields, point geometry, date window, and administrative-record caveats.",
    checked_files: ALL_OUTPUTS,
  ...
  if (errors.length) {
    throw new Error(`Round480 validation failed with ${errors.length} errors. See ${path.join(OUT_DIR, "validation.json")}`);
  }
...
async function main() {
  ...
  console.log(`Round480 validation passed for ${validation.checks.candidate_count} candidates.`);
}
