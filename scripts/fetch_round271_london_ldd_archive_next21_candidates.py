...
TASK_ID = "round271_london_ldd_archive_next21_candidates"
SCRIPT_PATH = "scripts/fetch_round271_london_ldd_archive_next21_candidates.py"
OUT_DIR = Path("tmp/subagents/round271_london_ldd_archive_next21")
...
ROUND260_SCRIPT_PATH = Path("scripts/fetch_round260_london_ldd_archive_next20_candidates.py")


def build_round_replacements():
    replacements = {
        "round260_london_ldd_archive_next20_candidates": TASK_ID,
        "scripts/fetch_round260_london_ldd_archive_next20_candidates.py": SCRIPT_PATH,
        "tmp/subagents/round260_london_ldd_archive_next20": OUT_DIR.as_posix(),
        "Round 260 London LDD archive next20 candidates": "Round 271 London LDD archive next21 candidates",
        "2026-05-19": ACCESSED_AT,
        "a administrative": "an administrative",
        "selected completion, started, or permission milestone dates": "selected permission milestone dates",
        "selected completion, work-start, or permission milestone records": "selected permission milestone records",
        "selected `Permission Date`, `Date work commenced on site (Started Date)`, or `Date construction completed (Completed Date)`": "selected LDD `Permission Date`",
    ...
    for round_number in [176, 182, 188, 194, 200, 206, 212, 220, 226, 233, 239, 245, 252, 260]:
        replacements[f"through round{round_number}"] = "through round260"
        replacements[f"after round{round_number}"] = "after round260"
        replacements[f"same fields inside the round{round_number} batch scan"] = (
            "same fields inside the round271 batch scan"
        )
        replacements[f"duplicate inside round{round_number} batch scan"] = "duplicate inside round271 batch scan"
    for next_number, round_number in [
        ...
        replacements[f"Round {round_number} London LDD archive next{next_number} candidates"] = (
            "Round 271 London LDD archive next21 candidates"
        )
...
def load_round260_module():
    spec = importlib.util.spec_from_file_location("round260_ldd_next20", ROUND260_SCRIPT_PATH)
    if not spec or not spec.loader:
        raise RuntimeError(f"Unable to load {ROUND260_SCRIPT_PATH}")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


round260 = load_round260_module()
original_rewrite_candidate = round260.rewrite_candidate


def configure_round271_globals():
    round260.ACCESSED_AT = ACCESSED_AT
    round260.SOURCE_ID = SOURCE_ID
    round260.TASK_ID = TASK_ID
    round260.SCRIPT_PATH = SCRIPT_PATH
    round260.OUT_DIR = OUT_DIR
    round260.OUT_PATH = OUT_PATH
    round260.SOURCE_AUDIT_PATH = SOURCE_AUDIT_PATH
    round260.SUMMARY_PATH = SUMMARY_PATH
    round260.NOTES_PATH = NOTES_PATH
    round260.REJECTED_PATH = REJECTED_PATH
    round260.ROUND_REPLACEMENTS = ROUND_REPLACEMENTS


def base_module():
    return round260.base_module()
...
def prior_ldd_pack_paths():
    ...
    for path in sorted(Path("tmp/subagents").glob("round*_london_ldd_archive*/candidates.json")):
        ...
        if not match or int(match.group(1)) > 260:
            continue
        paths.append(path)
    return paths
...
def rewrite_candidate(candidate, row, excel_row, milestone, milestone_date):
    ...
    event_id = (
        f"lon_ldd_archive_next21_round271_{milestone}_row_{excel_row}_"
...
def candidate_limit():
    return round260.candidate_limit()


def write_notes(candidate_count, summary):
    exclusion_counts = apply_replacements(summary.get("exclusion_counts", {}))
    text = f"""# Round 271 London LDD archive next21 candidates

Generated the next bounded London Development Database archive candidate pack after round260. This pack selects source-defined permission milestone records, while deduplicating against the manual corpus and previous/pending LDD archive packs through round260, including the round239, round245, round252, and round260 packs.
...
The generator excludes manual-corpus LDD records and previous/pending LDD archive candidate packs through round260 by workbook row, planning authority plus borough reference, source URL plus source record id, candidate title/date, and source/date key. It explicitly screens the prior/pending LDD packs through round239, round245, round252, and round260, and removes duplicate event IDs and source/date keys inside this batch.
...
def postprocess_outputs():
    for path in [OUT_PATH, SOURCE_AUDIT_PATH, SUMMARY_PATH, REJECTED_PATH]:
        ...
        payload = round260.normalize_permission_only_metadata(payload)
...
def main():
    configure_round271_globals()
    round260.prior_ldd_pack_paths = prior_ldd_pack_paths
    round260.rewrite_candidate = rewrite_candidate
    round260.write_notes = write_notes
    round260.postprocess_outputs = postprocess_outputs
    round260.main()
