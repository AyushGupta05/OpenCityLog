...
const ROUND_NAME = "round323_london_pld_lifecycle_next32";
...
function assertContains(value, needle, label) {
  if (!value.includes(needle)) {
    throw new Error(`Round323 source transform failed to include ${label}: ${needle}`);
  }
}

function assertAbsent(value, needle, label) {
  if (value.includes(needle)) {
    throw new Error(`Round323 source transform retained ${label}: ${needle}`);
  }
}
...
function replaceRequired(value, needle, replacement, label) {
  if (!value.includes(needle)) {
    ...
    throw new Error(`Round323 source transform could not find ${label}: ${needle}`);
  }
...
function transformRound284Fetch(source) {
  let transformed = source
    .replace(/const ROUND = "round284";/, 'const ROUND = "round323";')
    .replace(
      /const ROUND_NAME = "round284_london_pld_lifecycle_next25";/,
      'const ROUND_NAME = "round323_london_pld_lifecycle_next32";'
    )
    .replace(/const MAX_PRIOR_ROUND = 276;/, "const MAX_PRIOR_ROUND = 316;")
    .replace(/Bims5Round284PldLifecycleNext25/g, "Bims5Round323PldLifecycleNext32")
    .replace(/round284_london_pld_lifecycle_next25/g, "round323_london_pld_lifecycle_next32")
    .replace(/round284\.london_pld_lifecycle_next25/g, "round323.london_pld_lifecycle_next32")
    .replace(/Round284/g, "Round323")
    .replace(/Round 284/g, "Round 323")
    .replace(/round284/g, "round323")
    .replace(/next25/g, "next32")
    .replace(/through round276/g, "through round316")
    .replace(
      /round272, and round276/g,
      "round272, round276, round284, round288, round298, round301, round306, round311, and round316"
    );
  ...
  assertContains(transformed, 'const ROUND = "round323";', "round id");
  ...
  assertContains(transformed, "round311_london_pld_lifecycle_next30", "prior round311 pack");
  assertContains(transformed, "round316_london_pld_lifecycle_next31", "prior round316 pack");
  assertContains(transformed, "through round316", "dedupe cutoff");
...
function transformRound284Validator(source) {
  let transformed = source
    ...
    .replace(
      ...
      'const CURRENT_ROUND_NAME = "round323_london_pld_lifecycle_next32";'
    )
    .replace(/round284_london_pld_lifecycle_next25/g, "round323_london_pld_lifecycle_next32")
    .replace(/round284/g, "round323")
    .replace(/round284:/g, "round323:")
    .replace(/round276/g, "round316");
  ...
  assertContains(transformed, "MAX_PRIOR_ROUND = 316", "validator cutoff");
  assertContains(transformed, 'CURRENT_ROUND_NAME = "round323_london_pld_lifecycle_next32"', "validator current round");
...
async function runTransformedFetch() {
  ...
  vm.runInNewContext(transformed, sandbox, {
    filename: "generated_round323_london_pld_lifecycle_next32_candidates.js"
  });
  if (typeof sandboxModule.exports !== "function") {
    throw new Error("Round323 transform did not export the fetch main() function.");
  }
...
function writeValidator() {
  ...
  fs.writeFileSync(path.join(OUT_DIR, "validate_round323_pack.js"), transformed);
}
...
function normalizeSourceAuditProvenance() {
  ...
  const sourceAudit = JSON.parse(fs.readFileSync(file, "utf8").replace(/^\uFEFF/, ""));
  const license =
  ...
  fs.writeFileSync(file, `${JSON.stringify(sourceAudit, null, 2)}\n`);
}
