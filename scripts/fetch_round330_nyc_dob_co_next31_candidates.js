...
const GENERATED_FILENAME = "scripts/fetch_round330_nyc_dob_co_next31_candidates.generated.js";
...
function transformRound308Source(source) {
  let transformed = source.replace(/\r\n/g, "\n")
    .replaceAll("round308", "round330")
    .replaceAll("Round308", "Round330")
    .replaceAll("next26", "next31")
    .replaceAll("Next26", "Next31")
    .replaceAll("through round303", "through round326");
  ...
  transformed = replaceFirst(
    ...
    `${round303File}\n];`,
    `${priorRoundFiles}\n];`,
    "explicit duplicate Round308/Round313/Round318/Round322/Round326 files"
  );
  transformed = replaceOnce(
    transformed,
    `${round303File}\n];`,
    `${priorRoundFiles}\n];`,
    "required Round308/Round313/Round318/Round322/Round326 screened files"
  );
  transformed = replaceOnce(
    transformed,
    '  "300",\n  "303"\n];',
    '  "300",\n  "303",\n  "308",\n  "313",\n  "318",\n  "322",\n  "326"\n];',
    "screened rounds include Round308, Round313, Round318, Round322, and Round326"
  );
  transformed = replaceOnce(
    transformed,
    ...
    "Round308/Round313/Round318/Round322/Round326 validation check fields"
  );
  transformed = replaceOnce(
    transformed,
    "  const sourceAuditEntry = {\n    ...metadata,\n    reliability:",
    "  const sourceAuditEntry = {\n    ...metadata,\n    accessed_at: ACCESSED_AT,\n    confidence: \"documented\",\n    method: \"Round330 queried the official NYC Open Data legacy DOB Certificate Of Occupancy API, filtered and grouped issued rows, screened duplicates, and retained row-level provenance.\",\n    transformation_method: \"Round330 queried the official NYC Open Data legacy DOB Certificate Of Occupancy API, filtered and grouped issued rows, screened duplicates, and retained row-level provenance.\",\n    limitations: \"Legacy CO issuance is an administrative/legal DOB record. It is not actual occupancy, public opening, completion for all spaces, construction completion, final built form, safety outcome, or affordability outcome. Coordinates are DOB/Open Data geocoded address points.\",\n    reliability:",
  ...
  transformed = replaceOnce(
    ...
    "Round308/Round313/Round318/Round322/Round326 validation report lines"
  );
  transformed = replaceOnce(
    transformed,
    "main().catch((error) => {\n  console.error(error);\n  process.exit(1);\n});",
  ...
  assertContains(transformed, "round330_nyc_dob_co_next31", "Round330 output path");
  assertContains(transformed, "Round330 NYC DOB CO Next31 Candidate Pack", "Round330 notes title");
  assertContains(transformed, "nyc_dob_co_round330_legacy_", "Round330 candidate ID prefix");
  ...
  assertContains(transformed, '  "308",\n  "313",\n  "318",\n  "322",\n  "326"\n];', "screened rounds include 308, 313, 318, 322, and 326");
  assertContains(transformed, "through round326", "duplicate-screening text through Round326");
  ...
  assertContains(transformed, "transformation_method: \"Round330 queried", "source audit transformation method");
  assertContains(transformed, "scripts/fetch_round330_nyc_dob_co_next31_candidates.js standalone validation artifact", "Round330 validator label");
...
async function main() {
  ...
  if (typeof sandbox.module.exports.main !== "function") {
    throw new Error("Transformed Round330 generator did not export main().");
  }
