...
function replaceRequired(value, needle, replacement, label) {
  if (!value.includes(needle)) {
    ...
    throw new Error(`Round380 source transform could not find ${label}: ${needle}`);
  }
...
function assertContains(value, needle, label) {
  if (!value.includes(needle)) {
    throw new Error(`Round380 source transform failed to include ${label}: ${needle}`);
  }
}

function transformRound362Script(source) {
  let transformed = source
    .replace(/round362_london_pld_lifecycle_next42/g, "round380_london_pld_lifecycle_next47")
    .replace(/round362\.london_pld_lifecycle_next42/g, "round380.london_pld_lifecycle_next47")
    .replace(/Bims5Round362PldLifecycleNext42/g, "Bims5Round380PldLifecycleNext47")
    .replace(/Round362/g, "Round380")
    .replace(/Round 362/g, "Round 380")
    .replace(/round362/g, "round380")
    .replace(/next42/g, "next47")
    .replace(/MAX_PRIOR_ROUND = 358/g, "MAX_PRIOR_ROUND = 377")
    .replace(/through round358/g, "through round377");
  ...
  transformed = replaceRequired(
    ...
    '      "round272, round276, round284, round288, round298, round301, round306, round311, round316, round323, round327, round328, round333, round338, round343, round348, round350, round354, round358, round362, round365, round369, round373, and round377"\\n    );',
  ...
  transformed = replaceRequired(
    ...
    "prior round327 through round377 fetch assertions"
  );

  transformed = replaceRequired(
    transformed,
    '    .replace(/round276/g, "round358");',
    '    .replace(/round276/g, "round377");',
    "validator prior cutoff prose replacement"
  );

  assertContains(transformed, 'const ROUND_NAME = "round380_london_pld_lifecycle_next47";', "round name");
  assertContains(transformed, 'const ROUND = "round380";', "generated round id");
  assertContains(transformed, "MAX_PRIOR_ROUND = 377", "prior cutoff");
  ...
  assertContains(transformed, "through round377", "dedupe prose cutoff");
  return transformed;
}
...
function main() {
  ...
  vm.runInNewContext(
    ...
    {
      filename: "generated_round380_london_pld_lifecycle_next47_candidates.js"
    }
  );
}
