...
const OUTPUT_DIR = path.join(__dirname, "..", "tmp", "subagents", "round595_nyc_dob_legacy_permit_next25");
...
function normalizeOutputBoundaryText() {
  ...
  for (const name of fs.readdirSync(OUTPUT_DIR)) {
    ...
    const normalized = text
      .replaceAll("through Round568", "through Round593")
      .replaceAll("treating Round568", "treating Round593")
      .replaceAll("through Round572", "through Round593")
      .replaceAll("treating Round572", "treating Round593")
      .replaceAll("through Round574", "through Round593")
      .replaceAll("treating Round574", "treating Round593")
      .replaceAll("through Round578", "through Round593")
      .replaceAll("treating Round578", "treating Round593")
      .replaceAll("through Round583", "through Round593")
      .replaceAll("treating Round583", "treating Round593")
      .replaceAll("through Round588", "through Round593")
      .replaceAll("treating Round588", "treating Round593");
...
if (sourceHash !== EXPECTED_SOURCE_SHA256) {
  throw new Error(
    `Round595 expected Round572 worker source hash ${EXPECTED_SOURCE_SHA256}, got ${sourceHash}. ` +
      "Review the upstream pattern before regenerating this candidate pack."
  );
}

const round595Source = source
  .replace("const ROUND = 572;", "const ROUND = 595;")
  .replace(
    'const SLUG = "round572_nyc_dob_legacy_permit_next19";',
    'const SLUG = "round595_nyc_dob_legacy_permit_next25";'
  )
  .replace("const DEDUPE_BOUNDARY_ROUND = 568;", "const DEDUPE_BOUNDARY_ROUND = 593;")
  .replaceAll(
    "fetch_round572_nyc_dob_legacy_permit_next19_candidates.js",
    "fetch_round595_nyc_dob_legacy_permit_next25_candidates.js"
  )
  .replaceAll("round572_nyc_dob_legacy_permit_next19", "round595_nyc_dob_legacy_permit_next25")
  .replaceAll("Round572", "Round595")
  .replaceAll('"Round563", "Round568"', '"Round588", "Round593"')
  .replace(
  ...
  .replaceAll("through Round588", "through Round593")
  .replaceAll("treating Round588", "treating Round593")
  .replaceAll("next19", "next25");

vm.runInNewContext(round595Source, {
  __dirname,
  __filename,
  require,
  console,
