...
const ROUND = "round266";
const ROUND_NAME = "round266_london_pld_lifecycle_next22";
  ...
  for (let page = 0; page < maxPages; page += 1) {
    ...
    const response = await fetch(API_ENDPOINT, {
      ...
      headers: {
        ...
        "user-agent": "Bims5Round266PldLifecycleNext22/0.1"
      },
...
function rejectionReason(item, duplicateIndex, batchKeys) {
  ...
  if (batchKeys.has(fieldKey)) return "duplicate_row_date_field_inside_round266_batch";
  if (
...
async function main() {
  ...
  const candidatesPack = {
    schema_version: "round266.london_pld_lifecycle_next22.candidates.v1",
    generated_at: GENERATED_AT,
    accessed_at: ACCESSED_AT,
    task:
      "Round266 London Planning Datahub actual lifecycle candidate pack beyond current manual corpus and prior PLD lifecycle packs through round261",
  ...
  const summary = {
    ...
    dedupe_basis:
      "Live data/manual_drops/architecture_milestones/architecture_milestones_2008_2026.json plus all discovered London PLD lifecycle scratch packs through round261, explicitly including round126 direct/more, round140, round147, round150, round153, round157, round161, round166, round172, round178, round184, round190, round196, round202, round208, round217, round223, round230, round238, round243, round248, round253, and round261, scanned for PLD ids, source-record/source-url/date-field/date keys, source-date-field/date keys, and title/date keys.",
    ...
    caveats: [
      ...
      "Exact source row/date-field/date duplicates already present in the live manual corpus or prior PLD lifecycle packs through round261 were excluded.",
      "London Datastore licence is Not Specified for this source, so the pack retains factual metadata and attribution only."
    ]
  };

  const rejectedPack = {
    schema_version: "round266.london_pld_lifecycle_next22.rejected.v1",
    generated_at: GENERATED_AT,
    accessed_at: ACCESSED_AT,
  ...
  const notes = [
    "# Round 266 London PLD lifecycle next22 candidates",
    "",
    ...
    "",
    "Deduplication scanned the live manual architecture corpus plus all discovered prior London PLD lifecycle scratch packs through round261, explicitly including round126 direct, round126 more, round140, round147, round150, round153, round157, round161, round166, round172, round178, round184, round190, round196, round202, round208, round217, round223, round230, round238, round243, round248, round253, and round261 by PLD row id, source URL/source record/date-field/date, source date field/date, and title/date.",
    "",
