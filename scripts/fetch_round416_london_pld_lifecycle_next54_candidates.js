...
const ROUND_NAME = "round416_london_pld_lifecycle_next54";
...
const PRIOR_PROSE_AFTER_272 =
  "round272, round276, round284, round288, round298, round301, round306, round311, round316, round323, round327, round328, round333, round338, round343, round348, round350, round354, round358, round362, round365, round369, round373, round377, round380, round384, round387, round393, round398, round404, and round410";
...
function replaceRequired(value, needle, replacement, label) {
  if (!value.includes(needle)) {
    ...
    throw new Error(`Round416 source transform could not find ${label}: ${needle}`);
  }
...
function assertContains(value, needle, label) {
  if (!value.includes(needle)) {
    throw new Error(`Round416 source transform failed to include ${label}: ${needle}`);
  }
}
...
function transformRound404Source(source) {
  ...
  transformed = transformed
    .replace(/round404_london_pld_lifecycle_next52/g, "round416_london_pld_lifecycle_next54")
    .replace(/round404\.london_pld_lifecycle_next52/g, "round416.london_pld_lifecycle_next54")
    .replace(/Bims5Round404PldLifecycleNext52/g, "Bims5Round416PldLifecycleNext54")
    .replace(/Round 404/g, "Round 416")
    .replace(/Round404/g, "Round416")
    .replace(/round404/g, "round416")
    .replace(/next52/g, "next54")
    .replace(/MAX_PRIOR_ROUND = 398/g, "MAX_PRIOR_ROUND = 410")
    .replace(/through round398/g, "through round410");

  transformed = replaceRequired(
    transformed,
    '  "round398_london_pld_lifecycle_next51"\n];',
    '  "round398_london_pld_lifecycle_next51",\n  "round404_london_pld_lifecycle_next52",\n  "round410_london_pld_lifecycle_next53"\n];',
    "round404 and round410 prior PLD lifecycle packs"
  );
  ...
  assertContains(transformed, 'const ROUND_NAME = "round416_london_pld_lifecycle_next54";', "round name");
  assertContains(transformed, 'const ROUND = "round416";', "transformed fetch round id");
  assertContains(transformed, "MAX_PRIOR_ROUND = 410", "prior cutoff");
  assertContains(transformed, "through round410", "dedupe cutoff text");
  ...
  assertContains(transformed, 'CURRENT_ROUND_NAME = "round416_london_pld_lifecycle_next54"', "validator round name");
...
async function runTransformedRound404() {
  ...
  vm.runInNewContext(transformed, sandbox, {
    filename: "generated_round416_london_pld_lifecycle_next54_candidates.js"
  });

  if (typeof sandboxModule.exports !== "function") {
    throw new Error("Round416 transform did not export the fetch main() function.");
  }
...
function validateAndRewriteReadbackJson() {
  ...
  const ok =
    ...
    /through round410/i.test(summary.dedupe_basis || "");

  if (!ok) {
    throw new Error("Readback validation failed for Round416 output pack.");
  }
  ...
  writeJson(readbackFile, {
    ...
    readback_checks: [
      ...
      "dedupe basis includes PLD lifecycle packs through round410",
  ...
  if (!finalReadback.ok || !finalReadback.files?.["readback.json"]?.exists) {
    throw new Error("Final readback self-check failed for Round416 output pack.");
  }
}
