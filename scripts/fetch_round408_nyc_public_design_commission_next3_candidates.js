...
const OUT_DIR = path.join(ROOT, "tmp", "subagents", "round408_nyc_public_design_commission_next3");
const ACCESS_DATE = "2026-05-20";
const DATE_MIN = "2008-01-01";
const DATE_MAX = "2026-05-20";
const ROUND = "round408";
const TASK = "nyc_public_design_commission_next3_candidates";
...
const GEOCODE_QUERIES = {
  "30370": "Damrosch Park Lincoln Center West 62nd Street Columbus Avenue Amsterdam Avenue Manhattan",
  "30366": "St Andrew's Playground Atlantic Avenue St Andrew's Place Herkimer Street Kingston Avenue Brooklyn",
  "30356": "Building 41A Brooklyn Navy Yard Washington Avenue 4th Street Brooklyn",
  "30343": "Citi Field 41 Seaver Way Queens",
  "30341": "Downtown Manhattan Heliport Pier 6 20 South Street Manhattan",
  "30335": "Haffen Park Bronx",
  "30320": "10875 Avenue D Brooklyn NY",
  "30312": "431 East 34th Street Manhattan",
  "30311": "The Peninsula Tiffany Street Spofford Avenue Manida Street Bronx",
  "30295": "165 Bradford Street Brooklyn",
  "30291": "556 West 52 Street Manhattan",
  "30283": "8 Reilly Boulevard Randall's Island Manhattan",
  "30277": "P.S. 001 The Bergen 309 47 Street Brooklyn",
  "30274": "St Michael's Playground Astoria Queens",
  "30261": "Horizon Juvenile Center 560 Brooke Avenue Bronx",
  "30260": "Oakwood Beach Wastewater Resource Recovery Facility 751 Mill Road Staten Island",
  "30252": "423 Abbot Road Fort Totten Bayside Queens",
  "30251": "Jamaica Station Archer Avenue Sutphin Boulevard Queens",
  "30250": "Walker Park Davis Avenue Delafield Place Livingston Court Staten Island",
  "30249": "St James Recreation Center 2530 Jerome Avenue Bronx",
  "30248": "Sobel Court Park Staten Island",
  "30247": "ABC Playground East Houston Street Essex Street Norfolk Street Manhattan",
  "30242": "Richmond Hill Community Library 118-14 Hillside Avenue Queens",
  "30240": "Kissena Park Queens",
  "30239": "Brookville Park 232nd Street 144th Avenue Rosedale Queens",
  "30237": "Richmond Hill Community Library 118-14 Hillside Avenue Queens",
  "30236": "100 Centre Street New York NY",
  "30231": "East 236th Street Bronx Boulevard East 233rd Street Bronx River Parkway Bronx",
  "30230": "Lower East Side Ecology Center East River Park Jackson Street Cherry Street Manhattan",
  "30227": "745 East 141st Street Bronx",
  "30226": "Tompkinsville Pier Murray Hulbert Avenue Staten Island",
  "30224": "Willets Point Roosevelt Avenue 38th Avenue Willets Point Boulevard Corona Queens",
  "30221": "Hansborough Recreation Center 35 West 134th Street Manhattan",
  "30220": "High Line Park West 30th Street 11th Avenue New York NY",
  "30219": "Bensonhurst Park Cropsey Avenue Bay Parkway Brooklyn",
};

const MANUAL_GEOMETRY = {
  "30222": {
    coordinates: [-74.0063, 40.7435],
    label: "Approximate High Line Park access area near West 16th Street and Tenth Avenue, Manhattan",
    source: "Curated approximate point from PDC location text; not official PDC geometry.",
    precision: "approximate park access point",
...
const PROJECT_TYPE_RULES = [
  ...
  [/family residence|medical examiner|health/i, "public_health_or_social_service_facility"],
  [/wastewater|sewer|stormwater|bulkhead|flood|resilien|pump|conduit/i, "water_or_resilience_infrastructure"],
  [/greenway|esplanade|pier|heliport|waterfront|ferry|open space/i, "waterfront_or_transport_public_realm"],
  [/plaza|streetscape|sidewalk|ramp|staircase|elevator|path|retaining wall/i, "streetscape_access_or_plaza"],
  [/park|playground|field|public restroom|comfort station|compost|landscape/i, "park_or_playground"],
  [/garage|building|roof|facility|center|carport|training/i, "public_facility_architecture"],
];

const INCLUDED_SUBJECT_PATTERN =
  /\b(construction|reconstruction|rehabilitation|restoration|renovation|addition|facade|roof|library|community center|health center|criminal court|detention|juvenile|park|playground|plaza|greenway|esplanade|waterfront|shoreline|bridge|building|facility|public realm|streetscape|street|wastewater|sewer|garage|recreation center|public restroom|retaining wall|staircase|elevator|path|pier|bulkhead|resiliency|flood|pavilion|compost|heliport|ferry)\b/i;
const EXCLUDED_SUBJECT_PATTERN =
  /\b(artwork|artist|mural|sculpture|plaque|banner|wayfinding|security cameras?|camera|antenna|telecommunications|newsstand|bus shelter)\b/i;
const OBSERVED_CHANGE_FORBIDDEN_PATTERN =
  /\b(predicts?|forecast|simulat(?:e|ion)|caus(?:e|ed|al)|will increase|will decrease|impact score|opened|occupied|operational)\b/i;
...
function arrayFromCandidateFile(filePath) {
  ...
  return parsed.candidates || parsed.events || parsed.items || parsed.records || [];
}
...
function existingKeysFromGeneratedAtlas(indexPath) {
  ...
  const pdcRows = allRows.filter((row) => {
    const text = JSON.stringify(row).toLowerCase();
    return text.includes("public design commission") || text.includes("nyc_pdc") || text.includes("certificate-30");
  });
  return {
...
function cleanSubject(record) {
  ...
  const match = resolution.match(/considered (?:designs?|a proposal|a final report) for (.*?), submitted by/i);
  return normalizeText(match ? match[1] : direct || resolution);
}
...
function eventIdFor(record, subject) {
  ...
  return `nyc_pdc_round408_${datePart}_${record.certificate}_${slugify(stripSubjectPrefix(subject), 72)}`;
}
...
async function fetchText(url) {
  const response = await fetch(url, {
    headers: {
      "user-agent": "Bims-5 round408 source auditor (citation metadata only)",
...
async function checkUrl(url) {
  try {
    let response = await fetch(url, {
      method: "HEAD",
      headers: { "user-agent": "Bims-5 round408 source auditor (citation metadata only)" },
    });
...
async function geocodeCertificate(cert, borough) {
  ...
  try {
    const response = await fetch(url, {
      headers: {
        "user-agent": "Bims-5 round408 geocoding (approximate event map placement)",
...
function getPageExcerpt(record) {
  ...
  const source = match ? (match.input || pageText) : pageText;
  const start = match ? Math.max(0, match.index - 120) : 0;
  const excerpt = normalizeText(source.slice(start, start + 1100));
...
function makeCandidate(record, subject, geocode, sourceStatus, pageExcerpt) {
  ...
  return {
    ...
    transformation_method:
      "scripts/fetch_round408_nyc_public_design_commission_next3_candidates.js filtered parsed official PDC certificate records, screened live/prior PDC corpora for duplicates, rechecked official NYC.gov URLs, and added caveated approximate point geometry.",
    provenance: {
      source_id: SOURCE_ID,
      source_name: SOURCE_NAME,
      publisher: PUBLISHER,
      ...
      source_artifact_note:
        "Source row read from round124 parsed PDC certificate artifact created from official NYC.gov PDFs with pypdf text extraction; official NYC.gov PDF URL was rechecked for round408.",
      source_date_field: "PDC certificate adoption date",
      ...
      license_url: NYC_TERMS_URL,
      attribution_text: `Source: ${PUBLISHER}.`,
      transformation_script: "scripts/fetch_round408_nyc_public_design_commission_next3_candidates.js",
...
function auditRecordPool(records, existingKeys) {
  ...
  for (const record of records) {
    ...
    if (reasons.length === 0) {
      ...
      if (!seedSet.has(cert)) {
        counts.not_seeded_for_round408 += 1;
        reasons.push("available but not selected for clean round408 pack");
      }
    }
...
function makeValidationReport(validation, summary, sourcePages) {
  const checks = [
    ...
    },
    {
      ...
      detail: "Observed-change statements avoid prediction, causation, opening, operation, and outcome claims.",
    },
    {
...
async function main() {
  ...
  const rejected = {
    ...
    note:
      "The full PDC certificate pool is much larger than this clean next3 pack; sampled screening rejections are representative, while selected_rejections lists every seeded record dropped from round408.",
  };
  ...
  const sourceAudit = {
    ...
    accessed_at: ACCESS_DATE,
    auditor: "Codex worker Round408",
    task_scope:
      "Continue NYC Public Design Commission official design-review source ingestion after Round403; select fresh non-duplicate design-review/certificate/minutes records through the 2026-05-20 access date.",
    ...
    sources: [
      ...
      },
      {
        ...
        required_caveats: [
          ...
          "One selected High Line record uses a curated approximate point because the geocoder did not return a defensible site match.",
        ],
