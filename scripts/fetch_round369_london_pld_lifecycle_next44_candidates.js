...
function replaceRequired(value, needle, replacement, label) {
  if (!value.includes(needle)) {
    ...
    throw new Error(`Round369 source transform could not find ${label}: ${needle}`);
  }
...
function assertContains(value, needle, label) {
  if (!value.includes(needle)) {
    throw new Error(`Round369 source transform failed to include ${label}: ${needle}`);
  }
}

function transformRound362Script(source) {
  let transformed = source
    .replace(/round362_london_pld_lifecycle_next42/g, "round369_london_pld_lifecycle_next44")
    .replace(/round362\.london_pld_lifecycle_next42/g, "round369.london_pld_lifecycle_next44")
    .replace(/Bims5Round362PldLifecycleNext42/g, "Bims5Round369PldLifecycleNext44")
    .replace(/Round362/g, "Round369")
    .replace(/Round 362/g, "Round 369")
    .replace(/round362/g, "round369")
    .replace(/next42/g, "next44")
    .replace(/MAX_PRIOR_ROUND = 358/g, "MAX_PRIOR_ROUND = 365")
    .replace(/through round358/g, "through round365");
  ...
  transformed = replaceRequired(
    ...
    '      "round272, round276, round284, round288, round298, round301, round306, round311, round316, round323, round327, round328, round333, round338, round343, round348, round350, round354, round358, round362, and round365"\\n    );',
  ...
  transformed = replaceRequired(
    ...
    "prior round327 through round365 fetch assertions"
  );

  transformed = replaceRequired(
    transformed,
    '    .replace(/round276/g, "round358");',
    '    .replace(/round276/g, "round365");',
    "validator prior cutoff prose replacement"
  );

  assertContains(transformed, 'const ROUND_NAME = "round369_london_pld_lifecycle_next44";', "round name");
  assertContains(transformed, 'const ROUND = "round369";', "generated round id");
  assertContains(transformed, "MAX_PRIOR_ROUND = 365", "prior cutoff");
  ...
  assertContains(transformed, "through round365", "dedupe prose cutoff");
  return transformed;
}
...
function main() {
  ...
  vm.runInNewContext(
    ...
    {
      filename: "generated_round369_london_pld_lifecycle_next44_candidates.js"
    }
  );
}
