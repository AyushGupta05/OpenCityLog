...
const ROUND_NAME = "round508_london_pld_lifecycle_next77";
...
function replaceRequired(value, needle, replacement, label) {
  if (!value.includes(needle)) {
    ...
    throw new Error(`Round508 source transform could not find ${label}: ${needle}`);
  }
...
function transformRound488Script(source) {
  let transformed = source
    .replace(/round488_london_pld_lifecycle_next72/g, "round508_london_pld_lifecycle_next77")
    .replace(/round488\.london_pld_lifecycle_next72/g, "round508.london_pld_lifecycle_next77")
    .replace(/Bims5Round488PldLifecycleNext72/g, "Bims5Round508PldLifecycleNext77")
    .replace(/Round 488/g, "Round 508")
    .replace(/Round488/g, "Round508")
    .replace(/round488/g, "round508")
    .replace(/next72/g, "next77")
    .replace(/MAX_PRIOR_ROUND = 485/g, "MAX_PRIOR_ROUND = 504")
    .replace(/through round485/g, "through round504")
    .replace(/packRound\(entry\.name\) > 485/g, "packRound(entry.name) > 504")
    .replace(/\.replace\(\/round276\/g, "round485"\);/g, '.replace(/round276/g, "round504");');
  ...
  transformed = replaceRequired(
    ...
    "round488, round490, round496, round500, and round504 prior pack insertion"
  );

  transformed = transformed
    .replace(
      /pack\) => pack\.label === "round485_london_pld_lifecycle_next71" && pack\.exists === true/g,
      'pack) => pack.label === "round504_london_pld_lifecycle_next76" && pack.exists === true'
    )
    .replace(/round485 prior PLD lifecycle pack was scanned/g, "round504 prior PLD lifecycle pack was scanned");

  assertContains(transformed, 'const ROUND_NAME = "round508_london_pld_lifecycle_next77";', "round name");
  assertContains(transformed, 'const ROUND = "round508";', "transformed fetch round id");
  assertContains(transformed, "MAX_PRIOR_ROUND = 504", "prior cutoff");
  assertContains(transformed, "through round504", "dedupe cutoff text");
  ...
  assertContains(transformed, "addRound508RetrievedAtAlias", "candidate retrieved_at alias");
  assertContains(transformed, 'CURRENT_ROUND_NAME = "round508_london_pld_lifecycle_next77"', "validator round name");
  assertContains(transformed, "round508_strict_duplicate_audit", "strict duplicate audit label");
...
function addRound508SourceRowRefs() {
  ...
  pack.round508_postprocess = {
    source_row_ref_alias_count: aliasCount,
    note: "Round508 wrapper added explicit source_row_ref/source_row_refs aliases from the official PLD row/API URLs after the inherited lifecycle worker completed."
  };
  writeJson(file, pack);
}

function updateRound508Readback() {
  ...
  readback.round508_postprocess_checks = [
...
async function main() {
  ...
  vm.runInNewContext(transformed, sandbox, {
    filename: "generated_round508_london_pld_lifecycle_next77_candidates.js"
  });
  await waitForGeneratedOutputs(startedAtMs);
  addRound508SourceRowRefs();
  updateRound508Readback();
}
