...
const outDir = "tmp/subagents/round221_nyc_hpd_affordable_housing_next2";
...
function buildCandidate(row, existing, rejected) {
  ...
  return {
    ...
    raw_row: row,
    transformation_method: `Round221 official NYC Open Data HPD fetch from ${DATASET.id}; selected from rows with project/building identifiers, a source date, source coordinates, unit counts, and duplicate screening against the live manual corpus plus prior HPD candidate packs.`
  };
}
...
function sourceAudit(metadata, acceptedCount, selectedSummary) {
  ...
  return {
    schema_version: "round221.nyc_hpd_affordable_housing_next2_source_audit.v1",
    ...
    sources: [
      {
        ...
        published_coverage_note: "Round221 checked HPD building-level rows with coordinates and building/project completion or project start dates available through the access date.",
...
async function main() {
  ...
  const summaryPayload = {
    schema_version: "round221.nyc_hpd_affordable_housing_next2_summary.v1",
    generated_at: generatedAt,
    accessed_at: accessedAt,
    output_files: [
  ...
  const candidatesPayload = {
    schema_version: "round221.nyc_hpd_affordable_housing_next2_candidates.v1",
    generated_at: generatedAt,
    accessed_at: accessedAt,
  ...
  const rejectedPayload = {
    schema_version: "round221.nyc_hpd_affordable_housing_next2_rejected.v1",
    generated_at: generatedAt,
    accessed_at: accessedAt,
    source_id: DATASET.sourceId,
    dataset_id: DATASET.id,
  ...
  const notes = [
    "# Round221 NYC HPD Affordable Housing Production Candidate Pack",
    "",
