...
const OUT_DIR = path.join(ROOT, "tmp", "subagents", "round147_london_pld_lifecycle_next");
  ...
  for (let page = 0; page < 90; page += 1) {
    ...
    const response = await fetch(API_ENDPOINT, {
      ...
      headers: {
        ...
        "user-agent": "Bims5Round147PldLifecycleNext/0.1"
      },
...
function candidateFor(item) {
  ...
  const candidateId = `round147_lon_pld_${spec.kind}_${slugify(rowId)}_${date.replace(/-/g, "_")}_${hash8(
  ...
  return {
    ...
    duplicate_check_note:
      "Screened against the live manual architecture corpus, round126 London PLD lifecycle packs, and round140 London PLD lifecycle pack by PLD row id, source URL/source_record_id, source-date-field/date, and title/date.",
    limitations: [
    ...
    transformation_method:
      "scripts/fetch_round147_london_pld_lifecycle_next_candidates.js queried the official Planning London Datahub guest applications API for actual_commencement_date and actual_completion_date rows, normalized source dates, filtered to the London coordinate envelope and 2020-2026 access window, scored architecture/city-change signals, and removed rows already present in the live manual architecture corpus, round126 PLD lifecycle packs, or round140 PLD lifecycle pack."
  };
}
...
function shouldReject(row, spec, date, point, duplicateIndex, batchKeys, scoreValue, rejections) {
  ...
  if (batchKeys.has(rowKindKey)) return rejectWith(rejections, "duplicate_row_kind_inside_round147_batch");
  if (
...
async function main() {
  ...
  const candidatesPack = {
    schema_version: "round147.london_pld_lifecycle_next.candidates.v1",
    generated_at: GENERATED_AT,
    accessed_at: ACCESSED_AT,
    task:
      "Round147 London Planning Datahub actual lifecycle candidate pack beyond current manual corpus, round126 PLD packs, and round140 PLD pack",
  ...
  const sourceAudit = {
    generated_at: GENERATED_AT,
    accessed_at: ACCESSED_AT,
    task: "round147_london_pld_lifecycle_next",
    source_audits: [
      {
        source_id: "gla-planning-datahub-applications",
        source_name: "Planning London Datahub applications",
  ...
  const summary = {
    ...
    caveats: [
      ...
      "Rows with PLD ids already present in the live manual corpus, round126 PLD lifecycle packs, or round140 PLD lifecycle pack were excluded.",
      "London Datastore licence is Not Specified for this source, so the pack retains factual metadata and attribution only."
    ]
  };

  const notes = [
    "# Round 147 London PLD lifecycle next candidates",
    "",
    ...
    "",
    "Deduplication scanned the live manual architecture corpus plus the prior round126 direct, round126 more, and round140 London PLD lifecycle scratch packs by PLD row id, source URL/source record, source date field/date, and title/date.",
    "",
