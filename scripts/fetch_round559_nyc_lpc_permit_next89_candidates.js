...
source = source
  .replace(/const ROUND = 505;/g, "const ROUND = 559;")
  .replace(/round505_nyc_lpc_permit_next76/g, "round559_nyc_lpc_permit_next89")
  .replace(/round505/g, "round559")
  .replace(/Round505/g, "Round559")
  .replace(/next76/g, "next89")
  .replace(/Next76/g, "Next89")
  .replace(/through round501/g, "through round553");
...
source = replaceOnce(
  ...
  `function injectRound559MissingAddressFilter(innerSource) {
  ...
  if (innerSource.includes(runnerMarker)) {
    const bridge = injectRound559MissingAddressFilter.toString() + String.fromCharCode(10) + 'source = injectRound559MissingAddressFilter(source);' + String.fromCharCode(10) + runnerMarker;
    return innerSource.replace(runnerMarker, bridge);
  }
  throw new Error("Round559 missing-address injection target not found");
}

source = injectRound559MissingAddressFilter(source);
