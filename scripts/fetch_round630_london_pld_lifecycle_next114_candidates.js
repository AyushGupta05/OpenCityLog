...
const ROUND630_OUT_DIR = path.join(
  ROOT,
  "tmp",
  "subagents",
...
function replaceRequired(value, needle, replacement, label) {
  if (!value.includes(needle)) {
    ...
    throw new Error(`Round630 wrapper transform could not find ${label}: ${needle}`);
  }
...
function transformRound616Wrapper(source) {
  let transformed = source
    .replace(/round616_london_pld_lifecycle_next109/g, "round630_london_pld_lifecycle_next114")
    .replace(/round616\.london_pld_lifecycle_next109/g, "round630.london_pld_lifecycle_next114")
    .replace(/Bims5Round616PldLifecycleNext109/g, "Bims5Round630PldLifecycleNext114")
    .replace(/Round 616/g, "Round 630")
    .replace(/Round616/g, "Round630")
    .replace(/round616/g, "round630")
    .replace(/next109/g, "next114")
    .replace(/MAX_PRIOR_ROUND = 614/g, "MAX_PRIOR_ROUND = 629")
    .replace(/through round614/g, "through round629")
    .replace(/packRound\(entry\.name\) > 614/g, "packRound(entry.name) > 629")
    .replace(/\.replace\(\/round276\/g, "round614"\);/g, '.replace(/round276/g, "round629");')
    .replace(/round614 prior PLD lifecycle pack was scanned/g, "round629 prior PLD lifecycle pack was scanned")
    .replace(
  ...
  transformed = replaceRequired(
    ...
    "round629 prior pack insertion"
  );

  assertContains(transformed, 'const ROUND_NAME = "round630_london_pld_lifecycle_next114";', "round name");
  assertContains(transformed, 'const ROUND = "round630";', "transformed fetch round id");
  assertContains(transformed, "MAX_PRIOR_ROUND = 629", "prior cutoff");
  assertContains(transformed, "through round629", "dedupe cutoff text");
  ...
  assertContains(transformed, "addRound630RetrievedAtAlias", "candidate retrieved_at alias");
  assertContains(transformed, 'CURRENT_ROUND_NAME = "round630_london_pld_lifecycle_next114"', "validator round name");
  assertContains(transformed, "round630_strict_duplicate_audit", "strict duplicate audit label");
...
async function main() {
  process.once("beforeExit", () => {
    if (!fs.existsSync(ROUND630_OUT_DIR)) return;
    for (const entry of fs.readdirSync(ROUND630_OUT_DIR, { withFileTypes: true })) {
      if (!entry.isFile()) continue;
      if (!/\.(json|md)$/i.test(entry.name)) continue;
      const filePath = path.join(ROUND630_OUT_DIR, entry.name);
      const before = fs.readFileSync(filePath, "utf8");
      const after = before.split(INHERITED_ACCESS_DATE).join(ROUND630_ACCESS_DATE);
  ...
  vm.runInNewContext(transformed, sandbox, {
    filename: "generated_round630_london_pld_lifecycle_next114_candidates.js"
  });
}
