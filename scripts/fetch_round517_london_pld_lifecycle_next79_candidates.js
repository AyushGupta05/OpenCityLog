...
const ROUND_NAME = "round517_london_pld_lifecycle_next79";
...
function replaceRequired(value, needle, replacement, label) {
  if (!value.includes(needle)) {
    ...
    throw new Error(`Round517 source transform could not find ${label}: ${needle}`);
  }
...
function transformRound488Script(source) {
  let transformed = source
    .replace(/round488_london_pld_lifecycle_next72/g, "round517_london_pld_lifecycle_next79")
    .replace(/round488\.london_pld_lifecycle_next72/g, "round517.london_pld_lifecycle_next79")
    .replace(/Bims5Round488PldLifecycleNext72/g, "Bims5Round517PldLifecycleNext79")
    .replace(/Round 488/g, "Round 517")
    .replace(/Round488/g, "Round517")
    .replace(/round488/g, "round517")
    .replace(/next72/g, "next79")
    .replace(/MAX_PRIOR_ROUND = 485/g, "MAX_PRIOR_ROUND = 512")
    .replace(/through round485/g, "through round512")
    .replace(/packRound\(entry\.name\) > 485/g, "packRound(entry.name) > 512")
    .replace(/\.replace\(\/round276\/g, "round485"\);/g, '.replace(/round276/g, "round512");');
  ...
  transformed = replaceRequired(
    ...
    "round488, round490, round496, round500, round504, round508, and round512 prior pack insertion"
  );

  transformed = transformed
    .replace(
      /pack\) => pack\.label === "round485_london_pld_lifecycle_next71" && pack\.exists === true/g,
      'pack) => pack.label === "round512_london_pld_lifecycle_next78" && pack.exists === true'
    )
    .replace(/round485 prior PLD lifecycle pack was scanned/g, "round512 prior PLD lifecycle pack was scanned");

  assertContains(transformed, 'const ROUND_NAME = "round517_london_pld_lifecycle_next79";', "round name");
  assertContains(transformed, 'const ROUND = "round517";', "transformed fetch round id");
  assertContains(transformed, "MAX_PRIOR_ROUND = 512", "prior cutoff");
  assertContains(transformed, "through round512", "dedupe cutoff text");
  ...
  assertContains(transformed, "addRound517RetrievedAtAlias", "candidate retrieved_at alias");
  assertContains(transformed, 'CURRENT_ROUND_NAME = "round517_london_pld_lifecycle_next79"', "validator round name");
  assertContains(transformed, "round517_strict_duplicate_audit", "strict duplicate audit label");
...
function addRound517SourceRowRefs() {
  ...
  pack.round517_postprocess = {
    source_row_ref_alias_count: aliasCount,
    note: "Round517 wrapper added explicit source_row_ref/source_row_refs aliases from the official PLD row/API URLs after the inherited lifecycle worker completed."
  };
  writeJson(file, pack);
}

function updateRound517Readback() {
  ...
  readback.round517_postprocess_checks = [
...
async function main() {
  ...
  vm.runInNewContext(transformed, sandbox, {
    filename: "generated_round517_london_pld_lifecycle_next79_candidates.js"
  });
  await waitForGeneratedOutputs(startedAtMs);
  addRound517SourceRowRefs();
  updateRound517Readback();
}
