...
const OUTPUT_DIR = path.join(__dirname, "..", "tmp", "subagents", "round578_nyc_dob_legacy_permit_next21");
...
function normalizeOutputBoundaryText() {
  ...
  for (const name of fs.readdirSync(OUTPUT_DIR)) {
    ...
    const normalized = text
      ...
      .replaceAll("through Round572", "through Round574")
      .replaceAll("treating Round572", "treating Round574");
...
if (sourceHash !== EXPECTED_SOURCE_SHA256) {
  throw new Error(
    `Round578 expected Round572 worker source hash ${EXPECTED_SOURCE_SHA256}, got ${sourceHash}. ` +
      "Review the upstream pattern before regenerating this candidate pack."
  );
}

const round578Source = source
  .replace("const ROUND = 572;", "const ROUND = 578;")
  .replace(
    'const SLUG = "round572_nyc_dob_legacy_permit_next19";',
    'const SLUG = "round578_nyc_dob_legacy_permit_next21";'
  )
  .replace("const DEDUPE_BOUNDARY_ROUND = 568;", "const DEDUPE_BOUNDARY_ROUND = 574;")
  .replaceAll(
    "fetch_round572_nyc_dob_legacy_permit_next19_candidates.js",
    "fetch_round578_nyc_dob_legacy_permit_next21_candidates.js"
  )
  .replaceAll("round572_nyc_dob_legacy_permit_next19", "round578_nyc_dob_legacy_permit_next21")
  .replaceAll("Round572", "Round578")
  .replaceAll('"Round563", "Round568"', '"Round572", "Round574"')
  .replace(
  ...
  .replaceAll("through Round572", "through Round574")
  .replaceAll("treating Round572", "treating Round574")
  .replaceAll("next19", "next21");

vm.runInNewContext(round578Source, {
  __dirname,
  __filename,
  require,
  console,
