...
function replaceChecked(pattern, replacement, label) {
  ...
  if (next === source) {
    throw new Error(`Unable to adapt round251 HPD fetcher for round263: missing ${label}.`);
  }
  source = next;
}
...
replaceChecked(
  ...
  'const outDir = "tmp/subagents/round263_nyc_hpd_affordable_housing_next10";',
  "output directory"
);
replaceChecked('const accessedAt = "2026-05-19";', 'const accessedAt = "2026-05-20";', "access date");
replaceChecked('const roundName = "Round251";', 'const roundName = "Round263";', "round name");
replaceChecked("const maxScreeningRound = 246;", "const maxScreeningRound = 255;", "screening round");
replaceChecked(/Round251/g, "Round263", "round label references");
replaceChecked(/round251\.nyc/g, "round263.nyc", "schema round prefix");
replaceChecked(
  /nyc_hpd_affordable_housing_next8/g,
  "nyc_hpd_affordable_housing_next10",
  "next pack label"
);
replaceChecked(
...
replaceChecked(
  ...
  "candidate_files_considered_through_round255",
  "candidate-files summary key"
);
replaceChecked(
  "async function main()",
  `const round263SelectionCache = new WeakMap();
...
function isClearlySupportedRound263PreservationCandidate(candidate) {
...
function isRound263PreferredCandidate(candidate) {
  if (candidate.source_date_field !== "building_completion_date") return false;
  if (isPreservation(candidate.raw_row)) return isClearlySupportedRound263PreservationCandidate(candidate);
  return true;
}

function selectedRound263CandidateIds(rankedCandidates) {
  if (round263SelectionCache.has(rankedCandidates)) return round263SelectionCache.get(rankedCandidates);
  const selectedIds = new Set();

  for (const candidate of rankedCandidates) {
    if (selectedIds.size >= targetCount) break;
    if (isRound263PreferredCandidate(candidate)) selectedIds.add(candidate.event_id);
  }
  ...
  round263SelectionCache.set(rankedCandidates, selectedIds);
  return selectedIds;
}

async function main()`,
  "round263 supported preservation selection helper"
);
replaceChecked(
  ".slice(0, targetCount)",
  ".filter((candidate, index, rankedCandidates) => selectedRound263CandidateIds(rankedCandidates).has(candidate.event_id))",
  "round263 selection call"
);
replaceChecked(
  ...
  "Source date priority is applied first: building_completion_date rows, then project_completion_date rows, then project_start_date rows. Round263 keeps the accepted pack to building_completion_date rows where available; Preservation rows are admitted only when the selected row has HPD building_completion_date, source coordinates, positive unit counts, and Extended Affordability Only = No. Within each date-field tier, New Construction rows remain ahead of Preservation rows before unit-count score, senior/supportive text signals, and newer dates.",
  "round263 ranking summary"
);
replaceChecked(
...
replaceChecked(
  ...
  '      "Coordinates are source geocoded points and should not be displayed as exact building footprints or project boundaries.",\n      "Round263 Preservation rows are retained only when HPD reports building_completion_date, coordinates, positive units, and Extended Affordability Only = No; they remain administrative delivery records, not independent proof of full rehabilitation scope or occupancy.",',
...
replaceChecked(
  ...
  '    "- Coordinates are HPD/Open Data geocoded points, not surveyed building footprints or project boundaries.",\n    "- Round263 Preservation rows are included only when HPD reports building_completion_date, coordinates, positive units, and Extended Affordability Only = No; this is still administrative delivery evidence, not independent proof of full rehabilitation scope or occupancy.",',
  "notes preservation caveat"
);
replaceChecked(
  /including round229, round234, round236, round241, and round246/g,
  "including round229, round234, round236, round241, round246, round251, and round255",
  "explicit overlap caveat"
);
replaceChecked(/through round246/g, "through round255", "screening prose");

const runAdaptedRound251Fetcher = new Function(
  "require",
  "fetch",
  `${source}\n//# sourceURL=fetch_round263_nyc_hpd_affordable_housing_next10_candidates.adapted.js`
);
