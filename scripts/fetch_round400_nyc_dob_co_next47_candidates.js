...
const GENERATED_FILENAME = "scripts/fetch_round400_nyc_dob_co_next47_candidates.generated.js";
const ROUND = "round400";
const ROUND_TITLE = "Round400";
const NEXT_LABEL = "next47";
const NEXT_TITLE = "Next47";
...
function transformRound308Source(source) {
  let transformed = source.replace(/\r\n/g, "\n")
    ...
    .replaceAll("through round303", "through round395");
  ...
  transformed = replaceFirst(
    ...
    `${round303File}\n];`,
    `${priorRoundFiles}\n];`,
    "explicit duplicate files through Round395"
  );
  transformed = replaceOnce(
    transformed,
    `${round303File}\n];`,
    `${priorRoundFiles}\n];`,
    "required screened files through Round395"
  );
  ...
  transformed = replaceOnce(
    ...
    "screened rounds include Round308 through Round395"
  );
  ...
  transformed = replaceOnce(
    ...
    "validation check fields through Round395"
  );
  ...
  transformed = replaceOnce(
    ...
    "validation report lines through Round395"
  );
  ...
  assertContains(transformed, "round400_nyc_dob_co_next47", "Round400 output path");
  assertContains(transformed, "Round400 NYC DOB CO Next47 Candidate Pack", "Round400 notes title");
  assertContains(transformed, "nyc_dob_co_round400_legacy_", "Round400 candidate ID prefix");
  assertContains(transformed, "tmp/subagents/round395_nyc_dob_co_next46/candidates.json", "Round395 screened file");
  assertContains(transformed, "through round395", "duplicate-screening text through Round395");
  assertContains(transformed, "required_round395_screened", "Round395 validation result");
  ...
  assertContains(transformed, `transformation_method: "${ROUND_TITLE} queried`, "Round400 source audit transformation method");
  assertContains(transformed, "scripts/fetch_round400_nyc_dob_co_next47_candidates.js standalone validation artifact", "Round400 validator label");
...
async function main() {
  ...
  if (typeof sandbox.module.exports.main !== "function") {
    throw new Error("Transformed Round400 generator did not export main().");
  }
