...
const outDir = "tmp/subagents/round246_nyc_hpd_affordable_housing_next7";
...
const roundName = "Round246";
const maxScreeningRound = 241;
...
function appearsExisting(existing, eventId, sourceRecordId, sourceUrl, sourceDateField, date) {
  if (eventId && existing.eventIds.has(eventId)) {
    return "event_id already appears in the live corpus or HPD candidate packs through round241.";
  }
  if (sourceRecordId && (existing.sourceRecordIds.has(sourceRecordId) || existing.sourceRecordIds.has(`${DATASET.sourceId}:${sourceRecordId}`))) {
    return "HPD project/building source row already appears in the live corpus or HPD candidate packs through round241.";
  }
  if (sourceUrl && existing.sourceUrls.has(normalizeUrlToken(sourceUrl))) {
    return "HPD Socrata row URL already appears in the live corpus or HPD candidate packs through round241.";
  }
  ...
  if (existing.sourceDateKeys.has(key)) {
    return "HPD source/date key already appears in the live corpus or HPD candidate packs through round241.";
  }
  return "";
}
...
function buildCandidate(row, existing, rejected) {
  ...
  return {
    ...
    raw_row: row,
    transformation_method: `${roundName} official NYC Open Data HPD fetch from ${DATASET.id}; selected from rows with project/building identifiers, a source date, source coordinates, unit counts, and duplicate screening against the live manual architecture corpus plus HPD-family candidate packs through round241, explicitly including round229, round234, round236, and round241.`
  };
}
...
function sourceAudit(metadata, acceptedCount, selectedSummary) {
  ...
  return {
    schema_version: "round246.nyc_hpd_affordable_housing_next7_source_audit.v1",
    ...
    sources: [
      {
        ...
        published_coverage_note: "Round246 checked HPD building-level rows with coordinates and building/project completion or project start dates available through the access date.",
...
function validateGeneratedPack(candidatesPayload, sourceAuditPayload, summaryPayload, existing) {
  ...
  return {
    ...
    checks: [
      ...
      "no overlap with live corpus or HPD candidate packs through round241, including round229, round234, round236, and round241"
    ],
...
async function main() {
  ...
  const summaryPayload = {
    schema_version: "round246.nyc_hpd_affordable_housing_next7_summary.v1",
    generated_at: generatedAt,
    accessed_at: accessedAt,
    output_files: [
    ...
    selection_summary: {
      ...
      duplicate_screening: {
        ...
        candidate_files_considered_through_round241: existing.candidateFilesConsidered,
      ...
      ranking: "Source date priority is applied first: building_completion_date rows, then project_completion_date rows, then project_start_date rows. Within each date-field tier, new-construction rows are preferred before unit-count score, senior/supportive text signals, and newer dates.",
  ...
  const candidatesPayload = {
    schema_version: "round246.nyc_hpd_affordable_housing_next7_candidates.v1",
    generated_at: generatedAt,
    accessed_at: accessedAt,
  ...
  const rejectedPayload = {
    schema_version: "round246.nyc_hpd_affordable_housing_next7_rejected.v1",
    generated_at: generatedAt,
    accessed_at: accessedAt,
    source_id: DATASET.sourceId,
    dataset_id: DATASET.id,
