...
const OUT_DIR = "tmp/subagents/round143_nyc_dob_now_even_more";
...
function buildCandidate(app, permit, metadataContext, selectionMode) {
  ...
  const candidateId = `nyc_dob_now_even_more_rbx6_${slugify(cleanText(permit.work_permit), 64)}_${slugify(cleanText(permit.sequence_number) || "seq", 20)}_${slugify(cleanText(permit.tracking_number) || "tracking", 36)}_${issuedDate.replace(/-/g, "_")}`;
  ...
  return {
    ...
    limitations,
    transformation_method: `Round143 generator queried DOB NOW approved permit rows directly for ${START_DATE} through ${END_DATE}, joined them to DOB NOW job application rows, retained one nonduplicate high-signal administrative permit row per base DOB NOW job beyond the current manual corpus plus round133/round136 candidate packs when present, and preserved row-level permit/application provenance. Selection mode: ${selectionMode}.`,
    duplicate_check_note: "Exact job_filing_number, base DOB NOW job number, work_permit, tracking_number, source_record_id, source_url, candidate_id, and event_id tokens were screened against the manual architecture corpus and existing tmp candidate packs available during this run, including round133 and round136 outputs when present.",
...
async function main() {
  ...
  const selectionSummary = {
    ...
    duplicate_index_files: existing.candidateFiles.map((file) => file.replace(/\\/g, "/")).filter((file) => /round133|round136|nyc|architecture/i.test(file)).sort(),
  ...
  const sourceAudits = [
    {
      ...
      granularity: "One approved permit row; this generator retains one substantial initial permit row per base DOB NOW job filing beyond existing round133/round136/manual-corpus tokens.",
  ...
  const output = {
    schema_version: "round143.nyc_dob_now_even_more_candidates.v1",
    generated_at: GENERATED_AT,
    accessed_at: ACCESSED_AT,
    worker: "Round143 NYC DOB NOW even more candidates",
    scope: "Official NYC DOB NOW Build approved-permit/job-filing candidate pack beyond round133 and round136 when present. Records are administrative permit/job milestones and must not be presented as construction start, completion, opening, occupancy, or outcome evidence.",
    candidate_count: candidates.length,
  ...
  const sourceAudit = {
    schema_version: "round143.nyc_dob_now_even_more_source_audit.v1",
    generated_at: GENERATED_AT,
    accessed_at: ACCESSED_AT,
  ...
  const summary = {
    schema_version: "round143.nyc_dob_now_even_more_summary.v1",
    generated_at: GENERATED_AT,
    accessed_at: ACCESSED_AT,
    output_files: [
    ...
    duplicate_screening: {
      ...
      screened_round133_or_round136_files: selectionSummary.duplicate_index_files.filter((file) => /round133|round136/i.test(file))
    },
    selection_summary: selectionSummary,
    caveats: [
  ...
  const rejectOutput = {
    schema_version: "round143.nyc_dob_now_even_more_rejections.v1",
    generated_at: GENERATED_AT,
    accessed_at: ACCESSED_AT,
    rejected_counts: rejectionCounter(rejections),
  ...
  const notes = [
    "# Round143 NYC DOB NOW Even More Candidate Pack",
    "",
    ...
    "## Duplicate Screening",
    "",
    `Screened against ${existing.filesRead} manual-corpus/tmp candidate files and ${existing.tokens.size} exact identifier tokens, including round133/round136 outputs when present.`,
    "",
    "## Counts",
    "",
