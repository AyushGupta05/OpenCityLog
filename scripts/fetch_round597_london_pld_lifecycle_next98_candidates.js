...
function assertContains(value, needle, label) {
  if (!value.includes(needle)) {
    throw new Error(`Round597 wrapper transform failed to include ${label}: ${needle}`);
  }
}
...
function replaceRequired(value, needle, replacement, label) {
  if (!value.includes(needle)) {
    ...
    throw new Error(`Round597 wrapper transform could not find ${label}: ${needle}`);
  }
...
function transformRound579Wrapper(source) {
  let transformed = source
    .replace(/round579_london_pld_lifecycle_next94/g, "round597_london_pld_lifecycle_next98")
    .replace(/round579\.london_pld_lifecycle_next94/g, "round597.london_pld_lifecycle_next98")
    .replace(/Bims5Round579PldLifecycleNext94/g, "Bims5Round597PldLifecycleNext98")
    .replace(/Round 579/g, "Round 597")
    .replace(/Round579/g, "Round597")
    .replace(/round579/g, "round597")
    .replace(/next94/g, "next98")
    .replace(/MAX_PRIOR_ROUND = 576/g, "MAX_PRIOR_ROUND = 594")
    .replace(/through round576/g, "through round594")
    .replace(/packRound\(entry\.name\) > 576/g, "packRound(entry.name) > 594")
    .replace(/\.replace\(\/round276\/g, "round576"\);/g, '.replace(/round276/g, "round594");');
  ...
  transformed = replaceRequired(
    ...
    "round579, round584, round589, and round594 prior pack insertion"
  );

  transformed = transformed
    .replace(
      /pack\) => pack\.label === "round576_london_pld_lifecycle_next93" && pack\.exists === true/g,
      'pack) => pack.label === "round594_london_pld_lifecycle_next97" && pack.exists === true'
    )
    .replace(/round576 prior PLD lifecycle pack was scanned/g, "round594 prior PLD lifecycle pack was scanned");

  assertContains(transformed, 'const ROUND_NAME = "round597_london_pld_lifecycle_next98";', "round name");
  assertContains(transformed, 'const ROUND = "round597";', "transformed fetch round id");
  assertContains(transformed, "MAX_PRIOR_ROUND = 594", "prior cutoff");
  assertContains(transformed, "through round594", "dedupe cutoff text");
  ...
  assertContains(transformed, "round589_london_pld_lifecycle_next96", "round589 prior pack");
  assertContains(transformed, "round594_london_pld_lifecycle_next97", "round594 prior pack");
  assertContains(transformed, "addRound597RetrievedAtAlias", "candidate retrieved_at alias");
  assertContains(transformed, 'CURRENT_ROUND_NAME = "round597_london_pld_lifecycle_next98"', "validator round name");
  assertContains(transformed, "round597_strict_duplicate_audit", "strict duplicate audit label");
...
async function main() {
  ...
  vm.runInNewContext(transformed, sandbox, {
    filename: "generated_round597_london_pld_lifecycle_next98_candidates.js"
  });
}
