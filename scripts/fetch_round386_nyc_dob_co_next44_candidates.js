...
const GENERATED_FILENAME = "scripts/fetch_round386_nyc_dob_co_next44_candidates.generated.js";
const ROUND = "round386";
const ROUND_TITLE = "Round386";
const NEXT_LABEL = "next44";
const NEXT_TITLE = "Next44";
...
function transformRound308Source(source) {
  let transformed = source.replace(/\r\n/g, "\n")
    ...
    .replaceAll("through round303", "through round382");
  ...
  transformed = replaceFirst(
    ...
    `${round303File}\n];`,
    `${priorRoundFiles}\n];`,
    "explicit duplicate files through Round382"
  );
  transformed = replaceOnce(
    transformed,
    `${round303File}\n];`,
    `${priorRoundFiles}\n];`,
    "required screened files through Round382"
  );
  ...
  transformed = replaceOnce(
    ...
    "screened rounds include Round308 through Round382"
  );
  ...
  transformed = replaceOnce(
    ...
    "validation check fields through Round382"
  );
  ...
  transformed = replaceOnce(
    ...
    "validation report lines through Round382"
  );
  ...
  assertContains(transformed, "round386_nyc_dob_co_next44", "Round386 output path");
  assertContains(transformed, "Round386 NYC DOB CO Next44 Candidate Pack", "Round386 notes title");
  assertContains(transformed, "nyc_dob_co_round386_legacy_", "Round386 candidate ID prefix");
  assertContains(transformed, "tmp/subagents/round382_nyc_dob_co_next43/candidates.json", "Round382 screened file");
  assertContains(transformed, "through round382", "duplicate-screening text through Round382");
  assertContains(transformed, "required_round382_screened", "Round382 validation result");
  ...
  assertContains(transformed, `transformation_method: "${ROUND_TITLE} queried`, "Round386 source audit transformation method");
  assertContains(transformed, "scripts/fetch_round386_nyc_dob_co_next44_candidates.js standalone validation artifact", "Round386 validator label");
...
async function main() {
  ...
  if (typeof sandbox.module.exports.main !== "function") {
    throw new Error("Transformed Round386 generator did not export main().");
  }
