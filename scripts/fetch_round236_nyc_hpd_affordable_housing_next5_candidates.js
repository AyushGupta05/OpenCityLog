...
const outDir = "tmp/subagents/round236_nyc_hpd_affordable_housing_next5";
...
function hpdScore(row) {
  ...
  const newConstruction = isNewConstruction(row);
...
function buildCandidate(row, existing, rejected) {
  ...
  return {
    ...
    limitations: "HPD Affordable Housing Production by Building is administrative housing-program delivery evidence. It is not a complete citywide construction inventory, DOB final certificate, first occupancy record, tenant move-in record, as-built footprint, affordability-duration audit, causal outcome evidence, or proof of occupancy/outcomes.",
    area: `${address || row.project_name || "HPD affordable housing production row"}, ${borough || "New York City"}, New York City`,
    raw_row: row,
    transformation_method: `${roundName} official NYC Open Data HPD fetch from ${DATASET.id}; selected from rows with project/building identifiers, a source date, source coordinates, unit counts, and duplicate screening against the live manual corpus plus prior and pending HPD candidate packs including round229 and round234.`
  };
}
...
function sourceAudit(metadata, acceptedCount, selectedSummary) {
  ...
  return {
    schema_version: "round236.nyc_hpd_affordable_housing_next5_source_audit.v1",
    ...
    sources: [
      {
        ...
        published_coverage_note: "Round236 checked HPD building-level rows with coordinates and building/project completion or project start dates available through the access date.",
        ...
        required_caveats: [
          ...
          "No causality, forecast, neighborhood impact, first-occupancy, or outcome claim is made from this dataset."
        ],
...
function validateGeneratedPack(candidatesPayload, sourceAuditPayload, summaryPayload, existing) {
  ...
  for (const candidate of candidates) {
    ...
    if (candidate.date !== candidate.effective_date) throw new Error(`Validation failed: date/effective_date mismatch on ${candidate.event_id}.`);
    ...
    const sourceDateKeyValue = sourceDateKey({
    ...
    if (sourceDateKeys.has(sourceDateKeyValue)) {
      throw new Error(`Validation failed: duplicate source/date key ${sourceDateKeyValue}.`);
    }
    sourceDateKeys.add(sourceDateKeyValue);
  }
  ...
  return {
    ...
    checks: [
      ...
      "no overlap with live corpus or prior/pending HPD candidate packs through round234"
    ],
...
async function main() {
  ...
  const summaryPayload = {
    schema_version: "round236.nyc_hpd_affordable_housing_next5_summary.v1",
    generated_at: generatedAt,
    accessed_at: accessedAt,
    output_files: [
  ...
  const candidatesPayload = {
    schema_version: "round236.nyc_hpd_affordable_housing_next5_candidates.v1",
    generated_at: generatedAt,
    accessed_at: accessedAt,
  ...
  const rejectedPayload = {
    schema_version: "round236.nyc_hpd_affordable_housing_next5_rejected.v1",
    generated_at: generatedAt,
    accessed_at: accessedAt,
    source_id: DATASET.sourceId,
    dataset_id: DATASET.id,
  ...
  const notes = [
    "# Round236 NYC HPD Affordable Housing Production Candidate Pack",
    "",
    ...
    "## Caveats",
    "",
    "- HPD production rows are administrative housing-program delivery evidence, not a complete citywide construction inventory or full building-completion survey.",
    ...
    "- Duplicate screening used the live manual corpus and prior/pending HPD candidate packs including round229 and round234; this script did not edit the corpus, appender, or generated atlas files."
  ].join("\n");
