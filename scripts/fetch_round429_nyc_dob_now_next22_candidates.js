...
const SCRIPT_PATH = path.join("scripts", "fetch_round429_nyc_dob_now_next22_candidates.js");
const OUT_DIR = path.join("tmp", "subagents", "round429_nyc_dob_now_next22");
...
const DOB_NOW_ROUNDS_THROUGH_422 = [
...
const PRIOR_ROUND_REGEX_THROUGH_422 = [
  "round112",
  "round117",
  "round119",
  "round133",
...
function replaceRequired(source, needle, replacement, label) {
  if (!source.includes(needle)) {
    throw new Error(`Round429 wrapper patch failed; missing ${label || needle}`);
  }
...
function replaceOnce(source, needle, replacement, label) {
  ...
  if (count !== 1) {
    throw new Error(`Round429 wrapper patch failed for ${label}: expected one match, found ${count}`);
  }
...
function replaceRegexOnce(source, pattern, replacement, label) {
  ...
  if (count !== 1) {
    throw new Error(`Round429 wrapper patch failed for ${label}: expected one regex match, found ${count}`);
  }
  return output;
}
...
function transformRound293Source(source) {
  ...
  transformed = transformed
    .replace(/Round293/g, "Round429")
    .replace(/round293/g, "round429")
    .replace(/Next16/g, "Next22")
    .replace(/next16/g, "next22")
    .replace(/DOB_NOW_ROUNDS_THROUGH_237/g, "DOB_NOW_ROUNDS_THROUGH_422")
    .replace(/DOB NOW packs through round237/g, "DOB NOW packs through round422")
    .replace(/DOB NOW through round237/g, "DOB NOW through round422")
  ...
  transformed = replaceRegexOnce(
    transformed,
    /const DOB_NOW_ROUNDS_THROUGH_422 = \[[\s\S]*?\];/,
    `const DOB_NOW_ROUNDS_THROUGH_422 = ${JSON.stringify(DOB_NOW_ROUNDS_THROUGH_422, null, 2)};`,
...
async function runTransformedRound293Main() {
  ...
  if (typeof sandbox.module.exports.main !== "function") {
    throw new Error("Transformed Round429 DOB NOW generator did not export main().");
  }
...
function corpusStats() {
  ...
  stats.round422_records = records.filter((record) => {
    const text = JSON.stringify(record);
    return text.includes("round422") || text.includes("Round422") || text.includes("nyc_dob_now_next21");
  }).length;
  stats.contains_round422 = stats.round422_records > 0;
  return stats;
}

function addRound429ArtifactContext() {
  ...
  const priorScope = {
    ...
    dob_now_rounds_through: "round422",
    dob_co_rounds_screened: "available tmp/subagents DOB CO packs",
    dob_now_rounds_screened: DOB_NOW_ROUNDS_THROUGH_422
  };

  candidatesDoc.worker = "Round429 NYC DOB NOW next22 candidates";
  candidatesDoc.scope = "Official NYC DOB NOW Build approved-permit/job-filing candidate pack after screening the live manual architecture corpus and prior DOB NOW packs through Round422, with available DOB permit/application and DOB Certificate of Occupancy packs also screened. Records are administrative filing/permit/issued milestones and must not be presented as construction start, construction completion, opening, occupancy, impact, or causation evidence.";
  ...
  for (const candidate of candidates) {
    ...
    candidate.transformation_method = cleanText(candidate.transformation_method)
      .replace(/round237/gi, "round422")
      .replace(/round289/gi, "available tmp/subagents DOB CO packs")
      .replace("current manual corpus plus round117 DOB filings/permits, round119 DOB legacy permits, DOB NOW packs through round422, DOB Certificate of Occupancy packs available under tmp/subagents, and other DOB permit/application packs available under tmp/subagents candidate packs", "live manual corpus plus prior DOB permit/application packs, all DOB NOW packs through round422, and DOB Certificate of Occupancy packs available under tmp/subagents");
    candidate.method = candidate.transformation_method;
    candidate.duplicate_check_note = "Exact job_filing_number, base DOB NOW job number, work_permit, tracking_number, source_record_id, source_url, candidate_id, event_id, and title/date keys were screened against the live manual architecture corpus and prior NYC DOB NOW, DOB permit/application, and DOB Certificate of Occupancy candidate packs available during this run, including DOB NOW through Round422.";
    candidate.limitations = cleanText(candidate.limitations);
  }

  sourceAuditDoc.audit_scope = "Official NYC DOB NOW Build approved permits and job application filings, selected for residual administrative milestone candidate discovery after screening the live manual architecture corpus, DOB NOW packs through Round422, and available DOB permit/application and DOB Certificate of Occupancy packs.";
  ...
  summaryDoc.worker = "Round429 NYC DOB NOW next22 candidates";
  ...
  validationReport.validator = "scripts/fetch_round429_nyc_dob_now_next22_candidates.js --validate-only; reads emitted JSON plus manual corpus and prior DOB NOW/DOB permit/DOB CO candidate packs, then writes supplemental validation.json/readback.json";
  validationReport.checked_files = ALL_OUTPUTS;
  validationReport.checks = {
    ...(validationReport.checks || {}),
    manual_corpus: corpus,
    prior_dob_files_scanned: screenedDobAdminFiles,
    required_round422_screened: screenedDobAdminFiles.some((file) => /round422_nyc_dob_now/i.test(file)),
    required_round413_screened: screenedDobAdminFiles.some((file) => /round413_nyc_dob_now/i.test(file)),
    required_manual_corpus_round422_present: corpus.contains_round422,
  ...
  const notes = [
    "# Round429 NYC DOB NOW Next22 Candidate Pack",
    "",
    ...
    "## Duplicate Screening",
    "",
    `Screened against the live manual corpus (${corpus.records} records; Round422 present: ${corpus.contains_round422}) plus prior DOB NOW packs through Round422 and available DOB administrative candidate packs.`,
    "",
    "## Counts",
    "",
...
function buildReadbackAndValidation() {
  ...
  if (validationReport.checks?.required_round422_screened !== true) errors.push("validation_report.json does not confirm Round422 DOB NOW screening");
  if (validationReport.checks?.required_manual_corpus_round422_present !== true) errors.push("validation_report.json does not confirm Round422 is present in the screened manual corpus");
  ...
  const readback = {
    schema_version: "round429.nyc_dob_now_next22_readback.v1",
    ...
    prior_screening: {
      ...
      manual_corpus: corpus,
      dob_now_through_round422_screened: validationReport.checks?.required_round422_screened === true,
  ...
  const validation = {
    schema_version: "round429.nyc_dob_now_next22_validation.v1",
    generated_at: GENERATED_AT,
    accessed_at: ACCESSED_AT,
    ok: errors.length === 0,
    ...
    checks: {
      ...
      required_round422_screened: validationReport.checks?.required_round422_screened === true,
      required_manual_corpus_round422_present: validationReport.checks?.required_manual_corpus_round422_present === true,
      no_overlap_with_screened_corpus_and_prior_packs: validationReport.checks?.no_overlap_with_screened_corpus_and_prior_packs === true,
  ...
  if (errors.length) {
    throw new Error(`Round429 validation failed with ${errors.length} errors. See ${path.join(OUT_DIR, "validation.json")}`);
  }
...
async function main() {
  ...
  addRound429ArtifactContext();
  const { validation } = buildReadbackAndValidation();
  console.log(`Round429 validation passed for ${validation.checks.candidate_count} candidates.`);
}
