...
const SCRIPT_PATH = path.join("scripts", "fetch_round435_nyc_dob_now_next23_candidates.js");
const OUT_DIR = path.join("tmp", "subagents", "round435_nyc_dob_now_next23");
...
const DOB_NOW_ROUNDS_THROUGH_429 = [
...
const PRIOR_ROUND_REGEX_THROUGH_429 = [
  "round112",
  "round117",
  "round119",
  "round133",
...
function replaceRequired(source, needle, replacement, label) {
  if (!source.includes(needle)) {
    throw new Error(`Round435 wrapper patch failed; missing ${label || needle}`);
  }
...
function replaceOnce(source, needle, replacement, label) {
  ...
  if (count !== 1) {
    throw new Error(`Round435 wrapper patch failed for ${label}: expected one match, found ${count}`);
  }
...
function replaceRegexOnce(source, pattern, replacement, label) {
  ...
  if (count !== 1) {
    throw new Error(`Round435 wrapper patch failed for ${label}: expected one regex match, found ${count}`);
  }
  return output;
}
...
function transformRound293Source(source) {
  ...
  transformed = transformed
    .replace(/Round293/g, "Round435")
    .replace(/round293/g, "round435")
    .replace(/Next16/g, "Next23")
    .replace(/next16/g, "next23")
    .replace(/DOB_NOW_ROUNDS_THROUGH_237/g, "DOB_NOW_ROUNDS_THROUGH_429")
    .replace(/DOB NOW packs through round237/g, "DOB NOW packs through Round429")
    .replace(/DOB NOW through round237/g, "DOB NOW through Round429")
  ...
  transformed = replaceRegexOnce(
    transformed,
    /const DOB_NOW_ROUNDS_THROUGH_429 = \[[\s\S]*?\];/,
    `const DOB_NOW_ROUNDS_THROUGH_429 = ${JSON.stringify(DOB_NOW_ROUNDS_THROUGH_429, null, 2)};`,
  ...
  transformed = replaceRequired(
    ...
    "Round435 validation prior markers"
  );
...
async function runTransformedRound293Main() {
  ...
  if (typeof sandbox.module.exports.main !== "function") {
    throw new Error("Transformed Round435 DOB NOW generator did not export main().");
  }
...
function addRound435ArtifactContext() {
  ...
  const priorScope = {
    ...
    dob_now_rounds_through: "round429",
    dob_co_rounds_screened: "available tmp/subagents DOB CO packs",
    dob_now_rounds_screened: DOB_NOW_ROUNDS_THROUGH_429
  };

  candidatesDoc.worker = "Round435 NYC DOB NOW Next23 candidates";
  candidatesDoc.scope = "Official NYC DOB NOW Build approved-permit/job-filing candidate pack after screening the live manual architecture corpus and prior DOB NOW packs through Round429, with available DOB permit/application and DOB Certificate of Occupancy packs also screened. Records are administrative filing/permit/issued milestones and must not be presented as construction start, construction completion, opening, occupancy, impact, or causation evidence.";
  ...
  for (const candidate of candidates) {
    ...
    candidate.transformation_method = cleanText(candidate.transformation_method)
      .replace(/round237/gi, "round429")
      .replace(/round289/gi, "available tmp/subagents DOB CO packs")
      .replace("current manual corpus plus round117 DOB filings/permits, round119 DOB legacy permits, DOB NOW packs through Round429, DOB Certificate of Occupancy packs available under tmp/subagents, and other DOB permit/application packs available under tmp/subagents candidate packs", "live manual corpus plus prior DOB permit/application packs, all DOB NOW packs through Round429, and DOB Certificate of Occupancy packs available under tmp/subagents");
    candidate.method = candidate.transformation_method;
    candidate.duplicate_check_note = "Exact job_filing_number, base DOB NOW job number, work_permit, tracking_number, source_record_id, source_url, candidate_id, event_id, and title/date keys were screened against the live manual architecture corpus and prior NYC DOB NOW, DOB permit/application, and DOB Certificate of Occupancy candidate packs available during this run, including DOB NOW through Round429.";
    candidate.limitations = cleanText(candidate.limitations);
  }

  sourceAuditDoc.audit_scope = "Official NYC DOB NOW Build approved permits and job application filings, selected for residual administrative milestone candidate discovery after screening the live manual architecture corpus, DOB NOW packs through Round429, and available DOB permit/application and DOB Certificate of Occupancy packs.";
  ...
  summaryDoc.worker = "Round435 NYC DOB NOW Next23 candidates";
  ...
  validationReport.validator = "scripts/fetch_round435_nyc_dob_now_next23_candidates.js --validate-only; reads emitted JSON plus manual corpus and prior DOB NOW/DOB permit/DOB CO candidate packs, then writes supplemental validation.json/readback.json";
  ...
  const notes = [
    ...
    "## Duplicate Screening",
    "",
    `Screened against the live manual corpus (${corpus.records} records; Round429 present: ${corpus.contains_round429}) plus prior DOB NOW packs through Round429 and available DOB administrative candidate packs.`,
    "",
    "## Counts",
    "",
...
function buildReadbackAndValidation() {
  ...
  const readback = {
    schema_version: "round435.nyc_dob_now_next23_readback.v1",
    ...
    prior_screening: {
      ...
      manual_corpus: corpus,
      dob_now_through_round429_screened: validationReport.checks?.required_round429_screened === true,
  ...
  const validation = {
    schema_version: "round435.nyc_dob_now_next23_validation.v1",
    generated_at: GENERATED_AT,
    accessed_at: ACCESSED_AT,
    ok: errors.length === 0,
  ...
  if (errors.length) {
    throw new Error(`Round435 validation failed with ${errors.length} errors. See ${path.join(OUT_DIR, "validation.json")}`);
  }
...
async function main() {
  ...
  addRound435ArtifactContext();
  const { validation } = buildReadbackAndValidation();
  console.log(`Round435 validation passed for ${validation.checks.candidate_count} candidates.`);
}
