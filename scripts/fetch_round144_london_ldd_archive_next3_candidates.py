...
TASK_ID = "round144_london_ldd_archive_next3_candidates"
SCRIPT_PATH = "scripts/fetch_round144_london_ldd_archive_next3_candidates.py"
...
OUT_DIR = Path("tmp/subagents/round144_london_ldd_archive_next3")
...
def event_source_ids(event):
    ids = set()
    source_ids = event.get("source_ids")
    if isinstance(source_ids, list):
        ids.update(clean_text(source_id) for source_id in source_ids if clean_text(source_id))
    elif source_ids:
        ids.add(clean_text(source_ids))
...
def ensure_file(path, url):
    if path.exists():
        return path
    cache_path = OUT_DIR / "source_workbooks" / path.name
...
def existing_ldd_dedupe():
    ...
    for event in doc.get("events", []):
        source_ids = event_source_ids(event)
...
def previous_candidate_pack_dedupe(path):
    rows = set()
    refs = set()
    source_keys = set()
    title_dates = set()
    ...
    for candidate in candidates:
        ...
        if ref_match:
            refs.add(authority_ref_key(ref_match.group(1), ref_match.group(2)))
        source_keys.add(source_key(candidate.get("source_url"), record_id))
        title_dates.add(title_date_key(candidate.get("city_id"), candidate.get("title"), candidate.get("date")))
    return {
        "rows": rows,
        "refs": refs,
        "source_keys": source_keys,
...
def candidate_for(row, excel_row, score, flags, floorspace_refs, bedroom_refs):
    ...
    candidate_id = f"lon_ldd_archive_next3_round144_row_{excel_row}_{slugify(authority, 36)}_{slugify(borough_ref, 48)}"
    row_reference = f"{authority} / {borough_ref}"
    ...
    transformation_method = (
        ...
        "round132 and round135 candidate packs; filtered to Current permission status "
...
def write_notes(candidate_count, summary):
    text = f"""# Round 144 London LDD archive next3 candidates

Generated another bounded London Development Database archive candidate pack beyond already ingested LDD rows in the manual architecture corpus and the prior round132/round135 candidate packs.
...
The generator excludes manual-corpus LDD records by workbook row, planning authority plus borough reference, source URL plus source record id, and candidate title/date. It also checks the prior round132 and round135 candidate packs as fallback dedupe inputs and removes duplicate source rows and authority/reference pairs inside the new batch.
...
def main():
    ...
    permissions_xlsx = ensure_file(PERMISSIONS_XLSX, PERMISSIONS_DOWNLOAD)
    floorspace_xlsx = ensure_file(FLOORSPACE_XLSX, FLOORSPACE_DOWNLOAD)
    bedrooms_xlsx = ensure_file(BEDROOMS_XLSX, BEDROOMS_DOWNLOAD)

    dedupe = existing_ldd_dedupe()
    prior_round132 = previous_candidate_pack_dedupe(PREVIOUS_ROUND132_PATH)
    prior_round135 = previous_candidate_pack_dedupe(PREVIOUS_ROUND135_PATH)
    ...
    for excel_row, row in iter_xlsx_dict_rows(permissions_xlsx):
        ...
        elif excel_row in prior_round132["rows"]:
            reason = "previous round132 candidate workbook row not found in corpus"
        elif ref_key in prior_round132["refs"]:
            reason = "previous round132 candidate authority/reference not found in corpus"
        elif skey in prior_round132["source_keys"]:
            reason = "previous round132 candidate source key not found in corpus"
        elif tdate_key in prior_round132["title_dates"]:
            reason = "previous round132 candidate title/date not found in corpus"
        elif excel_row in prior_round135["rows"]:
            reason = "previous round135 candidate workbook row not found in corpus"
        elif ref_key in prior_round135["refs"]:
            reason = "previous round135 candidate authority/reference not found in corpus"
        elif skey in prior_round135["source_keys"]:
            reason = "previous round135 candidate source key not found in corpus"
        elif tdate_key in prior_round135["title_dates"]:
            reason = "previous round135 candidate title/date not found in corpus"
        elif excel_row in seen_rows or ref_key in seen_refs or skey in seen_source_keys or tdate_key in seen_title_dates:
            reason = "duplicate inside round144 batch scan"
        ...
        if len(rejected_sample) < 300 and reason not in {
            ...
            "previous round132 candidate workbook row not found in corpus",
            "previous round132 candidate authority/reference not found in corpus",
            "previous round135 candidate workbook row not found in corpus",
            "previous round135 candidate authority/reference not found in corpus",
        }:
    ...
    selection_summary = {
        ...
        "round132_candidate_pack_path": str(PREVIOUS_ROUND132_PATH),
        "round132_candidate_rows_seen": len(prior_round132["rows"]),
        "round132_candidate_rows_now_in_manual_corpus": len(prior_round132["rows"] & dedupe["rows"]),
        "round132_candidate_authority_reference_pairs_now_in_manual_corpus": len(prior_round132["refs"] & dedupe["refs"]),
        "round135_candidate_pack_path": str(PREVIOUS_ROUND135_PATH),
        "round135_candidate_rows_seen": len(prior_round135["rows"]),
        "round135_candidate_rows_now_in_manual_corpus": len(prior_round135["rows"] & dedupe["rows"]),
        "round135_candidate_authority_reference_pairs_now_in_manual_corpus": len(prior_round135["refs"] & dedupe["refs"]),
        "dedupe_fields": [
            ...
            "city/title/date",
            "prior round132 and round135 candidate packs as fallback dedupe inputs if corpus ingestion is incomplete",
            "same fields inside the round144 batch scan",
        ],
        "filters": [
    ...
    print(json.dumps({
        ...
        "round132CandidateRowsSeen": len(prior_round132["rows"]),
        "round132CandidateRowsNowInManualCorpus": len(prior_round132["rows"] & dedupe["rows"]),
        "round135CandidateRowsSeen": len(prior_round135["rows"]),
        "round135CandidateRowsNowInManualCorpus": len(prior_round135["rows"] & dedupe["rows"]),
