...
function replaceRequired(value, needle, replacement, label) {
  if (!value.includes(needle)) {
    ...
    throw new Error(`Round365 source transform could not find ${label}: ${needle}`);
  }
...
function assertContains(value, needle, label) {
  if (!value.includes(needle)) {
    throw new Error(`Round365 source transform failed to include ${label}: ${needle}`);
  }
}

function transformRound362Script(source) {
  let transformed = source
    .replace(/round362_london_pld_lifecycle_next42/g, "round365_london_pld_lifecycle_next43")
    .replace(/round362\.london_pld_lifecycle_next42/g, "round365.london_pld_lifecycle_next43")
    .replace(/Bims5Round362PldLifecycleNext42/g, "Bims5Round365PldLifecycleNext43")
    .replace(/Round362/g, "Round365")
    .replace(/Round 362/g, "Round 365")
    .replace(/round362/g, "round365")
    .replace(/next42/g, "next43")
    .replace(/MAX_PRIOR_ROUND = 358/g, "MAX_PRIOR_ROUND = 362")
    .replace(/through round358/g, "through round362");
  ...
  transformed = replaceRequired(
    ...
    '      "round272, round276, round284, round288, round298, round301, round306, round311, round316, round323, round327, round328, round333, round338, round343, round348, round350, round354, round358, and round362"\\n    );',
  ...
  assertContains(transformed, "through round362", "dedupe prose cutoff");
  return transformed;
}
...
function main() {
  ...
  vm.runInNewContext(
    ...
    {
      filename: "generated_round365_london_pld_lifecycle_next43_candidates.js"
    }
  );
}
