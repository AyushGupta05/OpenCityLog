...
source = source
  .replace(/const ROUND = 471;/g, "const ROUND = 492;")
  .replace(/round471_nyc_lpc_permit_next66/g, "round492_nyc_lpc_permit_next73")
  .replace(/round471/g, "round492")
  .replace(/Round471/g, "Round492")
  .replace(/next66/g, "next73")
  .replace(/Next66/g, "Next73")
  .replace(/through round466/g, "through round491");
...
function writeRound492DuplicateAudit() {
  if (duplicateAuditWritten || process.exitCode) return;
  duplicateAuditWritten = true;

  const root = path.resolve(__dirname, "..");
  const outDir = path.join(root, "tmp", "subagents", "round492_nyc_lpc_permit_next73");
  ...
  const audit = {
    ...
    audit_scope: "Round492 duplicate audit for NYC LPC Permit Application Information next73 candidates.",
    dedupe_boundary: {
...
process.on("beforeExit", writeRound492DuplicateAudit);
