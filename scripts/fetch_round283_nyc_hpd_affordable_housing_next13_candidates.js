...
function replaceChecked(pattern, replacement, label) {
  ...
  if (next === source) {
    throw new Error(`Unable to adapt round251 HPD fetcher for round283: missing ${label}.`);
  }
  source = next;
}
...
replaceChecked(
  ...
  'const outDir = "tmp/subagents/round283_nyc_hpd_affordable_housing_next13";',
  "output directory"
);
replaceChecked('const accessedAt = "2026-05-19";', 'const accessedAt = "2026-05-20";', "access date");
replaceChecked('const roundName = "Round251";', 'const roundName = "Round283";', "round name");
replaceChecked("const maxScreeningRound = 246;", "const maxScreeningRound = 275;", "screening round");
replaceChecked(/Round251/g, "Round283", "round label references");
replaceChecked(/round251\.nyc/g, "round283.nyc", "schema round prefix");
replaceChecked(
  /nyc_hpd_affordable_housing_next8/g,
  "nyc_hpd_affordable_housing_next13",
  "next pack label"
);
replaceChecked(
...
replaceChecked(
  ...
  "candidate_files_considered_through_round275",
  "candidate-files summary key"
);
replaceChecked(
  "async function main()",
  `const round283SelectionCache = new WeakMap();
...
function isClearlySupportedRound283PreservationCandidate(candidate) {
...
function isRound283PreferredCandidate(candidate) {
  if (candidate.source_date_field !== "building_completion_date") return false;
  if (isPreservation(candidate.raw_row)) return isClearlySupportedRound283PreservationCandidate(candidate);
  return true;
}

function selectedRound283CandidateIds(rankedCandidates) {
  if (round283SelectionCache.has(rankedCandidates)) return round283SelectionCache.get(rankedCandidates);
  const selectedIds = new Set();

  for (const candidate of rankedCandidates) {
    if (selectedIds.size >= targetCount) break;
    if (isRound283PreferredCandidate(candidate)) selectedIds.add(candidate.event_id);
  }
  ...
  round283SelectionCache.set(rankedCandidates, selectedIds);
  return selectedIds;
}

async function main()`,
  "round283 supported preservation selection helper"
);
replaceChecked(
  ".slice(0, targetCount)",
  ".filter((candidate, index, rankedCandidates) => selectedRound283CandidateIds(rankedCandidates).has(candidate.event_id))",
  "round283 selection call"
);
replaceChecked(
  ...
  "Source date priority is applied first: building_completion_date rows, then project_completion_date rows, then project_start_date rows. Round283 keeps the accepted pack to building_completion_date rows where available; Preservation rows are admitted only when the selected row has HPD building_completion_date, source coordinates, positive unit counts, and Extended Affordability Only = No. Within each date-field tier, New Construction rows remain ahead of Preservation rows before unit-count score, senior/supportive text signals, and newer dates.",
  "round283 ranking summary"
);
replaceChecked(
...
replaceChecked(
  ...
  '      "Coordinates are source geocoded points and should not be displayed as exact building footprints or project boundaries.",\n      "Round283 Preservation rows are retained only when HPD reports building_completion_date, coordinates, positive units, and Extended Affordability Only = No; they remain administrative delivery records, not independent proof of full rehabilitation scope or occupancy.",',
...
replaceChecked(
  ...
  '    "- Coordinates are HPD/Open Data geocoded points, not surveyed building footprints or project boundaries.",\n    "- Round283 Preservation rows are included only when HPD reports building_completion_date, coordinates, positive units, and Extended Affordability Only = No; this is still administrative delivery evidence, not independent proof of full rehabilitation scope or occupancy.",',
  "notes preservation caveat"
);
replaceChecked(
  /including round229, round234, round236, round241, and round246/g,
  "including round229, round234, round236, round241, round246, round251, round255, round263, round269, and round275",
  "explicit overlap caveat"
);
replaceChecked(/through round246/g, "through round275", "screening prose");

const runAdaptedRound251Fetcher = new Function(
  "require",
  "fetch",
  `${source}\n//# sourceURL=fetch_round283_nyc_hpd_affordable_housing_next13_candidates.adapted.js`
);
