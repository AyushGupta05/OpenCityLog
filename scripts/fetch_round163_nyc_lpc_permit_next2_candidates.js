...
const OUT_DIR = path.join(ROOT, "tmp", "subagents", "round163_nyc_lpc_permit_next2");
...
function collectExistingKeys() {
  ...
  if (corpus) {
    scannedFiles.push(path.relative(ROOT, CORPUS_PATH));
    for (const event of corpus.events || []) {
      if (event.city_id && event.city_id !== "nyc") continue;
      if (event.source_record_id) recordIds.add(normalizeRegulationNumber(event.source_record_id));
      if (event.source_url) sourceUrls.add(cleanText(event.source_url).toLowerCase());
      if (event.title && event.date) titleDates.add(titleDateKey(event.city_id || "nyc", event.title, event.date));
  ...
  for (const relative of PRIOR_PACK_PATHS) {
    ...
    for (const candidate of candidates) {
      if (candidate.source_record_id) recordIds.add(normalizeRegulationNumber(candidate.source_record_id));
  ...
  return { recordIds, titleDates, sourceUrls, scannedFiles, missingFiles };
}

async function fetchJson(url) {
  const response = await fetch(url, {
    headers: { "User-Agent": "Bims-5 round163 NYC LPC permit candidate fetcher" }
  });
  if (!response.ok) {
...
function candidateFrom(row) {
  ...
  const eventId = `nyc-lpc-permit-round163-${slugify(regulationNumber, 48)}`;
  ...
  return {
    ...
    transformation_method: `scripts/fetch_round163_nyc_lpc_permit_next2_candidates.js queried official NYC Open Data dpm2-m9mq rows with issue_date from ${START_DATE} through ${END_DATE}, required official latitude/longitude and regulation_number, scored architecture-facing WorkTypes/regulation_type values, deduplicated against the live manual corpus plus prior LPC permit/designation packs through round159 and any existing round163 output, and selected up to ${MAX_CANDIDATES} high-signal administrative milestones without inferring physical work.`,
...
function buildSourceAudit(metadata, queryUrl, sourceStats, existing) {
  ...
  return {
    ...
    audit_scope: "Official NYC LPC Permit Application Information source audit for round163 high-signal permit/application candidate extraction.",
    sources: [
      {
        source_id: SOURCE_ID,
        source_dataset_id: SOURCE_DATASET_ID,
...
function markdownNotes(summary) {
  return [
    "# Round163 NYC LPC Permit Application Information Next2 Candidates",
    "",
    ...
    "",
    "Rows were scored for architecture/preservation signal in WorkTypes and regulation_type, then balanced across years and regulation types. Dedupe checked the live manual architecture corpus and prior LPC permit/designation candidate packs named in the task.",
    "",
    "## Caveat",
    "",
...
async function main() {
  ...
  const summary = {
    generated_at: `${ACCESSED_AT}T00:00:00Z`,
    task: "Round163 NYC LPC Permit Application Information next2 candidate pack",
    source_id: SOURCE_ID,
    source_dataset_id: SOURCE_DATASET_ID,
