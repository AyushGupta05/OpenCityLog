...
function assertContains(value, needle, label) {
  if (!value.includes(needle)) {
    throw new Error(`Round594 wrapper transform failed to include ${label}: ${needle}`);
  }
}
...
function replaceRequired(value, needle, replacement, label) {
  if (!value.includes(needle)) {
    ...
    throw new Error(`Round594 wrapper transform could not find ${label}: ${needle}`);
  }
...
function transformRound579Wrapper(source) {
  let transformed = source
    .replace(/round579_london_pld_lifecycle_next94/g, "round594_london_pld_lifecycle_next97")
    .replace(/round579\.london_pld_lifecycle_next94/g, "round594.london_pld_lifecycle_next97")
    .replace(/Bims5Round579PldLifecycleNext94/g, "Bims5Round594PldLifecycleNext97")
    .replace(/Round 579/g, "Round 594")
    .replace(/Round579/g, "Round594")
    .replace(/round579/g, "round594")
    .replace(/next94/g, "next97")
    .replace(/MAX_PRIOR_ROUND = 576/g, "MAX_PRIOR_ROUND = 589")
    .replace(/through round576/g, "through round589")
    .replace(/packRound\(entry\.name\) > 576/g, "packRound(entry.name) > 589")
    .replace(/\.replace\(\/round276\/g, "round576"\);/g, '.replace(/round276/g, "round589");');
  ...
  transformed = replaceRequired(
    ...
    "round579, round584, and round589 prior pack insertion"
  );

  transformed = transformed
    .replace(
      /pack\) => pack\.label === "round576_london_pld_lifecycle_next93" && pack\.exists === true/g,
      'pack) => pack.label === "round589_london_pld_lifecycle_next96" && pack.exists === true'
    )
    .replace(/round576 prior PLD lifecycle pack was scanned/g, "round589 prior PLD lifecycle pack was scanned");

  assertContains(transformed, 'const ROUND_NAME = "round594_london_pld_lifecycle_next97";', "round name");
  assertContains(transformed, 'const ROUND = "round594";', "transformed fetch round id");
  assertContains(transformed, "MAX_PRIOR_ROUND = 589", "prior cutoff");
  assertContains(transformed, "through round589", "dedupe cutoff text");
  ...
  assertContains(transformed, "addRound594RetrievedAtAlias", "candidate retrieved_at alias");
  assertContains(transformed, 'CURRENT_ROUND_NAME = "round594_london_pld_lifecycle_next97"', "validator round name");
  assertContains(transformed, "round594_strict_duplicate_audit", "strict duplicate audit label");
...
async function main() {
  ...
  vm.runInNewContext(transformed, sandbox, {
    filename: "generated_round594_london_pld_lifecycle_next97_candidates.js"
  });
}
