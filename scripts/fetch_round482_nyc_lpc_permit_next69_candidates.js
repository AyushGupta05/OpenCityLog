...
source = source
  .replace(/const ROUND = 471;/g, "const ROUND = 482;")
  .replace(/round471_nyc_lpc_permit_next66/g, "round482_nyc_lpc_permit_next69")
  .replace(/round471/g, "round482")
  .replace(/Round471/g, "Round482")
  .replace(/next66/g, "next69")
  .replace(/Next66/g, "Next69")
  .replace(/through round466/g, "through round479");
...
function writeRound482DuplicateAudit() {
  if (duplicateAuditWritten || process.exitCode) return;
  duplicateAuditWritten = true;

  const root = path.resolve(__dirname, "..");
  const outDir = path.join(root, "tmp", "subagents", "round482_nyc_lpc_permit_next69");
  ...
  const audit = {
    ...
    audit_scope: "Round482 duplicate audit for NYC LPC Permit Application Information next69 candidates.",
    dedupe_boundary: {
      latest_lpc_permit_pack: "tmp/subagents/round479_nyc_lpc_permit_next68/candidates.json",
...
process.on("beforeExit", writeRound482DuplicateAudit);
