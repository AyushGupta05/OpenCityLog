...
function replaceRequired(value, needle, replacement, label) {
  if (!value.includes(needle)) {
    ...
    throw new Error(`Round358 source transform could not find ${label}: ${needle}`);
  }
...
function assertContains(value, needle, label) {
  if (!value.includes(needle)) {
    throw new Error(`Round358 source transform failed to include ${label}: ${needle}`);
  }
}

function transformRound327Script(source) {
  let transformed = source
    .replace(/round327_london_pld_lifecycle_next33/g, "round358_london_pld_lifecycle_next41")
    .replace(/round327\.london_pld_lifecycle_next33/g, "round358.london_pld_lifecycle_next41")
    .replace(/Bims5Round327PldLifecycleNext33/g, "Bims5Round358PldLifecycleNext41")
    .replace(/Round327/g, "Round358")
    .replace(/Round 327/g, "Round 358")
    .replace(/round327/g, "round358")
    .replace(/next33/g, "next41")
    .replace(/MAX_PRIOR_ROUND = 323/g, "MAX_PRIOR_ROUND = 354")
    .replace(/through round323/g, "through round354");

  transformed = replaceRequired(
    transformed,
    '      "round272, round276, round284, round288, round298, round301, round306, round311, round316, and round323"\n    );',
    '      "round272, round276, round284, round288, round298, round301, round306, round311, round316, round323, round327, round328, round333, round338, round343, round348, round350, and round354"\n    );',
  ...
  transformed = replaceRequired(
    ...
    "prior round327, round328, round333, round338, round343, round348, round350, and round354 fetch assertions"
  );

  transformed = replaceRequired(
    transformed,
    '    .replace(/round276/g, "round323");',
    '    .replace(/round276/g, "round354");',
    "validator prior cutoff prose replacement"
  );

  assertContains(transformed, 'const ROUND_NAME = "round358_london_pld_lifecycle_next41";', "round name");
  assertContains(transformed, 'const ROUND = "round358";', "generated round id");
  assertContains(transformed, "MAX_PRIOR_ROUND = 354", "prior cutoff");
  ...
  assertContains(transformed, "through round354", "dedupe prose cutoff");
  return transformed;
}
...
function main() {
  ...
  vm.runInNewContext(
    ...
    {
      filename: "generated_round358_london_pld_lifecycle_next41_candidates.js"
    }
  );
}
