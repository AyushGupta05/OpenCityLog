...
const ROUND_NAME = "round512_london_pld_lifecycle_next78";
...
function replaceRequired(value, needle, replacement, label) {
  if (!value.includes(needle)) {
    ...
    throw new Error(`Round512 source transform could not find ${label}: ${needle}`);
  }
...
function transformRound488Script(source) {
  let transformed = source
    .replace(/round488_london_pld_lifecycle_next72/g, "round512_london_pld_lifecycle_next78")
    .replace(/round488\.london_pld_lifecycle_next72/g, "round512.london_pld_lifecycle_next78")
    .replace(/Bims5Round488PldLifecycleNext72/g, "Bims5Round512PldLifecycleNext78")
    .replace(/Round 488/g, "Round 512")
    .replace(/Round488/g, "Round512")
    .replace(/round488/g, "round512")
    .replace(/next72/g, "next78")
    .replace(/MAX_PRIOR_ROUND = 485/g, "MAX_PRIOR_ROUND = 508")
    .replace(/through round485/g, "through round508")
    .replace(/packRound\(entry\.name\) > 485/g, "packRound(entry.name) > 508")
    .replace(/\.replace\(\/round276\/g, "round485"\);/g, '.replace(/round276/g, "round508");');
  ...
  transformed = replaceRequired(
    ...
    "round488, round490, round496, round500, round504, and round508 prior pack insertion"
  );

  transformed = transformed
    .replace(
      /pack\) => pack\.label === "round485_london_pld_lifecycle_next71" && pack\.exists === true/g,
      'pack) => pack.label === "round508_london_pld_lifecycle_next77" && pack.exists === true'
    )
    .replace(/round485 prior PLD lifecycle pack was scanned/g, "round508 prior PLD lifecycle pack was scanned");

  assertContains(transformed, 'const ROUND_NAME = "round512_london_pld_lifecycle_next78";', "round name");
  assertContains(transformed, 'const ROUND = "round512";', "transformed fetch round id");
  assertContains(transformed, "MAX_PRIOR_ROUND = 508", "prior cutoff");
  assertContains(transformed, "through round508", "dedupe cutoff text");
  ...
  assertContains(transformed, "addRound512RetrievedAtAlias", "candidate retrieved_at alias");
  assertContains(transformed, 'CURRENT_ROUND_NAME = "round512_london_pld_lifecycle_next78"', "validator round name");
  assertContains(transformed, "round512_strict_duplicate_audit", "strict duplicate audit label");
...
function addRound512SourceRowRefs() {
  ...
  pack.round512_postprocess = {
    source_row_ref_alias_count: aliasCount,
    note: "Round512 wrapper added explicit source_row_ref/source_row_refs aliases from the official PLD row/API URLs after the inherited lifecycle worker completed."
  };
  writeJson(file, pack);
}

function updateRound512Readback() {
  ...
  readback.round512_postprocess_checks = [
...
async function main() {
  ...
  vm.runInNewContext(transformed, sandbox, {
    filename: "generated_round512_london_pld_lifecycle_next78_candidates.js"
  });
  await waitForGeneratedOutputs(startedAtMs);
  addRound512SourceRowRefs();
  updateRound512Readback();
}
