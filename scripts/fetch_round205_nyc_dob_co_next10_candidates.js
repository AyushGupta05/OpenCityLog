...
const OUT_DIR = "tmp/subagents/round205_nyc_dob_co_next10";
...
const PRIOR_DOB_ROUND_PATTERN = /round(117|119|133|136|143|149|152|155|160|164|169|175|181|187|193|199).*nyc.*(dob|co)|nyc.*(dob|co).*round(117|119|133|136|143|149|152|155|160|164|169|175|181|187|193|199)/i;
...
function candidateFilesToScreen(root) {
  ...
  while (stack.length) {
    ...
    for (const entry of fs.readdirSync(current, { withFileTypes: true })) {
      ...
      if (entry.name === "candidates.json" && (PRIOR_DOB_ROUND_PATTERN.test(normalized) || /round(160|164|169|175|181|187|193|199)_nyc_dob_co_next(2|3|4|5|6|7|8|9)/i.test(normalized))) {
...
function candidateFromDobNow(row) {
  ...
  const candidateId = `nyc_dob_co_round205_dob_now_${slugify(row.application_number || row.c_of_o_number || row.job_filing_name, 72)}_${date.replace(/-/g, "_")}`;
...
function candidateFromLegacy(row) {
  ...
  const scaleText = scaleLabel(scale);
  const candidateId = `nyc_dob_co_round205_legacy_${slugify(row.job_number, 48)}_${slugify(issueType, 24)}_${date.replace(/-/g, "_")}`;
  ...
  return {
    ...sharedCandidateFields(dataset, row, {
      ...
      transformationMethod: `Round205 queried official legacy DOB Certificate Of Occupancy rows (${dataset.id}) with issued status and c_o_issue_date in ${START_DATE} through ${END_DATE}, retained rows within the dataset's stated pre-DOB-NOW coverage period through ${LEGACY_PREFERRED_END_DATE}, grouped repeated rows by DOB job, BIN, and address, preferred final rows over temporary rows, retained nonduplicate high-signal NB/A1 rows with at least 25 dwelling units or a source-row civic/public text signal, and preserved row-level provenance.`
    }),
...
function notesMarkdown(candidates, summary, duplicateIndex, fetchedCounts, rejectionCounts) {
  ...
  return [
    "# Round205 NYC DOB CO Next10 Candidate Pack",
    "",
    ...
    "## Duplicate Screening",
    "",
    `Screened ${duplicateIndex.files.length} files, ${duplicateIndex.tokens.size} identifier tokens, and ${duplicateIndex.sourceDateKeys.size} source-date keys, including the live manual corpus and prior DOB/CO candidate packs for rounds 117, 119, 133, 136, 143, 149, 152, 155, 160, 164, 169, 175, 181, 187, 193, and 199.`,
    "",
    "## Caveats",
    "",
...
async function main() {
  ...
  const sourceAudits = [
    ...
    },
    {
      ...
      coverage_years_checked: `${START_DATE} through ${END_DATE}`,
      published_coverage_note: "Dataset description says legacy CO coverage is primarily 2012-07-12 through March 2021; Round205 retained legacy candidates only through 2021-03-31.",
  ...
  const output = {
    schema_version: "round205.nyc_dob_co_next10_candidates.v1",
    generated_at: GENERATED_AT,
    accessed_at: ACCESSED_AT,
    worker: "Round205 NYC DOB CO next10 candidates",
  ...
  const sourceAudit = {
    schema_version: "round205.nyc_dob_co_next10_source_audit.v1",
    generated_at: GENERATED_AT,
    accessed_at: ACCESSED_AT,
  ...
  const summary = {
    schema_version: "round205.nyc_dob_co_next10_summary.v1",
    generated_at: GENERATED_AT,
    accessed_at: ACCESSED_AT,
    output_files: [
  ...
  const rejectedOutput = {
    schema_version: "round205.nyc_dob_co_next10_rejections.v1",
    generated_at: GENERATED_AT,
    accessed_at: ACCESSED_AT,
    rejected_counts: rejectedCounts,
