...
const ROUND_NAME = "round327_london_pld_lifecycle_next33";
...
function assertContains(value, needle, label) {
  if (!value.includes(needle)) {
    throw new Error(`Round327 source transform failed to include ${label}: ${needle}`);
  }
}

function assertAbsent(value, needle, label) {
  if (value.includes(needle)) {
    throw new Error(`Round327 source transform retained ${label}: ${needle}`);
  }
}
...
function replaceRequired(value, needle, replacement, label) {
  if (!value.includes(needle)) {
    ...
    throw new Error(`Round327 source transform could not find ${label}: ${needle}`);
  }
...
function transformRound284Fetch(source) {
  let transformed = source
    .replace(/const ROUND = "round284";/, 'const ROUND = "round327";')
    .replace(
      /const ROUND_NAME = "round284_london_pld_lifecycle_next25";/,
      'const ROUND_NAME = "round327_london_pld_lifecycle_next33";'
    )
    .replace(/const MAX_PRIOR_ROUND = 276;/, "const MAX_PRIOR_ROUND = 323;")
    .replace(/Bims5Round284PldLifecycleNext25/g, "Bims5Round327PldLifecycleNext33")
    .replace(/round284_london_pld_lifecycle_next25/g, "round327_london_pld_lifecycle_next33")
    .replace(/round284\.london_pld_lifecycle_next25/g, "round327.london_pld_lifecycle_next33")
    .replace(/Round284/g, "Round327")
    .replace(/Round 284/g, "Round 327")
    .replace(/round284/g, "round327")
    .replace(/next25/g, "next33")
    .replace(/through round276/g, "through round323")
    .replace(
  ...
  assertContains(transformed, 'const ROUND = "round327";', "round id");
  ...
  assertContains(transformed, "through round323", "dedupe cutoff");
...
function transformRound284Validator(source) {
  let transformed = source
    ...
    .replace(
      ...
      'const CURRENT_ROUND_NAME = "round327_london_pld_lifecycle_next33";'
    )
    .replace(/round284_london_pld_lifecycle_next25/g, "round327_london_pld_lifecycle_next33")
    .replace(/round284/g, "round327")
    .replace(/round284:/g, "round327:")
  ...
  assertContains(transformed, "MAX_PRIOR_ROUND = 323", "validator cutoff");
  assertContains(transformed, 'CURRENT_ROUND_NAME = "round327_london_pld_lifecycle_next33"', "validator current round");
...
async function runTransformedFetch() {
  ...
  vm.runInNewContext(transformed, sandbox, {
    filename: "generated_round327_london_pld_lifecycle_next33_candidates.js"
  });
  if (typeof sandboxModule.exports !== "function") {
    throw new Error("Round327 transform did not export the fetch main() function.");
  }
...
function writeValidator() {
  ...
  fs.writeFileSync(path.join(OUT_DIR, "validate_round327_pack.js"), transformed);
}
