...
const ROUND = "round161";
const ROUND_NAME = "round161_london_pld_lifecycle_next5";
  ...
  for (let page = 0; page < maxPages; page += 1) {
    ...
    const response = await fetch(API_ENDPOINT, {
      ...
      headers: {
        ...
        "user-agent": "Bims5Round161PldLifecycleNext5/0.1"
      },
...
function candidateFor(item) {
  ...
  return {
    ...
    duplicate_check_note:
      "Screened against the live manual architecture corpus, all discovered prior London PLD lifecycle packs including round126 direct/more, round140, round147, round150, round153, round157, and any existing round161 output by PLD row id, source-date-field/date, source URL/source_record/date-field/date, and title/date.",
    limitations: [
...
function rejectionReason(item, duplicateIndex, batchKeys) {
  ...
  if (batchKeys.has(fieldKey)) return "duplicate_row_date_field_inside_round161_batch";
  if (
...
async function main() {
  ...
  const candidatesPack = {
    schema_version: "round161.london_pld_lifecycle_next5.candidates.v1",
    generated_at: GENERATED_AT,
    accessed_at: ACCESSED_AT,
    task:
      "Round161 London Planning Datahub actual lifecycle candidate pack beyond current manual corpus and prior PLD lifecycle packs through round157",
  ...
  const summary = {
    ...
    caveats: [
      ...
      "Exact source row/date-field/date duplicates already present in the live manual corpus, prior PLD lifecycle packs through round157, or any existing round161 output were excluded.",
      "London Datastore licence is Not Specified for this source, so the pack retains factual metadata and attribution only."
    ]
  };

  const rejectedPack = {
    schema_version: "round161.london_pld_lifecycle_next5.rejected.v1",
    generated_at: GENERATED_AT,
    accessed_at: ACCESSED_AT,
  ...
  const notes = [
    "# Round 161 London PLD lifecycle next5 candidates",
    "",
    ...
    "",
    "Deduplication scanned the live manual architecture corpus plus all discovered prior London PLD lifecycle scratch packs, explicitly including round126 direct, round126 more, round140, round147, round150, round153, round157, and any existing round161 output by PLD row id, source URL/source record/date-field/date, source date field/date, and title/date.",
    "",
