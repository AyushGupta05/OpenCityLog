...
const ROUND = 495;
const SLUG = "round495_nyc_dob_legacy_permit_next3";
...
function buildExistingIndex() {
  ...
  while (stack.length) {
    ...
    for (const item of fs.readdirSync(dir, { withFileTypes: true })) {
      ...
      if (item.isDirectory()) {
        stack.push(full);
      } else if (/candidates.*\.json$/i.test(item.name) && /nyc|dob|permit/i.test(full)) {
...
function makeCandidate(appRow, permitRow, permitDate, scale) {
  ...
  return {
    ...
    transformation_method: "scripts/fetch_round495_nyc_dob_legacy_permit_next3_candidates.js queried official NYC Open Data DOB Job Application Filings high-signal rows, joined them by legacy job number to official DOB Permit Issuance rows, retained initial issued NB/AL permits within 2008-01-01 through 2026-05-20, and screened against the manual corpus plus prior NYC DOB/DOB legacy candidate packs through Round483 by permit SI number, job number, source/date key, source URL, title/date key, and address/date key.",
    address,
...
function validateCandidates(candidates, duplicateIndex, sourceAudits, summaryCandidateCount) {
  ...
  return {
    ...
    accessed_at: ACCESSED_AT,
    validator: "scripts/fetch_round495_nyc_dob_legacy_permit_next3_candidates.js standalone validation artifact",
    errors,
    checks: {
...
function duplicateAudit(candidates, duplicateIndex, validation) {
  return {
    ...
    generated_at: GENERATED_AT,
    accessed_at: ACCESSED_AT,
    audit_scope: "Round495 duplicate audit for NYC DOB Permit Issuance legacy permit next3 candidates, treating Round483 as the latest DOB legacy permit boundary.",
    dedupe_boundary: {
...
function buildNotes(summary, sourceAudits) {
  ...
  const lines = [
    "# Round495 NYC DOB legacy permit next3",
    "",
...
async function main() {
  ...
  const sourceAudits = [
    sourceAuditFromMetadata(DATASETS.permits, metadata[DATASETS.permits.dataset_id], {
      ...
      method: "Round495 queried official NYC Open Data DOB Permit Issuance rows and retained initial issued NB/AL permit rows joined to high-signal legacy DOB job application rows, with duplicate screening against the manual corpus and prior DOB/DOB legacy packs through Round483.",
    ...
    sourceAuditFromMetadata(DATASETS.filings, metadata[DATASETS.filings.dataset_id], {
      ...
      method: "Round495 used DOB Job Application Filings only as supporting context for selected DOB Permit Issuance rows, joining by legacy job number and retaining source-reported scale/status fields.",
  ...
  const summary = {
    ...
    task: "Round495 NYC DOB Permit Issuance legacy permit next3 candidate pack",
