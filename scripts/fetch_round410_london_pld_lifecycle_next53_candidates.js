...
const ROUND_NAME = "round410_london_pld_lifecycle_next53";
...
const PRIOR_PROSE_AFTER_272 =
  "round272, round276, round284, round288, round298, round301, round306, round311, round316, round323, round327, round328, round333, round338, round343, round348, round350, round354, round358, round362, round365, round369, round373, round377, round380, round384, round387, round393, round398, and round404";
...
function replaceRequired(value, needle, replacement, label) {
  if (!value.includes(needle)) {
    ...
    throw new Error(`Round410 source transform could not find ${label}: ${needle}`);
  }
...
function assertContains(value, needle, label) {
  if (!value.includes(needle)) {
    throw new Error(`Round410 source transform failed to include ${label}: ${needle}`);
  }
}
...
function transformRound404Source(source) {
  ...
  transformed = transformed
    .replace(/round404_london_pld_lifecycle_next52/g, "round410_london_pld_lifecycle_next53")
    .replace(/round404\.london_pld_lifecycle_next52/g, "round410.london_pld_lifecycle_next53")
    .replace(/Bims5Round404PldLifecycleNext52/g, "Bims5Round410PldLifecycleNext53")
    .replace(/Round 404/g, "Round 410")
    .replace(/Round404/g, "Round410")
    .replace(/round404/g, "round410")
    .replace(/next52/g, "next53")
    .replace(/MAX_PRIOR_ROUND = 398/g, "MAX_PRIOR_ROUND = 407")
    .replace(/through round398/g, "through round407");

  transformed = replaceRequired(
    transformed,
    '  "round398_london_pld_lifecycle_next51"\n];',
    '  "round398_london_pld_lifecycle_next51",\n  "round404_london_pld_lifecycle_next52"\n];',
    "round404 prior PLD lifecycle pack"
  );
  ...
  assertContains(transformed, 'const ROUND_NAME = "round410_london_pld_lifecycle_next53";', "round name");
  assertContains(transformed, 'const ROUND = "round410";', "transformed fetch round id");
  assertContains(transformed, "MAX_PRIOR_ROUND = 407", "prior cutoff");
  assertContains(transformed, "through round407", "dedupe cutoff text");
  assertContains(transformed, "round404_london_pld_lifecycle_next52", "round404 prior pack");
  assertContains(transformed, 'CURRENT_ROUND_NAME = "round410_london_pld_lifecycle_next53"', "validator round name");
...
async function runTransformedRound404() {
  ...
  vm.runInNewContext(transformed, sandbox, {
    filename: "generated_round410_london_pld_lifecycle_next53_candidates.js"
  });

  if (typeof sandboxModule.exports !== "function") {
    throw new Error("Round410 transform did not export the fetch main() function.");
  }
...
function validateAndRewriteReadbackJson() {
  ...
  if (!ok) {
    throw new Error("Readback validation failed for Round410 output pack.");
  }
  ...
  if (!finalReadback.ok || !finalReadback.files?.["readback.json"]?.exists) {
    throw new Error("Final readback self-check failed for Round410 output pack.");
  }
}
