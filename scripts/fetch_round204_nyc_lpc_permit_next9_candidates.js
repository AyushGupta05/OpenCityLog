...
const OUT_DIR = path.join(ROOT, "tmp", "subagents", "round204_nyc_lpc_permit_next9");
...
async function fetchJson(url) {
  const response = await fetch(url, {
    headers: { "User-Agent": "Bims-5 round204 NYC LPC permit candidate fetcher" }
  });
  if (!response.ok) {
...
function candidateFrom(row) {
  ...
  const eventId = `nyc-lpc-permit-round204-${slugify(regulationNumber, 48)}`;
  ...
  return {
    ...
    transformation_method: `scripts/fetch_round204_nyc_lpc_permit_next9_candidates.js queried official NYC Open Data dpm2-m9mq rows with issue_date from ${START_DATE} through ${END_DATE}, required official latitude/longitude and regulation_number, scored architecture-facing WorkTypes/regulation_type values, deduplicated against the current corpus plus prior LPC permit/designation packs through round198, and selected up to ${MAX_CANDIDATES} high-signal administrative milestones without inferring physical work.`,
...
function buildSourceAudit(metadata, queryUrl, sourceStats, existing) {
  ...
  return {
    ...
    audit_scope: "Official NYC LPC Permit Application Information source audit for round204 high-signal permit/application candidate extraction.",
    sources: [
      {
        source_id: SOURCE_ID,
        source_dataset_id: SOURCE_DATASET_ID,
...
function markdownNotes(summary) {
  return [
    "# Round204 NYC LPC Permit Application Information Next9 Candidates",
    "",
    ...
    "",
    "Rows were scored for architecture/preservation signal in WorkTypes and regulation_type, then balanced across years and regulation types. Dedupe checked the current corpus files and prior LPC permit/designation candidate packs through round198.",
    "",
    "## Caveat",
    "",
...
async function main() {
  ...
  const summary = {
    generated_at: `${ACCESSED_AT}T00:00:00Z`,
    task: "Round204 NYC LPC Permit Application Information next9 candidate pack",
    source_id: SOURCE_ID,
    source_dataset_id: SOURCE_DATASET_ID,
