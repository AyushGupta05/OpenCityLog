...
const ROUND_NAME = "round439_london_pld_lifecycle_next60";
...
function replaceRequired(value, needle, replacement, label) {
  if (!value.includes(needle)) {
    ...
    throw new Error(`Round439 source transform could not find ${label}: ${needle}`);
  }
...
function assertContains(value, needle, label) {
  if (!value.includes(needle)) {
    throw new Error(`Round439 source transform failed to include ${label}: ${needle}`);
  }
}
...
function tightenBareSourceDuplicateMatching(source) {
  ...
  transformed = replaceRequired(
    transformed,
    '  if (batchKeys.has(fieldKey)) return "duplicate_row_date_field_inside_round439_batch";',
    [
      '  if (batchKeys.has(fieldKey)) return "duplicate_row_date_field_inside_round439_batch";',
      '  if (batchKeys.has(`source_url|${sourceUrlKey}`)) return "duplicate_source_url_inside_round439_batch";',
      '  if (batchKeys.has(`source_record_id|${sourceRecordIdKey}`)) return "duplicate_source_record_id_inside_round439_batch";',
      '  if (batchKeys.has(`title_date|${titleDate}`)) return "duplicate_title_date_inside_round439_batch";'
    ].join("\n"),
...
function transformRound404Source(source) {
  ...
  transformed = transformed
    .replace(/round404_london_pld_lifecycle_next52/g, "round439_london_pld_lifecycle_next60")
    .replace(/round404\.london_pld_lifecycle_next52/g, "round439.london_pld_lifecycle_next60")
    .replace(/Bims5Round404PldLifecycleNext52/g, "Bims5Round439PldLifecycleNext60")
    .replace(/Round 404/g, "Round 439")
    .replace(/Round404/g, "Round439")
    .replace(/round404/g, "round439")
    .replace(/next52/g, "next60")
    .replace(/MAX_PRIOR_ROUND = 398/g, "MAX_PRIOR_ROUND = 436")
    .replace(/through round398/g, "through round436");
  ...
  transformed = replaceRequired(
    ...
    "round404 through round436 prior PLD lifecycle packs"
  );
  ...
  assertContains(transformed, 'const ROUND_NAME = "round439_london_pld_lifecycle_next60";', "round name");
  assertContains(transformed, 'const ROUND = "round439";', "transformed fetch round id");
  assertContains(transformed, "MAX_PRIOR_ROUND = 436", "prior cutoff");
  assertContains(transformed, "through round436", "dedupe cutoff text");
  ...
  assertContains(transformed, 'CURRENT_ROUND_NAME = "round439_london_pld_lifecycle_next60"', "validator round name");
...
async function runTransformedRound404() {
  ...
  vm.runInNewContext(transformed, sandbox, {
    filename: "generated_round439_london_pld_lifecycle_next60_candidates.js"
  });

  if (typeof sandboxModule.exports !== "function") {
    throw new Error("Round439 transform did not export the fetch main() function.");
  }
...
function addRound439RequestedAliases() {
...
function buildStrictDuplicateIndex() {
  ...
  if (fs.existsSync(subagentsDir)) {
    for (const entry of fs.readdirSync(subagentsDir, { withFileTypes: true })) {
      ...
      if (entry.name === ROUND_NAME || packRound(entry.name) > 436) continue;
...
function writeStrictDuplicateAuditAndAugmentReports() {
  ...
  const report = {
    ...
    validation_type: "round439_strict_duplicate_audit",
  ...
  if (!report.ok) {
    throw new Error(`Round439 strict duplicate audit failed with ${report.duplicate_count} duplicate keys.`);
  }
...
function validateAndRewriteReadbackJson() {
  ...
  const ok =
    ...
    /through round436/i.test(summary.dedupe_basis || "") &&
    (summary.duplicate_index?.prior_packs || []).some(
      (pack) => pack.label === "round436_london_pld_lifecycle_next59" && pack.exists === true
    );

  if (!ok) {
    throw new Error("Readback validation failed for Round439 output pack.");
  }
  ...
  writeJson(readbackFile, {
    ...
    readback_checks: [
      ...
      "dedupe basis includes PLD lifecycle packs through round436",
      "round436 prior PLD lifecycle pack was scanned",
  ...
  if (!finalReadback.ok || !finalReadback.files?.["readback.json"]?.exists) {
    throw new Error("Final readback self-check failed for Round439 output pack.");
  }
}
