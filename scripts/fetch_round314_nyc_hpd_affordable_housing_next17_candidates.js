...
function replaceChecked(pattern, replacement, label) {
  ...
  if (next === source) {
    throw new Error(`Unable to adapt round251 HPD fetcher for round314: missing ${label}.`);
  }
  source = next;
}
...
replaceChecked(
  ...
  'const outDir = "tmp/subagents/round314_nyc_hpd_affordable_housing_next17";',
  "output directory"
);
replaceChecked('const accessedAt = "2026-05-19";', 'const accessedAt = "2026-05-20";', "access date");
replaceChecked('const roundName = "Round251";', 'const roundName = "Round314";', "round name");
replaceChecked("const maxScreeningRound = 246;", "const maxScreeningRound = 309;", "screening round");
replaceChecked(/Round251/g, "Round314", "round label references");
replaceChecked(/round251\.nyc/g, "round314.nyc", "schema round prefix");
replaceChecked(
  /nyc_hpd_affordable_housing_next8/g,
  "nyc_hpd_affordable_housing_next17",
  "next pack label"
);
replaceChecked(
...
replaceChecked(
  ...
  "candidate_files_considered_through_round309",
...
replaceChecked(
  'published_coverage_note: "Round314 checked HPD building-level rows with coordinates and building/project completion or project start dates available through the access date.",',
  'published_coverage_note: "Round314 checked HPD building-level rows with coordinates and source-reported building_completion_date, project_completion_date, or project_start_date fields available through the access date; source date fields are retained explicitly.",',
...
replaceChecked(
  ...
  "including round229, round234, round236, round241, round246, round251, round255, round263, round269, round275, round283, round292, round304, and round309",
  "explicit overlap caveat"
);
replaceChecked(/through round246/g, "through round309", "screening prose");
...
replaceChecked(
  ...
  const validationPayload = {
    schema_version: "round314.nyc_hpd_affordable_housing_next17_validation.v1",
    status: summaryPayload.validation.status,
    checked_at: generatedAt,
    command: "node scripts/fetch_round314_nyc_hpd_affordable_housing_next17_candidates.js",
    checks: [
      ...
      "dedupe against manual corpus and HPD-family candidate packs through round309",
...
const runAdaptedRound251Fetcher = new Function(
  "require",
  "fetch",
  `${source}\n//# sourceURL=fetch_round314_nyc_hpd_affordable_housing_next17_candidates.adapted.js`
);
