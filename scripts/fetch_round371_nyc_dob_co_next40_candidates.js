...
const GENERATED_FILENAME = "scripts/fetch_round371_nyc_dob_co_next40_candidates.generated.js";
const ROUND = "round371";
const ROUND_TITLE = "Round371";
const NEXT_LABEL = "next40";
const NEXT_TITLE = "Next40";
...
function transformRound308Source(source) {
  let transformed = source.replace(/\r\n/g, "\n")
    ...
    .replaceAll("through round303", "through round367");
  ...
  transformed = replaceFirst(
    ...
    `${round303File}\n];`,
    `${priorRoundFiles}\n];`,
    "explicit duplicate files through Round367"
  );
  transformed = replaceOnce(
    transformed,
    `${round303File}\n];`,
    `${priorRoundFiles}\n];`,
    "required screened files through Round367"
  );
  ...
  transformed = replaceOnce(
    ...
    "screened rounds include Round308 through Round367"
  );
  ...
  transformed = replaceOnce(
    ...
    "validation check fields through Round367"
  );
  ...
  transformed = replaceOnce(
    ...
    "validation report lines through Round367"
  );
  ...
  assertContains(transformed, "round371_nyc_dob_co_next40", "Round371 output path");
  assertContains(transformed, "Round371 NYC DOB CO Next40 Candidate Pack", "Round371 notes title");
  assertContains(transformed, "nyc_dob_co_round371_legacy_", "Round371 candidate ID prefix");
  assertContains(transformed, "tmp/subagents/round367_nyc_dob_co_next39/candidates.json", "Round367 screened file");
  assertContains(transformed, "through round367", "duplicate-screening text through Round367");
  assertContains(transformed, "required_round367_screened", "Round367 validation result");
  ...
  assertContains(transformed, `transformation_method: "${ROUND_TITLE} queried`, "Round371 source audit transformation method");
  assertContains(transformed, "scripts/fetch_round371_nyc_dob_co_next40_candidates.js standalone validation artifact", "Round371 validator label");
...
async function main() {
  ...
  if (typeof sandbox.module.exports.main !== "function") {
    throw new Error("Transformed Round371 generator did not export main().");
  }
