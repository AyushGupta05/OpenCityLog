...
const ROUND_ID = "round577_belfast_official_architecture_sweep_next34";
const OUT_DIR = path.join("tmp", "subagents", ROUND_ID);
const SCRIPT_PATH =
  "scripts/fetch_round577_belfast_official_architecture_sweep_next34_candidates.js";
...
const DEDUPE_BOUNDARY_ROUND = 569;
...
const METHOD = [
  "Round577 official Belfast architecture sweep next34 after the completed Round569 next33 pack.",
  ...
  "Round569 is treated as the latest Belfast official architecture sweep dedupe boundary; official page-only, geometry-ref-only, aggregate, duplicate-project, status-only, signage-only, equipment-only, condition-only and lower-priority private-use leads are retained separately."
].join(" ");

const TRANSFORMATION_METHOD = `${SCRIPT_PATH}#round577OfficialArchitectureSweepNext34`;

const SOURCES = {
  dfiPlanningStats: {
    source_id: "dfi-planning-statistics-2024-25-round577",
...
const SEARCH_QUERIES_CHECKED = [
  "local DfI planning-statistics 2024-25 Belfast residual APP_ID scan after Round569 accepted-pack dedupe",
  "local duplicate scan: data/manual_drops/architecture_milestones plus official Belfast architecture sweep candidate packs through Round569",
...
function replaceRound577LeadBlocks() {
  const lead = (
    app_id,
    related_app_ids,
    event_id,
  ...
  const selectedLeads = [
    { app_id: "LA04/2023/4435/F" },
    { app_id: "LA04/2024/1518/F" },
    { app_id: "LA04/2024/1784/F" },
    { app_id: "LA04/2024/2127/F" },
    { app_id: "LA04/2024/2058/F" },
    { app_id: "LA04/2024/2075/F" },
    { app_id: "LA04/2024/2093/F" },
    { app_id: "LA04/2025/0066/F" },
    { app_id: "LA04/2025/0094/F" },
    { app_id: "LA04/2024/1408/F" },
    { app_id: "LA04/2024/2061/F" },
    { app_id: "LA04/2025/0029/F" },
    { app_id: "LA04/2024/2153/F" },
    { app_id: "LA04/2024/2149/F" },
    { app_id: "LA04/2024/2083/F" },
    { app_id: "LA04/2024/1972/F" },
    { app_id: "LA04/2024/1488/F" },
    { app_id: "LA04/2025/0009/F" },
    { app_id: "LA04/2024/1598/F" },
    { app_id: "LA04/2024/1779/F" },
    { app_id: "LA04/2024/1981/F" },
    { app_id: "LA04/2025/0076/F" },
    { app_id: "LA04/2024/2082/F" },
    { app_id: "LA04/2025/0034/F" },
    { app_id: "LA04/2024/1928/F" },
    { app_id: "LA04/2025/0092/F" },
    { app_id: "LA04/2025/0144/F" },
    { app_id: "LA04/2025/0192/F" },
    { app_id: "LA04/2023/4194/F" },
    { app_id: "LA04/2023/4584/F" }
  ];
  ...
  PLANNING_LEADS.splice(
    ...
    ...selectedLeads.map((selected) => {
      ...
      if (!row) throw new Error(`Selected Round577 app ID missing source row: ${selected.app_id}`);
      ...
      const eventId = `bfs_arch_round577_${slug(site)}_${appKey}_approval_${decisionYear}`;
...
replaceRound577LeadBlocks();

REJECTED_LEADS.splice(
  0,
  REJECTED_LEADS.length,
  {
    id: "bfs_arch_round577_reject_duplicate_project_rows_through_round569",
    title: "Residual duplicate-project planning rows through the Round569 boundary",
    rejection_category: "duplicate_project_boundary",
    reason:
      "Official point-backed DfI rows for already-covered project families or same-site milestones were retained outside the promoted pack because the manual architecture corpus or official Belfast architecture sweep candidate records through Round569 already cover the same project family or APP_ID.",
    source_record_id:
  ...
  {
    id: "bfs_arch_round577_reject_short_let_and_hmo_only_tail",
  ...
  {
    id: "bfs_arch_round577_reject_page_only_or_no_point_official_leads",
  ...
  {
    id: "bfs_arch_round577_reject_aggregate_only_dfi_quarterly_sources",
  ...
  {
    id: "bfs_arch_round577_reject_signage_equipment_condition_rows",
  ...
  {
    id: "bfs_arch_round577_reject_minor_domestic_tail_after_target_count",
...
function candidateFromPlanningLead(lead, rowsById, duplicateHaystack) {
  ...
  return {
    ...
    duplicate_review:
      `APP_ID, related APP_IDs, event_id and source_record_id were absent from the manual architecture corpus and official Belfast architecture sweep candidate outputs through Round${DEDUPE_BOUNDARY_ROUND} checked by the Round577 duplicate scan.`,
...
function rejectedRecord(lead, duplicateHaystack) {
  return {
    ...
    reason: lead.reason,
    source_id: lead.id.replace(/^bfs_arch_round577_reject_/, "round577-reject-"),
...
function validate(candidates, rejected, duplicateHaystack) {
  ...
  if (candidates.length !== EXPECTED_CANDIDATE_COUNT) {
    errors.push(
      `Expected ${EXPECTED_CANDIDATE_COUNT} promoted next34 candidates, found ${candidates.length}`
    );
  }
...
function main() {
  ...
  const sourceAudit = {
    ...
    sources_checked: [
      ...
      {
        source_id: "dfi-planning-statistics-2025-26-quarterly-reviewed-round577",
        source_name: "Northern Ireland planning statistics 2025/26 provisional quarterly publications",
        publisher: "Department for Infrastructure, Northern Ireland",
        source_url: DFI_2025_26_Q3_URL,
        source_type: "official provisional quarterly planning-statistics publication",
      ...
      {
        source_id: "hed-historic-buildings-and-harni-reviewed-round577",
        ...
        accepted_records: 0,
        disposition:
          "reviewed; Historic Buildings layer lacks a usable event date for new dated listing/designation changes, while Heritage at Risk Date_Added rows are retained as status/admin leads outside this DfI residual next34 pack"
      },
      ...rejected.map((item) => ({
  ...
  const notes = [
    ...
    "- Categories are primarily residential building-fabric administrative approvals from the March 2025 residual tail; lower-signal short-let, signage, equipment, condition-only and aggregate leads are retained in rejected.json.",
    "",
