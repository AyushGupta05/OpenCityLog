...
const report = JSON.parse(fs.readFileSync("tmp/subagents/belfast_arch_candidates_round87.json", "utf8"));
const retrievedAt = "2026-05-18";
const sourceId = "belfast-architecture-public-pages";

const records = (report.candidates || report).map((candidate) => {
...
if (records.length !== 8) {
  throw new Error(`Expected 8 selected records, found ${records.length}`);
}
