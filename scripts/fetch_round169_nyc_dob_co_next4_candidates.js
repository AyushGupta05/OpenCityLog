...
const OUT_DIR = "tmp/subagents/round169_nyc_dob_co_next4";
...
const PRIOR_DOB_ROUND_PATTERN = /round(117|119|133|136|143|149|152|155|160|164).*nyc.*(dob|co)|nyc.*(dob|co).*round(117|119|133|136|143|149|152|155|160|164)/i;
...
function candidateFilesToScreen(root) {
  ...
  while (stack.length) {
    ...
    for (const entry of fs.readdirSync(current, { withFileTypes: true })) {
      ...
      if (entry.name === "candidates.json" && (PRIOR_DOB_ROUND_PATTERN.test(normalized) || /round(160|164)_nyc_dob_co_next(2|3)/i.test(normalized))) {
...
function collectExistingTokensFromObject(tokens, addressDateKeys, value) {
  if (!value || typeof value !== "object") return;
  if (Array.isArray(value)) {
    for (const item of value) collectExistingTokensFromObject(tokens, addressDateKeys, item);
    return;
  }
  ...
  for (const key of ["source_row_ref", "source_fields", "raw_row", "row_fields", "group_key"]) {
    collectExistingTokensFromObject(tokens, addressDateKeys, value[key]);
  }
}
...
function buildExistingIndex() {
  ...
  if (corpus) {
    ...
    collectExistingTokensFromObject(tokens, addressDateKeys, rows);
  ...
  for (const file of candidateFilesToScreen("tmp/subagents")) {
    try {
      ...
      collectExistingTokensFromObject(tokens, addressDateKeys, candidateArray(doc));
  ...
  return { tokens, addressDateKeys, files, manualEventCount };
}
...
function candidateFromDobNow(row) {
  ...
  const candidateId = `nyc_dob_co_round169_dob_now_${slugify(row.application_number || row.c_of_o_number || row.job_filing_name, 72)}_${date.replace(/-/g, "_")}`;
...
function candidateFromLegacy(row) {
  ...
  const scaleText = scaleLabel(scale);
  const candidateId = `nyc_dob_co_round169_legacy_${slugify(row.job_number, 48)}_${slugify(issueType, 24)}_${date.replace(/-/g, "_")}`;
  ...
  return {
    ...sharedCandidateFields(dataset, row, {
      ...
      transformationMethod: `Round169 queried official legacy DOB Certificate Of Occupancy rows (${dataset.id}) with issued status and c_o_issue_date in ${START_DATE} through ${END_DATE}, retained rows within the dataset's stated pre-DOB-NOW coverage period through ${LEGACY_PREFERRED_END_DATE}, grouped repeated rows by DOB job, BIN, and address, preferred final rows over temporary rows, retained nonduplicate high-signal NB/A1 rows with at least 25 dwelling units or a source-row civic/public text signal, and preserved row-level provenance.`
    }),
...
function notesMarkdown(candidates, summary, duplicateIndex, fetchedCounts, rejectionCounts) {
  ...
  return [
    "# Round169 NYC DOB CO Next4 Candidate Pack",
    "",
    ...
    "## Duplicate Screening",
    "",
    `Screened ${duplicateIndex.files.length} files and ${duplicateIndex.tokens.size} identifier tokens, including the live manual corpus and prior DOB/CO candidate packs for rounds 117, 119, 133, 136, 143, 149, 152, 155, 160, and any existing round164 output.`,
    "",
    "## Caveats",
    "",
...
async function main() {
  ...
  const sourceAudits = [
    ...
    },
    {
      ...
      coverage_years_checked: `${START_DATE} through ${END_DATE}`,
      published_coverage_note: "Dataset description says legacy CO coverage is primarily 2012-07-12 through March 2021; Round169 retained legacy candidates only through 2021-03-31.",
  ...
  const output = {
    schema_version: "round169.nyc_dob_co_next4_candidates.v1",
    generated_at: GENERATED_AT,
    accessed_at: ACCESSED_AT,
    worker: "Round169 NYC DOB CO next4 candidates",
  ...
  const sourceAudit = {
    schema_version: "round169.nyc_dob_co_next4_source_audit.v1",
    generated_at: GENERATED_AT,
    accessed_at: ACCESSED_AT,
  ...
  const summary = {
    schema_version: "round169.nyc_dob_co_next4_summary.v1",
    generated_at: GENERATED_AT,
    accessed_at: ACCESSED_AT,
    output_files: [
  ...
  const rejectedOutput = {
    schema_version: "round169.nyc_dob_co_next4_rejections.v1",
    generated_at: GENERATED_AT,
    accessed_at: ACCESSED_AT,
    rejected_counts: rejectedCounts,
