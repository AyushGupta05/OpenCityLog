...
const ROUND_NAME = "round605_london_pld_lifecycle_next99";
...
function replaceRequired(value, needle, replacement, label) {
  if (!value.includes(needle)) {
    ...
    throw new Error(`Round605 wrapper transform could not find ${label}: ${needle}`);
  }
...
function transformRound579Wrapper(source) {
  let transformed = source
    .replace(/round579_london_pld_lifecycle_next94/g, "round605_london_pld_lifecycle_next99")
    .replace(/round579\.london_pld_lifecycle_next94/g, "round605.london_pld_lifecycle_next99")
    .replace(/Bims5Round579PldLifecycleNext94/g, "Bims5Round605PldLifecycleNext99")
    .replace(/Round 579/g, "Round 605")
    .replace(/Round579/g, "Round605")
    .replace(/round579/g, "round605")
    .replace(/next94/g, "next99")
    .replace(/MAX_PRIOR_ROUND = 576/g, "MAX_PRIOR_ROUND = 597")
    .replace(/through round576/g, "through round597")
    .replace(/packRound\(entry\.name\) > 576/g, "packRound(entry.name) > 597")
    .replace(/\.replace\(\/round276\/g, "round576"\);/g, '.replace(/round276/g, "round597");');
  ...
  transformed = replaceRequired(
    ...
    "round579, round584, round589, round594, and round597 prior pack insertion"
  );

  transformed = transformed
    .replace(
      /pack\) => pack\.label === "round576_london_pld_lifecycle_next93" && pack\.exists === true/g,
      'pack) => pack.label === "round597_london_pld_lifecycle_next98" && pack.exists === true'
    )
    .replace(/round576 prior PLD lifecycle pack was scanned/g, "round597 prior PLD lifecycle pack was scanned");

  assertContains(transformed, 'const ROUND_NAME = "round605_london_pld_lifecycle_next99";', "round name");
  assertContains(transformed, 'const ROUND = "round605";', "transformed fetch round id");
  assertContains(transformed, "MAX_PRIOR_ROUND = 597", "prior cutoff");
  assertContains(transformed, "through round597", "dedupe cutoff text");
  assertContains(transformed, "round594_london_pld_lifecycle_next97", "round594 prior pack");
  assertContains(transformed, "round597_london_pld_lifecycle_next98", "round597 prior pack");
  assertContains(transformed, "addRound605RetrievedAtAlias", "candidate retrieved_at alias");
  assertContains(transformed, 'CURRENT_ROUND_NAME = "round605_london_pld_lifecycle_next99"', "validator round name");
  assertContains(transformed, "round605_strict_duplicate_audit", "strict duplicate audit label");
...
async function main() {
  ...
  vm.runInNewContext(transformed, sandbox, {
    filename: "generated_round605_london_pld_lifecycle_next99_candidates.js"
  });
