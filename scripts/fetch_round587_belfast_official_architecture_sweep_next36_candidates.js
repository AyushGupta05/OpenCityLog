...
const ROUND_ID = "round587_belfast_official_architecture_sweep_next36";
const OUT_DIR = path.join("tmp", "subagents", ROUND_ID);
const SCRIPT_PATH =
  "scripts/fetch_round587_belfast_official_architecture_sweep_next36_candidates.js";
...
const DEDUPE_BOUNDARY_ROUND = 582;
...
const METHOD = [
  "Round587 official Belfast architecture sweep next36 after the completed Round582 next35 pack.",
  ...
  "Round582 is treated as the latest Belfast official architecture sweep dedupe boundary; official page-only, geometry-ref-only, aggregate, duplicate-project, status-only, signage-only, equipment-only, condition-only and lower-priority private-use leads are retained separately."
].join(" ");

const TRANSFORMATION_METHOD = `${SCRIPT_PATH}#round587OfficialArchitectureSweepNext36`;

const SOURCES = {
  dfiPlanningStats: {
    source_id: "dfi-planning-statistics-2024-25-round587",
...
const SEARCH_QUERIES_CHECKED = [
  "local DfI planning-statistics 2024-25 Belfast residual APP_ID scan after Round582 accepted-pack dedupe",
  "local duplicate scan: data/manual_drops/architecture_milestones plus official Belfast architecture sweep candidate/rejected/readback packs through Round582",
  ...
  "source review: DfI point-backed duplicate-project, minor domestic, low-signal HMO/short-let, signage, equipment, condition-only, transport and telecom rows retained outside promoted pack"
];
...
function replaceRound587LeadBlocks() {
  const lead = (
    app_id,
    related_app_ids,
    event_id,
  ...
  const selectedLeads = [
    { app_id: "LA04/2024/2007/F" },
    { app_id: "LA04/2024/1739/F" },
    { app_id: "LA04/2024/1395/F" },
    { app_id: "LA04/2024/1849/F" },
    { app_id: "LA04/2024/1925/F" },
    { app_id: "LA04/2024/0962/F" },
    { app_id: "LA04/2024/1712/F" },
    { app_id: "LA04/2024/1793/F" },
    { app_id: "LA04/2024/1838/F" },
    { app_id: "LA04/2023/3909/F" },
    { app_id: "LA04/2024/1540/F" },
    { app_id: "LA04/2024/1691/F" },
    { app_id: "LA04/2024/1861/DCA" },
    { app_id: "LA04/2024/1887/F" },
    { app_id: "LA04/2024/1198/F" },
    { app_id: "LA04/2024/1673/F" },
    { app_id: "LA04/2024/1886/F" },
    { app_id: "LA04/2024/1683/F" },
    { app_id: "LA04/2024/1796/F" },
    { app_id: "LA04/2024/1897/F" },
    { app_id: "LA04/2024/2105/F" },
    { app_id: "LA04/2024/1679/F" },
    { app_id: "LA04/2024/1881/F" },
    { app_id: "LA04/2024/1039/F" },
    { app_id: "LA04/2024/1212/F" },
    { app_id: "LA04/2024/1369/F" },
    { app_id: "LA04/2024/1929/F" },
    { app_id: "LA04/2024/1732/F" },
    { app_id: "LA04/2024/1995/F" },
    { app_id: "LA04/2024/0934/F" }
  ];
  ...
  const classify = (row) => {
    ...
    if (row.AppType === "Other consents" && text.includes("demolition")) {
      return {
    ...
    if (/hmo|multiple occupancy/.test(text)) {
      return {
    ...
    if (/roof|dormer|fenestration|window/.test(text)) {
      return {
        event_type: "planning_decision_residential_roof_or_external_alteration",
        category: "architecture_residential_fabric_admin"
      };
    }
    if (/garden room|tree playhouse|outbuilding/.test(text)) {
      return {
        event_type: "planning_decision_residential_outbuilding",
        category: "architecture_residential_fabric_admin"
      };
    }
    if (/proposal for .*dwellings|five dwellings|new end terrace house|new house|proposed dwelling|new dwelling|detached dwelling|residential development|apartments|dwellinghouses|student accommodation/.test(text)) {
      return {
  ...
  PLANNING_LEADS.splice(
    ...
    ...selectedLeads.map((selected) => {
      ...
      if (!row) throw new Error(`Selected Round587 app ID missing source row: ${selected.app_id}`);
      ...
      const decisionYear = (parseDate(row.DecisionIssuedDate || row.DateValid) || GENERATED_AT).slice(0, 4);
      const eventId = `bfs_arch_round587_${slug(site)}_${appKey}_approval_${decisionYear}`;
...
replaceRound587LeadBlocks();

REJECTED_LEADS.splice(
  0,
  REJECTED_LEADS.length,
  {
    id: "bfs_arch_round587_reject_duplicate_project_rows_through_round582",
    title: "Residual duplicate-project planning rows through the Round582 boundary",
    rejection_category: "duplicate_project_boundary",
    reason:
      "Official point-backed DfI rows for already-covered project families or same-site milestones were retained outside the promoted pack because the manual architecture corpus or official Belfast architecture sweep candidate records through Round582 already cover the same project family or APP_ID.",
    source_record_id:
      "EXAMPLES:APP_ID:LA04/2024/1831/F; APP_ID:LA04/2024/1832/DCA; APP_ID:LA04/2024/1453/F; APP_ID:LA04/2024/1493/DCA; APP_ID:LA04/2024/1189/F; APP_ID:LA04/2024/1519/F; FILE:data/raw/planning_statistics/planning-statistics-2024-25-dataset.csv",
    source_url: DFI_2024_25_PUBLICATION_URL,
  ...
  {
    id: "bfs_arch_round587_reject_short_let_and_hmo_only_tail",
    ...
    reason:
      "Residual point-backed DfI rows limited to apartment-to-short-let, dwelling-to-HMO, holiday-let or guest-accommodation use changes with little or no substantial building-fabric signal were retained for a possible housing-use pass rather than promoted in this architecture/development batch.",
    source_record_id:
      "EXAMPLES:APP_ID:LA04/2024/1938/F; APP_ID:LA04/2024/1660/F; APP_ID:LA04/2023/4395/F; APP_ID:LA04/2024/1870/F; APP_ID:LA04/2024/0894/F; APP_ID:LA04/2024/1225/F; FILE:data/raw/planning_statistics/planning-statistics-2024-25-dataset.csv",
    source_url: DFI_2024_25_PUBLICATION_URL,
    source_name: SOURCES.dfiPlanningStats.source_name,
    publisher: SOURCES.dfiPlanningStats.publisher,
    source_type: SOURCES.dfiPlanningStats.source_type,
    ...
    license_url: SOURCES.dfiPlanningStats.license_url,
    attribution: SOURCES.dfiPlanningStats.attribution,
    duplicate_terms: ["short term let", "holiday-let", "HMO", "Sui Generis", "guest accommodation"]
  },
  {
    id: "bfs_arch_round587_reject_page_only_or_no_point_official_leads",
  ...
  {
    id: "bfs_arch_round587_reject_aggregate_only_dfi_quarterly_sources",
    ...
    reason:
      "DfI 2025/26 provisional quarterly publications were reviewed as current planning-statistics context, but they were not promoted because this pack requires application-level source rows with application IDs, decision dates and reusable Easting/Northing point coordinates.",
    source_record_id:
      "PUBLICATION: Northern Ireland planning statistics October - December 2025; TABLES: provisional quarterly statistical tables; no promoted application-level point rows",
  ...
  {
    id: "bfs_arch_round587_reject_signage_equipment_condition_rows",
    title: "Signage, equipment-only, condition-only, transport and telecom residual rows",
    rejection_category: "not_architecture_atlas_priority",
    reason:
      "Point-backed DfI rows for advertisements, display signs, EV charger/parking-bay relocation, equipment-only changes, transport/telecom changes, or condition-only/compliance-timing variations were reviewed but not promoted because they do not add a higher-signal building-fabric, use, civic-facility or heritage-works milestone for this pack.",
    source_record_id:
      "EXAMPLES:APP_ID:LA04/2024/2000/A; APP_ID:LA04/2024/1711/A; APP_ID:LA04/2024/1014/A; APP_ID:LA04/2024/0982/F; APP_ID:LA04/2024/0983/LBC; APP_ID:LA04/2024/1892/A; FILE:data/raw/planning_statistics/planning-statistics-2024-25-dataset.csv",
    source_url: DFI_2024_25_PUBLICATION_URL,
    source_name: SOURCES.dfiPlanningStats.source_name,
    publisher: SOURCES.dfiPlanningStats.publisher,
    source_type: SOURCES.dfiPlanningStats.source_type,
    ...
    license_url: SOURCES.dfiPlanningStats.license_url,
    attribution: SOURCES.dfiPlanningStats.attribution,
    duplicate_terms: ["advertisement", "shop sign", "projecting sign", "EV charger", "condition", "decommissioning"]
  },
  {
    id: "bfs_arch_round587_reject_minor_domestic_tail_after_target_count",
    title: "Additional point-backed minor domestic rows retained after target count",
    rejection_category: "lower_priority_minor_domestic_tail_not_promoted",
    reason:
      "A small remaining tail of point-backed, non-duplicate minor domestic extension/demolition-consent rows was retained outside the promoted pack to keep this next sweep close to the requested 30-candidate target. These are not page-only and may be reconsidered in a later tail pass.",
    source_record_id:
      "EXAMPLES:APP_ID:LA04/2024/0704/F; APP_ID:LA04/2024/1495/F; APP_ID:LA04/2024/1680/F; APP_ID:LA04/2024/1083/F; APP_ID:LA04/2024/1176/F; FILE:data/raw/planning_statistics/planning-statistics-2024-25-dataset.csv",
    source_url: DFI_2024_25_PUBLICATION_URL,
    source_name: SOURCES.dfiPlanningStats.source_name,
    publisher: SOURCES.dfiPlanningStats.publisher,
    source_type: SOURCES.dfiPlanningStats.source_type,
    ...
    license_url: SOURCES.dfiPlanningStats.license_url,
    attribution: SOURCES.dfiPlanningStats.attribution,
    duplicate_terms: ["49 Malone Park", "13 Beechgrove Park", "3 Woodberry Lane", "minor domestic tail", "single-storey rear extension"]
  }
);
...
function candidateFromPlanningLead(lead, rowsById, duplicateHaystack) {
  ...
  return {
    ...
    evidence_basis: [
      ...
      `APP_ID and related APP_IDs were absent from the manual architecture corpus and official Belfast architecture sweep candidate/rejected/readback packs through Round${DEDUPE_BOUNDARY_ROUND} during this sweep.`
    ],
    ...
    duplicate_review:
      `APP_ID, related APP_IDs, event_id and source_record_id were absent from the manual architecture corpus and official Belfast architecture sweep candidate/rejected/readback outputs through Round${DEDUPE_BOUNDARY_ROUND} checked by the Round587 duplicate scan.`,
...
function rejectedRecord(lead, duplicateHaystack) {
  return {
    ...
    reason: lead.reason,
    source_id: lead.id.replace(/^bfs_arch_round587_reject_/, "round587-reject-"),
...
function validate(candidates, rejected, duplicateHaystack) {
  ...
  if (candidates.length !== EXPECTED_CANDIDATE_COUNT) {
    errors.push(
      `Expected ${EXPECTED_CANDIDATE_COUNT} promoted next36 candidates, found ${candidates.length}`
    );
  }
...
function main() {
  ...
  const sourceAudit = {
    ...
    sources_checked: [
      ...
      {
        source_id: "dfi-planning-statistics-2025-26-quarterly-reviewed-round587",
        source_name: "Northern Ireland planning statistics 2025/26 provisional quarterly publications",
        publisher: "Department for Infrastructure, Northern Ireland",
        source_url: DFI_2025_26_Q3_URL,
        source_type: "official provisional quarterly planning-statistics publication",
      ...
      {
        source_id: "hed-historic-buildings-and-harni-reviewed-round587",
        ...
        accepted_records: 0,
        disposition:
          "reviewed; Historic Buildings layer lacks a usable event date for new dated listing/designation changes, while Heritage at Risk Date_Added rows are retained as status/admin leads outside this DfI residual next36 pack"
      },
      ...rejected.map((item) => ({
    ...
    caveats: [
      ...
      "Rejected official pages and rows include duplicate-project consents, 2025/26 quarterly aggregate-only releases, lower-priority fixture/equipment-only changes, lower-signal HMO/short-let-only rows, condition-only/compliance variations, telecom/transport/street-furniture rows, minor domestic rows, sign/advertisement/display rows, page-only leads and status-only heritage sources.",
  ...
  const notes = [
    ...
    "- Categories are primarily residential building-fabric administrative approvals from the December 2024 to February 2025 residual tail; lower-signal short-let, signage, equipment, condition-only and aggregate leads are retained in rejected.json.",
    "",
