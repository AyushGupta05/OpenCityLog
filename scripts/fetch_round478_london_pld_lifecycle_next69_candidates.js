...
function replaceRequired(value, needle, replacement, label) {
  if (!value.includes(needle)) {
    ...
    throw new Error(`Round478 source transform could not find ${label}: ${needle}`);
  }
...
function transformRound467Script(source) {
  let transformed = source
    .replace(/round467_london_pld_lifecycle_next66/g, "round478_london_pld_lifecycle_next69")
    .replace(/round467\.london_pld_lifecycle_next66/g, "round478.london_pld_lifecycle_next69")
    .replace(/Bims5Round467PldLifecycleNext66/g, "Bims5Round478PldLifecycleNext69")
    .replace(/Round 467/g, "Round 478")
    .replace(/Round467/g, "Round478")
    .replace(/round467/g, "round478")
    .replace(/next66/g, "next69")
    .replace(/MAX_PRIOR_ROUND = 460/g, "MAX_PRIOR_ROUND = 474")
    .replace(/through round460/g, "through round474")
    .replace(/packRound\(entry\.name\) > 460/g, "packRound(entry.name) > 474")
    .replace(/\.replace\(\/round276\/g, "round460"\);/g, '.replace(/round276/g, "round474");');
  ...
  transformed = replaceRequired(
    ...
    "round467, round469, and round474 prior pack insertion"
  );

  transformed = transformed
    .replace(
      /pack\) => pack\.label === "round460_london_pld_lifecycle_next65" && pack\.exists === true/g,
      'pack) => pack.label === "round474_london_pld_lifecycle_next68" && pack.exists === true'
    )
    .replace(/round460 prior PLD lifecycle pack was scanned/g, "round474 prior PLD lifecycle pack was scanned");

  assertContains(transformed, 'const ROUND_NAME = "round478_london_pld_lifecycle_next69";', "round name");
  assertContains(transformed, 'const ROUND = "round478";', "transformed fetch round id");
  assertContains(transformed, "MAX_PRIOR_ROUND = 474", "prior cutoff");
  assertContains(transformed, "through round474", "dedupe cutoff text");
  ...
  assertContains(transformed, "addRound478RetrievedAtAlias", "candidate retrieved_at alias");
  assertContains(transformed, 'CURRENT_ROUND_NAME = "round478_london_pld_lifecycle_next69"', "validator round name");
  assertContains(transformed, "round478_strict_duplicate_audit", "strict duplicate audit label");
...
async function main() {
  ...
  vm.runInNewContext(transformed, sandbox, {
    filename: "generated_round478_london_pld_lifecycle_next69_candidates.js"
  });
}
