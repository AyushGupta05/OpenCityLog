...
const GENERATED_FILENAME = "scripts/fetch_round375_nyc_dob_co_next41_candidates.generated.js";
const ROUND = "round375";
const ROUND_TITLE = "Round375";
const NEXT_LABEL = "next41";
const NEXT_TITLE = "Next41";
...
function transformRound308Source(source) {
  let transformed = source.replace(/\r\n/g, "\n")
    ...
    .replaceAll("through round303", "through round371");
  ...
  transformed = replaceFirst(
    ...
    `${round303File}\n];`,
    `${priorRoundFiles}\n];`,
    "explicit duplicate files through Round371"
  );
  transformed = replaceOnce(
    transformed,
    `${round303File}\n];`,
    `${priorRoundFiles}\n];`,
    "required screened files through Round371"
  );
  ...
  transformed = replaceOnce(
    ...
    "screened rounds include Round308 through Round371"
  );
  ...
  transformed = replaceOnce(
    ...
    "validation check fields through Round371"
  );
  ...
  transformed = replaceOnce(
    ...
    "validation report lines through Round371"
  );
  ...
  assertContains(transformed, "round375_nyc_dob_co_next41", "Round375 output path");
  assertContains(transformed, "Round375 NYC DOB CO Next41 Candidate Pack", "Round375 notes title");
  assertContains(transformed, "nyc_dob_co_round375_legacy_", "Round375 candidate ID prefix");
  assertContains(transformed, "tmp/subagents/round371_nyc_dob_co_next40/candidates.json", "Round371 screened file");
  assertContains(transformed, "through round371", "duplicate-screening text through Round371");
  assertContains(transformed, "required_round371_screened", "Round371 validation result");
  ...
  assertContains(transformed, `transformation_method: "${ROUND_TITLE} queried`, "Round375 source audit transformation method");
  assertContains(transformed, "scripts/fetch_round375_nyc_dob_co_next41_candidates.js standalone validation artifact", "Round375 validator label");
...
async function main() {
  ...
  if (typeof sandbox.module.exports.main !== "function") {
    throw new Error("Transformed Round375 generator did not export main().");
  }
