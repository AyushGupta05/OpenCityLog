...
const candidateFiles = [
  "tmp/subagents/london_arch_candidates_round105_civic_public_realm.json",
  "tmp/subagents/london_arch_candidates_round105_health_education.json",
  "tmp/subagents/london_arch_candidates_round105_transport_regeneration.json",
  "tmp/subagents/nyc_arch_candidates_round105_civic_parks_libraries.json",
  "tmp/subagents/nyc_arch_candidates_round105_health_education_culture.json",
  "tmp/subagents/nyc_arch_candidates_round105_housing_mixeduse_landmarks.json",
  "tmp/subagents/belfast_arch_candidates_round105_council_public_realm.json",
  "tmp/subagents/belfast_arch_candidates_round105_institutions_culture.json",
  "tmp/subagents/belfast_arch_candidates_round105_heritage_restoration.json"
];
...
const publisherFor = (candidate) => {
  ...
  if (url.includes("communities-ni.gov.uk")) return "Department for Communities Historic Environment Division / nidirect Buildings Database";
  if (url.includes("belfastcity.gov.uk") || url.includes("minutes.belfastcity.gov.uk")) return "Belfast City Council";
  if (url.includes("belfastbuildingstrust.org")) return "Belfast Buildings Trust";
  if (url.includes("heritagefund.org.uk")) return "The National Lottery Heritage Fund";
  if (url.includes("nyc.gov/site/ddc") || url.includes("nyc.gov/html/ddc")) return "NYC Department of Design and Construction";
  if (url.includes("nyc.gov/html/dep") || url.includes("nyc.gov/site/dep")) return "NYC Department of Environmental Protection";
  if (url.includes("nycgovparks.org")) return "NYC Parks";
  if (url.includes("nyc.gov/site/hpd")) return "NYC Department of Housing Preservation and Development";
  if (url.includes("nychdc.com")) return "NYC Housing Development Corporation";
  if (url.includes("cuny.edu")) return "City University of New York";
  if (url.includes("nhs.uk")) return "NHS public or trust website";
  if (url.includes("tfl.gov.uk")) return "Transport for London";
  if (url.includes("london.gov.uk")) return "Greater London Authority";
...
const sourceTypeFor = (candidate) => {
  ...
  if (text.includes("ddc") || text.includes("dep")) return "official agency press release";
  if (text.includes("news") || text.includes("press") || text.includes("release")) return "public news or institutional update page";
...
const termsNoteFor = (candidate) => {
  ...
  const url = String(candidate.source_url || "").toLowerCase();
  if (url.includes("communities-ni.gov.uk") || url.includes("nidirect.gov.uk")) {
  ...
  if (url.includes("nyc.gov") || url.includes("nycgovparks.org") || url.includes("nychdc.com")) {
    return "NYC public or partner website terms apply; factual metadata and source URL only, no page text or images reproduced.";
  }
  if (url.includes("nhs.uk")) {
    return "NHS website terms apply; factual metadata and source URL only, no page text or media reproduced.";
  }
...
if (candidateRows.length !== 76) {
  throw new Error(`Expected 76 round105 candidates, found ${candidateRows.length}`);
}
...
const toRecord = (candidate) => {
  ...
  return {
    ...
    transformation_method: safeText(
      ...
      `Manual round105 source-family extraction from ${candidate.__file}; duplicate checked by event_id, source URL, source record ID, and title terms; normalized into the architecture milestone schema with date, source, confidence, geometry, and limitation fields preserved.`
    )
  };
};
...
const tmpPath = `${path}.round105.tmp`;
