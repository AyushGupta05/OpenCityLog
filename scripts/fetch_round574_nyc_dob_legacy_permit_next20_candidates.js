...
const OUTPUT_DIR = path.join(__dirname, "..", "tmp", "subagents", "round574_nyc_dob_legacy_permit_next20");
...
function normalizeOutputBoundaryText() {
  ...
  for (const name of fs.readdirSync(OUTPUT_DIR)) {
    ...
    const normalized = text
      .replaceAll("through Round568", "through Round572")
      .replaceAll("treating Round568", "treating Round572");
...
if (sourceHash !== EXPECTED_SOURCE_SHA256) {
  throw new Error(
    `Round574 expected Round572 worker source hash ${EXPECTED_SOURCE_SHA256}, got ${sourceHash}. ` +
      "Review the upstream pattern before regenerating this candidate pack."
  );
}

const round574Source = source
  .replace("const ROUND = 572;", "const ROUND = 574;")
  .replace(
    'const SLUG = "round572_nyc_dob_legacy_permit_next19";',
    'const SLUG = "round574_nyc_dob_legacy_permit_next20";'
  )
  .replace("const DEDUPE_BOUNDARY_ROUND = 568;", "const DEDUPE_BOUNDARY_ROUND = 572;")
  .replaceAll(
    "fetch_round572_nyc_dob_legacy_permit_next19_candidates.js",
    "fetch_round574_nyc_dob_legacy_permit_next20_candidates.js"
  )
  .replaceAll("round572_nyc_dob_legacy_permit_next19", "round574_nyc_dob_legacy_permit_next20")
  .replaceAll("Round572", "Round574")
  .replaceAll('"Round563", "Round568"', '"Round568", "Round572"')
  .replace(
  ...
  .replaceAll("through Round568", "through Round572")
  .replaceAll("treating Round568", "treating Round572")
  .replaceAll("next19", "next20");

vm.runInNewContext(round574Source, {
  __dirname,
  __filename,
  require,
  console,
