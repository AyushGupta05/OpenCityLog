...
const GENERATED_FILENAME = "scripts/fetch_round367_nyc_dob_co_next39_candidates.generated.js";
const ROUND = "round367";
const ROUND_TITLE = "Round367";
const NEXT_LABEL = "next39";
const NEXT_TITLE = "Next39";
...
function transformRound308Source(source) {
  let transformed = source.replace(/\r\n/g, "\n")
    ...
    .replaceAll("through round303", "through round364");
  ...
  transformed = replaceFirst(
    ...
    `${round303File}\n];`,
    `${priorRoundFiles}\n];`,
    "explicit duplicate files through Round364"
  );
  transformed = replaceOnce(
    transformed,
    `${round303File}\n];`,
    `${priorRoundFiles}\n];`,
    "required screened files through Round364"
  );
  ...
  transformed = replaceOnce(
    ...
    "screened rounds include Round308 through Round364"
  );
  ...
  transformed = replaceOnce(
    ...
    "validation check fields through Round364"
  );
  ...
  transformed = replaceOnce(
    ...
    "validation report lines through Round364"
  );
  ...
  assertContains(transformed, "round367_nyc_dob_co_next39", "Round367 output path");
  assertContains(transformed, "Round367 NYC DOB CO Next39 Candidate Pack", "Round367 notes title");
  assertContains(transformed, "nyc_dob_co_round367_legacy_", "Round367 candidate ID prefix");
  assertContains(transformed, "tmp/subagents/round364_nyc_dob_co_next38/candidates.json", "Round364 screened file");
  assertContains(transformed, "through round364", "duplicate-screening text through Round364");
  assertContains(transformed, "required_round364_screened", "Round364 validation result");
  ...
  assertContains(transformed, `transformation_method: "${ROUND_TITLE} queried`, "Round367 source audit transformation method");
  assertContains(transformed, "scripts/fetch_round367_nyc_dob_co_next39_candidates.js standalone validation artifact", "Round367 validator label");
...
async function main() {
  ...
  if (typeof sandbox.module.exports.main !== "function") {
    throw new Error("Transformed Round367 generator did not export main().");
  }
