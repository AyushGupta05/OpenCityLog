...
const outDir = "tmp/subagents/round213_nyc_hpd_affordable_housing_next";
...
function buildCandidate(row, existing, rejected) {
  ...
  return {
    ...
    raw_row: row,
    transformation_method: `Round213 official NYC Open Data HPD fetch from ${DATASET.id}; selected from rows with project/building identifiers, a source date, source coordinates, unit counts, and duplicate screening against the live manual corpus plus prior HPD candidate packs.`
  };
}
...
function sourceAudit(metadata, acceptedCount, selectedSummary) {
  ...
  return {
    schema_version: "round213.nyc_hpd_affordable_housing_next_source_audit.v1",
    ...
    sources: [
      {
        ...
        published_coverage_note: "Round213 checked HPD building-level rows with coordinates and building/project completion or project start dates available through the access date.",
...
async function main() {
  ...
  const selected = candidates
    .sort((a, b) => {
      ...
      return hpdScore(b.raw_row, bDate) - hpdScore(a.raw_row, aDate) ||
  ...
  const summaryPayload = {
    schema_version: "round213.nyc_hpd_affordable_housing_next_summary.v1",
    generated_at: generatedAt,
    accessed_at: accessedAt,
    output_files: [
    ...
    selection_summary: {
      ...
      ranking: "Completion-date rows are preferred over project-start-only rows, then higher total/all-counted unit counts, then new-construction text signals, then newer dates.",
  ...
  const candidatesPayload = {
    schema_version: "round213.nyc_hpd_affordable_housing_next_candidates.v1",
    generated_at: generatedAt,
    accessed_at: accessedAt,
  ...
  const rejectedPayload = {
    schema_version: "round213.nyc_hpd_affordable_housing_next_rejected.v1",
    generated_at: generatedAt,
    accessed_at: accessedAt,
    source_id: DATASET.sourceId,
    dataset_id: DATASET.id,
  ...
  const notes = [
    "# Round213 NYC HPD Affordable Housing Production Candidate Pack",
    "",
