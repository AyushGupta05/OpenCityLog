...
const ROUND_ID = "round554_belfast_official_architecture_sweep_next30";
const OUT_DIR = path.join("tmp", "subagents", ROUND_ID);
const SCRIPT_PATH =
  "scripts/fetch_round554_belfast_official_architecture_sweep_next30_candidates.js";
...
const DEDUPE_BOUNDARY_ROUND = 544;
...
const METHOD = [
  "round554 official Belfast architecture sweep next30 after the completed Round544 next29 pack.",
  ...
  "Round544 is treated as the latest Belfast official architecture sweep dedupe boundary; official page-only, geometry-ref-only, duplicate-project, status-only, signage-only, equipment-only and lower-priority private-use leads are retained separately."
].join(" ");

const TRANSFORMATION_METHOD = `${SCRIPT_PATH}#round554OfficialArchitectureSweepnext30`;

const SOURCES = {
  dfiPlanningStats: {
    source_id: "dfi-planning-statistics-2024-25-round554",
...
const SEARCH_QUERIES_CHECKED = [
  "local DfI planning-statistics 2024-25 Belfast residual APP_ID scan after Round544 accepted-pack dedupe",
  "local duplicate scan: data/manual_drops/architecture_milestones plus official Belfast architecture sweep candidate packs through Round544",
...
function replaceRound554LeadBlocks() {
  const lead = (
    app_id,
    related_app_ids,
    event_id,
  ...
  const classify = (row) => {
    ...
    if (/proposed dwelling|new dwelling|residential development/.test(text)) {
      return {
  ...
  PLANNING_LEADS.splice(
    ...
    ...selectedAppIds.map((appId) => {
      const row = rowsById.get(appId);
      if (!row) throw new Error(`Selected Round554 app ID missing source row: ${appId}`);
      const site = siteLabel(row);
      const proposal = clean(row.Proposal).replace(/\.$/, "");
      const appKey = appId.toLowerCase().replace(/^la04\//, "").replace(/[^a-z0-9]+/g, "_");
      const eventId = `bfs_arch_round554_${slug(site)}_${appKey}_approval_2024`;
...
replaceRound554LeadBlocks();

REJECTED_LEADS.splice(
  0,
  REJECTED_LEADS.length,
  {
    id: "bfs_arch_round554_reject_duplicate_project_rows_through_round544",
    title: "Residual duplicate-project planning rows through the Round544 boundary",
    rejection_category: "duplicate_project_boundary",
    reason:
      "Official point-backed DfI rows for already-covered project families or same-site milestones were retained outside the promoted pack because the manual architecture corpus or official Belfast architecture sweep candidate records through Round544 already cover the same project family.",
    source_record_id:
  ...
  {
    id: "bfs_arch_round554_reject_short_let_and_hmo_only_tail",
  ...
  {
    id: "bfs_arch_round554_reject_page_only_or_no_point_official_leads",
  ...
  {
    id: "bfs_arch_round554_reject_signage_equipment_transport_rows",
...
function candidateFromPlanningLead(lead, rowsById, duplicateHaystack) {
  ...
  return {
    ...
    duplicate_review:
      `APP_ID, related APP_IDs, event_id and source_record_id were absent from the manual architecture corpus and official Belfast architecture sweep candidate outputs through Round${DEDUPE_BOUNDARY_ROUND} checked by the round554 duplicate scan.`,
...
function rejectedRecord(lead, duplicateHaystack) {
  return {
    ...
    reason: lead.reason,
    source_id: lead.id.replace(/^bfs_arch_round554_reject_/, "round554-reject-"),
...
function validate(candidates, rejected, duplicateHaystack) {
  ...
  if (candidates.length !== EXPECTED_CANDIDATE_COUNT) {
    errors.push(
      `Expected ${EXPECTED_CANDIDATE_COUNT} promoted next30 candidates, found ${candidates.length}`
    );
  }
...
function main() {
  ...
  const sourceAudit = {
    ...
    sources_checked: [
      ...
      {
        source_id: "dfi-planning-statistics-2025-26-quarterly-reviewed-round554",
      ...
      {
        source_id: "hed-historic-buildings-and-harni-reviewed-round554",
        ...
        accepted_records: 0,
        disposition:
          "reviewed; Historic Buildings layer lacks a usable event date for new dated listing/designation changes, while Heritage at Risk Date_Added rows are retained as status/admin leads outside this DfI residual next30 pack"
      },
      ...rejected.map((item) => ({
