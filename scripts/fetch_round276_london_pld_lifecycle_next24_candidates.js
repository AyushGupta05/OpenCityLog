...
const ROUND = "round276";
const ROUND_NAME = "round276_london_pld_lifecycle_next24";
...
const EXPLICIT_PRIOR_PACKS = [
  ...
  "round196_london_pld_lifecycle_next11",
  "round202_london_pld_lifecycle_next12",
  "round208_london_pld_lifecycle_next13",
  "round217_london_pld_lifecycle_next14",
  "round223_london_pld_lifecycle_next15",
  "round230_london_pld_lifecycle_next16",
  "round238_london_pld_lifecycle_next17",
  "round243_london_pld_lifecycle_next18",
  "round248_london_pld_lifecycle_next19",
  "round253_london_pld_lifecycle_next20",
  "round261_london_pld_lifecycle_next21",
  "round266_london_pld_lifecycle_next22",
  "round272_london_pld_lifecycle_next23"
];
...
function discoverPriorPacks() {
  ...
  if (fs.existsSync(subagentsDir)) {
    for (const entry of fs.readdirSync(subagentsDir, { withFileTypes: true })) {
      ...
      if (
        /^round\d+_london_pld_lifecycle/.test(entry.name) &&
        entry.name !== ROUND_NAME &&
        packSortValue(entry.name) <= MAX_PRIOR_ROUND
      ) {
  ...
  for (let page = 0; page < maxPages; page += 1) {
    ...
    const response = await fetch(API_ENDPOINT, {
      ...
      headers: {
        ...
        "user-agent": "Bims5Round276PldLifecycleNext23/0.1"
      },
...
function candidateFor(item) {
  ...
  return {
    ...
    license: "Not Specified on London Datastore dataset page checked 2026-05-20",
    source_license: "Not Specified",
    license_or_terms_note:
      "London Datastore Planning London Datahub applications page lists Licence: Not Specified as checked on 2026-05-20; this candidate pack retains factual row metadata, official row/API URLs, attribution, and retrieval date, and terms should be reviewed before redistribution.",
    attribution:
    ...
    duplicate_check_note:
      "Screened against the live manual architecture corpus and all discovered prior London PLD lifecycle packs through round272, including round126 direct/more, round140, round147, round150, round153, round157, round161, round166, round172, round178, round184, round190, round196, round202, round208, round217, round223, round230, round238, round243, round248, round253, round261, round266, and round272 by PLD row id, source-date-field/date, source URL/source_record/date-field/date, and title/date.",
    limitations: [
    ...
    transformation_method:
      "scripts/fetch_round276_london_pld_lifecycle_next24_candidates.js queried the official Planning London Datahub guest applications API for actual_commencement_date and actual_completion_date rows, normalized source dates, filtered to the London coordinate envelope and 2008-2026 access window, scored architecture/city-change signals, and removed exact source row/date-field/date duplicates already present in the live manual architecture corpus or prior PLD lifecycle packs through round272."
  };
}
...
function rejectionReason(item, duplicateIndex, batchKeys) {
  ...
  if (batchKeys.has(fieldKey)) return "duplicate_row_date_field_inside_round276_batch";
  if (
...
function validateCandidates(pack, duplicateIndex) {
  ...
  for (const candidate of candidates) {
    ...
    if (candidate.source_date_field !== candidate.source_lifecycle_field) {
      throw new Error(`source_date_field must be the literal PLD lifecycle field on ${candidate.candidate_id}`);
    }
...
async function main() {
  ...
  const candidatesPack = {
    schema_version: "round276.london_pld_lifecycle_next24.candidates.v1",
    generated_at: GENERATED_AT,
    accessed_at: ACCESSED_AT,
    task:
      "Round276 London Planning Datahub actual lifecycle candidate pack beyond current manual corpus and prior PLD lifecycle packs through round272",
    ...
    source_audits: [
      {
        ...
        license_or_terms_note:
          "London Datastore dataset page lists Licence: Not Specified as checked on 2026-05-20. This scratch pack keeps factual row metadata, official row/API URLs, normalized coordinates, attribution, and retrieval date only. Review terms before bulk redistribution.",
        ...
        update_frequency: "Daily according to the London Datastore dataset page checked on 2026-05-20.",
        geographic_scope:
  ...
  const summary = {
    ...
    dedupe_basis:
      "Live data/manual_drops/architecture_milestones/architecture_milestones_2008_2026.json plus all discovered London PLD lifecycle scratch packs through round272, explicitly including round126 direct/more, round140, round147, round150, round153, round157, round161, round166, round172, round178, round184, round190, round196, round202, round208, round217, round223, round230, round238, round243, round248, round253, round261, round266, and round272, scanned for PLD ids, source-record/source-url/date-field/date keys, source-date-field/date keys, and title/date keys.",
    ...
    caveats: [
      ...
      "Exact source row/date-field/date duplicates already present in the live manual corpus or prior PLD lifecycle packs through round272 were excluded.",
      "London Datastore licence is Not Specified for this source, so the pack retains factual metadata and attribution only."
    ]
  };

  const rejectedPack = {
    schema_version: "round276.london_pld_lifecycle_next24.rejected.v1",
    generated_at: GENERATED_AT,
    accessed_at: ACCESSED_AT,
  ...
  const notes = [
    "# Round 276 London PLD lifecycle next24 candidates",
    "",
    ...
    "",
    "Deduplication scanned the live manual architecture corpus plus all discovered prior London PLD lifecycle scratch packs through round272, explicitly including round126 direct, round126 more, round140, round147, round150, round153, round157, round161, round166, round172, round178, round184, round190, round196, round202, round208, round217, round223, round230, round238, round243, round248, round253, round261, round266, and round272 by PLD row id, source URL/source record/date-field/date, source date field/date, and title/date.",
    "",
