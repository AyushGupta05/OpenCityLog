...
function replaceRequired(value, needle, replacement, label) {
  if (!value.includes(needle)) {
    ...
    throw new Error(`Round542 wrapper transform could not find ${label}: ${needle}`);
  }
...
function transformRound529Wrapper(source) {
  let transformed = source
    .replace(/round529_london_pld_lifecycle_next82/g, "round542_london_pld_lifecycle_next85")
    .replace(/round529\.london_pld_lifecycle_next82/g, "round542.london_pld_lifecycle_next85")
    .replace(/Bims5Round529PldLifecycleNext82/g, "Bims5Round542PldLifecycleNext85")
    .replace(/Round 529/g, "Round 542")
    .replace(/Round529/g, "Round542")
    .replace(/round529/g, "round542")
    .replace(/next82/g, "next85")
    .replace(/MAX_PRIOR_ROUND = 526/g, "MAX_PRIOR_ROUND = 537")
    .replace(/through round526/g, "through round537")
    .replace(/packRound\(entry\.name\) > 526/g, "packRound(entry.name) > 537")
    .replace(/\.replace\(\/round276\/g, "round526"\);/g, '.replace(/round276/g, "round537");');
  ...
  transformed = replaceRequired(
    ...
    "round529, round534, and round537 prior pack insertion"
  );

  transformed = transformed
    .replace(
      /pack\) => pack\.label === "round526_london_pld_lifecycle_next81" && pack\.exists === true/g,
      'pack) => pack.label === "round537_london_pld_lifecycle_next84" && pack.exists === true'
    )
    .replace(/round526 prior PLD lifecycle pack was scanned/g, "round537 prior PLD lifecycle pack was scanned");

  assertContains(transformed, 'const ROUND_NAME = "round542_london_pld_lifecycle_next85";', "round name");
  assertContains(transformed, 'const ROUND = "round542";', "transformed fetch round id");
  assertContains(transformed, "MAX_PRIOR_ROUND = 537", "prior cutoff");
  assertContains(transformed, "through round537", "dedupe cutoff text");
  ...
  assertContains(transformed, "addRound542RetrievedAtAlias", "candidate retrieved_at alias");
  assertContains(transformed, 'CURRENT_ROUND_NAME = "round542_london_pld_lifecycle_next85"', "validator round name");
  assertContains(transformed, "round542_strict_duplicate_audit", "strict duplicate audit label");
...
async function main() {
  ...
  vm.runInNewContext(transformed, sandbox, {
    filename: "generated_round542_london_pld_lifecycle_next85_candidates.js"
  });
}
