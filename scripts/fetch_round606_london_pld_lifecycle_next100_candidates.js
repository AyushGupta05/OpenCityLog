...
const ROUND_NAME = "round606_london_pld_lifecycle_next100";
...
function replaceRequired(value, needle, replacement, label) {
  if (!value.includes(needle)) {
    ...
    throw new Error(`Round606 wrapper transform could not find ${label}: ${needle}`);
  }
...
function transformRound579Wrapper(source) {
  let transformed = source
    .replace(/round579_london_pld_lifecycle_next94/g, "round606_london_pld_lifecycle_next100")
    .replace(/round579\.london_pld_lifecycle_next94/g, "round606.london_pld_lifecycle_next100")
    .replace(/Bims5Round579PldLifecycleNext94/g, "Bims5Round606PldLifecycleNext100")
    .replace(/Round 579/g, "Round 606")
    .replace(/Round579/g, "Round606")
    .replace(/round579/g, "round606")
    .replace(/next94/g, "next100")
    .replace(/MAX_PRIOR_ROUND = 576/g, "MAX_PRIOR_ROUND = 605")
    .replace(/through round576/g, "through round605")
    .replace(/packRound\(entry\.name\) > 576/g, "packRound(entry.name) > 605")
    .replace(/\.replace\(\/round276\/g, "round576"\);/g, '.replace(/round276/g, "round605");');
  ...
  transformed = replaceRequired(
    ...
    "round579, round584, round589, round594, round597, and round605 prior pack insertion"
  );

  transformed = transformed
    .replace(
      /pack\) => pack\.label === "round576_london_pld_lifecycle_next93" && pack\.exists === true/g,
      'pack) => pack.label === "round605_london_pld_lifecycle_next99" && pack.exists === true'
    )
    .replace(/round576 prior PLD lifecycle pack was scanned/g, "round605 prior PLD lifecycle pack was scanned");

  assertContains(transformed, 'const ROUND_NAME = "round606_london_pld_lifecycle_next100";', "round name");
  assertContains(transformed, 'const ROUND = "round606";', "transformed fetch round id");
  assertContains(transformed, "MAX_PRIOR_ROUND = 605", "prior cutoff");
  assertContains(transformed, "through round605", "dedupe cutoff text");
  ...
  assertContains(transformed, "addRound606RetrievedAtAlias", "candidate retrieved_at alias");
  assertContains(transformed, 'CURRENT_ROUND_NAME = "round606_london_pld_lifecycle_next100"', "validator round name");
  assertContains(transformed, "round606_strict_duplicate_audit", "strict duplicate audit label");
...
async function main() {
  ...
  vm.runInNewContext(transformed, sandbox, {
    filename: "generated_round606_london_pld_lifecycle_next100_candidates.js"
  });
