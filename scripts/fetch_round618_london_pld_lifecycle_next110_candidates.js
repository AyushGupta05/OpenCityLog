...
function replaceRequired(value, needle, replacement, label) {
  if (!value.includes(needle)) {
    ...
    throw new Error(`Round618 wrapper transform could not find ${label}: ${needle}`);
  }
...
function transformRound616Wrapper(source) {
  let transformed = source
    .replace(/round616_london_pld_lifecycle_next109/g, "round618_london_pld_lifecycle_next110")
    .replace(/round616\.london_pld_lifecycle_next109/g, "round618.london_pld_lifecycle_next110")
    .replace(/Bims5Round616PldLifecycleNext109/g, "Bims5Round618PldLifecycleNext110")
    .replace(/Round 616/g, "Round 618")
    .replace(/Round616/g, "Round618")
    .replace(/round616/g, "round618")
    .replace(/next109/g, "next110")
    .replace(/MAX_PRIOR_ROUND = 614/g, "MAX_PRIOR_ROUND = 616")
    .replace(/through round614/g, "through round616")
    .replace(/packRound\(entry\.name\) > 614/g, "packRound(entry.name) > 616")
    .replace(/\.replace\(\/round276\/g, "round614"\);/g, '.replace(/round276/g, "round616");')
    .replace(/round614 prior PLD lifecycle pack was scanned/g, "round616 prior PLD lifecycle pack was scanned")
    .replace(
      /pack\) => pack\.label === "round614_london_pld_lifecycle_next108" && pack\.exists === true/g,
      'pack) => pack.label === "round616_london_pld_lifecycle_next109" && pack.exists === true'
    );
  ...
  transformed = replaceRequired(
    ...
    "round616 prior pack insertion"
  );

  assertContains(transformed, 'const ROUND_NAME = "round618_london_pld_lifecycle_next110";', "round name");
  assertContains(transformed, 'const ROUND = "round618";', "transformed fetch round id");
  assertContains(transformed, "MAX_PRIOR_ROUND = 616", "prior cutoff");
  assertContains(transformed, "through round616", "dedupe cutoff text");
  ...
  assertContains(transformed, "addRound618RetrievedAtAlias", "candidate retrieved_at alias");
  assertContains(transformed, 'CURRENT_ROUND_NAME = "round618_london_pld_lifecycle_next110"', "validator round name");
  assertContains(transformed, "round618_strict_duplicate_audit", "strict duplicate audit label");
...
async function main() {
  ...
  vm.runInNewContext(transformed, sandbox, {
    filename: "generated_round618_london_pld_lifecycle_next110_candidates.js"
  });
}
