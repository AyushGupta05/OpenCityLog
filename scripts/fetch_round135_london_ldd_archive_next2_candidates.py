...
TASK_ID = "round135_london_ldd_archive_next2_candidates"
SCRIPT_PATH = "scripts/fetch_round135_london_ldd_archive_next2_candidates.py"
...
OUT_DIR = Path("tmp/subagents/round135_london_ldd_archive_next2")
...
def candidate_for(row, excel_row, score, flags, floorspace_refs, bedroom_refs):
    ...
    candidate_id = f"lon_ldd_archive_next2_round135_row_{excel_row}_{slugify(authority, 36)}_{slugify(borough_ref, 48)}"
    row_reference = f"{authority} / {borough_ref}"
    ...
    transformation_method = (
        ...
        "planning authority/borough reference, source URL/record id, and title/date, with a fallback check against the "
        "round132 candidate pack; filtered to Current permission status "
...
def write_notes(candidate_count, summary):
    text = f"""# Round 135 London LDD archive next2 candidates

Generated another bounded London Development Database archive candidate pack beyond already ingested LDD rows in the manual architecture corpus, including the round132 rows now present in the corpus.
...
The generator excludes manual-corpus LDD records by workbook row, planning authority plus borough reference, source URL plus source record id, and candidate title/date. It also checks the prior round132 candidate pack as a fallback and removes duplicate source rows and authority/reference pairs inside the new batch.
...
def main():
    ...
    for excel_row, row in iter_xlsx_dict_rows(permissions_xlsx):
        ...
        if excel_row in dedupe["rows"]:
            ...
            reason = "duplicate inside round135 batch scan"
    ...
    selection_summary = {
        ...
        "dedupe_fields": [
            ...
            "city/title/date",
            "prior round132 candidate pack as a fallback if corpus ingestion is incomplete",
            "same fields inside the round135 batch scan",
        ],
        "filters": [
