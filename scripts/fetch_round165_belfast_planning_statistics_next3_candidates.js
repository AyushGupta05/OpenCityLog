...
const ROUND_ID = "round165_belfast_planning_statistics_next3";
...
function assessNext3Quality(record, baseAssessment, existingApps) {
  ...
  if (baseAssessment.score < MIN_SCORE) {
    return { passes: false, reason: "below_round165_score_floor" };
  }
  ...
  if (hasResidentialUse && strongScale && hasBuiltFacility) {
    return {
      passes: true,
      qualityGate: "scaled_residential_residual_next3",
  ...
  if (
    ...
    return {
      passes: true,
      qualityGate: "listed_adaptive_reuse_or_built_use_next3",
  ...
  if (
    ...
    return {
      passes: true,
      qualityGate: "civic_or_public_facility_next3",
  ...
  if (
    ...
    return {
      passes: true,
      qualityGate: "industrial_or_commercial_built_use_next3",
      qualityReasons: ["industrial/commercial built-use residual approval"]
    };
  }

  return { passes: false, reason: "not_substantial_enough_for_round165" };
}
...
function limitationsFor() {
  return [
    ...
    "Round165 keeps only residual rows that are post-dedupe against the live manual corpus and the round131, round134, round137, round145, and round151 Belfast planning-statistics packs, approved, in the Belfast coordinate envelope, and strong enough for a scaled residential, listed/adaptive reuse, civic/community facility, or industrial/commercial built-use residual gate. Existing same-round output is read for rerun awareness because the script overwrites this pack rather than appending to it.",
...
function candidateFor(record) {
  ...
  return {
    ...
    transformation_method: "scripts/fetch_round165_belfast_planning_statistics_next3_candidates.js reused the round145 local DfI planning-statistics CSV parser, deduped against all planning application IDs in the live manual architecture corpus plus round131, round134, round137, round145, and round151 Belfast planning-statistics candidate application IDs/source rows, read any existing same-round output for rerun awareness, rejected residual rows that referenced already-used application IDs, kept only approved 2016-2025 Belfast rows with approximate in-city coordinates and residual scaled residential, listed/adaptive reuse, civic/community facility, or industrial/commercial built-use signals, excluded domestic/minor/admin/signage/telecom/frontage-only/internal-fitout/site-only rows, and preserved file/row provenance."
  };
}
...
function sourceAudit(fileAudits, summary, decision) {
  return {
    ...
    transformation_method: "Live manual-corpus app-ID/source-row dedupe, explicit round131/round134/round137/round145/round151 candidate app-ID/source-row dedupe, same-round output readback for rerun awareness, linked-application rejection, Belfast authority/LPA and coordinate-envelope check, approved decision filter, 2016-2025 date window bounded by available CSVs, residual high-signal scoring, exclusion of minor domestic/admin/signage/telecom/frontage-only/internal-fitout/site-only rows, deterministic sorting, and required-provenance validation.",
    scoring_summary: {
      min_score: MIN_SCORE,
      max_candidates: MAX_CANDIDATES,
      emitted_candidates: summary.accepted_candidates,
      quality_gate: "A residual row must pass one round165 gate: scaled residential approval, listed/adaptive built-use approval, civic/community facility approval, or industrial/commercial built-use approval."
    },
...
function buildPack() {
  ...
  for (const record of rows) {
    ...
    if (!record.date || record.date < DATE_START || record.date > DATE_END) {
      addRejection(rejections, rejectionCounts, record, "outside_round165_window");
      continue;
    }
    ...
    if (selectedAppIds.has(record.appIdKey)) {
      addRejection(rejections, rejectionCounts, record, "duplicate_app_id_in_round165_selection");
      continue;
    }

    const baseAssessment = helpers.assessQuality(record);
    const next3Assessment = assessNext3Quality(record, baseAssessment, existingApps);
    if (!next3Assessment.passes) {
      addRejection(rejections, rejectionCounts, record, next3Assessment.reason, {
        score: baseAssessment.score,
        reasons: baseAssessment.qualitySignals,
        linked_existing_application_ids: next3Assessment.linkedExistingAppIds
      });
      continue;
    }
    ...
    selectedRecords.push({
      ...
      qualitySignals: [...new Set([
        ...
        ...next3Assessment.qualityReasons
      ])],
      qualityGate: next3Assessment.qualityGate
    });
  }
  ...
  const decision = candidates.length > 0
    ...
    : "Do not append another pack from this tranche; remaining post-dedupe rows did not clear the round165 residual high-signal quality gate.";
