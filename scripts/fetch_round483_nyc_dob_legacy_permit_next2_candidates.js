...
const ROUND = 483;
const SLUG = "round483_nyc_dob_legacy_permit_next2";
...
function makeCandidate(appRow, permitRow, permitDate, scale) {
  ...
  return {
    ...
    transformation_method: "scripts/fetch_round483_nyc_dob_legacy_permit_next2_candidates.js queried official NYC Open Data DOB Job Application Filings high-signal rows, joined them by legacy job number to official DOB Permit Issuance rows, retained initial issued NB/AL permits within 2008-01-01 through 2026-05-20, and screened against the manual corpus plus prior NYC DOB/DOB legacy candidate packs by permit SI number, job number, source/date key, source URL, title/date key, and address/date key.",
    address,
...
function validateCandidates(candidates, duplicateIndex, sourceAudits, summaryCandidateCount) {
  ...
  return {
    ...
    accessed_at: ACCESSED_AT,
    validator: "scripts/fetch_round483_nyc_dob_legacy_permit_next2_candidates.js standalone validation artifact",
    errors,
    checks: {
...
function duplicateAudit(candidates, duplicateIndex, validation) {
  return {
    ...
    generated_at: GENERATED_AT,
    accessed_at: ACCESSED_AT,
    audit_scope: "Round483 duplicate audit for NYC DOB Permit Issuance legacy permit next2 candidates.",
    dedupe_boundary: {
...
async function main() {
  ...
  const sourceAudits = [
    sourceAuditFromMetadata(DATASETS.permits, metadata[DATASETS.permits.dataset_id], {
      ...
      method: "Round483 queried official NYC Open Data DOB Permit Issuance rows and retained initial issued NB/AL permit rows joined to high-signal legacy DOB job application rows, with duplicate screening against the manual corpus and prior DOB packs.",
    ...
    sourceAuditFromMetadata(DATASETS.filings, metadata[DATASETS.filings.dataset_id], {
      ...
      method: "Round483 used DOB Job Application Filings only as supporting context for selected DOB Permit Issuance rows, joining by legacy job number and retaining source-reported scale/status fields.",
  ...
  const summary = {
    ...
    task: "Round483 NYC DOB Permit Issuance legacy permit next2 candidate pack",
