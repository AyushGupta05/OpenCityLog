...
source = source
  .replace(/const ROUND = 471;/g, "const ROUND = 479;")
  .replace(/round471_nyc_lpc_permit_next66/g, "round479_nyc_lpc_permit_next68")
  .replace(/round471/g, "round479")
  .replace(/Round471/g, "Round479")
  .replace(/next66/g, "next68")
  .replace(/Next66/g, "Next68")
  .replace(/through round466/g, "through round476");
...
function writeRound479DuplicateAudit() {
  if (duplicateAuditWritten || process.exitCode) return;
  duplicateAuditWritten = true;

  const root = path.resolve(__dirname, "..");
  const outDir = path.join(root, "tmp", "subagents", "round479_nyc_lpc_permit_next68");
  ...
  const audit = {
    ...
    audit_scope: "Round479 duplicate audit for NYC LPC Permit Application Information next68 candidates.",
    dedupe_boundary: {
      latest_lpc_permit_pack: "tmp/subagents/round476_nyc_lpc_permit_next67/candidates.json",
...
process.on("beforeExit", writeRound479DuplicateAudit);
