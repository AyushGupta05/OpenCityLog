...
const candidateFiles = [
  "tmp/subagents/london_arch_candidates_round110_official.json"
];
...
const acceptedEventIds = new Set([
  "lon_arch_liverpool_st_east_anglians_memorial_listing_2022",
  "lon_arch_26_the_avenue_bickley_listing_2022",
  "lon_arch_highgate_shepherds_cottage_listing_2021",
  "lon_arch_lauriston_road_jewish_cemetery_lodge_listing_2021",
  "lon_arch_hays_mews_41_43_listing_2021",
  "lon_arch_club_row_whitby_street_listing_2019"
]);

const acceptedRows = rows.filter((candidate) => acceptedEventIds.has(candidate.event_id));
const missingAcceptedRows = [...acceptedEventIds].filter((id) => !acceptedRows.some((candidate) => candidate.event_id === id));

if (rows.length !== 16 || acceptedRows.length !== 6 || missingAcceptedRows.length > 0) {
  throw new Error(`Expected 16 round110 candidates with 6 accepted rows; found ${rows.length} candidates and ${acceptedRows.length} accepted rows; missing ${missingAcceptedRows.join(", ")}`);
}

const records = acceptedRows.map((candidate) => ({
  city_id: "london",
  ...
  bucket: safeText(candidate.category || "architecture/heritage_designation"),
  title: safeText(candidate.title),
  summary: safeText(candidate.summary),
  observed_change: safeText(
    candidate.observed_change ||
    `${candidate.source_name || "Historic England NHLE"} records this statutory heritage designation.`
  ),
  area: safeText(candidate.location || candidate.address || "London"),
  ...
  source_type: safeText(candidate.source_type || "official NHLE listed-building point record"),
  ...
  project_type: safeText(candidate.event_type || candidate.category || "heritage designation"),
  geometry_source: safeText(candidate.geometry_source || "Historic England NHLE Open Data Hub WGS84 point geometry."),
  geometry_precision: safeText(candidate.geometry_precision || "NHLE point marker; not a measured footprint or statutory boundary."),
  ...
  transformation_method: safeText(
    ...
    `Manual round110 NHLE extraction from ${candidate.__file}; duplicate checked by event_id, source URL, source record ID, and title terms; normalized into the architecture milestone schema with date, source, confidence, geometry, and limitation fields preserved.`
  )
}));

const nhleSource = doc.sources.find((source) => source.source_id === "historic-england-nhle");
if (nhleSource) {
  nhleSource.title = "National Heritage List for England listed-building records";
  nhleSource.access_url = "https://historicengland.org.uk/listing/the-list/data-downloads/";
  nhleSource.licence = "Historic England Open Data Hub GIS data is licensed under Open Government Licence v3.0; list-entry page text, images, and map products require separate terms review. Factual metadata and source URLs are retained here.";
  nhleSource.licence_url = "https://historicengland.org.uk/terms/website-terms-conditions/open-data-hub/";
  nhleSource.retrieved_at = retrievedAt;
  nhleSource.limitations = "NHLE records document heritage designation changes, not construction completion or comprehensive development activity. Coordinates are source point markers for atlas navigation; Historic England notes spatial data is provided to indicate location and may not define full curtilage or statutory boundary.";
}
...
const tmpPath = `${path}.round110.tmp`;
