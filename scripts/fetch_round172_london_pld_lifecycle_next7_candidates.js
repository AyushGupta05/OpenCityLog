...
const ROUND = "round172";
const ROUND_NAME = "round172_london_pld_lifecycle_next7";
  ...
  for (let page = 0; page < maxPages; page += 1) {
    ...
    const response = await fetch(API_ENDPOINT, {
      ...
      headers: {
        ...
        "user-agent": "Bims5Round172PldLifecycleNext7/0.1"
      },
...
function candidateFor(item) {
  ...
  return {
    ...
    duplicate_check_note:
      "Screened against the live manual architecture corpus and all discovered prior London PLD lifecycle packs through round166, including round126 direct/more, round140, round147, round150, round153, round157, round161, and round166 by PLD row id, source-date-field/date, source URL/source_record/date-field/date, and title/date.",
    limitations: [
    ...
    transformation_method:
      "scripts/fetch_round172_london_pld_lifecycle_next7_candidates.js queried the official Planning London Datahub guest applications API for actual_commencement_date and actual_completion_date rows, normalized source dates, filtered to the London coordinate envelope and 2008-2026 access window, scored architecture/city-change signals, and removed exact source row/date-field/date duplicates already present in the live manual architecture corpus or prior PLD lifecycle packs through round166."
  };
}
...
function rejectionReason(item, duplicateIndex, batchKeys) {
  ...
  if (batchKeys.has(fieldKey)) return "duplicate_row_date_field_inside_round172_batch";
  if (
...
async function main() {
  ...
  const candidatesPack = {
    schema_version: "round172.london_pld_lifecycle_next7.candidates.v1",
    generated_at: GENERATED_AT,
    accessed_at: ACCESSED_AT,
    task:
      "Round172 London Planning Datahub actual lifecycle candidate pack beyond current manual corpus and prior PLD lifecycle packs through round166",
  ...
  const summary = {
    ...
    dedupe_basis:
      "Live data/manual_drops/architecture_milestones/architecture_milestones_2008_2026.json plus all discovered London PLD lifecycle scratch packs through round166, explicitly including round126 direct/more, round140, round147, round150, round153, round157, round161, and round166, scanned for PLD ids, source-record/source-url/date-field/date keys, source-date-field/date keys, and title/date keys.",
    ...
    caveats: [
      ...
      "Exact source row/date-field/date duplicates already present in the live manual corpus or prior PLD lifecycle packs through round166 were excluded.",
      "London Datastore licence is Not Specified for this source, so the pack retains factual metadata and attribution only."
    ]
  };

  const rejectedPack = {
    schema_version: "round172.london_pld_lifecycle_next7.rejected.v1",
    generated_at: GENERATED_AT,
    accessed_at: ACCESSED_AT,
  ...
  const notes = [
    "# Round 172 London PLD lifecycle next7 candidates",
    "",
    ...
    "",
    "Deduplication scanned the live manual architecture corpus plus all discovered prior London PLD lifecycle scratch packs through round166, explicitly including round126 direct, round126 more, round140, round147, round150, round153, round157, round161, and round166 by PLD row id, source URL/source record/date-field/date, source date field/date, and title/date.",
    "",
