...
function replaceRequired(value, needle, replacement, label) {
  if (!value.includes(needle)) {
    ...
    throw new Error(`Round398 source transform could not find ${label}: ${needle}`);
  }
...
function assertContains(value, needle, label) {
  if (!value.includes(needle)) {
    throw new Error(`Round398 source transform failed to include ${label}: ${needle}`);
  }
}

function transformRound362Script(source) {
  let transformed = source
    .replace(/round362_london_pld_lifecycle_next42/g, "round398_london_pld_lifecycle_next51")
    .replace(/round362\.london_pld_lifecycle_next42/g, "round398.london_pld_lifecycle_next51")
    .replace(/Bims5Round362PldLifecycleNext42/g, "Bims5Round398PldLifecycleNext51")
    .replace(/Round362/g, "Round398")
    .replace(/Round 362/g, "Round 398")
    .replace(/round362/g, "round398")
    .replace(/next42/g, "next51")
    .replace(/MAX_PRIOR_ROUND = 358/g, "MAX_PRIOR_ROUND = 393")
    .replace(/through round358/g, "through round393");
  ...
  transformed = replaceRequired(
    ...
    '      "round272, round276, round284, round288, round298, round301, round306, round311, round316, round323, round327, round328, round333, round338, round343, round348, round350, round354, round358, round362, round365, round369, round373, round377, round380, round384, round387, and round393"\\n    );',
  ...
  assertContains(transformed, "round377_london_pld_lifecycle_next46", "prior round377 exclusion");
  assertContains(transformed, "round380_london_pld_lifecycle_next47", "prior round380 exclusion");
  assertContains(transformed, "round384_london_pld_lifecycle_next48", "prior round384 exclusion");
  assertContains(transformed, "round387_london_pld_lifecycle_next49", "prior round387 exclusion");
  assertContains(transformed, "round393_london_pld_lifecycle_next50", "prior round393 exclusion");
  assertContains(transformed, "through round393", "dedupe prose cutoff");
  return transformed;
}
...
function main() {
  ...
  vm.runInNewContext(
    ...
    {
      filename: "generated_round398_london_pld_lifecycle_next51_candidates.js"
    }
  );
}
