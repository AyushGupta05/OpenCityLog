...
TASK_ID = "round285_london_ldd_archive_next23_candidates"
SCRIPT_PATH = "scripts/fetch_round285_london_ldd_archive_next23_candidates.py"
OUT_DIR = Path("tmp/subagents/round285_london_ldd_archive_next23")
...
ROUND176_SCRIPT_PATH = Path("scripts/fetch_round176_london_ldd_archive_next7_candidates.py")
...
def load_round176_module():
    spec = importlib.util.spec_from_file_location("round176_ldd_mixed_base", ROUND176_SCRIPT_PATH)
    if not spec or not spec.loader:
        raise RuntimeError(f"Unable to load {ROUND176_SCRIPT_PATH}")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


round176 = load_round176_module()
original_rewrite_candidate = round176.rewrite_candidate
original_source_audit = round176.source_audit
...
ROUND_REPLACEMENTS = {
    ...
    "Round 176 London LDD archive next7 candidates": "Round 285 London LDD archive next23 candidates",
    "after round170": "after round277",
    "through round170": "through round277",
    "same fields inside the round176 batch scan": "same fields inside the round285 batch scan",
    "duplicate inside round176 batch scan": "duplicate inside round285 batch scan",
...
def configure_round285_globals():
    ...
    round176.base.title_date_key = strict_title_date_key


def strict_ldd_title_token(title):
    base = base_module()
    text = base.clean_text(title).lower()
    text = re.sub(
        r"^ldd (?:administrative )?(?:permission|work-start|completion(?:-status)?) record:\s*",
        "",
        text,
    )
...
def strict_title_date_key(city_id, title, event_date):
    base = base_module()
    return (base.clean_text(city_id).lower(), strict_ldd_title_token(title), base.clean_text(event_date))
...
def prior_ldd_pack_paths():
    ...
    for path in sorted(Path("tmp/subagents").glob("round*_london_ldd_archive*/candidates.json")):
        ...
        if not match or int(match.group(1)) > 277:
            continue
        paths.append(path)
    return paths
...
def rewrite_candidate(candidate, row, excel_row, milestone, milestone_date):
    ...
    event_id = (
        f"lon_ldd_archive_next23_round285_{milestone}_row_{excel_row}_"
    ...
    candidate["transformation_method"] = (
        ...
        "candidate packs through round277 by LDD workbook row, planning authority/borough reference, source "
...
def write_notes(candidate_count, summary):
    ...
    text = f"""# Round 285 London LDD archive next23 candidates
...
- Validation report: `{VALIDATION_PATH.as_posix()}`
...
def title_date_key(candidate):
    ...
    title = re.sub(r"^ldd administrative (?:permission|work-start|completion-status) record:\s*", "", title)
    date_value = clean_text(candidate.get("date") or candidate.get("effective_date"))
...
def validate_generated_pack():
    ...
    if not (200 <= len(candidates) <= 240):
        issues.append(f"candidate count {len(candidates)} is outside target range 200-240")
    ...
    for index, candidate in enumerate(candidates):
        ...
        if candidate.get("license") != LICENSE:
            issues.append(f"{label}: candidate license is not OGL v3")
    ...
    if set(accessed_at_values) != {ACCESSED_AT}:
        issues.append(f"accessed_at values are not all {ACCESSED_AT}: {dict(accessed_at_values)}")
    if not milestone_mix:
        issues.append("milestone mix is empty")
    if set(milestone_mix) == {"permission"}:
        warnings.append("No commencement/started or completion rows remained after dedupe through round277 and signal filters.")
    ...
    validation = {
        ...
        "prior_ldd_packs_checked_through_round277": len(prior_paths),
...
def postprocess_outputs(validation=None):
    for path in [OUT_PATH, SOURCE_AUDIT_PATH, SUMMARY_PATH, REJECTED_PATH]:
        ...
        if path == SUMMARY_PATH:
            ...
            if validation_file not in output_files:
...
def main():
    ...
    if not validation["ok"]:
        raise RuntimeError(f"Round285 validation failed: {validation['issues']}")
