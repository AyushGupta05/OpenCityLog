...
if (sourceHash !== EXPECTED_SOURCE_SHA256) {
  throw new Error(
    `Round532 expected Round509 worker source hash ${EXPECTED_SOURCE_SHA256}, got ${sourceHash}. ` +
      "Review the upstream pattern before regenerating this candidate pack."
  );
}

const round532Source = source
  .replace("const ROUND = 509;", "const ROUND = 532;")
  .replace(
    'const SLUG = "round509_nyc_dob_legacy_permit_next6";',
    'const SLUG = "round532_nyc_dob_legacy_permit_next10";'
  )
  .replace("const DEDUPE_BOUNDARY_ROUND = 506;", "const DEDUPE_BOUNDARY_ROUND = 521;")
  .replaceAll(
    "fetch_round509_nyc_dob_legacy_permit_next6_candidates.js",
    "fetch_round532_nyc_dob_legacy_permit_next10_candidates.js"
  )
  .replaceAll("round509_nyc_dob_legacy_permit_next6", "round532_nyc_dob_legacy_permit_next10")
  .replaceAll("Round509", "Round532")
  .replaceAll("Round506", "Round521")
  .replaceAll(
...
vm.runInNewContext(round532Source, {
  __dirname,
  __filename,
  require,
  console,
