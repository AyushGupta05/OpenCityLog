...
TASK_ID = "round176_london_ldd_archive_next7_candidates"
SCRIPT_PATH = "scripts/fetch_round176_london_ldd_archive_next7_candidates.py"
OUT_DIR = Path("tmp/subagents/round176_london_ldd_archive_next7")
...
def prior_ldd_pack_paths():
    ...
    for path in sorted(Path("tmp/subagents").glob("round*_london_ldd_archive*/candidates.json")):
        ...
        if not match or int(match.group(1)) > 170:
            continue
        paths.append(path)
    return paths
...
def corpus_event_ids():
    ...
    payload = json.loads(corpus_path.read_text(encoding="utf-8-sig"))
    for event in payload.get("events", []):
        event_id = base.clean_text(event.get("event_id") or event.get("id"))
        if event_id:
            event_ids.add(event_id)
    return event_ids


def prior_event_ids(paths):
    event_ids = set()
    for path in paths:
        if not path.exists():
            continue
        payload = json.loads(path.read_text(encoding="utf-8-sig"))
        for candidate in payload.get("candidates", []):
            event_id = base.clean_text(candidate.get("event_id") or candidate.get("candidate_id"))
            if event_id:
                event_ids.add(event_id)
    return event_ids
...
def rewrite_candidate(candidate, row, excel_row, milestone, milestone_date):
    ...
    event_id = (
        f"lon_ldd_archive_next7_round176_{milestone}_row_{excel_row}_"
...
def write_notes(candidate_count, summary):
    text = f"""# Round 176 London LDD archive next7 candidates

Generated the next bounded London Development Database archive candidate pack after round170. This pack expands beyond exhausted strict completion rows by selecting source-defined completion, work-start, or permission milestone records, while deduplicating against the manual corpus and previous LDD archive packs through round170.
...
The generator excludes manual-corpus LDD records and previous LDD archive candidate packs through round170 by workbook row, planning authority plus borough reference, source URL plus source record id, candidate title/date, and source/date key. It also removes duplicate event IDs and source/date keys inside this batch.
...
def main():
    ...
    existing_event_ids = corpus_event_ids() | prior_event_ids(prior_paths)
    ...
    for excel_row, row in base.iter_xlsx_dict_rows(permissions_xlsx):
        ...
        if excel_row in dedupe["rows"]:
            ...
            reason = "duplicate inside round176 batch scan"
        elif not milestone:
    ...
    if set(event_ids) & existing_event_ids:
        raise ValueError("Generated event_id already exists in corpus or previous packs")
    ...
    selection_summary = {
        ...
        **prior_pack_summary(prior_packs, current_dedupe),
        "dedupe_fields": [
            ...
            "event_id/id against the manual corpus and previous LDD archive candidate packs through round170",
            ...
            "same fields inside the round176 batch scan",
        ],
        "filters": [
    ...
    summary = {
        ...
        "duplicate_validation": {
            "generated_event_ids_unique": len(event_ids) == len(set(event_ids)),
            "generated_source_date_keys_unique": len(batch_source_date_keys) == len(set(batch_source_date_keys)),
            "generated_event_ids_seen_in_corpus_or_prior_ldd_packs": len(set(event_ids) & existing_event_ids),
            "generated_source_date_keys_seen_in_corpus_or_prior_ldd_packs": len(set(batch_source_date_keys) & existing_source_date_keys),
        },
