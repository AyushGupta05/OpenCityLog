...
const ROUND_NAME = "round421_london_pld_lifecycle_next55";
...
const PRIOR_PROSE_AFTER_272 =
  "round272, round276, round284, round288, round298, round301, round306, round311, round316, round323, round327, round328, round333, round338, round343, round348, round350, round354, round358, round362, round365, round369, round373, round377, round380, round384, round387, round393, round398, round404, round410, and round416";
...
function replaceRequired(value, needle, replacement, label) {
  if (!value.includes(needle)) {
    ...
    throw new Error(`Round421 source transform could not find ${label}: ${needle}`);
  }
...
function assertContains(value, needle, label) {
  if (!value.includes(needle)) {
    throw new Error(`Round421 source transform failed to include ${label}: ${needle}`);
  }
}
...
function transformRound404Source(source) {
  ...
  transformed = transformed
    .replace(/round404_london_pld_lifecycle_next52/g, "round421_london_pld_lifecycle_next55")
    .replace(/round404\.london_pld_lifecycle_next52/g, "round421.london_pld_lifecycle_next55")
    .replace(/Bims5Round404PldLifecycleNext52/g, "Bims5Round421PldLifecycleNext55")
    .replace(/Round 404/g, "Round 421")
    .replace(/Round404/g, "Round421")
    .replace(/round404/g, "round421")
    .replace(/next52/g, "next55")
    .replace(/MAX_PRIOR_ROUND = 398/g, "MAX_PRIOR_ROUND = 416")
    .replace(/through round398/g, "through round416");
  ...
  transformed = replaceRequired(
    ...
    "round404, round410, and round416 prior PLD lifecycle packs"
  );
  ...
  assertContains(transformed, 'const ROUND_NAME = "round421_london_pld_lifecycle_next55";', "round name");
  assertContains(transformed, 'const ROUND = "round421";', "transformed fetch round id");
  assertContains(transformed, "MAX_PRIOR_ROUND = 416", "prior cutoff");
  assertContains(transformed, "through round416", "dedupe cutoff text");
  ...
  assertContains(transformed, 'CURRENT_ROUND_NAME = "round421_london_pld_lifecycle_next55"', "validator round name");
...
async function runTransformedRound404() {
  ...
  vm.runInNewContext(transformed, sandbox, {
    filename: "generated_round421_london_pld_lifecycle_next55_candidates.js"
  });

  if (typeof sandboxModule.exports !== "function") {
    throw new Error("Round421 transform did not export the fetch main() function.");
  }
...
function validateAndRewriteReadbackJson() {
  ...
  const ok =
    ...
    /through round416/i.test(summary.dedupe_basis || "") &&
    (summary.duplicate_index?.prior_packs || []).some(
      (pack) => pack.label === "round416_london_pld_lifecycle_next54" && pack.exists === true
    );

  if (!ok) {
    throw new Error("Readback validation failed for Round421 output pack.");
  }
  ...
  writeJson(readbackFile, {
    ...
    readback_checks: [
      ...
      "dedupe basis includes PLD lifecycle packs through round416",
      "round416 prior PLD lifecycle pack was scanned",
  ...
  if (!finalReadback.ok || !finalReadback.files?.["readback.json"]?.exists) {
    throw new Error("Final readback self-check failed for Round421 output pack.");
  }
}
