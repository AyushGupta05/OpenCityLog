...
function replaceRequired(value, needle, replacement, label) {
  if (!value.includes(needle)) {
    ...
    throw new Error(`Round393 source transform could not find ${label}: ${needle}`);
  }
...
function assertContains(value, needle, label) {
  if (!value.includes(needle)) {
    throw new Error(`Round393 source transform failed to include ${label}: ${needle}`);
  }
}

function transformRound362Script(source) {
  let transformed = source
    .replace(/round362_london_pld_lifecycle_next42/g, "round393_london_pld_lifecycle_next50")
    .replace(/round362\.london_pld_lifecycle_next42/g, "round393.london_pld_lifecycle_next50")
    .replace(/Bims5Round362PldLifecycleNext42/g, "Bims5Round393PldLifecycleNext50")
    .replace(/Round362/g, "Round393")
    .replace(/Round 362/g, "Round 393")
    .replace(/round362/g, "round393")
    .replace(/next42/g, "next50")
    .replace(/MAX_PRIOR_ROUND = 358/g, "MAX_PRIOR_ROUND = 387")
    .replace(/through round358/g, "through round387");
  ...
  transformed = replaceRequired(
    ...
    '      "round272, round276, round284, round288, round298, round301, round306, round311, round316, round323, round327, round328, round333, round338, round343, round348, round350, round354, round358, round362, round365, round369, round373, round377, round380, round384, and round387"\\n    );',
  ...
  transformed = replaceRequired(
    ...
    "prior round327 through round387 fetch assertions"
  );

  transformed = replaceRequired(
    transformed,
    '    .replace(/round276/g, "round358");',
    '    .replace(/round276/g, "round387");',
    "validator prior cutoff prose replacement"
  );

  assertContains(transformed, 'const ROUND_NAME = "round393_london_pld_lifecycle_next50";', "round name");
  assertContains(transformed, 'const ROUND = "round393";', "generated round id");
  assertContains(transformed, "MAX_PRIOR_ROUND = 387", "prior cutoff");
  ...
  assertContains(transformed, "through round387", "dedupe prose cutoff");
  return transformed;
}
...
function main() {
  ...
  vm.runInNewContext(
    ...
    {
      filename: "generated_round393_london_pld_lifecycle_next50_candidates.js"
    }
  );
}
