...
const ROUND_NAME = "round522_london_pld_lifecycle_next80";
...
function replaceRequired(value, needle, replacement, label) {
  if (!value.includes(needle)) {
    ...
    throw new Error(`Round522 source transform could not find ${label}: ${needle}`);
  }
...
function transformRound488Script(source) {
  let transformed = source
    .replace(/round488_london_pld_lifecycle_next72/g, "round522_london_pld_lifecycle_next80")
    .replace(/round488\.london_pld_lifecycle_next72/g, "round522.london_pld_lifecycle_next80")
    .replace(/Bims5Round488PldLifecycleNext72/g, "Bims5Round522PldLifecycleNext80")
    .replace(/Round 488/g, "Round 522")
    .replace(/Round488/g, "Round522")
    .replace(/round488/g, "round522")
    .replace(/next72/g, "next80")
    .replace(/MAX_PRIOR_ROUND = 485/g, "MAX_PRIOR_ROUND = 517")
    .replace(/through round485/g, "through round517")
    .replace(/packRound\(entry\.name\) > 485/g, "packRound(entry.name) > 517")
    .replace(/\.replace\(\/round276\/g, "round485"\);/g, '.replace(/round276/g, "round517");');
  ...
  transformed = replaceRequired(
    ...
    "round488, round490, round496, round500, round504, round508, round512, and round517 prior pack insertion"
  );

  transformed = transformed
    .replace(
      /pack\) => pack\.label === "round485_london_pld_lifecycle_next71" && pack\.exists === true/g,
      'pack) => pack.label === "round517_london_pld_lifecycle_next79" && pack.exists === true'
    )
    .replace(/round485 prior PLD lifecycle pack was scanned/g, "round517 prior PLD lifecycle pack was scanned");

  assertContains(transformed, 'const ROUND_NAME = "round522_london_pld_lifecycle_next80";', "round name");
  assertContains(transformed, 'const ROUND = "round522";', "transformed fetch round id");
  assertContains(transformed, "MAX_PRIOR_ROUND = 517", "prior cutoff");
  assertContains(transformed, "through round517", "dedupe cutoff text");
  ...
  assertContains(transformed, "addRound522RetrievedAtAlias", "candidate retrieved_at alias");
  assertContains(transformed, 'CURRENT_ROUND_NAME = "round522_london_pld_lifecycle_next80"', "validator round name");
  assertContains(transformed, "round522_strict_duplicate_audit", "strict duplicate audit label");
...
function addRound522SourceRowRefs() {
  ...
  pack.round522_postprocess = {
    source_row_ref_alias_count: aliasCount,
    note: "Round522 wrapper added explicit source_row_ref/source_row_refs aliases from the official PLD row/API URLs after the inherited lifecycle worker completed."
  };
  writeJson(file, pack);
}

function updateRound522Readback() {
  ...
  readback.round522_postprocess_checks = [
...
async function main() {
  ...
  vm.runInNewContext(transformed, sandbox, {
    filename: "generated_round522_london_pld_lifecycle_next80_candidates.js"
  });
  await waitForGeneratedOutputs(startedAtMs);
  addRound522SourceRowRefs();
  updateRound522Readback();
}
