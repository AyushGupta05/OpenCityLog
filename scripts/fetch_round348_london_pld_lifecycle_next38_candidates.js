...
function replaceRequired(value, needle, replacement, label) {
  if (!value.includes(needle)) {
    ...
    throw new Error(`Round348 source transform could not find ${label}: ${needle}`);
  }
...
function assertContains(value, needle, label) {
  if (!value.includes(needle)) {
    throw new Error(`Round348 source transform failed to include ${label}: ${needle}`);
  }
}

function transformRound327Script(source) {
  let transformed = source
    .replace(/round327_london_pld_lifecycle_next33/g, "round348_london_pld_lifecycle_next38")
    .replace(/round327\.london_pld_lifecycle_next33/g, "round348.london_pld_lifecycle_next38")
    .replace(/Bims5Round327PldLifecycleNext33/g, "Bims5Round348PldLifecycleNext38")
    .replace(/Round327/g, "Round348")
    .replace(/Round 327/g, "Round 348")
    .replace(/round327/g, "round348")
    .replace(/next33/g, "next38")
    .replace(/MAX_PRIOR_ROUND = 323/g, "MAX_PRIOR_ROUND = 343")
    .replace(/through round323/g, "through round343");

  transformed = replaceRequired(
    transformed,
    '      "round272, round276, round284, round288, round298, round301, round306, round311, round316, and round323"\n    );',
    '      "round272, round276, round284, round288, round298, round301, round306, round311, round316, round323, round327, round328, round333, round338, and round343"\n    );',
  ...
  transformed = replaceRequired(
    ...
    "prior round327, round328, round333, round338, and round343 fetch assertions"
  );

  transformed = replaceRequired(
    transformed,
    '    .replace(/round276/g, "round323");',
    '    .replace(/round276/g, "round343");',
    "validator prior cutoff prose replacement"
  );

  assertContains(transformed, 'const ROUND_NAME = "round348_london_pld_lifecycle_next38";', "round name");
  assertContains(transformed, 'const ROUND = "round348";', "generated round id");
  assertContains(transformed, "MAX_PRIOR_ROUND = 343", "prior cutoff");
  ...
  assertContains(transformed, "through round343", "dedupe prose cutoff");
  return transformed;
}
...
function main() {
  ...
  vm.runInNewContext(
    ...
    {
      filename: "generated_round348_london_pld_lifecycle_next38_candidates.js"
    }
  );
}
