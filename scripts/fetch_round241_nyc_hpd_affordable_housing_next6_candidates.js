...
const outDir = "tmp/subagents/round241_nyc_hpd_affordable_housing_next6";
...
function addExistingRow(index, row, file) {
  ...
  for (const recordId of [sourceRecordId, urlRecordId]) {
  ...
  if (sourceRecordId || urlRecordId || sourceUrl || date || sourceDateField) {
    const recordId = sourceRecordId || urlRecordId;
...
function appearsExisting(existing, eventId, sourceRecordId, sourceUrl, sourceDateField, date) {
  if (eventId && existing.eventIds.has(eventId)) return "event_id already appears in corpus or prior/pending HPD candidates.";
  if (sourceRecordId && (existing.sourceRecordIds.has(sourceRecordId) || existing.sourceRecordIds.has(`${DATASET.sourceId}:${sourceRecordId}`))) {
    return "HPD project/building source row already appears in corpus or prior/pending HPD candidates.";
  }
  if (sourceUrl && existing.sourceUrls.has(normalizeUrlToken(sourceUrl))) return "HPD Socrata row URL already appears in corpus or prior/pending HPD candidates.";
  ...
  if (existing.sourceDateKeys.has(key)) return "HPD source/date key already appears in corpus or prior/pending HPD candidates.";
  return "";
}
...
function buildCandidate(row, existing, rejected) {
  ...
  return {
    ...
    raw_row: row,
    transformation_method: `${roundName} official NYC Open Data HPD fetch from ${DATASET.id}; selected from rows with project/building identifiers, a source date, source coordinates, unit counts, and duplicate screening against the live manual corpus plus prior/pending HPD candidate packs including round229, round234, and round236.`
  };
}
...
function summarizeCandidates(candidates) {
  ...
  for (const candidate of candidates) {
    ...
    unitCounts.push(Math.max(candidate.units.total_units || 0, candidate.units.all_counted_units || 0));
  ...
  return {
    ...
    total_or_counted_units: {
      ...
      median: unitCounts.length ? unitCounts[Math.floor(unitCounts.length / 2)] : 0,
...
function sourceAudit(metadata, acceptedCount, selectedSummary) {
  ...
  return {
    schema_version: "round241.nyc_hpd_affordable_housing_next6_source_audit.v1",
    ...
    sources: [
      {
        ...
        published_coverage_note: "Round241 checked HPD building-level rows with coordinates and building/project completion or project start dates available through the access date.",
...
function validateGeneratedPack(candidatesPayload, sourceAuditPayload, summaryPayload, existing) {
  ...
  return {
    ...
    checks: [
      ...
      "no overlap with live corpus or prior/pending HPD candidate packs including round229, round234, and round236"
    ],
...
async function main() {
  ...
  const summaryPayload = {
    schema_version: "round241.nyc_hpd_affordable_housing_next6_summary.v1",
    generated_at: generatedAt,
    accessed_at: accessedAt,
    output_files: [
  ...
  const candidatesPayload = {
    schema_version: "round241.nyc_hpd_affordable_housing_next6_candidates.v1",
    generated_at: generatedAt,
    accessed_at: accessedAt,
  ...
  const rejectedPayload = {
    schema_version: "round241.nyc_hpd_affordable_housing_next6_rejected.v1",
    generated_at: generatedAt,
    accessed_at: accessedAt,
    source_id: DATASET.sourceId,
    dataset_id: DATASET.id,
  ...
  const notes = [
    "# Round241 NYC HPD Affordable Housing Production Candidate Pack",
    "",
    ...
    "- Duplicate screening used the live manual corpus and prior/pending HPD candidate packs including round229, round234, and round236; this script did not edit the corpus, appender, or generated atlas files."
  ].join("\n");
