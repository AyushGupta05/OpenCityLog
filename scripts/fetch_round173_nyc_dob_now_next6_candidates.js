...
const SCRIPT_PATH = path.join("scripts", "fetch_round173_nyc_dob_now_next6_candidates.js");

function replaceRequired(source, needle, replacement) {
  if (!source.includes(needle)) {
    throw new Error(`Round173 template patch failed; missing expected text: ${needle}`);
  }
...
source = replaceRequired(
  ...
  'const OUT_DIR = "tmp/subagents/round173_nyc_dob_now_next6";'
);

source = source
  .replace(/round149/g, "round173")
  .replace(/Round149/g, "Round173")
  .replace(/nyc_dob_now_next(?!6)/g, "nyc_dob_now_next6")
  .replace(/NYC DOB NOW Next/g, "NYC DOB NOW Next6")
  .replace(/NYC DOB NOW next/g, "NYC DOB NOW next6")
  .replace(
    /round133, round136, and round143/g,
    "round133, round136, round143, round149, round152, round158, round162, and round167"
  )
  .replace(
