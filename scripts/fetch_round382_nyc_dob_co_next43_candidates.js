...
const GENERATED_FILENAME = "scripts/fetch_round382_nyc_dob_co_next43_candidates.generated.js";
const ROUND = "round382";
const ROUND_TITLE = "Round382";
const NEXT_LABEL = "next43";
const NEXT_TITLE = "Next43";
...
function transformRound308Source(source) {
  let transformed = source.replace(/\r\n/g, "\n")
    ...
    .replaceAll("through round303", "through round379");
  ...
  transformed = replaceFirst(
    ...
    `${round303File}\n];`,
    `${priorRoundFiles}\n];`,
    "explicit duplicate files through Round379"
  );
  transformed = replaceOnce(
    transformed,
    `${round303File}\n];`,
    `${priorRoundFiles}\n];`,
    "required screened files through Round379"
  );
  ...
  transformed = replaceOnce(
    ...
    "screened rounds include Round308 through Round379"
  );
  ...
  transformed = replaceOnce(
    ...
    "validation check fields through Round379"
  );
  ...
  transformed = replaceOnce(
    ...
    "validation report lines through Round379"
  );
  ...
  assertContains(transformed, "round382_nyc_dob_co_next43", "Round382 output path");
  assertContains(transformed, "Round382 NYC DOB CO Next43 Candidate Pack", "Round382 notes title");
  assertContains(transformed, "nyc_dob_co_round382_legacy_", "Round382 candidate ID prefix");
  assertContains(transformed, "tmp/subagents/round379_nyc_dob_co_next42/candidates.json", "Round379 screened file");
  assertContains(transformed, "through round379", "duplicate-screening text through Round379");
  assertContains(transformed, "required_round379_screened", "Round379 validation result");
  ...
  assertContains(transformed, `transformation_method: "${ROUND_TITLE} queried`, "Round382 source audit transformation method");
  assertContains(transformed, "scripts/fetch_round382_nyc_dob_co_next43_candidates.js standalone validation artifact", "Round382 validator label");
...
async function main() {
  ...
  if (typeof sandbox.module.exports.main !== "function") {
    throw new Error("Transformed Round382 generator did not export main().");
  }
