...
const OUT_DIR = path.join(ROOT, "tmp", "subagents", "round159_nyc_lpc_permit_next");
...
async function fetchJson(url) {
  const response = await fetch(url, {
    headers: { "User-Agent": "Bims-5 round159 NYC LPC permit candidate fetcher" }
  });
  if (!response.ok) {
...
function candidateFrom(row) {
  ...
  const eventId = `nyc-lpc-permit-round159-${slugify(regulationNumber, 48)}`;
  ...
  return {
    ...
    transformation_method: `scripts/fetch_round159_nyc_lpc_permit_next_candidates.js queried official NYC Open Data dpm2-m9mq rows with issue_date from ${START_DATE} through ${END_DATE}, required official latitude/longitude and regulation_number, scored architecture-facing WorkTypes/regulation_type values, deduplicated against the live manual corpus plus prior LPC permit/designation packs and any existing round159 output, and selected up to ${MAX_CANDIDATES} high-signal administrative milestones without inferring physical work.`,
...
function buildSourceAudit(metadata, queryUrl, sourceStats, existing) {
  ...
  return {
    ...
    audit_scope: "Official NYC LPC Permit Application Information source audit for round159 high-signal permit/application candidate extraction.",
    sources: [
      {
        source_id: SOURCE_ID,
        source_dataset_id: SOURCE_DATASET_ID,
...
function markdownNotes(summary) {
  return [
    "# Round159 NYC LPC Permit Application Information Next Candidates",
    "",
...
async function main() {
  ...
  const summary = {
    generated_at: `${ACCESSED_AT}T00:00:00Z`,
    task: "Round159 NYC LPC Permit Application Information next candidate pack",
    source_id: SOURCE_ID,
    source_dataset_id: SOURCE_DATASET_ID,
