...
function replaceRequired(value, needle, replacement, label) {
  if (!value.includes(needle)) {
    ...
    throw new Error(`Round485 source transform could not find ${label}: ${needle}`);
  }
...
function transformRound467Script(source) {
  let transformed = source
    .replace(/round467_london_pld_lifecycle_next66/g, "round485_london_pld_lifecycle_next71")
    .replace(/round467\.london_pld_lifecycle_next66/g, "round485.london_pld_lifecycle_next71")
    .replace(/Bims5Round467PldLifecycleNext66/g, "Bims5Round485PldLifecycleNext71")
    .replace(/Round 467/g, "Round 485")
    .replace(/Round467/g, "Round485")
    .replace(/round467/g, "round485")
    .replace(/next66/g, "next71")
    .replace(/MAX_PRIOR_ROUND = 460/g, "MAX_PRIOR_ROUND = 481")
    .replace(/through round460/g, "through round481")
    .replace(/packRound\(entry\.name\) > 460/g, "packRound(entry.name) > 481")
    .replace(/\.replace\(\/round276\/g, "round460"\);/g, '.replace(/round276/g, "round481");');
  ...
  transformed = replaceRequired(
    ...
    "round467, round469, round474, round478, and round481 prior pack insertion"
  );

  transformed = transformed
    .replace(
      /pack\) => pack\.label === "round460_london_pld_lifecycle_next65" && pack\.exists === true/g,
      'pack) => pack.label === "round481_london_pld_lifecycle_next70" && pack.exists === true'
    )
    .replace(/round460 prior PLD lifecycle pack was scanned/g, "round481 prior PLD lifecycle pack was scanned");

  assertContains(transformed, 'const ROUND_NAME = "round485_london_pld_lifecycle_next71";', "round name");
  assertContains(transformed, 'const ROUND = "round485";', "transformed fetch round id");
  assertContains(transformed, "MAX_PRIOR_ROUND = 481", "prior cutoff");
  assertContains(transformed, "through round481", "dedupe cutoff text");
  ...
  assertContains(transformed, "addRound485RetrievedAtAlias", "candidate retrieved_at alias");
  assertContains(transformed, 'CURRENT_ROUND_NAME = "round485_london_pld_lifecycle_next71"', "validator round name");
  assertContains(transformed, "round485_strict_duplicate_audit", "strict duplicate audit label");
...
async function main() {
  ...
  vm.runInNewContext(transformed, sandbox, {
    filename: "generated_round485_london_pld_lifecycle_next71_candidates.js"
  });
}
