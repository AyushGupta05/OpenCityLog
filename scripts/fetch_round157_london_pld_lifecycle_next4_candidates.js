...
const OUT_DIR = path.join(ROOT, "tmp", "subagents", "round157_london_pld_lifecycle_next4");
...
const PRIOR_PACKS = [
  ...
  {
    label: "round153_london_pld_lifecycle_next3",
    file: path.join(ROOT, "tmp", "subagents", "round153_london_pld_lifecycle_next3", "candidates.json")
  },
  {
    label: "round157_london_pld_lifecycle_next4_existing_output",
...
function buildDuplicateIndex() {
  ...
  for (const event of corpus.events || []) {
    ...
    if (event.source_url || event.source_record_id) {
      const field = normalizeLifecycleField(event.source_lifecycle_field || event.source_date_field);
      index.sourceKeys.add(
        `london|${event.source_url || ""}|${event.source_record_id || ""}|${field}|${event.source_lifecycle_date || date}`.toLowerCase()
      );
    }
    const text = [
      ...
      .filter(Boolean)
      .join(" ");
    for (const id of collectPldIdsFromText(text)) {
      index.manualPldIds.add(id);
      const fieldKey = sourceFieldDateKey(
        id,
        event.source_lifecycle_field || event.source_date_field,
        event.source_lifecycle_date || date
      );
  ...
  for (const prior of PRIOR_PACKS) {
    ...
    const fieldKeysForPack = new Set();
    for (const candidate of rows) {
      ...
      for (const id of ids) {
        idsForPack.add(id);
        const field = normalizeLifecycleField(candidate.source_lifecycle_field || candidate.source_date_field);
        const date = dateFromCandidate(candidate);
        const fieldKey = sourceFieldDateKey(id, field, date);
        if (fieldKey) {
          fieldKeysForPack.add(fieldKey);
      ...
      if (candidate.source_url || candidate.source_record_id) {
        const field = normalizeLifecycleField(candidate.source_lifecycle_field || candidate.source_date_field);
        index.sourceKeys.add(
          `london|${candidate.source_url || ""}|${candidate.source_record_id || ""}|${field}|${date}`.toLowerCase()
        );
      }
    }
    index.priorPldIdsByPack.set(prior.label, idsForPack);
    index.priorFieldKeysByPack.set(prior.label, fieldKeysForPack);
    index.counts.prior_packs.push({
      ...
      field_date_keys: fieldKeysForPack.size
    });
  }
  ...
  for (let page = 0; page < maxPages; page += 1) {
    ...
    const response = await fetch(API_ENDPOINT, {
      ...
      headers: {
        ...
        "user-agent": "Bims5Round157PldLifecycleNext4/0.1"
      },
...
function candidateFor(item) {
  ...
  const candidateId = `round157_lon_pld_${spec.kind}_${slugify(rowId)}_${date.replace(/-/g, "_")}_${hash8(
  ...
  return {
    ...
    duplicate_check_note:
      "Screened against the live manual architecture corpus, prior London PLD lifecycle packs through round153, and any existing round157 output by PLD row id, source-date-field/date, source URL/source_record/date-field/date, and title/date.",
    limitations: [
...
function rejectionReason(item, duplicateIndex, batchKeys) {
  ...
  if (batchKeys.has(fieldKey)) return "duplicate_row_date_field_inside_round157_batch";
  if (
...
async function main() {
  ...
  const candidatesPack = {
    schema_version: "round157.london_pld_lifecycle_next4.candidates.v1",
    generated_at: GENERATED_AT,
    accessed_at: ACCESSED_AT,
    task:
      "Round157 London Planning Datahub actual lifecycle candidate pack beyond current manual corpus and prior PLD lifecycle packs through round153",
  ...
  const sourceAudit = {
    generated_at: GENERATED_AT,
    accessed_at: ACCESSED_AT,
    task: "round157_london_pld_lifecycle_next4",
    source_audits: [
      {
        source_id: "gla-planning-datahub-applications",
        source_name: "Planning London Datahub applications",
  ...
  const summary = {
    ...
    dedupe_basis:
      "Live data/manual_drops/architecture_milestones/architecture_milestones_2008_2026.json plus round126_london_pld_lifecycle_direct, round126_london_pld_lifecycle_more, round140_london_pld_lifecycle_more, round147_london_pld_lifecycle_next, round150_london_pld_lifecycle_next2, round153_london_pld_lifecycle_next3, and any existing round157_london_pld_lifecycle_next4 candidates were scanned for PLD ids, source-record/source-url/date-field/date keys, source-date-field/date keys, and title/date keys.",
    ...
    caveats: [
      ...
      "Exact source row/date-field/date duplicates already present in the live manual corpus, prior PLD lifecycle packs through round153, or any existing round157 output were excluded.",
      "London Datastore licence is Not Specified for this source, so the pack retains factual metadata and attribution only."
    ]
  };

  const rejectedPack = {
    schema_version: "round157.london_pld_lifecycle_next4.rejected.v1",
    generated_at: GENERATED_AT,
    accessed_at: ACCESSED_AT,
  ...
  const notes = [
    "# Round 157 London PLD lifecycle next4 candidates",
    "",
    ...
    "",
    "Deduplication scanned the live manual architecture corpus plus the prior round126 direct, round126 more, round140, round147, round150, and round153 London PLD lifecycle scratch packs, and any existing round157 output by PLD row id, source URL/source record/date-field/date, source date field/date, and title/date.",
    "",
