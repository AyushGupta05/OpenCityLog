...
const ROUND_NAME = "round428_london_pld_lifecycle_next57";
...
function replaceRequired(value, needle, replacement, label) {
  if (!value.includes(needle)) {
    ...
    throw new Error(`Round428 source transform could not find ${label}: ${needle}`);
  }
...
function assertContains(value, needle, label) {
  if (!value.includes(needle)) {
    throw new Error(`Round428 source transform failed to include ${label}: ${needle}`);
  }
}
...
function transformRound404Source(source) {
  ...
  transformed = transformed
    .replace(/round404_london_pld_lifecycle_next52/g, "round428_london_pld_lifecycle_next57")
    .replace(/round404\.london_pld_lifecycle_next52/g, "round428.london_pld_lifecycle_next57")
    .replace(/Bims5Round404PldLifecycleNext52/g, "Bims5Round428PldLifecycleNext57")
    .replace(/Round 404/g, "Round 428")
    .replace(/Round404/g, "Round428")
    .replace(/round404/g, "round428")
    .replace(/next52/g, "next57")
    .replace(/MAX_PRIOR_ROUND = 398/g, "MAX_PRIOR_ROUND = 425")
    .replace(/through round398/g, "through round425");
  ...
  transformed = replaceRequired(
    ...
    "round404, round410, round416, round421, and round425 prior PLD lifecycle packs"
  );
  ...
  assertContains(transformed, 'const ROUND_NAME = "round428_london_pld_lifecycle_next57";', "round name");
  assertContains(transformed, 'const ROUND = "round428";', "transformed fetch round id");
  assertContains(transformed, "MAX_PRIOR_ROUND = 425", "prior cutoff");
  assertContains(transformed, "through round425", "dedupe cutoff text");
  ...
  assertContains(transformed, 'CURRENT_ROUND_NAME = "round428_london_pld_lifecycle_next57"', "validator round name");
...
async function runTransformedRound404() {
  ...
  vm.runInNewContext(transformed, sandbox, {
    filename: "generated_round428_london_pld_lifecycle_next57_candidates.js"
  });

  if (typeof sandboxModule.exports !== "function") {
    throw new Error("Round428 transform did not export the fetch main() function.");
  }
...
function addRound428RequestedAliases() {
...
function validateAndRewriteReadbackJson() {
  ...
  const ok =
    ...
    /through round425/i.test(summary.dedupe_basis || "") &&
    (summary.duplicate_index?.prior_packs || []).some(
      (pack) => pack.label === "round425_london_pld_lifecycle_next56" && pack.exists === true
    );

  if (!ok) {
    throw new Error("Readback validation failed for Round428 output pack.");
  }
  ...
  writeJson(readbackFile, {
    ...
    readback_checks: [
      ...
      "candidate aliases include method, planning_reference, status, decision, and lifecycle_milestone",
      "dedupe basis includes PLD lifecycle packs through round425",
      "round425 prior PLD lifecycle pack was scanned",
  ...
  if (!finalReadback.ok || !finalReadback.files?.["readback.json"]?.exists) {
    throw new Error("Final readback self-check failed for Round428 output pack.");
  }
}
