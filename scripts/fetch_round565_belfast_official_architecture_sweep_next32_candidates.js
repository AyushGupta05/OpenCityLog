...
const ROUND_ID = "round565_belfast_official_architecture_sweep_next32";
const OUT_DIR = path.join("tmp", "subagents", ROUND_ID);
const SCRIPT_PATH =
  "scripts/fetch_round565_belfast_official_architecture_sweep_next32_candidates.js";
...
const DEDUPE_BOUNDARY_ROUND = 560;
...
const METHOD = [
  "Round565 official Belfast architecture sweep next32 after the completed Round560 next30 pack.",
  ...
  "Round560 is treated as the latest Belfast official architecture sweep dedupe boundary; official page-only, geometry-ref-only, duplicate-project, status-only, signage-only, equipment-only and lower-priority private-use leads are retained separately."
].join(" ");

const TRANSFORMATION_METHOD = `${SCRIPT_PATH}#round565OfficialArchitectureSweepNext32`;

const SOURCES = {
  dfiPlanningStats: {
    source_id: "dfi-planning-statistics-2024-25-round565",
...
const SEARCH_QUERIES_CHECKED = [
  "local DfI planning-statistics 2024-25 Belfast residual APP_ID scan after Round560 accepted-pack dedupe",
  "local duplicate scan: data/manual_drops/architecture_milestones plus official Belfast architecture sweep candidate packs through Round560",
...
function replaceRound565LeadBlocks() {
  const lead = (
    app_id,
    related_app_ids,
    event_id,
  ...
  const selectedLeads = [
    { app_id: "LA04/2023/2694/F" },
    { app_id: "LA04/2024/0168/F" },
    { app_id: "LA04/2024/0357/F" },
    { app_id: "LA04/2024/0406/F" },
    { app_id: "LA04/2024/0107/F" },
    { app_id: "LA04/2024/0684/F" },
    { app_id: "LA04/2024/0772/F" },
    { app_id: "LA04/2023/3316/F" },
    { app_id: "LA04/2024/0494/F" },
    { app_id: "LA04/2023/3305/O" },
    { app_id: "LA04/2024/0395/F" },
    { app_id: "LA04/2024/0942/F" },
    { app_id: "LA04/2024/0873/F" },
    { app_id: "LA04/2023/4474/F" },
    { app_id: "LA04/2024/0819/F" },
    { app_id: "LA04/2023/2496/F" },
    { app_id: "LA04/2024/0259/F" },
    { app_id: "LA04/2023/4557/F" },
    { app_id: "LA04/2024/0776/F" },
    { app_id: "LA04/2023/4495/F" },
    { app_id: "LA04/2024/0525/F" },
    { app_id: "LA04/2024/1129/F" },
    { app_id: "LA04/2024/0669/F" },
    { app_id: "LA04/2024/0725/F" },
    { app_id: "LA04/2024/0042/O" },
    { app_id: "LA04/2024/0884/F" },
    { app_id: "LA04/2024/1061/F" },
    { app_id: "LA04/2024/0730/F" },
    { app_id: "LA04/2023/4412/RM" },
    { app_id: "LA04/2024/0607/F", related_app_ids: ["LA04/2024/0879/DCA"] }
  ];
  ...
  PLANNING_LEADS.splice(
    ...
    ...selectedLeads.map((selected) => {
      ...
      if (!row) throw new Error(`Selected Round565 app ID missing source row: ${selected.app_id}`);
      ...
      const eventId = `bfs_arch_round565_${slug(site)}_${appKey}_approval_2024`;
...
replaceRound565LeadBlocks();

REJECTED_LEADS.splice(
  0,
  REJECTED_LEADS.length,
  {
    id: "bfs_arch_round565_reject_duplicate_project_rows_through_round560",
    title: "Residual duplicate-project planning rows through the Round560 boundary",
    rejection_category: "duplicate_project_boundary",
    reason:
      "Official point-backed DfI rows for already-covered project families or same-site milestones were retained outside the promoted pack because the manual architecture corpus or official Belfast architecture sweep candidate records through Round560 already cover the same project family.",
    source_record_id:
  ...
  {
    id: "bfs_arch_round565_reject_short_let_and_hmo_only_tail",
  ...
  {
    id: "bfs_arch_round565_reject_page_only_or_no_point_official_leads",
  ...
  {
    id: "bfs_arch_round565_reject_signage_equipment_transport_rows",
...
function candidateFromPlanningLead(lead, rowsById, duplicateHaystack) {
  ...
  return {
    ...
    duplicate_review:
      `APP_ID, related APP_IDs, event_id and source_record_id were absent from the manual architecture corpus and official Belfast architecture sweep candidate outputs through Round${DEDUPE_BOUNDARY_ROUND} checked by the Round565 duplicate scan.`,
...
function rejectedRecord(lead, duplicateHaystack) {
  return {
    ...
    reason: lead.reason,
    source_id: lead.id.replace(/^bfs_arch_round565_reject_/, "round565-reject-"),
...
function validate(candidates, rejected, duplicateHaystack) {
  ...
  if (candidates.length !== EXPECTED_CANDIDATE_COUNT) {
    errors.push(
      `Expected ${EXPECTED_CANDIDATE_COUNT} promoted next32 candidates, found ${candidates.length}`
    );
  }
...
function main() {
  ...
  const sourceAudit = {
    ...
    sources_checked: [
      ...
      {
        source_id: "dfi-planning-statistics-2025-26-quarterly-reviewed-round565",
      ...
      {
        source_id: "hed-historic-buildings-and-harni-reviewed-round565",
        ...
        accepted_records: 0,
        disposition:
          "reviewed; Historic Buildings layer lacks a usable event date for new dated listing/designation changes, while Heritage at Risk Date_Added rows are retained as status/admin leads outside this DfI residual next32 pack"
      },
      ...rejected.map((item) => ({
