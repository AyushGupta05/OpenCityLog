...
source = source
  .replace(/const ROUND = 471;/g, "const ROUND = 486;")
  .replace(/round471_nyc_lpc_permit_next66/g, "round486_nyc_lpc_permit_next70")
  .replace(/round471/g, "round486")
  .replace(/Round471/g, "Round486")
  .replace(/next66/g, "next70")
  .replace(/Next66/g, "Next70")
  .replace(/through round466/g, "through round482");
...
function writeRound486DuplicateAudit() {
  if (duplicateAuditWritten || process.exitCode) return;
  duplicateAuditWritten = true;

  const root = path.resolve(__dirname, "..");
  const outDir = path.join(root, "tmp", "subagents", "round486_nyc_lpc_permit_next70");
  ...
  const audit = {
    ...
    audit_scope: "Round486 duplicate audit for NYC LPC Permit Application Information next70 candidates.",
    dedupe_boundary: {
      latest_lpc_permit_pack: "tmp/subagents/round482_nyc_lpc_permit_next69/candidates.json",
...
process.on("beforeExit", writeRound486DuplicateAudit);
