...
const ROUND_ID = "round569_belfast_official_architecture_sweep_next33";
const OUT_DIR = path.join("tmp", "subagents", ROUND_ID);
const SCRIPT_PATH =
  "scripts/fetch_round569_belfast_official_architecture_sweep_next33_candidates.js";
...
const DEDUPE_BOUNDARY_ROUND = 565;
...
const METHOD = [
  "Round569 official Belfast architecture sweep next33 after the completed Round565 next32 pack.",
  ...
  "Round565 is treated as the latest Belfast official architecture sweep dedupe boundary; official page-only, geometry-ref-only, duplicate-project, status-only, signage-only, equipment-only and lower-priority private-use leads are retained separately."
].join(" ");

const TRANSFORMATION_METHOD = `${SCRIPT_PATH}#round569OfficialArchitectureSweepNext33`;

const SOURCES = {
  dfiPlanningStats: {
    source_id: "dfi-planning-statistics-2024-25-round569",
...
const SEARCH_QUERIES_CHECKED = [
  "local DfI planning-statistics 2024-25 Belfast residual APP_ID scan after Round565 accepted-pack dedupe",
  "local duplicate scan: data/manual_drops/architecture_milestones plus official Belfast architecture sweep candidate packs through Round565",
...
function replaceRound569LeadBlocks() {
  const lead = (
    app_id,
    related_app_ids,
    event_id,
  ...
  const selectedLeads = [
    { app_id: "LA04/2024/0436/F" },
    { app_id: "LA04/2024/0995/F" },
    { app_id: "LA04/2022/1425/F" },
    { app_id: "LA04/2023/4446/F" },
    { app_id: "LA04/2024/0964/F" },
    { app_id: "LA04/2024/1194/F" },
    { app_id: "LA04/2022/0777/O" },
    { app_id: "LA04/2024/1107/F" },
    { app_id: "LA04/2024/1007/F" },
    { app_id: "LA04/2024/1075/DCA" },
    { app_id: "LA04/2024/1402/DCA" },
    { app_id: "LA04/2023/2831/F", related_app_ids: ["LA04/2023/2787/LBC"] },
    { app_id: "LA04/2024/0390/O" },
    { app_id: "LA04/2024/1276/LBC" },
    { app_id: "LA04/2024/0989/DCA" },
    { app_id: "LA04/2024/1142/F" },
    { app_id: "LA04/2024/1685/F", related_app_ids: ["LA04/2024/1168/F"] },
    { app_id: "LA04/2024/1187/F" },
    { app_id: "LA04/2023/4603/O" },
    { app_id: "LA04/2024/1453/F", related_app_ids: ["LA04/2024/1493/DCA"] },
    { app_id: "LA04/2024/0805/F" },
    { app_id: "LA04/2024/1829/DCA" },
    { app_id: "LA04/2024/0649/F" },
    { app_id: "LA04/2024/0312/F" },
    { app_id: "LA04/2024/1189/F" },
    { app_id: "LA04/2024/1468/F" },
    { app_id: "LA04/2024/0521/RM" },
    { app_id: "LA04/2024/1432/F" },
    { app_id: "LA04/2024/1519/F" },
    { app_id: "LA04/2024/1831/F", related_app_ids: ["LA04/2024/1832/DCA"] }
  ];
  ...
  PLANNING_LEADS.splice(
    ...
    ...selectedLeads.map((selected) => {
      ...
      if (!row) throw new Error(`Selected Round569 app ID missing source row: ${selected.app_id}`);
      ...
      const eventId = `bfs_arch_round569_${slug(site)}_${appKey}_approval_2024`;
...
replaceRound569LeadBlocks();

REJECTED_LEADS.splice(
  0,
  REJECTED_LEADS.length,
  {
    id: "bfs_arch_round569_reject_duplicate_project_rows_through_round565",
    title: "Residual duplicate-project planning rows through the Round565 boundary",
    rejection_category: "duplicate_project_boundary",
    reason:
      "Official point-backed DfI rows for already-covered project families or same-site milestones were retained outside the promoted pack because the manual architecture corpus or official Belfast architecture sweep candidate records through Round565 already cover the same project family.",
    source_record_id:
  ...
  {
    id: "bfs_arch_round569_reject_short_let_and_hmo_only_tail",
  ...
  {
    id: "bfs_arch_round569_reject_page_only_or_no_point_official_leads",
  ...
  {
    id: "bfs_arch_round569_reject_signage_equipment_transport_rows",
...
function candidateFromPlanningLead(lead, rowsById, duplicateHaystack) {
  ...
  return {
    ...
    duplicate_review:
      `APP_ID, related APP_IDs, event_id and source_record_id were absent from the manual architecture corpus and official Belfast architecture sweep candidate outputs through Round${DEDUPE_BOUNDARY_ROUND} checked by the Round569 duplicate scan.`,
...
function rejectedRecord(lead, duplicateHaystack) {
  return {
    ...
    reason: lead.reason,
    source_id: lead.id.replace(/^bfs_arch_round569_reject_/, "round569-reject-"),
...
function validate(candidates, rejected, duplicateHaystack) {
  ...
  if (candidates.length !== EXPECTED_CANDIDATE_COUNT) {
    errors.push(
      `Expected ${EXPECTED_CANDIDATE_COUNT} promoted next33 candidates, found ${candidates.length}`
    );
  }
...
function main() {
  ...
  const sourceAudit = {
    ...
    sources_checked: [
      ...
      {
        source_id: "dfi-planning-statistics-2025-26-quarterly-reviewed-round569",
      ...
      {
        source_id: "hed-historic-buildings-and-harni-reviewed-round569",
        ...
        accepted_records: 0,
        disposition:
          "reviewed; Historic Buildings layer lacks a usable event date for new dated listing/designation changes, while Heritage at Risk Date_Added rows are retained as status/admin leads outside this DfI residual next33 pack"
      },
      ...rejected.map((item) => ({
