...
TASK_ID = "round226_london_ldd_archive_next15_candidates"
SCRIPT_PATH = "scripts/fetch_round226_london_ldd_archive_next15_candidates.py"
OUT_DIR = Path("tmp/subagents/round226_london_ldd_archive_next15")
...
ROUND_REPLACEMENTS = {
    ...
    "through round182": "through round220",
    "through round188": "through round220",
    "through round194": "through round220",
    "through round200": "through round220",
    "through round206": "through round220",
    "through round212": "through round220",
    "after round182": "after round220",
    "after round188": "after round220",
    "after round194": "after round220",
    "after round200": "after round220",
    "after round206": "after round220",
    "after round212": "after round220",
    "same fields inside the round182 batch scan": "same fields inside the round226 batch scan",
    "same fields inside the round188 batch scan": "same fields inside the round226 batch scan",
    "same fields inside the round194 batch scan": "same fields inside the round226 batch scan",
    "same fields inside the round200 batch scan": "same fields inside the round226 batch scan",
    "same fields inside the round206 batch scan": "same fields inside the round226 batch scan",
    "same fields inside the round212 batch scan": "same fields inside the round226 batch scan",
    "same fields inside the round220 batch scan": "same fields inside the round226 batch scan",
    "duplicate inside round182 batch scan": "duplicate inside round226 batch scan",
    "duplicate inside round188 batch scan": "duplicate inside round226 batch scan",
    "duplicate inside round194 batch scan": "duplicate inside round226 batch scan",
    "duplicate inside round200 batch scan": "duplicate inside round226 batch scan",
    "duplicate inside round206 batch scan": "duplicate inside round226 batch scan",
    "duplicate inside round212 batch scan": "duplicate inside round226 batch scan",
    "duplicate inside round220 batch scan": "duplicate inside round226 batch scan",
    "Round 200 London LDD archive next11 candidates": "Round 226 London LDD archive next15 candidates",
    "Round 206 London LDD archive next12 candidates": "Round 226 London LDD archive next15 candidates",
    "Round 212 London LDD archive next13 candidates": "Round 226 London LDD archive next15 candidates",
    "Round 220 London LDD archive next14 candidates": "Round 226 London LDD archive next15 candidates",
...
def configure_round226_globals():
...
def rewrite_candidate(candidate, row, excel_row, milestone, milestone_date):
    ...
    event_id = (
        f"lon_ldd_archive_next15_round226_{milestone}_row_{excel_row}_"
...
def write_notes(candidate_count, summary):
    ...
    text = f"""# Round 226 London LDD archive next15 candidates

Generated the next bounded London Development Database archive candidate pack after round220. This pack selects source-defined completion, work-start, or permission milestone records, while deduplicating against the manual corpus and previous LDD archive packs through round220.
...
The generator excludes manual-corpus LDD records and previous LDD archive candidate packs through round220 by workbook row, planning authority plus borough reference, source URL plus source record id, candidate title/date, and source/date key. It also removes duplicate event IDs and source/date keys inside this batch.
