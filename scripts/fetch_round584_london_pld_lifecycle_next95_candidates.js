...
function replaceRequired(value, needle, replacement, label) {
  if (!value.includes(needle)) {
    ...
    throw new Error(`Round584 wrapper transform could not find ${label}: ${needle}`);
  }
...
function transformRound579Wrapper(source) {
  let transformed = source
    .replace(/round579_london_pld_lifecycle_next94/g, "round584_london_pld_lifecycle_next95")
    .replace(/round579\.london_pld_lifecycle_next94/g, "round584.london_pld_lifecycle_next95")
    .replace(/Bims5Round579PldLifecycleNext94/g, "Bims5Round584PldLifecycleNext95")
    .replace(/Round 579/g, "Round 584")
    .replace(/Round579/g, "Round584")
    .replace(/round579/g, "round584")
    .replace(/next94/g, "next95")
    .replace(/MAX_PRIOR_ROUND = 576/g, "MAX_PRIOR_ROUND = 579")
    .replace(/through round576/g, "through round579")
    .replace(/packRound\(entry\.name\) > 576/g, "packRound(entry.name) > 579")
    .replace(/\.replace\(\/round276\/g, "round576"\);/g, '.replace(/round276/g, "round579");');
  ...
  transformed = replaceRequired(
    ...
    "round579 prior pack insertion"
  );

  transformed = transformed
    .replace(
      /pack\) => pack\.label === "round576_london_pld_lifecycle_next93" && pack\.exists === true/g,
      'pack) => pack.label === "round579_london_pld_lifecycle_next94" && pack.exists === true'
    )
    .replace(/round576 prior PLD lifecycle pack was scanned/g, "round579 prior PLD lifecycle pack was scanned");

  assertContains(transformed, 'const ROUND_NAME = "round584_london_pld_lifecycle_next95";', "round name");
  assertContains(transformed, 'const ROUND = "round584";', "transformed fetch round id");
  assertContains(transformed, "MAX_PRIOR_ROUND = 579", "prior cutoff");
  assertContains(transformed, "through round579", "dedupe cutoff text");
  ...
  assertContains(transformed, "addRound584RetrievedAtAlias", "candidate retrieved_at alias");
  assertContains(transformed, 'CURRENT_ROUND_NAME = "round584_london_pld_lifecycle_next95"', "validator round name");
  assertContains(transformed, "round584_strict_duplicate_audit", "strict duplicate audit label");
...
async function main() {
  ...
  vm.runInNewContext(transformed, sandbox, {
    filename: "generated_round584_london_pld_lifecycle_next95_candidates.js"
  });
}
