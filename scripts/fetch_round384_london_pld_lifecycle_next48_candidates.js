...
function replaceRequired(value, needle, replacement, label) {
  if (!value.includes(needle)) {
    ...
    throw new Error(`Round384 source transform could not find ${label}: ${needle}`);
  }
...
function assertContains(value, needle, label) {
  if (!value.includes(needle)) {
    throw new Error(`Round384 source transform failed to include ${label}: ${needle}`);
  }
}

function transformRound362Script(source) {
  let transformed = source
    .replace(/round362_london_pld_lifecycle_next42/g, "round384_london_pld_lifecycle_next48")
    .replace(/round362\.london_pld_lifecycle_next42/g, "round384.london_pld_lifecycle_next48")
    .replace(/Bims5Round362PldLifecycleNext42/g, "Bims5Round384PldLifecycleNext48")
    .replace(/Round362/g, "Round384")
    .replace(/Round 362/g, "Round 384")
    .replace(/round362/g, "round384")
    .replace(/next42/g, "next48")
    .replace(/MAX_PRIOR_ROUND = 358/g, "MAX_PRIOR_ROUND = 380")
    .replace(/through round358/g, "through round380");
  ...
  transformed = replaceRequired(
    ...
    '      "round272, round276, round284, round288, round298, round301, round306, round311, round316, round323, round327, round328, round333, round338, round343, round348, round350, round354, round358, round362, round365, round369, round373, round377, and round380"\\n    );',
  ...
  transformed = replaceRequired(
    ...
    "prior round327 through round380 fetch assertions"
  );

  transformed = replaceRequired(
    transformed,
    '    .replace(/round276/g, "round358");',
    '    .replace(/round276/g, "round380");',
    "validator prior cutoff prose replacement"
  );

  assertContains(transformed, 'const ROUND_NAME = "round384_london_pld_lifecycle_next48";', "round name");
  assertContains(transformed, 'const ROUND = "round384";', "generated round id");
  assertContains(transformed, "MAX_PRIOR_ROUND = 380", "prior cutoff");
  ...
  assertContains(transformed, "through round380", "dedupe prose cutoff");
  return transformed;
}
...
function main() {
  ...
  vm.runInNewContext(
    ...
    {
      filename: "generated_round384_london_pld_lifecycle_next48_candidates.js"
    }
  );
}
