...
TASK_ID = "round182_london_ldd_archive_next8_candidates"
SCRIPT_PATH = "scripts/fetch_round182_london_ldd_archive_next8_candidates.py"
OUT_DIR = Path("tmp/subagents/round182_london_ldd_archive_next8")
...
def prior_ldd_pack_paths():
    ...
    for path in sorted(Path("tmp/subagents").glob("round*_london_ldd_archive*/candidates.json")):
        ...
        if not match or int(match.group(1)) > 176:
            continue
        paths.append(path)
    return paths
...
def rewrite_candidate(candidate, row, excel_row, milestone, milestone_date):
    ...
    event_id = (
        f"lon_ldd_archive_next8_round182_{milestone}_row_{excel_row}_"
...
def write_notes(candidate_count, summary):
    text = f"""# Round 182 London LDD archive next8 candidates

Generated the next bounded London Development Database archive candidate pack after round176. This pack expands beyond exhausted strict completion rows by selecting source-defined completion, work-start, or permission milestone records, while deduplicating against the manual corpus and previous LDD archive packs through round176.
...
The generator excludes manual-corpus LDD records and previous LDD archive candidate packs through round176 by workbook row, planning authority plus borough reference, source URL plus source record id, candidate title/date, and source/date key. It also removes duplicate event IDs and source/date keys inside this batch.
...
def main():
    ...
    for excel_row, row in base.iter_xlsx_dict_rows(permissions_xlsx):
        ...
        if excel_row in dedupe["rows"]:
            ...
            reason = "duplicate inside round182 batch scan"
        elif not milestone:
    ...
    selection_summary = {
        ...
        **prior_pack_summary(prior_packs, current_dedupe),
        "dedupe_fields": [
            ...
            "event_id/id against the manual corpus and previous LDD archive candidate packs through round176",
            ...
            "same fields inside the round182 batch scan",
        ],
        "filters": [
