...
source = source
  .replace(/const ROUND = 471;/g, "const ROUND = 489;")
  .replace(/round471_nyc_lpc_permit_next66/g, "round489_nyc_lpc_permit_next71")
  .replace(/round471/g, "round489")
  .replace(/Round471/g, "Round489")
  .replace(/next66/g, "next71")
  .replace(/Next66/g, "Next71")
  .replace(/through round466/g, "through round486");
...
function writeRound489DuplicateAudit() {
  if (duplicateAuditWritten || process.exitCode) return;
  duplicateAuditWritten = true;

  const root = path.resolve(__dirname, "..");
  const outDir = path.join(root, "tmp", "subagents", "round489_nyc_lpc_permit_next71");
  ...
  const audit = {
    ...
    audit_scope: "Round489 duplicate audit for NYC LPC Permit Application Information next71 candidates.",
    dedupe_boundary: {
      latest_lpc_permit_pack: "tmp/subagents/round486_nyc_lpc_permit_next70/candidates.json",
...
process.on("beforeExit", writeRound489DuplicateAudit);
