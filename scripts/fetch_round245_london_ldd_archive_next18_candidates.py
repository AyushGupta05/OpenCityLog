...
TASK_ID = "round245_london_ldd_archive_next18_candidates"
SCRIPT_PATH = "scripts/fetch_round245_london_ldd_archive_next18_candidates.py"
OUT_DIR = Path("tmp/subagents/round245_london_ldd_archive_next18")
...
ROUND239_SCRIPT_PATH = Path("scripts/fetch_round239_london_ldd_archive_next17_candidates.py")


def build_round_replacements():
    replacements = {
        "round239_london_ldd_archive_next17_candidates": TASK_ID,
        "scripts/fetch_round239_london_ldd_archive_next17_candidates.py": SCRIPT_PATH,
        "tmp/subagents/round239_london_ldd_archive_next17": OUT_DIR.as_posix(),
        "Round 239 London LDD archive next17 candidates": "Round 245 London LDD archive next18 candidates",
        "a administrative": "an administrative",
    }
    for round_number in [176, 182, 188, 194, 200, 206, 212, 220, 226, 233]:
        replacements[f"through round{round_number}"] = "through round239"
        replacements[f"after round{round_number}"] = "after round239"
    for round_number in [176, 182, 188, 194, 200, 206, 212, 220, 226, 233, 239]:
        replacements[f"same fields inside the round{round_number} batch scan"] = (
            "same fields inside the round245 batch scan"
        )
        replacements[f"duplicate inside round{round_number} batch scan"] = "duplicate inside round245 batch scan"
...
def load_round239_module():
    spec = importlib.util.spec_from_file_location("round239_ldd_next17", ROUND239_SCRIPT_PATH)
    if not spec or not spec.loader:
        raise RuntimeError(f"Unable to load {ROUND239_SCRIPT_PATH}")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


round239 = load_round239_module()
original_rewrite_candidate = round239.rewrite_candidate


def configure_round245_globals():
    round239.ACCESSED_AT = ACCESSED_AT
    round239.SOURCE_ID = SOURCE_ID
    round239.TASK_ID = TASK_ID
    round239.SCRIPT_PATH = SCRIPT_PATH
    round239.OUT_DIR = OUT_DIR
    round239.OUT_PATH = OUT_PATH
    round239.SOURCE_AUDIT_PATH = SOURCE_AUDIT_PATH
    round239.SUMMARY_PATH = SUMMARY_PATH
    round239.NOTES_PATH = NOTES_PATH
    round239.REJECTED_PATH = REJECTED_PATH
    round239.ROUND_REPLACEMENTS = ROUND_REPLACEMENTS


def base_module():
    return round239.base_module()
...
def prior_ldd_pack_paths():
    ...
    for path in sorted(Path("tmp/subagents").glob("round*_london_ldd_archive*/candidates.json")):
        ...
        if not match or int(match.group(1)) > 239:
            continue
        paths.append(path)
    return paths
...
def rewrite_candidate(candidate, row, excel_row, milestone, milestone_date):
    ...
    event_id = (
        f"lon_ldd_archive_next18_round245_{milestone}_row_{excel_row}_"
...
def main():
    configure_round245_globals()
    round239.prior_ldd_pack_paths = prior_ldd_pack_paths
    round239.rewrite_candidate = rewrite_candidate
    round239.write_notes = write_notes
    round239.main()
    postprocess_outputs()
