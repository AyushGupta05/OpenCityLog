...
const outDir = "tmp/subagents/round234_nyc_hpd_affordable_housing_next4";
...
function buildCandidate(row, existing, rejected) {
  ...
  return {
    ...
    raw_row: row,
    transformation_method: `${roundName} official NYC Open Data HPD fetch from ${DATASET.id}; selected from rows with project/building identifiers, a source date, source coordinates, unit counts, and duplicate screening against the live manual corpus plus prior HPD candidate packs through round229.`
  };
}
...
function sourceAudit(metadata, acceptedCount, selectedSummary) {
  ...
  return {
    schema_version: "round234.nyc_hpd_affordable_housing_next4_source_audit.v1",
    ...
    sources: [
      {
        ...
        published_coverage_note: "Round234 checked HPD building-level rows with coordinates and building/project completion or project start dates available through the access date.",
...
function validateGeneratedPack(candidatesPayload, sourceAuditPayload, summaryPayload, existing) {
  ...
  return {
    ...
    checks: [
      ...
      "no overlap with live corpus or prior HPD candidate packs"
    ],
...
async function main() {
  ...
  const summaryPayload = {
    schema_version: "round234.nyc_hpd_affordable_housing_next4_summary.v1",
    generated_at: generatedAt,
    accessed_at: accessedAt,
    output_files: [
  ...
  const candidatesPayload = {
    schema_version: "round234.nyc_hpd_affordable_housing_next4_candidates.v1",
    generated_at: generatedAt,
    accessed_at: accessedAt,
  ...
  const rejectedPayload = {
    schema_version: "round234.nyc_hpd_affordable_housing_next4_rejected.v1",
    generated_at: generatedAt,
    accessed_at: accessedAt,
    source_id: DATASET.sourceId,
    dataset_id: DATASET.id,
