...
const ROUND_NAME = "round306_london_pld_lifecycle_next29";
...
function assertContains(value, needle, label) {
  if (!value.includes(needle)) {
    throw new Error(`Round306 source transform failed to include ${label}: ${needle}`);
  }
}

function assertAbsent(value, needle, label) {
  if (value.includes(needle)) {
    throw new Error(`Round306 source transform retained ${label}: ${needle}`);
  }
}
...
function replaceRequired(value, needle, replacement, label) {
  if (!value.includes(needle)) {
    ...
    throw new Error(`Round306 source transform could not find ${label}: ${needle}`);
  }
...
function transformRound284Fetch(source) {
  let transformed = source
    .replace(/const ROUND = "round284";/, 'const ROUND = "round306";')
    .replace(
      /const ROUND_NAME = "round284_london_pld_lifecycle_next25";/,
      'const ROUND_NAME = "round306_london_pld_lifecycle_next29";'
    )
    .replace(/const MAX_PRIOR_ROUND = 276;/, "const MAX_PRIOR_ROUND = 301;")
    .replace(/Bims5Round284PldLifecycleNext25/g, "Bims5Round306PldLifecycleNext29")
    .replace(/round284_london_pld_lifecycle_next25/g, "round306_london_pld_lifecycle_next29")
    .replace(/round284\.london_pld_lifecycle_next25/g, "round306.london_pld_lifecycle_next29")
    .replace(/Round284/g, "Round306")
    .replace(/Round 284/g, "Round 306")
    .replace(/round284/g, "round306")
    .replace(/next25/g, "next29")
    .replace(/through round276/g, "through round301")
    .replace(
      /round272, and round276/g,
      "round272, round276, round284, round288, round298, and round301"
    );
  ...
  assertContains(transformed, 'const ROUND = "round306";', "round id");
  ...
  assertContains(transformed, "through round301", "dedupe cutoff");
...
function transformRound284Validator(source) {
  let transformed = source
    ...
    .replace(
      ...
      'const CURRENT_ROUND_NAME = "round306_london_pld_lifecycle_next29";'
    )
    .replace(/round284_london_pld_lifecycle_next25/g, "round306_london_pld_lifecycle_next29")
    .replace(/round284/g, "round306")
    .replace(/round284:/g, "round306:")
    .replace(/round276/g, "round301");
  ...
  assertContains(transformed, "MAX_PRIOR_ROUND = 301", "validator cutoff");
  assertContains(transformed, 'CURRENT_ROUND_NAME = "round306_london_pld_lifecycle_next29"', "validator current round");
...
async function runTransformedFetch() {
  ...
  vm.runInNewContext(transformed, sandbox, {
    filename: "generated_round306_london_pld_lifecycle_next29_candidates.js"
  });
  if (typeof sandboxModule.exports !== "function") {
    throw new Error("Round306 transform did not export the fetch main() function.");
  }
...
function writeValidator() {
  ...
  fs.writeFileSync(path.join(OUT_DIR, "validate_round306_pack.js"), transformed);
}
