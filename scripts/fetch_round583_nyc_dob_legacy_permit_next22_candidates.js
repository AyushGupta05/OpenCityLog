...
const OUTPUT_DIR = path.join(__dirname, "..", "tmp", "subagents", "round583_nyc_dob_legacy_permit_next22");
...
function normalizeOutputBoundaryText() {
  ...
  for (const name of fs.readdirSync(OUTPUT_DIR)) {
    ...
    const normalized = text
      .replaceAll("through Round568", "through Round578")
      .replaceAll("treating Round568", "treating Round578")
      .replaceAll("through Round572", "through Round578")
      .replaceAll("treating Round572", "treating Round578")
...
if (sourceHash !== EXPECTED_SOURCE_SHA256) {
  throw new Error(
    `Round583 expected Round572 worker source hash ${EXPECTED_SOURCE_SHA256}, got ${sourceHash}. ` +
      "Review the upstream pattern before regenerating this candidate pack."
  );
}

const round583Source = source
  .replace("const ROUND = 572;", "const ROUND = 583;")
  .replace(
    'const SLUG = "round572_nyc_dob_legacy_permit_next19";',
    'const SLUG = "round583_nyc_dob_legacy_permit_next22";'
  )
  .replace("const DEDUPE_BOUNDARY_ROUND = 568;", "const DEDUPE_BOUNDARY_ROUND = 578;")
  .replaceAll(
    "fetch_round572_nyc_dob_legacy_permit_next19_candidates.js",
    "fetch_round583_nyc_dob_legacy_permit_next22_candidates.js"
  )
  .replaceAll("round572_nyc_dob_legacy_permit_next19", "round583_nyc_dob_legacy_permit_next22")
  .replaceAll("Round572", "Round583")
  .replaceAll('"Round563", "Round568"', '"Round574", "Round578"')
  .replace(
  ...
  .replaceAll("through Round574", "through Round578")
  .replaceAll("treating Round574", "treating Round578")
  .replaceAll("next19", "next22");

vm.runInNewContext(round583Source, {
  __dirname,
  __filename,
  require,
  console,
