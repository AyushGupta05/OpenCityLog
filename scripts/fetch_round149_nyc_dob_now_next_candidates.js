...
const PUBLIC_OWNER_PATTERN = /NYCHA|SCHOOL CONSTRUCTION AUTHORITY|NYC|HOSPITAL|LIBRARY|PARKS|DORMITORY AUTHORITY|MTA|CUNY|HHC|HEALTH|UNIVERSITY|DEPARTMENT|AUTHORITY|HOUSING PRESERVATION|ECONOMIC DEVELOPMENT|PORT AUTHORITY|TRANSIT/i;

const HIGH_SIGNAL_THRESHOLDS = {
  total_construction_floor_area_gte: 25000,
  proposed_dwelling_units_gte: 20,
  proposed_height_gte: 45,
  proposed_stories_gte: 5,
  initial_cost_or_estimated_job_costs_gte: 3000000,
  public_owner_name_pattern: "NYCHA|SCHOOL CONSTRUCTION AUTHORITY|NYC|HOSPITAL|LIBRARY|PARKS|DORMITORY AUTHORITY|MTA|CUNY|HHC|HEALTH|UNIVERSITY|DEPARTMENT|AUTHORITY|HOUSING PRESERVATION|ECONOMIC DEVELOPMENT|PORT AUTHORITY|TRANSIT"
};
...
function buildExistingIndex() {
  ...
  return { tokens, filesRead, candidateFiles };
}
...
function scaleForApplication(row) {
  return {
    ...
    publicLike: PUBLIC_OWNER_PATTERN.test(cleanText(row.owner_s_business_name))
  };
}

function scaleLabel(scale, permit) {
  const parts = [];
  ...
  if (permitCost) parts.push(`permit estimated job cost $${Math.round(permitCost).toLocaleString("en-US")}`);
  return parts.length ? parts.join("; ") : "no large scale fields retained";
}

function applicationScore(row, scale) {
  let score = 0;
  score += Math.min(scale.floorArea / 800, 900);
  score += Math.min(scale.units * 2.5, 700);
  score += Math.min(scale.height * 1.2, 350);
  score += Math.min(scale.stories * 10, 250);
  score += Math.min(scale.cost / 85000, 280);
  if (cleanText(row.job_type) === "New Building") score += 180;
  if (cleanText(row.job_type).includes("ALT-CO")) score += 120;
  if (scale.publicLike) score += 160;
...
function isHighSignal(app, permit, scale) {
  const permitCost = parseNumber(permit.estimated_job_costs);
  const permitOwnerPublic = PUBLIC_OWNER_PATTERN.test(cleanText(permit.owner_business_name));
  const jobType = cleanText(app.job_type);
  return APPLICATION_JOB_TYPES.includes(jobType) &&
    (scale.floorArea >= HIGH_SIGNAL_THRESHOLDS.total_construction_floor_area_gte ||
      scale.units >= HIGH_SIGNAL_THRESHOLDS.proposed_dwelling_units_gte ||
      scale.height >= HIGH_SIGNAL_THRESHOLDS.proposed_height_gte ||
      scale.stories >= HIGH_SIGNAL_THRESHOLDS.proposed_stories_gte ||
      scale.cost >= HIGH_SIGNAL_THRESHOLDS.initial_cost_or_estimated_job_costs_gte ||
      permitCost >= HIGH_SIGNAL_THRESHOLDS.initial_cost_or_estimated_job_costs_gte ||
      scale.publicLike ||
      permitOwnerPublic);
}
...
function rankPermit(row) {
  ...
  const costRank = Math.min(parseNumber(row.estimated_job_costs) / 200000, 80);
  return typeRank + statusRank + costRank;
}

function chooseApplicationForBase(base, applicationsByBaseJob) {
  const options = applicationsByBaseJob.get(base) || [];
  return options
    .slice()
    .sort((a, b) => {
...
function buildCandidate(app, permit, metadataContext, selectionMode) {
  const issuedDate = parseDate(permit.issued_date);
  const approvedDate = parseDate(permit.approved_date);
  const filingDate = parseDate(app.filing_date);
  const firstPermitDate = parseDate(app.first_permit_date);
  ...
  const candidateId = `nyc_dob_now_next_rbx6_${slugify(cleanText(permit.work_permit), 64)}_${slugify(cleanText(permit.sequence_number) || "seq", 20)}_${slugify(cleanText(permit.tracking_number) || "tracking", 36)}_${issuedDate.replace(/-/g, "_")}`;
  ...
  const summary = [
    `NYC Department of Buildings Open Data records DOB NOW approved permit ${cleanText(permit.work_permit)} for ${address}, ${borough} with issued_date ${issuedDate}.`,
    `The permit row is tied to job filing ${cleanText(app.job_filing_number)}, whose DOB NOW job application row lists job_type '${jobType}', filing_status '${cleanText(app.filing_status)}', filing_date ${filingDate || "not supplied"}, and scale fields: ${scaleLabel(scale, permit)}.`
  ].join(" ");
  const limitations = [
  ...
  return {
    ...
    permit_approved_date: approvedDate,
    ...
    source_page_url: DATASETS.permits.page,
    source_api_url: DATASETS.permits.api,
    ...
    source_date_field: "issued_date from DOB NOW: Build - Approved Permits; filing_date, job_type, and scale fields joined from DOB NOW: Build - Job Application Filings by job_filing_number or DOB NOW base job number",
    ...
    limitations,
    transformation_method: `Round149 generator queried DOB NOW approved permit rows directly for ${START_DATE} through ${END_DATE}, joined them to DOB NOW job application rows, retained one nonduplicate high-signal administrative permit row per base DOB NOW job beyond the current manual corpus plus round133/round136/round143 candidate packs when present, and preserved row-level permit/application provenance. Selection mode: ${selectionMode}.`,
    duplicate_check_note: "Exact job_filing_number, base DOB NOW job number, work_permit, tracking_number, source_record_id, source_url, candidate_id, and event_id tokens were screened against the manual architecture corpus and existing tmp candidate packs available during this run, including round133, round136, and round143 outputs when present.",
    application_source_url: appUrl,
    application_source_page_url: DATASETS.applications.page,
    application_source_api_url: DATASETS.applications.api,
...
function summarizeCandidates(candidates) {
  ...
  for (const candidate of candidates) {
    ...
    byWorkType[candidate.work_type] = (byWorkType[candidate.work_type] || 0) + 1;
  ...
  return { by_borough: byBorough, by_work_type: byWorkType, by_job_type: byJobType, by_year: byYear };
}
...
async function main() {
  ...
  const applicationsByBaseJob = new Map();
  for (const app of applicationRows) {
    ...
    const base = dobNowBaseNumber(job);
    if (!applicationsByBaseJob.has(base)) applicationsByBaseJob.set(base, []);
    applicationsByBaseJob.get(base).push(app);
  }
  ...
  for (const permit of candidatePermits) {
    ...
    const app = exactApp || chooseApplicationForBase(baseJob, applicationsByBaseJob);
    if (!app) {
  ...
  const selectionSummary = {
    ...
    duplicate_index_files: existing.candidateFiles.map((file) => file.replace(/\\/g, "/")).filter((file) => /round133|round136|round143|nyc|architecture/i.test(file)).sort(),
    ...
    retained_less_than_target_reason: candidates.length < TARGET_COUNT ? "Source/dedupe/high-signal filters yielded fewer candidates than target." : null,
    ...
    high_signal_thresholds: HIGH_SIGNAL_THRESHOLDS,
  ...
  const sourceAudits = [
    {
      ...
      granularity: "One approved permit row; this generator retains one substantial initial permit row per base DOB NOW job filing beyond existing round133/round136/round143/manual-corpus tokens.",
  ...
  const output = {
    schema_version: "round149.nyc_dob_now_next_candidates.v1",
    generated_at: GENERATED_AT,
    accessed_at: ACCESSED_AT,
    worker: "Round149 NYC DOB NOW next candidates",
    scope: "Official NYC DOB NOW Build approved-permit/job-filing candidate pack beyond the live manual corpus plus round133, round136, and round143 when present. Records are administrative permit/job milestones and must not be presented as construction start, completion, opening, occupancy, or outcome evidence.",
    candidate_count: candidates.length,
  ...
  const sourceAudit = {
    schema_version: "round149.nyc_dob_now_next_source_audit.v1",
    generated_at: GENERATED_AT,
    accessed_at: ACCESSED_AT,
    audit_scope: "Official NYC DOB NOW Build approved permits and job application filings, selected for additional architecture-change candidate discovery with administrative-record caveats.",
  ...
  const summary = {
    schema_version: "round149.nyc_dob_now_next_summary.v1",
    generated_at: GENERATED_AT,
    accessed_at: ACCESSED_AT,
    output_files: [
    ...
    candidate_count: candidates.length,
    selected_summary: selectedSummary,
    source_ids: [DATASETS.permits.sourceId, DATASETS.applications.sourceId, DATASETS.permits.canonicalSourceId],
    date_window: selectionSummary.date_window,
    duplicate_screening: {
      files_read: existing.filesRead,
      tokens: existing.tokens.size,
      screened_round133_round136_round143_files: selectionSummary.duplicate_index_files.filter((file) => /round133|round136|round143/i.test(file))
    },
    selection_summary: selectionSummary,
    caveats: [
      "Administrative DOB NOW filing/permit milestone only.",
      "Not proof that a building was built, completed, opened, occupied, or causally linked to outcomes.",
    ...
    sample_candidates: candidates.slice(0, 10).map((candidate) => ({
  ...
  const rejectOutput = {
    schema_version: "round149.nyc_dob_now_next_rejections.v1",
    generated_at: GENERATED_AT,
    accessed_at: ACCESSED_AT,
    rejected_counts: rejectionCounter(rejections),
  ...
  const notes = [
    "# Round149 NYC DOB NOW Next Candidate Pack",
    "",
    `Generated ${candidates.length} additional administrative DOB NOW approved-permit candidates on ${ACCESSED_AT}.`,
    "",
    "## Scope",
    "",
    ...
    "## Duplicate Screening",
    "",
    `Screened against ${existing.filesRead} manual-corpus/tmp candidate files and ${existing.tokens.size} exact identifier tokens, including round133/round136/round143 outputs when present.`,
    "",
    "## Counts",
    "",
    `- Permit rows fetched: ${permitRows.length}`,
    `- Permit rows after duplicate/geography filters: ${candidatePermits.length}`,
    `- Application rows fetched: ${applicationRows.length}`,
    `- Candidate pool before balancing: ${candidatePool.length}`,
