...
function replaceRequired(value, needle, replacement, label) {
  if (!value.includes(needle)) {
    ...
    throw new Error(`Round547 wrapper transform could not find ${label}: ${needle}`);
  }
...
function transformRound529Wrapper(source) {
  let transformed = source
    .replace(/round529_london_pld_lifecycle_next82/g, "round547_london_pld_lifecycle_next86")
    .replace(/round529\.london_pld_lifecycle_next82/g, "round547.london_pld_lifecycle_next86")
    .replace(/Bims5Round529PldLifecycleNext82/g, "Bims5Round547PldLifecycleNext86")
    .replace(/Round 529/g, "Round 547")
    .replace(/Round529/g, "Round547")
    .replace(/round529/g, "round547")
    .replace(/next82/g, "next86")
    .replace(/MAX_PRIOR_ROUND = 526/g, "MAX_PRIOR_ROUND = 542")
    .replace(/through round526/g, "through round542")
    .replace(/packRound\(entry\.name\) > 526/g, "packRound(entry.name) > 542")
    .replace(/\.replace\(\/round276\/g, "round526"\);/g, '.replace(/round276/g, "round542");');
  ...
  transformed = replaceRequired(
    ...
    "round529, round534, round537, and round542 prior pack insertion"
  );

  transformed = transformed
    .replace(
      /pack\) => pack\.label === "round526_london_pld_lifecycle_next81" && pack\.exists === true/g,
      'pack) => pack.label === "round542_london_pld_lifecycle_next85" && pack.exists === true'
    )
    .replace(/round526 prior PLD lifecycle pack was scanned/g, "round542 prior PLD lifecycle pack was scanned");

  assertContains(transformed, 'const ROUND_NAME = "round547_london_pld_lifecycle_next86";', "round name");
  assertContains(transformed, 'const ROUND = "round547";', "transformed fetch round id");
  assertContains(transformed, "MAX_PRIOR_ROUND = 542", "prior cutoff");
  assertContains(transformed, "through round542", "dedupe cutoff text");
  ...
  assertContains(transformed, "addRound547RetrievedAtAlias", "candidate retrieved_at alias");
  assertContains(transformed, 'CURRENT_ROUND_NAME = "round547_london_pld_lifecycle_next86"', "validator round name");
  assertContains(transformed, "round547_strict_duplicate_audit", "strict duplicate audit label");
...
async function main() {
  ...
  vm.runInNewContext(transformed, sandbox, {
    filename: "generated_round547_london_pld_lifecycle_next86_candidates.js"
  });
}
