...
const MAX_PRIOR_ROUND = 369;
const CURRENT_ROUND_NAME = "round373_london_pld_lifecycle_next45";
...
function validate() {
  ...
  for (const candidate of candidates) {
    ...
    assert(!fieldDateKeys.has(fieldKey), `Duplicate source-date key inside round373: ${fieldKey}`);
  ...
  const report = {
    ...
    checks: [
      ...
      "no overlap with PLD lifecycle packs through round369",
      "source/row/API URLs",
