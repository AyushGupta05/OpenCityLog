...
const MAX_PRIOR_ROUND = 348;
const CURRENT_ROUND_NAME = "round350_london_pld_lifecycle_next39";
...
function validate() {
  ...
  for (const candidate of candidates) {
    ...
    assert(!fieldDateKeys.has(fieldKey), `Duplicate source-date key inside round350: ${fieldKey}`);
  ...
  const report = {
    ...
    checks: [
      ...
      "no overlap with PLD lifecycle packs through round348",
      "source/row/API URLs",
