...
ROOT = Path("tmp/subagents/round392_london_nhle_heritage_tail5")
...
def validate():
    ...
    if pack.get("metadata", {}).get("date_window") != {"start": DATE_START, "end": DATE_END}:
        errors.append("candidates metadata date_window is not the required round392 window.")
    ...
    for index, candidate in enumerate(candidates):
        ...
        if not key or not re.fullmatch(r"(?:\d{6,8}|article-\d{6,8})\|[A-Za-z]+\|\d{4}-\d{2}-\d{2}", key):
            ...
            errors.append(f"Duplicate NHLE/admin key within round392: {key}.")
        else:
            nhle_keys.add(key)
    ...
    if duplicate_keys:
        errors.append(f"Round392 candidates duplicate existing NHLE/admin keys: {duplicate_keys[:20]}")
    ...
    for artifact_name in artifact_names:
        ...
        file_readback[artifact_name] = {
            "exists": artifact_path.exists(),
            "size_bytes": artifact_path.stat().st_size if artifact_path.exists() else 0,
        }
        if artifact_name.endswith(".json") and artifact_path.exists():
            try:
                read_json(artifact_path)
                file_readback[artifact_name]["json_readback_ok"] = True
            except (OSError, json.JSONDecodeError) as exc:
                file_readback[artifact_name]["json_readback_ok"] = False
                file_readback[artifact_name]["error"] = str(exc)

    readback = {
        "generated_at": f"{ACCESS_DATE}T00:00:00Z",
        "passed": all(item["exists"] for item in file_readback.values())
        and all(item.get("json_readback_ok", True) for item in file_readback.values()),
        "files": file_readback,
        "pack_type": summary.get("pack_type"),
        "candidate_count": len(candidates),
        "summary_candidate_count": summary.get("candidate_count"),
        "metadata_candidate_count": pack.get("metadata", {}).get("candidate_count"),
        "source_rows_fetched": summary.get("source_rows_fetched"),
        "source_rows_fetched_by_source": summary.get("source_rows_fetched_by_source"),
        "source_names": [audit.get("source_name") for audit in source_audit.get("source_audits", [])],
        "event_source_publishers": sorted(
            {audit.get("publisher") for audit in source_audit.get("source_audits", []) if audit.get("publisher")}
        ),
        "supporting_boundary_source": source_audit.get("supporting_boundary_source"),
        "rejected_reason_counts": rejected.get("reason_counts", {}),
        "critical_interpretation_note": summary.get("critical_interpretation_note"),
    }
    (ROOT / "readback.json").write_text(json.dumps(readback, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    readback_lines = [
        "# Round392 Readback Report",
        "",
        f"- Passed: {readback['passed']}",
        f"- Pack type: {readback['pack_type']}",
        f"- Candidate count: {readback['candidate_count']}",
        f"- Source rows fetched: {readback['source_rows_fetched']}",
        f"- Event source publishers: {', '.join(readback['event_source_publishers'])}",
        "",
        "Artifacts:",
        "",
    ]
    readback_lines.extend(
        f"- {name}: exists={details['exists']}, size={details['size_bytes']}"
        for name, details in file_readback.items()
    )
    (ROOT / "readback_report.md").write_text("\n".join(readback_lines) + "\n", encoding="utf-8")
    report_lines = [
        "# Round392 Validation Report",
        "",
