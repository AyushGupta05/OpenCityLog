...
const MAX_PRIOR_ROUND = 384;
const CURRENT_ROUND_NAME = "round387_london_pld_lifecycle_next49";
...
function validate() {
  ...
  for (const candidate of candidates) {
    ...
    assert(!fieldDateKeys.has(fieldKey), `Duplicate source-date key inside round387: ${fieldKey}`);
  ...
  const report = {
    ...
    checks: [
      ...
      "no overlap with PLD lifecycle packs through round384",
      "source/row/API URLs",
