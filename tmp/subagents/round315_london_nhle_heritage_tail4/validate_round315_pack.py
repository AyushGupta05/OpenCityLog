...
ROOT = Path("tmp/subagents/round315_london_nhle_heritage_tail4")
...
def validate():
    ...
    if pack.get("metadata", {}).get("date_window") != {"start": DATE_START, "end": DATE_END}:
        errors.append("candidates metadata date_window is not the required round315 window.")
    ...
    for index, candidate in enumerate(candidates):
        ...
        if not key or not re.fullmatch(r"(?:\d{6,8}|article-\d{6,8})\|[A-Za-z]+\|\d{4}-\d{2}-\d{2}", key):
            ...
            errors.append(f"Duplicate NHLE/admin key within round315: {key}.")
        else:
            nhle_keys.add(key)
    ...
    if duplicate_keys:
        errors.append(f"Round315 candidates duplicate existing NHLE/admin keys: {duplicate_keys[:20]}")
