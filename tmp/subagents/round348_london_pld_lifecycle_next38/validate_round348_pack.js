...
const MAX_PRIOR_ROUND = 343;
const CURRENT_ROUND_NAME = "round348_london_pld_lifecycle_next38";
...
function validate() {
  ...
  for (const candidate of candidates) {
    ...
    assert(!fieldDateKeys.has(fieldKey), `Duplicate source-date key inside round348: ${fieldKey}`);
  ...
  const report = {
    ...
    checks: [
      ...
      "no overlap with PLD lifecycle packs through round343",
      "source/row/API URLs",
