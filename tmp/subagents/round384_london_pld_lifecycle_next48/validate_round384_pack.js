...
const MAX_PRIOR_ROUND = 380;
const CURRENT_ROUND_NAME = "round384_london_pld_lifecycle_next48";
...
function validate() {
  ...
  for (const candidate of candidates) {
    ...
    assert(!fieldDateKeys.has(fieldKey), `Duplicate source-date key inside round384: ${fieldKey}`);
  ...
  const report = {
    ...
    checks: [
      ...
      "no overlap with PLD lifecycle packs through round380",
      "source/row/API URLs",
