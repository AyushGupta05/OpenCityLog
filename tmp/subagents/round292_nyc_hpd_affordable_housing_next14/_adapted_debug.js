...
const roundName = "Round292";
const maxScreeningRound = 283;
...
function addExistingRow(index, row, file) {
  ...
  const canonicalRecordId = sourceRecordId || urlRecordId || rawRecordId || Array.from(hpdRecordIdsFromText(`${sourceRecordId} ${sourceUrl}`))[0] || "";
...
function buildExistingIndex() {
  ...
  if (index.hpdRowsIndexed > 0) index.filesWithHpdRows.add(path.normalize(corpusPath));
...
function appearsExisting(existing, eventId, sourceRecordId, sourceUrl, sourceDateField, date) {
  if (eventId && existing.eventIds.has(eventId)) {
    return "event_id already appears in the live corpus or HPD candidate packs through round283.";
  }
  if (sourceRecordId && (existing.sourceRecordIds.has(sourceRecordId) || existing.sourceRecordIds.has(`${DATASET.sourceId}:${sourceRecordId}`))) {
    return "HPD project/building source row already appears in the live corpus or HPD candidate packs through round283.";
  }
  if (sourceUrl && existing.sourceUrls.has(normalizeUrlToken(sourceUrl))) {
    return "HPD Socrata row URL already appears in the live corpus or HPD candidate packs through round283.";
  }
  ...
  if (existing.sourceDateKeys.has(key)) {
    return "HPD source/date key already appears in the live corpus or HPD candidate packs through round283.";
  }
  return "";
}
...
function buildCandidate(row, existing, rejected) {
  ...
  return {
    ...
    limitations: "HPD Affordable Housing Production by Building is administrative affordable-housing program/building delivery evidence. It is not a complete citywide construction inventory, DOB final certificate, first occupancy record, tenant move-in record, as-built footprint, affordability-duration audit, causal outcome evidence, or proof of occupancy/outcomes. Preservation rows in this pack are retained only where HPD reports a building or project completion date, source coordinates, positive units, and Extended Affordability Only = No; they are not independent evidence of full rehabilitation scope or occupancy.",
    area: `${address || row.project_name || "HPD affordable housing production row"}, ${borough || "New York City"}, New York City`,
    raw_row: row,
    transformation_method: `${roundName} official NYC Open Data HPD fetch from ${DATASET.id}; selected from rows with project/building identifiers, a source date, source coordinates, unit counts, and duplicate screening against the live manual architecture corpus plus HPD-family candidate packs through round283, explicitly including round229, round234, round236, round241, round246, round251, round255, round263, round269, round275, and round283.`
  };
}
...
function summarizeCandidates(candidates) {
  ...
  const median = unitCounts.length ? unitCounts[Math.floor(unitCounts.length / 2)] : null;
  return {
    by_year: byYear,
...
function sourceAudit(metadata, acceptedCount, selectedSummary) {
  ...
  return {
    schema_version: "round292.nyc_hpd_affordable_housing_next14_source_audit.v1",
...
function validateGeneratedPack(candidatesPayload, sourceAuditPayload, summaryPayload, existing) {
  ...
  if (candidates.length === 0) throw new Error("Validation failed: no candidates generated.");
  if (candidates.length > targetCount) throw new Error(`Validation failed: too many candidates generated (${candidates.length}).`);
  if (!Array.isArray(sourceAuditPayload.sources) || sourceAuditPayload.sources.length !== 1) {
    throw new Error("Validation failed: expected exactly one source audit.");
  }
  if (summaryPayload.candidate_count !== candidates.length) {
    throw new Error("Validation failed: summary candidate_count does not match candidates payload.");
  }

  for (const candidate of candidates) {
    for (const field of requiredFields) {
      if (!candidate[field] || (Array.isArray(candidate[field]) && candidate[field].length === 0)) {
        throw new Error(`Validation failed: missing required field ${field} on ${candidate.event_id || candidate.title}.`);
      }
    }
    if (candidate.city_id !== "nyc") throw new Error(`Validation failed: non-NYC candidate ${candidate.event_id}.`);
    if (candidate.accessed_at !== accessedAt) throw new Error(`Validation failed: accessed_at mismatch on ${candidate.event_id}.`);
    if (candidate.confidence !== "documented") throw new Error(`Validation failed: unexpected confidence ${candidate.confidence} on ${candidate.event_id}.`);
    if (candidate.source_dataset_id !== DATASET.sourceId) throw new Error(`Validation failed: unexpected source_dataset_id on ${candidate.event_id}.`);
    if (!candidate.source_ids.includes(DATASET.sourceId)) throw new Error(`Validation failed: missing HPD source id on ${candidate.event_id}.`);
    if (!candidate.source_url.includes(DATASET.id)) throw new Error(`Validation failed: source_url is not the HPD building dataset on ${candidate.event_id}.`);
    if (!allowedDateFields.has(candidate.source_date_field)) throw new Error(`Validation failed: unexpected source_date_field on ${candidate.event_id}.`);
    if (candidate.date !== candidate.effective_date || candidate.date !== candidate.source_date) {
      throw new Error(`Validation failed: date/effective/source_date mismatch on ${candidate.event_id}.`);
    }
    if (!inDateWindow(candidate.date)) throw new Error(`Validation failed: out-of-window date ${candidate.date} on ${candidate.event_id}.`);
    if (!candidate.project_id || !candidate.building_id || candidate.source_record_id !== `${candidate.project_id}:${candidate.building_id}`) {
      throw new Error(`Validation failed: project/building/source_record_id mismatch on ${candidate.event_id}.`);
    }
    ...
    if (candidate.geometry.type !== "Point" || !isNycPoint(lat, lon)) {
      throw new Error(`Validation failed: geometry outside NYC bounds on ${candidate.event_id}.`);
    }
    if (Number(candidate.latitude) !== lat || Number(candidate.longitude) !== lon) {
      throw new Error(`Validation failed: geometry and latitude/longitude disagree on ${candidate.event_id}.`);
    }
    ...
    if (/\bparks?\b|4hcv-tc5r/.test(sourceFamily)) {
      throw new Error(`Validation failed: parks source detected on ${candidate.event_id}.`);
    }
    ...
    if (overlapReason) throw new Error(`Validation failed: overlap for ${candidate.event_id}: ${overlapReason}`);

    if (eventIds.has(candidate.event_id)) throw new Error(`Validation failed: duplicate event_id ${candidate.event_id}.`);
    eventIds.add(candidate.event_id);
    if (candidateIds.has(candidate.candidate_id)) throw new Error(`Validation failed: duplicate candidate_id ${candidate.candidate_id}.`);
    ...
    if (sourceDateKeys.has(key)) throw new Error(`Validation failed: duplicate source/date key ${key}.`);
  ...
  return {
    ...
    checks: [
      ...
      "unique event_id and candidate_id",
      "unique source/date keys",
      "date window",
      "NYC coordinate bounds",
      "no parks source records",
      "no overlap with live corpus or HPD candidate packs through round283, including round229, round234, round236, round241, round246, round251, round255, round263, round269, round275, and round283"
    ],
...
async function main() {
  ...
  const summaryPayload = {
    schema_version: "round292.nyc_hpd_affordable_housing_next14_summary.v1",
    generated_at: generatedAt,
    accessed_at: accessedAt,
    output_files: [
    ...
    selection_summary: {
      ...
      duplicate_screening: {
        ...
        candidate_files_considered_through_round283: existing.candidateFilesConsidered,
  ...
  const candidatesPayload = {
    schema_version: "round292.nyc_hpd_affordable_housing_next14_candidates.v1",
    generated_at: generatedAt,
    accessed_at: accessedAt,
  ...
  const rejectedPayload = {
    schema_version: "round292.nyc_hpd_affordable_housing_next14_rejected.v1",
    generated_at: generatedAt,
    accessed_at: accessedAt,
    source_id: DATASET.sourceId,
    dataset_id: DATASET.id,
  ...
  const notes = [
    "# Round292 NYC HPD Affordable Housing Production Candidate Pack",
    "",
    ...
    ...Object.entries(selectedSummary.by_prevailing_wage_status).map(([field, count]) => `- Prevailing wage status ${field}: ${count}`),
    "",
    "## Caveats",
    "",
    ...
    "- Duplicate screening used the live manual corpus and HPD-family candidate packs through round283, explicitly including round229, round234, round236, round241, round246, round251, round255, round263, round269, round275, and round283; this script did not edit the corpus, appender, or generated atlas files."
  ].join("\n");
