...
const MAX_PRIOR_ROUND = 377;
const CURRENT_ROUND_NAME = "round380_london_pld_lifecycle_next47";
...
function validate() {
  ...
  for (const candidate of candidates) {
    ...
    assert(!fieldDateKeys.has(fieldKey), `Duplicate source-date key inside round380: ${fieldKey}`);
  ...
  const report = {
    ...
    checks: [
      ...
      "no overlap with PLD lifecycle packs through round377",
      "source/row/API URLs",
