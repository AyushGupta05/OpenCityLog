...
const MAX_PRIOR_ROUND = 333;
const CURRENT_ROUND_NAME = "round338_london_pld_lifecycle_next36";
...
function validate() {
  ...
  for (const candidate of candidates) {
    ...
    assert(!fieldDateKeys.has(fieldKey), `Duplicate source-date key inside round338: ${fieldKey}`);
  ...
  const report = {
    ...
    checks: [
      ...
      "no overlap with PLD lifecycle packs through round333",
      "source/row/API URLs",
