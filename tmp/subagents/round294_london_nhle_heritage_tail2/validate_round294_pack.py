...
ROOT = Path("tmp/subagents/round294_london_nhle_heritage_tail2")
...
def extract_keys(row):
    ...
    for item in [row, source_row]:
        if not isinstance(item, dict):
            continue
        entry = one_line(item.get("ListEntry") or item.get("nhle_list_entry") or "")
        if not entry:
            continue
...
def validate():
    ...
    if pack.get("metadata", {}).get("date_window") != {"start": DATE_START, "end": DATE_END}:
        errors.append("candidates metadata date_window is not the required round294 window.")
    ...
    for index, candidate in enumerate(candidates):
        ...
        if not key or not re.fullmatch(r"\d{6,8}\|[A-Za-z]+\|\d{4}-\d{2}-\d{2}", key):
            ...
            errors.append(f"Duplicate NHLE key within round294: {key}.")
        else:
            nhle_keys.add(key)
    ...
    if duplicate_keys:
        errors.append(f"Round294 candidates duplicate existing NHLE keys: {duplicate_keys[:20]}")
    ...
    result = {
        ...
        "independent_duplicate_scan": {
            ...
            "existing_nhle_keys_seen": len(existing_keys),
    ...
    report_lines = [
        ...
        f"- Existing NHLE keys seen: {len(existing_keys)}",
        "",
