...
const MAX_PRIOR_ROUND = 350;
const CURRENT_ROUND_NAME = "round354_london_pld_lifecycle_next40";
...
function validate() {
  ...
  for (const candidate of candidates) {
    ...
    assert(!fieldDateKeys.has(fieldKey), `Duplicate source-date key inside round354: ${fieldKey}`);
  ...
  const report = {
    ...
    checks: [
      ...
      "no overlap with PLD lifecycle packs through round350",
      "source/row/API URLs",
