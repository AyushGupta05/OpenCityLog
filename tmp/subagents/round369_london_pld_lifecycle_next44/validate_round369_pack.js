...
const MAX_PRIOR_ROUND = 365;
const CURRENT_ROUND_NAME = "round369_london_pld_lifecycle_next44";
...
function validate() {
  ...
  for (const candidate of candidates) {
    ...
    assert(!fieldDateKeys.has(fieldKey), `Duplicate source-date key inside round369: ${fieldKey}`);
  ...
  const report = {
    ...
    checks: [
      ...
      "no overlap with PLD lifecycle packs through round365",
      "source/row/API URLs",
