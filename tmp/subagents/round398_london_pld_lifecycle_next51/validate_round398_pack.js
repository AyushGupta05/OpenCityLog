...
const MAX_PRIOR_ROUND = 393;
const CURRENT_ROUND_NAME = "round398_london_pld_lifecycle_next51";
...
function validate() {
  ...
  for (const candidate of candidates) {
    ...
    assert(!fieldDateKeys.has(fieldKey), `Duplicate source-date key inside round398: ${fieldKey}`);
  ...
  const report = {
    ...
    checks: [
      ...
      "no overlap with PLD lifecycle packs through round393",
      "source/row/API URLs",
