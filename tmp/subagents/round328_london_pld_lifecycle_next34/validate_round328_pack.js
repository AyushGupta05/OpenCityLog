...
const MAX_PRIOR_ROUND = 327;
const CURRENT_ROUND_NAME = "round328_london_pld_lifecycle_next34";
...
function validate() {
  ...
  for (const candidate of candidates) {
    ...
    assert(!fieldDateKeys.has(fieldKey), `Duplicate source-date key inside round328: ${fieldKey}`);
  ...
  const report = {
    ...
    checks: [
      ...
      "no overlap with PLD lifecycle packs through round327",
      "source/row/API URLs",
