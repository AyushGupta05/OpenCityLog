...
const MAX_PRIOR_ROUND = 338;
const CURRENT_ROUND_NAME = "round343_london_pld_lifecycle_next37";
...
function validate() {
  ...
  for (const candidate of candidates) {
    ...
    assert(!fieldDateKeys.has(fieldKey), `Duplicate source-date key inside round343: ${fieldKey}`);
  ...
  const report = {
    ...
    checks: [
      ...
      "no overlap with PLD lifecycle packs through round338",
      "source/row/API URLs",
