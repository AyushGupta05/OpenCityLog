...
const MAX_PRIOR_ROUND = 354;
const CURRENT_ROUND_NAME = "round358_london_pld_lifecycle_next41";
...
function validate() {
  ...
  for (const candidate of candidates) {
    ...
    assert(!fieldDateKeys.has(fieldKey), `Duplicate source-date key inside round358: ${fieldKey}`);
  ...
  const report = {
    ...
    checks: [
      ...
      "no overlap with PLD lifecycle packs through round354",
      "source/row/API URLs",
