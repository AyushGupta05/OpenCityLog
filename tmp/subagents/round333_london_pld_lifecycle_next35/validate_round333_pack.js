...
const MAX_PRIOR_ROUND = 328;
const CURRENT_ROUND_NAME = "round333_london_pld_lifecycle_next35";
...
function validate() {
  ...
  for (const candidate of candidates) {
    ...
    assert(!fieldDateKeys.has(fieldKey), `Duplicate source-date key inside round333: ${fieldKey}`);
  ...
  const report = {
    ...
    checks: [
      ...
      "no overlap with PLD lifecycle packs through round328",
      "source/row/API URLs",
