...
const MAX_PRIOR_ROUND = 373;
const CURRENT_ROUND_NAME = "round377_london_pld_lifecycle_next46";
...
function validate() {
  ...
  for (const candidate of candidates) {
    ...
    assert(!fieldDateKeys.has(fieldKey), `Duplicate source-date key inside round377: ${fieldKey}`);
  ...
  const report = {
    ...
    checks: [
      ...
      "no overlap with PLD lifecycle packs through round373",
      "source/row/API URLs",
