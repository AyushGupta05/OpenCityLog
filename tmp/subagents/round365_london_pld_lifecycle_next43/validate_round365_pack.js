...
const MAX_PRIOR_ROUND = 362;
const CURRENT_ROUND_NAME = "round365_london_pld_lifecycle_next43";
...
function validate() {
  ...
  for (const candidate of candidates) {
    ...
    assert(!fieldDateKeys.has(fieldKey), `Duplicate source-date key inside round365: ${fieldKey}`);
  ...
  const report = {
    ...
    checks: [
      ...
      "no overlap with PLD lifecycle packs through round362",
      "source/row/API URLs",
