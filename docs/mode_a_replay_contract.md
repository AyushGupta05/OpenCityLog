...
The former Mode A replay artifacts are not checked-in atlas data and are not public atlas endpoints. The local server returns `410 Gone` for `/data/mode-a/...`, the legacy-path verifier fails if tracked Mode A files return, and browser smoke tests assert that this retired path stays unavailable.
