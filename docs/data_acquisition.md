...
Architecture-related planning, permit, heritage, public-building and completion/opening sources are frozen separately in `config/architecture_source_inventory.json`. That inventory pins the working window to `2008-01-01` through `2026-05-23`, records API/CSV/portal/docs URLs, licence and redistribution caveats, row id fields, date fields, geometry fields, supported event types, current raw/candidate/generated artifacts, and the next checks for London, New York City, and Belfast priority source families. The methodology is documented in `docs/architecture_methodology.md`. Sync the priority source rows into the general registry and validate them with:
