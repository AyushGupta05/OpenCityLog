...
class EnergyInfrastructureProposalTests(unittest.TestCase):
    def test_energy_proposal_is_screening_context_not_grid_approval(self) -> None:
        script = """
const proposalImpact = require('./lib/proposal-impact');
const result = proposalImpact.assessProposal({
  category: 'energy_infrastructure',
  title: 'Energy infrastructure near York Street',
  description: 'An energy-infrastructure proposal for local connection needs.',
...
"""
        ...
        self.assertFalse(
            any("found in the 2026 grid" in reason for reason in result["confidence"]["reasons"]),
