...
// Legacy scenario-studio module. The core 2026+ product path now uses
// lib/proposal-impact.js for evidence-backed proposal screening.
...
const TRANSFORMER_MODEL_VERSION_FALLBACK = "bims5-transformer-impact-fallback";

const TRANSFORMER_CONFIDENCE_RANK = {
  low: 1,
  medium: 2,
  "medium-high": 3,
  high: 4
...
function roundConcrete(value, places = 0) {
  if (!Number.isFinite(value)) return 0;
  const factor = Math.pow(10, places);
  return Math.round(value * factor) / factor;
}

function forecastStartYear(source = {}, fallback = START_YEAR) {
  const raw = source?.delivery?.startYear ??
    source?.delivery?.start_year ??
    source?.startYear ??
    source?.start_year ??
    source?.commissionYear ??
    source?.commission_year ??
    source?.year ??
    fallback;
  const parsed = Number(raw);
  return Math.max(START_YEAR, Math.min(HORIZON_YEAR, Number.isFinite(parsed) ? parsed : fallback));
}
...
function loadTransformerArtifacts(rootDir) {
  const cacheKey = path.join(rootDir, "web", "data", "mode-a", "transformer_impact_model.json");
  const cached = dataCache.get(`${cacheKey}:transformer-artifacts`);
  if (cached) return cached;
  const model = readJsonIfExists(rootDir, path.join("web", "data", "mode-a", "transformer_impact_model.json"), null);
  const capacityByCell = readJsonIfExists(rootDir, path.join("web", "data", "mode-a", "transformer_capacity_by_cell.json"), null);
  const capacityForecast = readJsonIfExists(rootDir, path.join("web", "data", "mode-a", "transformer_capacity_forecast.json"), null);
  const valid = model?.kind === "belfast.transformerImpactModel" && Array.isArray(model.years);
  const artifacts = {
    valid,
    model: valid ? model : null,
    capacityByCell: capacityByCell?.kind === "belfast.transformerCapacityByCell" ? capacityByCell : null,
    capacityForecast: capacityForecast?.kind === "belfast.transformerCapacityForecast" ? capacityForecast : null,
    warning: valid ? null : "Transformer impact model artifact is missing or invalid; deterministic transformer fallback is being used."
  };
  dataCache.set(`${cacheKey}:transformer-artifacts`, artifacts);
  return artifacts;
}

function canonicalTransformerAssetClass(value) {
  const normalized = String(value || "secondary").toLowerCase().replace(/[-\s]+/g, "_");
  if (normalized.includes("primary")) return "primary";
  return "secondary";
}

function confidenceRank(label) {
  return TRANSFORMER_CONFIDENCE_RANK[String(label || "medium").toLowerCase()] || TRANSFORMER_CONFIDENCE_RANK.medium;
}

function lowerConfidence(a, b) {
  return confidenceRank(a) <= confidenceRank(b) ? (a || "medium") : (b || "medium");
}

function transformerDefault(model, assetClass, key, fallback) {
  return Number(model?.transformerDefaults?.[assetClass]?.[key] ?? fallback);
}

function capacityKvaFromValue(value, unit, assetClass) {
  const number = Number(value);
  if (!Number.isFinite(number) || number <= 0) return null;
  const unitText = String(unit || "").toLowerCase();
  if (unitText.includes("mva")) return number * 1000;
  if (unitText.includes("kva")) return number;
  if (unitText.includes("va")) return number / 1000;
  if (assetClass === "primary" && number < 100) return number * 1000;
  return number;
}

function voltageKvFromValue(value, unit) {
  if (String(value || "").includes(";")) {
    const parts = String(value).split(";").map((part) => voltageKvFromValue(part, unit)).filter(Number.isFinite);
    return parts.length ? Math.max(...parts) : null;
  }
  const number = Number(value);
  if (!Number.isFinite(number) || number <= 0) return null;
  const unitText = String(unit || value || "").toLowerCase();
  if (unitText.includes("kv")) return number;
  if (unitText.includes("v") || number >= 230) return number / 1000;
  return number;
}

function normalizeTransformerIntervention(intervention = {}, model = null) {
  const assetClass = canonicalTransformerAssetClass(intervention.assetClass || intervention.asset_class || intervention.class || intervention.kind);
  const defaultKva = transformerDefault(model, assetClass, "capacityKva", assetClass === "primary" ? 16_000 : 500);
  const defaultVoltage = transformerDefault(model, assetClass, "voltageKv", assetClass === "primary" ? 33 : 11);
  const defaultRadius = transformerDefault(model, assetClass, "serviceRadiusM", assetClass === "primary" ? 2500 : 650);
  const defaultConfidence = model?.transformerDefaults?.[assetClass]?.confidence || "medium";
  const unit = intervention.capacityUnit || intervention.capacity_unit || intervention.ratingUnit || intervention.rating_unit || intervention.nominalRatingUnit;
  const rawCapacity = intervention.capacityKva ?? intervention.capacity_kva ?? intervention.nominalRatingKva ?? intervention.nominal_rating_kva ?? intervention.nominalRating ?? intervention.nominal_rating;
  const capacityKva = capacityKvaFromValue(rawCapacity, unit, assetClass) || defaultKva;
  const voltageKv = voltageKvFromValue(intervention.voltageKv ?? intervention.voltage_kv ?? intervention.voltage, intervention.voltageUnit || intervention.voltage_unit) || defaultVoltage;
  const commissionYear = Math.max(START_YEAR, Math.min(HORIZON_YEAR, Number(intervention.commissionYear || intervention.commission_year || intervention.year || START_YEAR) || START_YEAR));
  const serviceRadiusM = Number(intervention.serviceRadiusM || intervention.service_radius_m || intervention.radiusM || intervention.radius_m || defaultRadius);
  return {
    ...intervention,
    type: "transformer",
    assetClass,
    ...
    commissionYear,
    serviceRadiusM,
    radiusM: serviceRadiusM,
    confidence: String(intervention.confidence || defaultConfidence)
  };
}

function transformerUsableKw(intervention, model = null) {
  const assetClass = canonicalTransformerAssetClass(intervention.assetClass);
  const conversion = model?.conversion || {};
  const powerFactor = Number(conversion.powerFactor ?? 0.9);
  const usableShare = assetClass === "primary"
    ? Number(conversion.primaryUsableShare ?? 0.46)
    : Number(conversion.secondaryUsableShare ?? 0.58);
  return Math.max(0, Number(intervention.capacityKva || 0) * powerFactor * usableShare);
}

function transformerUncertaintyBand(value, confidence = "medium", model = null) {
  const uncertainty = model?.uncertainty || {};
  const p10Multiplier = Number(uncertainty.p10Multiplier ?? 0.62);
  const p90Multiplier = Number(uncertainty.p90Multiplier ?? 1.38);
  const widening = confidenceRank(confidence) <= 1
    ? Number(uncertainty.lowConfidenceWidening ?? 0.28)
    : confidenceRank(confidence) === 2
      ? Number(uncertainty.mediumConfidenceWidening ?? 0.14)
      : 0;
  const magnitude = Math.abs(Number(value) || 0);
  const lowerMagnitude = magnitude * Math.max(0, p10Multiplier - widening);
  const upperMagnitude = magnitude * (p90Multiplier + widening);
  const sign = Number(value) < 0 ? -1 : 1;
  const p10 = sign < 0 ? -upperMagnitude : lowerMagnitude;
  const p90 = sign < 0 ? -lowerMagnitude : upperMagnitude;
  return {
    p10: roundConcrete(p10, 2),
    p50: roundConcrete(value, 2),
    p90: roundConcrete(p90, 2)
  };
}

function fallbackTransformerPlannerImpact(context, intervention, year, distanceM) {
  const radiusM = Number(intervention.serviceRadiusM || intervention.radiusM || intervention.radius_m || 650);
  const ramp = operationRamp(year, Number(intervention.commissionYear || START_YEAR), HORIZON_YEAR);
  const w = plannerWeight(distanceM, radiusM) * ramp;
  if (!w) return null;
  const deprivation = clamp(Number(context.cell?.deprivationWeight || 0));
  return {
    weight: w,
    electricityIndexDelta: -0.16 * w,
    servicesDelta: 0.03 * w,
    economyDelta: 0.02 * w,
    jobsDelta: 0.01 * w,
    environmentAirDelta: 0.006 * w,
    fairnessDelta: 0.025 * Math.max(0.25, deprivation) * w,
    fiscalBalanceDelta: -0.014 * w,
    planningViabilityDelta: 0.04 * w,
    transformerReliefKw: 900 * w,
    overloadRiskDelta: -0.08 * w,
    loadIndexDelta: -0.16 * w,
    confidence: "medium",
    modelBacked: false,
    warning: "Deterministic transformer fallback used."
  };
}

function transformerPlannerImpact(context, intervention, year, distanceM, rootDir) {
  const artifacts = loadTransformerArtifacts(rootDir);
  const model = artifacts.model;
  const normalized = normalizeTransformerIntervention(intervention, model);
  const fallback = () => fallbackTransformerPlannerImpact(context, normalized, year, distanceM);
  if (!artifacts.valid || !model) return fallback();
  const ramp = operationRamp(year, normalized.commissionYear, HORIZON_YEAR);
  const w = plannerWeight(distanceM, normalized.serviceRadiusM) * ramp;
  if (!w) return null;
  const cellFeature = model.cellFeatures?.[context.cell?.id] || artifacts.capacityByCell?.cells?.[context.cell?.id] || null;
  if (!cellFeature) return fallback();
  const coeff = model.coefficients || {};
  const deprivation = clamp(Number(context.cell?.deprivationWeight || 0));
  const electricityPressure = clamp(Number(context.cell?.baseline2025?.electricity ?? context.cell?.forecast2036?.electricity ?? 0.45));
  const dataSupportScore = clamp(Number(cellFeature.dataSupportScore ?? 0.5));
  const usableKw = transformerUsableKw(normalized, model);
  const localReliefKw = usableKw * w;
  const demandSensitivity = clamp(0.72 + electricityPressure * 0.52 + dataSupportScore * 0.12, 0.6, 1.25);
  const electricityIndexDelta = -localReliefKw / Number(coeff.electricityIndexKwScale || 4800) * demandSensitivity;
  const overloadRiskDelta = -localReliefKw / Number(coeff.headroomRiskKwScale || 5500) * (0.72 + electricityPressure * 0.55);
  const loadIndexDelta = -localReliefKw / Number(coeff.loadIndexKwScale || 4200) * demandSensitivity;
  const confidence = lowerConfidence(normalized.confidence, cellFeature.confidence || "medium");
  return {
    weight: w,
    electricityIndexDelta,
    servicesDelta: localReliefKw * Number(coeff.servicesUpliftPerKw || 0.000018) * Math.max(0.35, dataSupportScore),
    economyDelta: localReliefKw * Number(coeff.economyUpliftPerKw || 0.000012) * Math.max(0.35, dataSupportScore),
    jobsDelta: localReliefKw * Number(coeff.jobsUpliftPerKw || 0.00001) * Math.max(0.25, dataSupportScore),
    environmentAirDelta: 0.0025 * w,
    fairnessDelta: 0.018 * Math.max(0.25, deprivation) * w,
    fiscalBalanceDelta: -0.010 * w * (normalized.assetClass === "primary" ? 1.35 : 1),
    planningViabilityDelta: Math.min(0.052 * w, Math.abs(loadIndexDelta) * 0.72),
    transformerReliefKw: localReliefKw,
    overloadRiskDelta,
    loadIndexDelta,
    confidence,
    modelBacked: true,
    modelVersion: model.modelVersion || TRANSFORMER_MODEL_VERSION_FALLBACK
  };
}
...
function validatePlacement(payload = {}, rootDir = process.cwd()) {
  ...
  if (!payload.allowExistingBuildingOverlap && !payload.allow_existing_building_overlap && intersectsLayerAtPointOrBbox(data.buildings.features, point, footprintBbox, 0.00001)) {
    invalidReasons.push("Overlaps existing buildings");
  }
  if (!payload.allowRoadProximity && !payload.allow_road_proximity && closestFeature(data.majorRoads.features, point, 18).feature) {
...
function createBuildingIntervention(payload = {}, rootDir = process.cwd()) {
  const rawType = payload.type || payload.interventionType || payload.intervention_type;
  const isRemoval = rawType === "building_removal" || payload.removal === true || payload.remove === true;
  const resolvedPostcode = payload.resolvedPostcode || payload.postcodeResolution || (payload.postcode ? resolvePostcode(payload.postcode, rootDir) : null);
  ...
  const validation = payload.validation || validatePlacement({
    ...
    allowExistingBuildingOverlap: isRemoval,
    allowRoadProximity: isRemoval
  }, rootDir);
  const id = payload.id || `building-${Date.now().toString(36)}`;
  const startYear = forecastStartYear(payload, START_YEAR);
  const completionYear = Math.max(startYear, Math.min(HORIZON_YEAR, Number(payload.delivery?.completionYear || payload.delivery?.completion_year || payload.completionYear || payload.completion_year || HORIZON_YEAR) || HORIZON_YEAR));
  return {
    id,
    scenarioId: payload.scenarioId || payload.scenario_id || "housing_growth",
    interventionType: isRemoval ? "building_removal" : "building",
    type: isRemoval ? "building_removal" : "building",
    removal: isRemoval,
    year: startYear,
    startYear,
    existingBuildingId: payload.existingBuildingId || payload.existing_building_id || null,
    existingBuildingName: payload.existingBuildingName || payload.existing_building_name || null,
...
function buildingInterventionFrom(baseBuilding, overrides = {}) {
  ...
  const baseType = baseBuilding.type || baseBuilding.interventionType || "building";
  const type = overrides.type || overrides.interventionType || overrides.intervention_type || baseType;
  const startYear = forecastStartYear(overrides, forecastStartYear(baseBuilding, START_YEAR));
  const completionYear = Math.max(startYear, Math.min(HORIZON_YEAR, Number(overrides.delivery?.completionYear || overrides.delivery?.completion_year || baseBuilding.delivery?.completionYear || HORIZON_YEAR) || HORIZON_YEAR));
  return {
    id: overrides.id || baseBuilding.id,
    type,
    interventionType: type,
    removal: type === "building_removal" || Boolean(baseBuilding.removal),
    year: startYear,
    startYear,
    existingBuildingId: overrides.existingBuildingId || baseBuilding.existingBuildingId,
    existingBuildingName: overrides.existingBuildingName || baseBuilding.existingBuildingName,
...
function locationFromPointLike(value) {
...
function sanitizeCoordinatePath(path) {
  if (!Array.isArray(path)) return [];
  return path
    .map((coord) => {
      const location = locationFromPointLike(coord);
      return location ? [roundCoord(location.lng), roundCoord(location.lat)] : null;
    })
    .filter(Boolean);
}

function locationFromPath(path) {
  const coords = sanitizeCoordinatePath(path);
  if (!coords.length) return null;
  const midpoint = coords.reduce((sum, coord) => [sum[0] + coord[0], sum[1] + coord[1]], [0, 0]);
  return { lng: roundCoord(midpoint[0] / coords.length), lat: roundCoord(midpoint[1] / coords.length) };
}
...
function pathLengthMeters(path) {
  ...
  for (let index = 1; index < coords.length; index += 1) {
    length += haversineMeters(coords[index - 1], coords[index]);
  }
  return Math.round(length);
}

function locationForIntervention(rawIntervention = {}, fallbackLocation = null) {
  return locationFromPointLike(rawIntervention.location) ||
    locationFromPointLike(rawIntervention) ||
    locationFromPath(rawIntervention.path) ||
    locationFromPointLike(rawIntervention.start) ||
    fallbackLocation;
}

function normalizeScenarioIntervention(rawIntervention = {}, baseBuilding, rootDir = process.cwd()) {
  const rawType = rawIntervention.type || rawIntervention.interventionType || rawIntervention.intervention_type;
  const type = rawType === "infrastructure" ? "transformer" : rawType;
  if (type === "building") return buildingInterventionFrom(baseBuilding, rawIntervention);
  if (type === "building_removal") return buildingInterventionFrom(baseBuilding, { ...rawIntervention, type: "building_removal", interventionType: "building_removal", removal: true });
  if (type === "mobility_corridor" || type === "green_corridor" || type === "opportunity_hub") {
    return {
      ...rawIntervention,
      type,
      location: locationForIntervention(rawIntervention, baseBuilding.location),
  ...
  if (type === "road" || type === "road_corridor") {
    const path = sanitizeCoordinatePath(rawIntervention.path);
    const start = locationFromPointLike(rawIntervention.start) || (path.length ? locationFromPointLike(path[0]) : null);
    const end = locationFromPointLike(rawIntervention.end) || (path.length ? locationFromPointLike(path[path.length - 1]) : null);
    const route = path.length >= 2 ? path : [start, end].filter(Boolean).map((location) => [roundCoord(location.lng), roundCoord(location.lat)]);
    if (route.length < 2) return null;
    const lengthM = Number(rawIntervention.lengthM || rawIntervention.length_m || pathLengthMeters(route));
    return {
      ...rawIntervention,
      type: "road",
      path: route,
      ...
      location: locationForIntervention(rawIntervention, locationFromPath(route) || baseBuilding.location),
      radiusM: Number(rawIntervention.radiusM || rawIntervention.radius_m || clamp(lengthM / 2, 450, 1200)),
      lengthM,
      mode: rawIntervention.mode || "road_capacity",
      rationale: rawIntervention.rationale || "User-staged road is included as a deterministic mobility intervention."
    };
  }
  if (type === "transformer") {
    const location = locationForIntervention(rawIntervention, baseBuilding.location);
    if (!location) return null;
    const normalized = normalizeTransformerIntervention(rawIntervention, loadTransformerArtifacts(rootDir).model);
    return {
      ...normalized,
      type: "transformer",
      location,
      serviceRadiusM: normalized.serviceRadiusM,
      radiusM: normalized.serviceRadiusM,
      rationale: rawIntervention.rationale || "User-staged transformer is included as a model-backed energy-capacity intervention."
    };
  }
  return null;
}

function interventionIdentity(intervention = {}) {
  if (intervention.id) return `${intervention.type}:${intervention.id}`;
  const location = locationFromPointLike(intervention.location);
  return `${intervention.type}:${location ? `${roundCoord(location.lng, 5)},${roundCoord(location.lat, 5)}` : intervention.rationale || ""}`;
}

function mergeUserInterventionsIntoVariants(variants = [], userInterventions = []) {
  if (!userInterventions.length) return variants;
  return variants.map((variant) => {
    const interventions = [...(variant.interventions || [])];
    const seen = new Set(interventions.map(interventionIdentity));
    for (const intervention of userInterventions) {
      const key = interventionIdentity(intervention);
    ...
    return {
      ...
      assumptions: [
        ...(variant.assumptions || []),
        "User-staged branch infrastructure is included in the deterministic forecast."
      ].slice(0, 6)
    };
  });
}

function extractUserInterventions(input = {}, baseBuilding, rootDir = process.cwd()) {
  const rawList = Array.isArray(input.interventions)
    ? input.interventions
    : Array.isArray(input.branchItems)
      ? input.branchItems
      : Array.isArray(input.items)
        ? input.items
        : [];
  return rawList
    .map((item) => normalizeScenarioIntervention(item, baseBuilding, rootDir))
    .filter(Boolean);
}
...
function sanitizeScenarioVariants(variants, baseBuilding, rootDir = process.cwd(), options = {}) {
  ...
  const allowedTypes = new Set(["building", "building_removal", "mobility_corridor", "green_corridor", "opportunity_hub", "road", "road_corridor", "transformer", "infrastructure"]);
  const sanitized = [];
  ...
  for (const item of list.slice(0, 6)) {
    ...
    for (const rawIntervention of item.interventions || []) {
      ...
      const normalized = normalizeScenarioIntervention(rawIntervention, baseBuilding, rootDir);
      if (normalized) interventions.push(normalized);
    }
...
function simulateVariant(variant, baseCells, contextIds, baselineMetrics, baseBuilding, rootDir) {
  ...
  for (const intervention of variant.interventions || []) {
    ...
    } else if (type === "building_removal") {
      const building = createBuildingIntervention({
        ...baseBuilding,
        ...intervention,
        type: "building_removal",
        interventionType: "building_removal",
        removal: true
      }, rootDir);
      applyBuildingRemovalToCells(cells, building);
      agentNotes.push({
        agent: "Population Agent",
        risk: "medium",
        reason: `Removes about ${building.config.estimatedResidents} residents and ${building.config.estimatedJobs} jobs as a proxy estimate.`
      });
...
function applyBuildingRemovalToCells(cells, building) {
  const radiusM = 800;
  const residents = building.config.estimatedResidents;
  const jobs = building.config.estimatedJobs;
  const electricity = building.config.estimatedElectricityDemand;
  ...
  for (const cell of cells) {
    ...
    const w = distanceWeight(distanceM, radiusM);
    if (!w) continue;
    cell.values.populationPressure -= 0.1 * popImpact * w;
    cell.values.mobilityStrain -= 0.075 * popImpact * w;
    cell.values.roadPressure -= 0.065 * popImpact * w;
    cell.values.electricityDemand -= 0.095 * energyImpact * w;
    cell.values.economicOpportunity -= 0.09 * jobImpact * w;
    cell.values.jobAccess -= 0.075 * jobImpact * w;
    cell.values.servicePressure -= 0.035 * popImpact * w;
    cell.values.greenScore += 0.025 * w;
    clampCellValues(cell.values);
  }
}
...
function applyBuildingPlanner(values, context, building, siteContext, year, distanceM) {
  ...
  const ramp = operationRamp(year, forecastStartYear(building, START_YEAR), HORIZON_YEAR);
...
function applyBuildingRemovalPlanner(values, context, building, siteContext, year, distanceM) {
  const s = plannerScalars(building, siteContext);
  const ramp = operationRamp(year, forecastStartYear(building, START_YEAR), HORIZON_YEAR);
  const w = plannerWeight(distanceM, 850) * ramp;
  if (!w) return;
  ...
  const serviceDemand = s.residentsN * 0.055 + s.jobsN * 0.018;
  const localRevenue = s.jobsN * 0.04 + s.unitsN * 0.018 + commercialShare * 0.026;
  const avoidedInfra = s.electricityN * 0.014 + tripLoad * 0.01;

  addMetricDelta(values, "population", -0.092 * s.residentsN * residentialShare * w);
  addMetricDelta(values, "traffic", -0.058 * tripLoad * w);
  addMetricDelta(values, "jobs", -(0.096 * s.jobsN + commercialShare * 0.028 + communityShare * 0.01) * w);
  addMetricDelta(values, "economy", -(0.082 * s.jobsN + 0.018 * s.unitsN + commercialShare * 0.024) * w);
  addMetricDelta(values, "housingPressure", (0.072 * s.unitsN * (0.55 + affordabilityRatio) - 0.018 * s.residentsN) * w);
  addMetricDelta(values, "services", (serviceDemand - communityShare * 0.052) * w);
  addMetricDelta(values, "electricity", -0.09 * s.electricityN * s.energyFactor * w);
  addMetricDelta(values, "environmentAir", -(0.032 * tripLoad + 0.036 * s.electricityN * s.energyFactor) * w);
  addMetricDelta(values, "greenScore", 0.024 * s.footprintN * w);
  addMetricDelta(values, "fairness", -0.035 * Math.max(0.25, deprivation) * (s.unitsN + affordabilityRatio * 0.45) * w);
  addMetricDelta(values, "fiscalBalance", (avoidedInfra - localRevenue * 0.72) * w);
  addMetricDelta(values, "planningViability", 0.012 * w);
}

function applyMobilityPlanner(values, context, intervention, year, distanceM) {
  const radiusM = Number(intervention.radiusM || intervention.radius_m || 700);
  const ramp = operationRamp(year, forecastStartYear(intervention, START_YEAR), HORIZON_YEAR);
  const w = plannerWeight(distanceM, radiusM) * ramp;
  if (!w) return;
...
function applyRoadPlanner(values, context, intervention, year, distanceM) {
  const lengthM = Number(intervention.lengthM || intervention.length_m || pathLengthMeters(intervention.path || []));
  const radiusM = Number(intervention.radiusM || intervention.radius_m || clamp(lengthM / 2, 500, 1200));
  const ramp = operationRamp(year, forecastStartYear(intervention, START_YEAR), HORIZON_YEAR);
  const w = plannerWeight(distanceM, radiusM) * ramp;
  if (!w) return;
  const deprivation = clamp(Number(context.cell?.deprivationWeight || 0));
  const lengthN = normalize(lengthM, 80, 1800);
  const inducedDemand = 0.018 + lengthN * 0.018;
  const localRelief = 0.064 - inducedDemand;
  addMetricDelta(values, "traffic", -localRelief * w);
  addMetricDelta(values, "jobs", (0.012 + lengthN * 0.01) * w);
  addMetricDelta(values, "economy", (0.018 + lengthN * 0.012) * w);
  addMetricDelta(values, "services", 0.01 * w);
  addMetricDelta(values, "electricity", (0.004 + lengthN * 0.003) * w);
  addMetricDelta(values, "environmentAir", (0.024 + inducedDemand) * w);
  addMetricDelta(values, "greenScore", -(0.014 + lengthN * 0.012) * w);
  addMetricDelta(values, "fairness", -(0.026 + lengthN * 0.018) * Math.max(0.35, deprivation) * w);
  addMetricDelta(values, "fiscalBalance", -(0.014 + lengthN * 0.024) * w);
  addMetricDelta(values, "planningViability", -(0.012 + lengthN * 0.014) * w);
}

function applyTransformerPlanner(values, context, intervention, year, distanceM, rootDir) {
  const impact = transformerPlannerImpact(context, intervention, year, distanceM, rootDir);
  if (!impact) return;
  addMetricDelta(values, "electricity", impact.electricityIndexDelta);
  addMetricDelta(values, "services", impact.servicesDelta);
  addMetricDelta(values, "economy", impact.economyDelta);
  addMetricDelta(values, "jobs", impact.jobsDelta);
  addMetricDelta(values, "environmentAir", impact.environmentAirDelta);
  addMetricDelta(values, "fairness", impact.fairnessDelta);
  addMetricDelta(values, "fiscalBalance", impact.fiscalBalanceDelta);
  addMetricDelta(values, "planningViability", impact.planningViabilityDelta);
}
  ...
  for (const intervention of variant.interventions || []) {
    ...
    for (const item of contextRows) {
      ...
      if (type === "building") {
        ...
        applyBuildingRemovalPlanner(values, item, building, siteContext, year, distanceM);
      } else if (type === "mobility_corridor") {
        ...
        applyRoadPlanner(values, item, intervention, year, distanceM);
      } else if (type === "transformer" || type === "infrastructure") {
        applyTransformerPlanner(values, item, intervention, year, distanceM, rootDir);
      }
...
function branchAgentNotes(variant, building, siteContext) {
  ...
  if ((variant.interventions || []).some((item) => {
    const type = item.type || item.interventionType || item.intervention_type;
    return type === "road" || type === "road_corridor";
  })) {
    notes.push({ agent: "Mobility Agent", risk: "medium-high", reason: "User-staged roads affect congestion, access, environment, fiscal and fairness proxies through deterministic road planning." });
  }
  if ((variant.interventions || []).some((item) => {
    const type = item.type || item.interventionType || item.intervention_type;
    return type === "transformer" || type === "infrastructure";
  })) {
    notes.push({ agent: "Energy Agent", risk: "medium", reason: "User-staged transformers use model-backed capacity relief, headroom, overload-risk and confidence-band screening when the artifact is available." });
  }
  return notes;
}

function concreteImpactZero(year) {
  return {
    year,
    modelBasis: "2025 no-build forecast",
    estimateType: "baseline",
    traffic: {
      dailyTripsAdded: 0,
      dailyTripsReducedByRoad: 0,
      inducedDailyTrips: 0,
      netDailyTrips: 0,
      peakHourVehicleChange: 0,
      congestionIndexDelta: 0,
      delayMinutesPerPeakHourChange: 0,
      method: "no-build baseline"
    },
    jobs: {
      directJobs: 0,
      accessibilitySupportedJobs: 0,
      temporaryConstructionJobs: 0,
      operationsJobs: 0,
      capacityEnabledJobs: 0,
      netJobsEstimate: 0,
      jobsIndexDelta: 0,
      confidence: "medium",
      p10: { netJobsEstimate: 0 },
      p50: { netJobsEstimate: 0 },
      p90: { netJobsEstimate: 0 },
      method: "no-build baseline"
    },
    electricity: {
      annualMwhChange: 0,
      peakKwChange: 0,
      transformerReliefKw: 0,
      localCapacityHeadroomKwChange: 0,
      overloadRiskDelta: 0,
      loadIndexDelta: 0,
      localLoadIndexDelta: 0,
      confidence: "medium",
      p10: { peakKwChange: 0, transformerReliefKw: 0, localCapacityHeadroomKwChange: 0 },
      p50: { peakKwChange: 0, transformerReliefKw: 0, localCapacityHeadroomKwChange: 0 },
      p90: { peakKwChange: 0, transformerReliefKw: 0, localCapacityHeadroomKwChange: 0 },
      method: "no-build baseline"
    },
    services: {
      residentServiceDemand: 0,
      workerServiceDemand: 0,
      serviceCapacityEquivalent: 0,
      netServiceDemand: 0,
      serviceIndexDelta: 0,
      method: "no-build baseline"
    },
    evidence: ["web/data/mode-a/baseline_2025_forecast.json"]
  };
}

function interventionType(intervention = {}) {
  const type = intervention.type || intervention.interventionType || intervention.intervention_type;
  return type === "infrastructure" ? "transformer" : type;
}

function buildingStatsFromVariant(variant, baseBuilding) {
  const buildingInterventions = (variant.interventions || []).filter((item) => {
    const type = interventionType(item);
    return type === "building" || type === "building_removal";
  });
  const sources = buildingInterventions.length ? buildingInterventions : [baseBuilding];
  return sources.map((item) => {
    const overrideConfig = item.config || {};
    const config = {
      ...baseBuilding.config,
      ...overrideConfig,
      size: item.size || overrideConfig.size || baseBuilding.config.size,
      buildingType: item.buildingType || item.building_type || overrideConfig.buildingType || baseBuilding.config.buildingType,
      affordabilityMix: item.affordabilityMix || item.affordability_mix || overrideConfig.affordabilityMix || baseBuilding.config.affordabilityMix,
      energyStandard: item.energyStandard || overrideConfig.energyStandard || baseBuilding.config.energyStandard,
      parkingTransitAssumption: item.parkingTransitAssumption || overrideConfig.parkingTransitAssumption || baseBuilding.config.parkingTransitAssumption,
      mitigation: {
        ...(baseBuilding.config.mitigation || {}),
        ...(overrideConfig.mitigation || {})
      }
    };
    const stats = deriveBuildingStats(config);
    stats.removal = interventionType(item) === "building_removal" || item.removal === true;
    return stats;
  });
}

function sumBuildingStats(stats = []) {
  return stats.reduce((sum, item) => {
    const type = TYPE_PRESETS[item.buildingType] || TYPE_PRESETS.apartments;
    const grossFloorAreaSqm = Number(item.grossFloorAreaSqm || item.footprintSqm * item.floors || 0);
    const sign = item.removal ? -1 : 1;
    sum.units += sign * Number(item.units || 0);
    sum.estimatedResidents += sign * Number(item.estimatedResidents || 0);
    sum.estimatedJobs += sign * Number(item.estimatedJobs || 0);
    sum.estimatedElectricityDemand += sign * Number(item.estimatedElectricityDemand || 0);
    sum.grossFloorAreaSqm += sign * grossFloorAreaSqm;
    sum.communityAreaSqm += sign * grossFloorAreaSqm * Number(type.communityShare || 0);
    sum.commercialAreaSqm += sign * grossFloorAreaSqm * Number(type.commercialShare || 0);
    sum.residentialAreaSqm += sign * grossFloorAreaSqm * Number(type.residentialShare || 0);
    sum.energyFactor = Math.min(sum.energyFactor, item.energyStandard === "low_energy" || item.energyStandard === "passive" ? 0.68 : item.energyStandard === "net_zero_ready" ? 0.52 : 1);
    if (item.parkingTransitAssumption === "parking_heavy") sum.parkingFactor = Math.max(sum.parkingFactor, 1.22);
    if (item.parkingTransitAssumption === "transit_first") sum.parkingFactor = Math.min(sum.parkingFactor, 0.82);
    if (item.mitigation?.mobility) sum.mobilityMitigation = true;
    if (item.mitigation?.energy) sum.energyMitigation = true;
    return sum;
  }, {
    units: 0,
    estimatedResidents: 0,
    estimatedJobs: 0,
    estimatedElectricityDemand: 0,
    grossFloorAreaSqm: 0,
    ...
    parkingFactor: 1,
    energyFactor: 1,
    mobilityMitigation: false,
    energyMitigation: false
  });
}

function roadConcreteStats(interventions = [], year, baselineTraffic = 0) {
  let count = 0;
  let lengthM = 0;
  let capacityDailyTrips = 0;
  let inducedDailyTrips = 0;
  let dailyTripRelief = 0;
  let accessJobs = 0;
  let serviceCapacityEquivalent = 0;
  let annualLightingMwh = 0;
  for (const intervention of interventions) {
    const type = interventionType(intervention);
    if (type !== "road" && type !== "road_corridor") continue;
    const ramp = operationRamp(year, forecastStartYear(intervention, START_YEAR), HORIZON_YEAR);
    const itemLengthM = Number(intervention.lengthM || intervention.length_m || pathLengthMeters(intervention.path || []));
    const itemLengthKm = itemLengthM / 1000;
    const itemCapacityDailyTrips = itemLengthKm * 1800 * ramp;
    count += 1;
    lengthM += itemLengthM;
    capacityDailyTrips += itemCapacityDailyTrips;
    inducedDailyTrips += itemCapacityDailyTrips * clamp(0.24 + Number(baselineTraffic || 0) * 0.22, 0.2, 0.42);
    dailyTripRelief += itemCapacityDailyTrips * 0.58;
    accessJobs += itemLengthKm * 28 * ramp;
    serviceCapacityEquivalent += itemLengthKm * 38 * ramp;
    annualLightingMwh += itemLengthKm * 5.5 * ramp;
  }
  const lengthKm = lengthM / 1000;
  return {
    count,
    lengthM: roundConcrete(lengthM, 1),
    lengthKm,
    capacityDailyTrips,
    inducedDailyTrips,
    dailyTripRelief,
    accessJobs,
    serviceCapacityEquivalent,
    annualLightingMwh
  };
}

function transformerConcreteStats(interventions = [], year, rootDir = process.cwd()) {
  const artifacts = loadTransformerArtifacts(rootDir);
  const model = artifacts.model;
  const coeff = model?.coefficients || {};
  const stats = {
    ...
    overloadRiskDelta: 0,
    loadIndexDelta: 0,
    confidence: artifacts.valid ? "medium" : "medium",
    modelBacked: Boolean(artifacts.valid),
    modelVersion: model?.modelVersion || TRANSFORMER_MODEL_VERSION_FALLBACK,
    warnings: artifacts.warning ? [artifacts.warning] : []
  };
  for (const intervention of interventions || []) {
    if (interventionType(intervention) !== "transformer") continue;
    const normalized = normalizeTransformerIntervention(intervention, model);
    const ramp = operationRamp(year, normalized.commissionYear, HORIZON_YEAR);
    const capacityKw = transformerUsableKw(normalized, model);
    const reliefKw = capacityKw * ramp;
    const isConstructionYear = year === normalized.commissionYear;
    const constructionJobs = normalized.assetClass === "primary"
      ? Number(coeff.constructionJobsPerPrimary || 8)
      : Number(coeff.constructionJobsPerSecondary || 0.8);
    const operationsJobs = normalized.assetClass === "primary"
      ? Number(coeff.operationsJobsPerPrimary || 1.4)
      : Number(coeff.operationsJobsPerSecondary || 0.18);
    stats.count += 1;
    stats.transformerReliefKw += reliefKw;
    stats.serviceCapacityEquivalent += (normalized.assetClass === "primary" ? 95 : 35) * ramp;
    stats.temporaryConstructionJobs += isConstructionYear ? constructionJobs : 0;
    stats.operationsJobs += operationsJobs * ramp;
    stats.overloadRiskDelta += -reliefKw / Number(coeff.headroomRiskKwScale || 5500);
    stats.loadIndexDelta += -reliefKw / Number(coeff.loadIndexKwScale || 4200);
    stats.confidence = lowerConfidence(stats.confidence, normalized.confidence || "medium");
  }
  return stats;
}

function variantStartYear(variant, baseBuilding) {
  const starts = (variant?.interventions || [])
    .map((item) => forecastStartYear(item, forecastStartYear(baseBuilding, START_YEAR)))
    .filter(Number.isFinite);
  if (!starts.length) starts.push(forecastStartYear(baseBuilding, START_YEAR));
  return Math.min(...starts);
}

function concreteImpactsForVariant(variant, baseBuilding, siteContext, year, diffFromBaseline, baselineMetrics, rootDir = process.cwd()) {
  const ramp = operationRamp(year, variantStartYear(variant, baseBuilding), HORIZON_YEAR);
  const buildingStats = sumBuildingStats(buildingStatsFromVariant(variant, baseBuilding));
  const interventions = variant.interventions || [];
  const hasBuildingRemoval = interventions.some((item) => interventionType(item) === "building_removal" || item.removal === true);
  const hasBuildingAddition = interventions.some((item) => interventionType(item) === "building");
  const roadStats = roadConcreteStats(interventions, year, baselineMetrics?.traffic);
  const transformerStats = transformerConcreteStats(interventions, year, rootDir);
  const hasMobilityCorridor = interventions.some((item) => interventionType(item) === "mobility_corridor");
  const hasOpportunityHub = interventions.some((item) => interventionType(item) === "opportunity_hub");
  const transitGood = Number(siteContext?.nearbyTransport?.distanceM || 9999) <= 500 ||
    Number(siteContext?.baselineMetrics?.transportAccess || 0) >= 0.42;
  const parkingFactor = transitGood ? Math.min(buildingStats.parkingFactor, 0.9) : buildingStats.parkingFactor;
  const mobilityFactor = hasMobilityCorridor || buildingStats.mobilityMitigation ? 0.82 : 1;
  const energyFactor = buildingStats.energyMitigation ? Math.min(buildingStats.energyFactor, 0.68) : buildingStats.energyFactor;
  const grossDailyTrips = (
    buildingStats.estimatedResidents * 2.35 +
    buildingStats.estimatedJobs * 1.65 +
    buildingStats.units * 0.35
  ) * parkingFactor * mobilityFactor * ramp;
  const forecastTrafficSignal = Number(diffFromBaseline.traffic || 0) * 2400;
  let netDailyTrips = grossDailyTrips + roadStats.inducedDailyTrips - roadStats.dailyTripRelief + forecastTrafficSignal * 0.25;
  if (hasBuildingRemoval && !hasBuildingAddition) {
    netDailyTrips = Math.min(0, netDailyTrips);
  }
  const peakHourVehicleChange = netDailyTrips * 0.11;
  const trafficIndexDelta = hasBuildingRemoval && !hasBuildingAddition
    ? Math.min(0, Number(diffFromBaseline.traffic || 0))
    : Number(diffFromBaseline.traffic || 0);
  const delayMinutesPerPeakHourChange = trafficIndexDelta * 160;
  const directJobs = buildingStats.estimatedJobs * ramp;
  const opportunityJobs = hasOpportunityHub ? Math.max(18, buildingStats.commercialAreaSqm / 90) * ramp : 0;
  const forecastSupportedJobs = Math.max(0, Number(diffFromBaseline.jobs || 0) * 900);
  const capacityJobsDemand = clamp(
    buildingStats.commercialAreaSqm / 45_000 +
      buildingStats.estimatedJobs / 1200 +
      Number(baselineMetrics?.jobs || 0) * 0.35 +
      Number(baselineMetrics?.economy || 0) * 0.25 +
      (hasOpportunityHub ? 0.18 : 0),
    0,
    1
  );
  const capacityEnabledJobs = transformerStats.transformerReliefKw * Number(loadTransformerArtifacts(rootDir).model?.coefficients?.capacityEnabledJobsPerHeadroomKw || 0.01) * capacityJobsDemand;
  const accessibilitySupportedJobs = roadStats.accessJobs + opportunityJobs + transformerStats.operationsJobs + capacityEnabledJobs + forecastSupportedJobs * 0.25;
  const annualMwhChange = (buildingStats.estimatedElectricityDemand * energyFactor + roadStats.annualLightingMwh) * ramp;
  const peakKwChange = annualMwhChange * 1000 / (8760 * 0.42);
  const residentServiceDemand = buildingStats.estimatedResidents * ramp;
  const workerServiceDemand = buildingStats.estimatedJobs * 0.22 * ramp;
  const forecastServiceEquivalent = Number(diffFromBaseline.services || 0) * 600;
  const serviceCapacityEquivalent = buildingStats.communityAreaSqm * 0.045 * ramp +
    roadStats.serviceCapacityEquivalent +
    transformerStats.serviceCapacityEquivalent +
    Math.max(0, forecastServiceEquivalent) * 0.35;
  const netServiceDemand = residentServiceDemand + workerServiceDemand - serviceCapacityEquivalent - Math.min(0, forecastServiceEquivalent) * 0.25;

  return {
    year,
    modelBasis: transformerStats.modelBacked
      ? "2016-2026 transformer impact artifact plus trained forecast planners"
      : "2016-2025 trained forecast artifact plus deterministic planners",
    estimateType: "planning estimate",
    traffic: {
      dailyTripsAdded: roundConcrete(grossDailyTrips, 0),
      dailyTripsReducedByRoad: roundConcrete(roadStats.dailyTripRelief, 0),
      inducedDailyTrips: roundConcrete(roadStats.inducedDailyTrips, 0),
      netDailyTrips: roundConcrete(netDailyTrips, 0),
      peakHourVehicleChange: roundConcrete(peakHourVehicleChange, 0),
      congestionIndexDelta: round(trafficIndexDelta, 3),
      delayMinutesPerPeakHourChange: roundConcrete(delayMinutesPerPeakHourChange, 1),
      roadLengthM: roundConcrete(roadStats.lengthM, 1),
      method: hasBuildingRemoval && !hasBuildingAddition
        ? "removed-building trip relief + road capacity relief + forecast traffic delta capped at no added traffic"
        : "trip generation + road capacity relief + induced demand + forecast traffic delta"
    },
    jobs: {
      directJobs: roundConcrete(directJobs, 0),
      accessibilitySupportedJobs: roundConcrete(accessibilitySupportedJobs, 0),
      temporaryConstructionJobs: roundConcrete(transformerStats.temporaryConstructionJobs, 1),
      operationsJobs: roundConcrete(transformerStats.operationsJobs, 1),
      capacityEnabledJobs: roundConcrete(capacityEnabledJobs, 0),
      netJobsEstimate: roundConcrete(directJobs + accessibilitySupportedJobs, 0),
      jobsIndexDelta: round(diffFromBaseline.jobs || 0, 3),
      confidence: transformerStats.confidence,
      p10: {
        temporaryConstructionJobs: transformerUncertaintyBand(transformerStats.temporaryConstructionJobs, transformerStats.confidence, loadTransformerArtifacts(rootDir).model).p10,
        operationsJobs: transformerUncertaintyBand(transformerStats.operationsJobs, transformerStats.confidence, loadTransformerArtifacts(rootDir).model).p10,
        capacityEnabledJobs: transformerUncertaintyBand(capacityEnabledJobs, transformerStats.confidence, loadTransformerArtifacts(rootDir).model).p10,
        netJobsEstimate: transformerUncertaintyBand(directJobs + accessibilitySupportedJobs, transformerStats.confidence, loadTransformerArtifacts(rootDir).model).p10
      },
      p50: {
        temporaryConstructionJobs: roundConcrete(transformerStats.temporaryConstructionJobs, 1),
        operationsJobs: roundConcrete(transformerStats.operationsJobs, 1),
      ...
      p90: {
        temporaryConstructionJobs: transformerUncertaintyBand(transformerStats.temporaryConstructionJobs, transformerStats.confidence, loadTransformerArtifacts(rootDir).model).p90,
        operationsJobs: transformerUncertaintyBand(transformerStats.operationsJobs, transformerStats.confidence, loadTransformerArtifacts(rootDir).model).p90,
        capacityEnabledJobs: transformerUncertaintyBand(capacityEnabledJobs, transformerStats.confidence, loadTransformerArtifacts(rootDir).model).p90,
        netJobsEstimate: transformerUncertaintyBand(directJobs + accessibilitySupportedJobs, transformerStats.confidence, loadTransformerArtifacts(rootDir).model).p90
      },
      method: "building employment density + road/accessibility uplift + transformer headroom constraint + trained jobs forecast delta"
    },
    electricity: {
      annualMwhChange: roundConcrete(annualMwhChange, 1),
      peakKwChange: roundConcrete(peakKwChange, 0),
      transformerReliefKw: roundConcrete(transformerStats.transformerReliefKw, 0),
      localCapacityHeadroomKwChange: roundConcrete(transformerStats.transformerReliefKw - peakKwChange, 0),
      overloadRiskDelta: round((peakKwChange - transformerStats.transformerReliefKw) / Number(loadTransformerArtifacts(rootDir).model?.coefficients?.headroomRiskKwScale || 5500), 3),
      loadIndexDelta: round(diffFromBaseline.electricity || transformerStats.loadIndexDelta || 0, 3),
      localLoadIndexDelta: round(diffFromBaseline.electricity || transformerStats.loadIndexDelta || 0, 3),
      confidence: transformerStats.confidence,
      p10: {
        annualMwhChange: transformerUncertaintyBand(annualMwhChange, transformerStats.confidence, loadTransformerArtifacts(rootDir).model).p10,
        peakKwChange: transformerUncertaintyBand(peakKwChange, transformerStats.confidence, loadTransformerArtifacts(rootDir).model).p10,
        transformerReliefKw: transformerUncertaintyBand(transformerStats.transformerReliefKw, transformerStats.confidence, loadTransformerArtifacts(rootDir).model).p10,
        localCapacityHeadroomKwChange: transformerUncertaintyBand(transformerStats.transformerReliefKw - peakKwChange, transformerStats.confidence, loadTransformerArtifacts(rootDir).model).p10
      },
      p50: {
        annualMwhChange: roundConcrete(annualMwhChange, 1),
        peakKwChange: roundConcrete(peakKwChange, 0),
      ...
      p90: {
        annualMwhChange: transformerUncertaintyBand(annualMwhChange, transformerStats.confidence, loadTransformerArtifacts(rootDir).model).p90,
        peakKwChange: transformerUncertaintyBand(peakKwChange, transformerStats.confidence, loadTransformerArtifacts(rootDir).model).p90,
        transformerReliefKw: transformerUncertaintyBand(transformerStats.transformerReliefKw, transformerStats.confidence, loadTransformerArtifacts(rootDir).model).p90,
        localCapacityHeadroomKwChange: transformerUncertaintyBand(transformerStats.transformerReliefKw - peakKwChange, transformerStats.confidence, loadTransformerArtifacts(rootDir).model).p90
      },
      method: "floor-area electricity demand + road lighting load + transformer impact artifact + forecast load delta"
    },
    services: {
      residentServiceDemand: roundConcrete(residentServiceDemand, 0),
      workerServiceDemand: roundConcrete(workerServiceDemand, 0),
      serviceCapacityEquivalent: roundConcrete(serviceCapacityEquivalent, 0),
      netServiceDemand: roundConcrete(netServiceDemand, 0),
      serviceIndexDelta: round(diffFromBaseline.services || 0, 3),
      method: "resident/job service demand + community floorspace + road/transformer access + services forecast delta"
    },
    evidence: [
      ...
      "lib/scenario-studio.js planners"
    ],
    warning: "Concrete values are calibrated planning estimates. Transformer outputs are planning-grade screening only, not NIE engineering approval."
  };
}
...
function simulateForecastVariant(variant, contextRows, baselineTimeline, baseBuilding, siteContext, rootDir) {
  ...
  for (const year of FORECAST_YEARS) {
    ...
    const concreteImpacts = concreteImpactsForVariant(variant, baseBuilding, siteContext, year, diffFromBaseline, baselineMetrics, rootDir);
  ...
  const branch = {
    ...
    concreteImpacts: horizon.concreteImpacts,
    timelineByYear,
    affectedCellsByYear,
...
function evidenceForScenarioBranch(variant, building) {
  ...
  for (const intervention of variant.interventions || []) {
    ...
    if (type === "building_removal") evidence.push(`${building.config.size} ${building.config.buildingType} building-removal planner`);
    ...
    if (type === "road" || type === "road_corridor") evidence.push("Road planner: congestion relief, induced demand, severance/equity, environment and fiscal effects");
    if (type === "transformer" || type === "infrastructure") evidence.push("Transformer impact model: capacity relief, headroom, overload-risk and confidence-band screening");
  }
...
function baselineForecastBranch(contextRows) {
  ...
  for (const year of FORECAST_YEARS) {
    ...
    timelineByYear[String(year)] = {
      ...
      concreteImpacts: concreteImpactZero(year)
    };
  ...
  for (const year of FORECAST_YEARS) {
    timeline[String(year)] = {
      ...
      branches: scenarioBranches.map((branch) => ({
        ...
        concreteImpacts: branch.timelineByYear[String(year)].concreteImpacts,
...
function runForecastScenario(input = {}, rootDir = process.cwd()) {
  ...
  const suppliedLocation = input.building?.location || input.location;
  const resolvedPostcode = input.resolvedPostcode || input.postcodeResolution || (postcodeValue ? resolvePostcode(postcodeValue, rootDir) : null);
  const useResolvedPostcode = Boolean(resolvedPostcode?.canPlace);
  if (!useResolvedPostcode && !suppliedLocation) {
    const error = new Error(resolvedPostcode?.warnings?.[0] || "A full Belfast postcode or a valid map point is required before placing a building.");
  ...
  const buildingPayload = {
    ...
    year: input.building?.year || input.building?.delivery?.startYear || input.building?.delivery?.start_year || input.building?.startYear || input.building?.start_year || startYear,
    startYear: input.building?.delivery?.startYear || input.building?.delivery?.start_year || input.building?.startYear || input.building?.start_year || input.building?.year || startYear,
  ...
  // Validation results are recorded on the building (so the UI can surface
  // warnings) but NEVER block scenario execution — the user picks the spot,
  // the simulation reports what happens there.
  ...
  const userInterventions = extractUserInterventions(input, building, rootDir);
  const variants = mergeUserInterventionsIntoVariants(
    sanitizeScenarioVariants(input.branches || input.variants || generateFallbackVariants({ building, siteContext }, rootDir), building, rootDir, { strict: false }),
    userInterventions
  );
  const cells = cityCells(rootDir);
  ...
  const transformerArtifacts = loadTransformerArtifacts(rootDir);
  const warnings = [
    ...
    "Transformer capacity/headroom estimates are planning-grade screening only, not NIE engineering approval."
  ];
  ...
  return {
    ...
    transformerModelVersion: transformerArtifacts.model?.modelVersion || TRANSFORMER_MODEL_VERSION_FALLBACK,
...
function deterministicAgentTrace({ building, siteContext, scenarioBranches, recommendedBranch }) {
  return [
    ...
    { agent: "Energy", status: "complete", summary: "Transformer interventions use the screening model when available; outputs are not grid approval." },
...
function buildSpecialistRecommendations(input = {}) {
  ...
  return [
    ...
    },
    {
      ...
      reason: "Electricity demand and transformer headroom are planning-screening estimates; local grid capacity is not asserted as approval.",
      recommended_variant: {
...
function critiqueSimulation(input = {}) {
  ...
  return {
    ...
    rejected_claims: [
      ...
      "Do not claim exact electricity capacity availability or NIE approval."
    ],
    unsupportedClaims: [
      "Exact congestion reduction is not supported by this proxy model.",
      "Transformer headroom is a screening estimate and does not validate feeder-level capacity."
    ],
    warnings,
    recommendations: [
...
function reportSimulation(input = {}) {
  ...
  const isRemoval = branchHasRemoval(userBranch);
  const summary = `The original ${isRemoval ? "removal" : "proposal"} changes the population proxy ${formatDelta(popDelta)} against the no-build forecast. ${recommendedName} keeps the scenario testable while balancing mobility, fairness, and environmental exposure. Planner review is ${critic.humanReviewRequired || critic.human_review_required ? "recommended" : "not automatically required"} because these are proxy estimates, not engineering guarantees.`;
  return {
    headline,
    summary,
    city_commits: [
      `${isRemoval ? "-" : "+"} ${isRemoval ? "Removed" : "Added"} ${describeBuildingFromBranch(userBranch)} near the selected Belfast site`,
...
function describeBuildingFromBranch(branch) {
  const intervention = branch?.interventions?.find((item) => {
    const type = item.type || item.interventionType;
    return type === "building" || type === "building_removal";
  });
...
function branchHasRemoval(branch) {
  return Boolean(branch?.interventions?.some((item) => (item.type || item.interventionType) === "building_removal"));
}
...
module.exports = {
  ...
  loadTransformerArtifacts,
  normalizeBuildingConfig,
  normalizeTransformerIntervention,
  normalizePostcode,
