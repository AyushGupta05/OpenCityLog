...
const CATEGORY_SEMANTIC_RULES = {
  building_development: {
    strong: /\b(mixed[-\s]?use|housing|residential|dwelling|dwellings|apartment|office|retail|commercial|hotel|building|storey|floor|development|construction|extension|change of use|brownfield)\b/i,
    context: /\b(planning|approval|permission|listed|heritage|conservation|frontage|site)\b/i,
    minor: /\b(signs?|signage|totem|hoarding|advertis(?:e|ing|ement)?|fascia|atm|cash machine|window|doorway?|boundary wall|garden structure|satellite dish|antenna)\b/i,
  },
  road_transport_change: {
    strong: /\b(bus|junction|road|street|cycle|cycling|station|transport|transit|corridor|route|traffic|pedestrian|walking|crossing|public realm)\b/i,
    context: /\b(access|mobility|lane|stop|interchange|servicing|loading)\b/i,
    minor: /\b(signs?|advertis(?:e|ing|ement)?|totem|hoarding)\b/i,
  },
  energy_infrastructure: {
    strong: /\b(substation|transformer|electricity|electrical|grid|power|energy|utility|utilities|connection|capacity)\b/i,
    context: /\b(asset|network|supply|cable|infrastructure|constraint)\b/i,
    minor: /\b(signs?|advertis(?:e|ing|ement)?|totem|hoarding)\b/i,
  },
  green_public_space: {
    strong: /\b(park|public space|public realm|tree|planting|green|open space|playground|drainage|flood|biodiversity|canopy)\b/i,
    context: /\b(landscape|comfort|shade|access|maintenance|environment|resilience)\b/i,
    minor: /\b(signs?|advertis(?:e|ing|ement)?|totem|hoarding)\b/i,
  },
  service_civic_infrastructure: {
    strong: /\b(community|school|health|library|hospital|facility|service|hub|university|college|leisure|civic|public access)\b/i,
    context: /\b(catchment|opening|capacity|accessibility|service users|staff|amenity)\b/i,
    minor: /\b(signs?|advertis(?:e|ing|ement)?|totem|hoarding)\b/i,
  },
};

const GENERIC_PROPOSAL_TERMS = new Set([
  "proposal",
  "proposed",
  "screen",
  "review",
...
function staticCandidateScore(event) {
  ...
  if (/\b(mixed[-\s]?use|housing|residential|dwelling|dwellings|transport|station|substation|park|community|school|hospital|public realm)\b/i.test(text)) score += 16;
  if (Object.values(CATEGORY_SEMANTIC_RULES).some((rule) => rule.minor?.test(title))) score -= 24;
...
function proposalSemanticText(proposal) {
  return [
    proposal.title,
...
function semanticTerms(text) {
  return uniqueStrings(String(text || "")
    ...
    .filter((term) => term.length >= 4 && !GENERIC_PROPOSAL_TERMS.has(term)));
}

function eventSemanticScore(event, proposal) {
  const rule = CATEGORY_SEMANTIC_RULES[proposal.category] || CATEGORY_SEMANTIC_RULES.building_development;
  const proposalText = proposalSemanticText(proposal);
  const eventText = eventSignalText(event);
  const eventLower = eventText.toLowerCase();
  let score = 0.18;
  if (rule.strong?.test(eventText)) score += 0.38;
  if (rule.context?.test(eventText)) score += 0.16;
  const matchingTerms = semanticTerms(proposalText).filter((term) => eventLower.includes(term));
  score += Math.min(0.26, matchingTerms.length * 0.055);
  if (rule.minor?.test(eventText) && !rule.minor.test(proposalText)) score -= 0.34;
  if (/^other planning approval/i.test(String(event.title || "")) && !/\b(other|minor|sign|advert|boundary|alteration|extension)\b/i.test(proposalText)) {
    score -= 0.08;
  }
  if (/\b(temporary|retrospective)\b/i.test(eventText) && !/\b(temporary|meanwhile|retrospective)\b/i.test(proposalText)) {
    score -= 0.05;
  }
...
function scoreEventForProposal(event, proposal, sourceById, options = {}) {
  ...
  const semanticScore = eventSemanticScore(event, proposal);
  const weighted =
    categoryScore * 0.26 +
    distanceScore * 0.18 +
    recencyScore * 0.14 +
  ...
  return {
    ...
    score_breakdown: {
      ...
      semantic_relevance: round(semanticScore, 2),
    },
...
function matchFactors(scoreBreakdown = {}) {
  const labels = {
    ...
    semantic_relevance: "proposal text relevance",
  };
...
function directionPhrase(direction) {
  if (direction === "positive") return "review as a possible gain or capacity theme; no direction is asserted";
  if (direction === "negative") return "review as a possible pressure theme; no direction is asserted";
  if (direction === "mixed") return "observed records vary, so review more than one direction";
...
function eventIdValue(event) {
  return event?.event_id || event?.id || null;
}

function eventsInYearWindow(rows, startYear, endYear) {
  return (rows || [])
...
function countsByCategory(events) {
  const counts = {};
  for (const event of events || []) {
    const key = String(event.category || event.lens || "unknown");
    counts[key] = (counts[key] || 0) + 1;
  }
  return counts;
}

function categoryCountSummary(counts) {
  const entries = Object.entries(counts || {})
    .sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0]))
    .slice(0, 3);
  if (!entries.length) return "no nearby source-backed records";
  return entries
    .map(([category, count]) => `${count} ${category.replace(/_/g, " ")}`)
    .join(", ");
}

function observedPatternStrength(anchor, beforeEvents, afterEvents) {
  const count = beforeEvents.length + afterEvents.length;
  const documented = ["documented", "corroborated"].includes(String(anchor?.confidence || "").toLowerCase());
  if (documented && count >= 4) return { status: "ready_to_review", label: "Ready to review" };
  if (documented || count > 0) return { status: "thin_evidence", label: "Thin evidence" };
  return { status: "gap", label: "Evidence gap" };
}

function formatPatternEvent(event) {
  return {
    event_id: eventIdValue(event),
    title: event.title || "Historical event",
    year: event.year || null,
    category: event.category || null,
    confidence: event.confidence || "inferred",
...
function buildObservedPatterns(similarEvents, events, sourceById, options = {}) {
  const byId = new Map((events || []).map((event) => [eventIdValue(event), event]));
  const radiusM = Number(options.radius_m || options.radiusM || DEFAULT_RADIUS_M);
  return (similarEvents || []).slice(0, MAX_OBSERVED_PATTERNS).map((match) => {
    const anchor = byId.get(match.event_id) || match;
    const point = eventPoint(anchor) || match.location;
    const year = Number(match.year || anchor.year);
    const hasYear = Number.isFinite(year);
    const beforeStart = hasYear ? year - OBSERVED_PATTERN_WINDOW_YEARS : null;
    const beforeEnd = hasYear ? year - 1 : null;
    const afterStart = hasYear ? year + 1 : null;
    const afterEnd = hasYear ? year + OBSERVED_PATTERN_WINDOW_YEARS : null;
    const nearbyRows = point && hasYear
      ? eventsNearLocation(point, events || [], radiusM).filter((row) => eventIdValue(row.event) !== match.event_id)
      : [];
    const beforeEvents = hasYear ? eventsInYearWindow(nearbyRows, beforeStart, beforeEnd) : [];
    const afterEvents = hasYear ? eventsInYearWindow(nearbyRows, afterStart, afterEnd) : [];
    const beforeCategories = countsByCategory(beforeEvents);
    const afterCategories = countsByCategory(afterEvents);
    const strength = observedPatternStrength(anchor, beforeEvents, afterEvents);
    const observedPattern = afterEvents.length
      ? `In the ${OBSERVED_PATTERN_WINDOW_YEARS} years after this analogue, the atlas records ${afterEvents.length} nearby source-backed record${afterEvents.length === 1 ? "" : "s"}: ${categoryCountSummary(afterCategories)}. The preceding ${OBSERVED_PATTERN_WINDOW_YEARS}-year window records ${beforeEvents.length}: ${categoryCountSummary(beforeCategories)}.`
      : `The atlas has no nearby source-backed follow-up records in the ${OBSERVED_PATTERN_WINDOW_YEARS} years after this analogue; treat the after-window as a coverage gap. The preceding ${OBSERVED_PATTERN_WINDOW_YEARS}-year window records ${beforeEvents.length}: ${categoryCountSummary(beforeCategories)}.`;
    return {
      analogue_event_id: match.event_id,
      title: match.title || anchor.title || "Historical analogue",
      year: Number.isFinite(year) ? year : null,
      effective_date: match.effective_date || anchor.effective_date || null,
      radius_m: radiusM,
      before_window: { start_year: beforeStart, end_year: beforeEnd, event_count: beforeEvents.length, categories: beforeCategories },
      after_window: { start_year: afterStart, end_year: afterEnd, event_count: afterEvents.length, categories: afterCategories },
      observed_pattern: observedPattern,
      evidence_strength: strength.status,
      evidence_strength_label: strength.label,
      caveat: "Before/after records are observations around the analogue. They do not show why later records occurred.",
      baseline_events: beforeEvents.slice(0, 4).map(formatPatternEvent),
      follow_up_events: afterEvents.slice(0, 4).map(formatPatternEvent),
      evidence: (match.evidence || []).length ? match.evidence : evidenceLinks(anchor, sourceById, 3),
    };
  });
}
...
function buildProposalBrief(proposal, similarEvents, affectedSignals, localContext, confidence, designReview, observedPatterns = []) {
...
function summarySentence(proposal, affectedSignals, confidence, observedPatterns = []) {
  ...
  const patternText = observedPatterns.length
    ? `${observedPatterns.length} historical analogue${observedPatterns.length === 1 ? "" : "s"} include observed before/after record windows`
    : "Historical analogues and local context are available for review";
  return `${patternText} for this ${proposal.category_label}, with review themes across ${signalText}. Evidence strength is ${confidence.label}. This is not a forecast; check the linked source rows and caveats before making a proposal claim.`;
}

function framingNote(confidence) {
  return {
    label: "Historical analogue lens",
    not_a_forecast: true,
    claim_standard: "Historically similar changes were followed by these observed records; causation is not claimed.",
    evidence_strength: confidence.label,
    safe_labels: [
      "historical analogue",
      "observed change",
      "not a forecast",
...
function assessProposal(input = {}, options = {}) {
  ...
  const observedPatterns = buildObservedPatterns(similarEvents, atlas.events, sourceById, options);
  const proposalBrief = buildProposalBrief(proposal, similarEvents, affectedSignals, localContext, confidence, designReview, observedPatterns);
  const proposalPathway = buildProposalPathway(proposal, similarEvents, localContext, affectedSignals, confidence);
  return {
    ok: true,
    mode: "proposal_analogue_lens",
    generated_at: new Date().toISOString(),
    framing: framingNote(confidence),
    proposal,
    summary: summarySentence(proposal, affectedSignals, confidence, observedPatterns),
