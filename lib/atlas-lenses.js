...
function eventExcludedFromLens(event, lens) {
  ...
  return exclusions.includes(String(lens.slug || "").toLowerCase());
}
...
function eventMatchesLens(event, lensOrSlug, sourceById = new Map()) {
  ...
  const sourceEvidence = sourceTextForEvent(event, sourceById);
  return Boolean(sourceEvidence && GROUP_PATTERNS[group].test(sourceEvidence));
}
...
function sourceWithholdsMapGeometry(source) {
  if (!source) return false;
  if (source.map_geometry_review_required === true) return true;
  if (String(source.map_geometry_status || "").toLowerCase() === "withheld_rights_review") return true;
  if (String(source.geometry_status || "").toLowerCase() === "withheld_rights_review") return true;
  return false;
}

function eventMapGeometryStatus(event) {
  if (!event) return "";
  const explicitStatus = String(
    event.map_geometry_status
      || event.geometry_status
      || event.provenance?.map_geometry_status
      || event.provenance?.geometry_status
      || "",
  ).toLowerCase();
  if (explicitStatus.startsWith("withheld_")) return explicitStatus;

  const precision = String(event.provenance?.geometry_precision || "").toLowerCase();
  const source = String(event.provenance?.geometry_source || "").toLowerCase();
  const sourceBasis = String(event.provenance?.source_basis || "").toLowerCase();
  const combined = `${source} ${precision} ${sourceBasis}`;
  const sourceIds = (event.source_ids || event.sourceIds || []).join(" ").toLowerCase();
  const identity = [
    sourceIds,
    event.id,
    event.event_id,
  ...
  if (
    /atlas reference point|lacks row-level coordinates|source seed lacks row-level coordinates|not an exact event geometry|not exact event geometry/.test(combined)
    || /area\/city reference marker|city\/area keywords/.test(combined)
    || /borough aggregate|borough centroid for atlas navigation|aggregate (?:row|record)|statistical housing-market record|house price index|uk[-_\s]?hpi/.test(combined)
    || /\blon-extra-uk-house-price-index\b|house price index|uk[-_\s]?hpi/.test(identity)
    || /dataset\/layer marker|current-state source marker|represents a dataset\/layer/.test(combined)
    || /not a project site, footprint, route, parcel, or facility coordinate/.test(combined)
  ) {
    return "withheld_non_site_scope";
  }
  return "";
}

function eventWithholdsMapGeometry(event) {
  return Boolean(eventMapGeometryStatus(event));
}
